...
/* athena-bookmarks-window.c - implementation of bookmark-editing window.
 */
...
static AthenaBookmarkList *bookmarks = NULL;
static GtkTreeView	    *bookmark_list_widget = NULL; /* awkward name to distinguish from AthenaBookmarkList */
...
static void     name_or_uri_field_activate                  (AthenaEntry        *entry);
static void     athena_bookmarks_window_restore_geometry  (GtkWidget            *window);
static void     on_bookmark_list_changed                    (AthenaBookmarkList *list,
							     gpointer              user_data);
...
/* We store a pointer to the bookmark in a column so when an item is moved
   ...
   athena_bookmark_list_insert_item() makes a copy of the bookmark, so we
...
static void
athena_bookmarks_window_response_callback (GtkDialog *dialog,
					     int response_id,
					     gpointer callback_data)
{
	if (response_id == GTK_RESPONSE_HELP) {
		...
		gtk_show_uri (gtk_window_get_screen (GTK_WINDOW (dialog)),
			      "help:gnome-help/athena-bookmarks-edit",
...
bookmarks_set_empty (gboolean empty)
{
	...
	if (empty) {
		...
		if (athena_bookmark_list_length (bookmarks) > 0 &&
 ...
 * @list: The AthenaBookmarkList that this window will edit.
 *
 * Return value: A pointer to the new window.
 **/
GtkWindow *
create_bookmarks_window (AthenaBookmarkList *list, GObject *undo_manager_source)
{
	GtkWidget         *window;
	...
	athena_undo_share_undo_manager (G_OBJECT (window), undo_manager_source);

	gtk_window_set_wmclass (GTK_WINDOW (window), "bookmarks", "Athena");
	athena_bookmarks_window_restore_geometry (window);
	...
	g_signal_connect (window, "response",
			  G_CALLBACK (athena_bookmarks_window_response_callback), NULL);
...
get_selected_bookmark (void)
{
	...
	if (athena_bookmark_list_length (bookmarks) < 1)
		return NULL;

	return athena_bookmark_list_item_at(bookmarks, get_selected_row ());
}
...
static void
athena_bookmarks_window_restore_geometry (GtkWidget *window)
{
	...
	window_geometry = athena_bookmark_list_get_window_geometry (bookmarks);
 ...
 **/
void
athena_bookmarks_window_save_geometry (GtkWindow *window)
{
	g_return_if_fail (GTK_IS_WINDOW (window));
	g_return_if_fail (ATHENA_IS_BOOKMARK_LIST (bookmarks));
	...
	if (gtk_widget_get_visible (GTK_WIDGET (window))) {
		...
		athena_bookmark_list_set_window_geometry (bookmarks, geometry_string);
		g_free (geometry_string);
	}
}

static void
on_bookmark_list_changed (AthenaBookmarkList *bookmarks, gpointer data)
{
	g_return_if_fail (ATHENA_IS_BOOKMARK_LIST (bookmarks));
...
open_selected_bookmark (gpointer user_data, GdkScreen *screen)
{
	...
	location = athena_bookmark_get_location (selected);
	if (location == NULL) { 
		return;
	}
...
on_row_changed (GtkListStore *store,
		...
		gpointer user_data)
{
	AthenaBookmark *bookmark = NULL, *bookmark_in_list;
	gint *indices, row;
	...
	if (row < (gint) athena_bookmark_list_length (bookmarks)) {
		bookmark_in_list = athena_bookmark_list_item_at (bookmarks,
								   row);
	...
	if (insert_bookmark) {
		g_signal_handler_block (bookmarks,
					bookmark_list_changed_signal_id);
		athena_bookmark_list_insert_item (bookmarks, bookmark, row);
		...
		bookmark = athena_bookmark_list_item_at (bookmarks, row);
		...
		gpointer user_data)
{
	...
	athena_bookmark_list_delete_item_at (bookmarks, row);
		      ...
		      gpointer user_data)
{
	...
	if (selected) {
		name = athena_bookmark_get_name (selected);
		location = athena_bookmark_get_location (selected);
	...
	athena_entry_set_text (ATHENA_ENTRY (name_field),
				 name ? name : "");
	...
	athena_entry_set_text (ATHENA_ENTRY (uri_field),
...
{
	if (text_changed) {
		AthenaBookmark *bookmark, *bookmark_in_list;
		const char *name;
		GIcon *icon;
		guint selected_row;
		GtkTreeIter iter;
		...
		bookmark = athena_bookmark_new (location,
		...
		g_signal_handler_block (bookmarks, 
					bookmark_list_changed_signal_id);
		athena_bookmark_list_delete_item_at (bookmarks, selected_row);
		athena_bookmark_list_insert_item (bookmarks, bookmark, selected_row);
		g_signal_handler_unblock (bookmarks, 
					  bookmark_list_changed_signal_id);
		...
		name = athena_bookmark_get_name (bookmark_in_list);
		icon = athena_bookmark_get_icon (bookmark_in_list);
...
on_text_field_focus_out_event (GtkWidget *widget,
			       ...
			       gpointer user_data)
{
	g_assert (ATHENA_IS_ENTRY (widget));
...
static void
name_or_uri_field_activate (AthenaEntry *entry)
{
	g_assert (ATHENA_IS_ENTRY (entry));

	update_bookmark_from_text ();
	athena_entry_select_all_at_idle (entry);
}
...
restore_geometry (gpointer data)
{
	...
	athena_bookmarks_window_restore_geometry (GTK_WIDGET (data));
...
on_window_hide_event (GtkWidget *widget,
		      gpointer user_data)
{
	athena_bookmarks_window_save_geometry (GTK_WINDOW (widget));

	/* Disable undo for entry widgets */
	athena_undo_unregister (G_OBJECT (name_field));
	athena_undo_unregister (G_OBJECT (uri_field));
...
repopulate (void)
{
	...
	for (index = 0; index < athena_bookmark_list_length (bookmarks); ++index) {
		...
		bookmark_icon = athena_bookmark_get_icon (bookmark);
