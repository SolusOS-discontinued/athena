...
G_DEFINE_TYPE (AthenaNavigationAction, athena_navigation_action, GTK_TYPE_ACTION);
#define ATHENA_NAVIGATION_ACTION_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), ATHENA_TYPE_NAVIGATION_ACTION, AthenaNavigationActionPrivate))

struct AthenaNavigationActionPrivate
{
	AthenaWindow *window;
	AthenaNavigationDirection direction;
	char *arrow_tooltip;
				    ...
				    gboolean back)
{
	...
	athena_window_back_or_forward (window, back, index, should_open_in_new_tab ());
}
...
static void
activate_forward_menu_item_callback (GtkMenuItem *menu_item, AthenaWindow *window)
{
	activate_back_or_forward_menu_item (menu_item, window, FALSE);
}

static void
fill_menu (AthenaWindow *window,
	   GtkWidget *menu,
	   gboolean back)
{
	...
	index = 0;
	while (list != NULL) {
		menu_item = athena_bookmark_menu_item_new (ATHENA_BOOKMARK (list->data));
...
static void
show_menu (AthenaNavigationAction *self,
           guint button,
           guint32 event_time)
{
	AthenaWindow *window;
	...
	case ATHENA_NAVIGATION_DIRECTION_FORWARD:
		fill_menu (window, menu, FALSE);
		break;
	case ATHENA_NAVIGATION_DIRECTION_BACK:
		fill_menu (window, menu, TRUE);
		break;
 	case ATHENA_NAVIGATION_DIRECTION_UP:
 		return;
 	case ATHENA_NAVIGATION_DIRECTION_RELOAD:
 		return;
 	case ATHENA_NAVIGATION_DIRECTION_HOME:
 		return;
 	case ATHENA_NAVIGATION_DIRECTION_COMPUTER:
 		return;
 	case ATHENA_NAVIGATION_DIRECTION_EDIT:
 		return;
	default:
...
{
        AthenaNavigationAction *self = data;
...
static void
unschedule_menu_popup_timeout (AthenaNavigationAction *self)
{
...
static void
schedule_menu_popup_timeout (AthenaNavigationAction *self)
{
...
tool_button_press_cb (GtkButton *button,
                      ...
                      gpointer user_data)
{
        AthenaNavigationAction *self = user_data;
               ...
               GtkWidget *proxy)
{
	...
	(* GTK_ACTION_CLASS (athena_navigation_action_parent_class)->connect_proxy) (action, proxy);
}
                  ...
                  GtkWidget *proxy)
{
	...
	if (GTK_IS_TOOL_BUTTON (proxy)) {
                ...
                unschedule_menu_popup_timeout (ATHENA_NAVIGATION_ACTION (action));
	...
	(* GTK_ACTION_CLASS (athena_navigation_action_parent_class)->disconnect_proxy) (action, proxy);
}

static void
athena_navigation_action_finalize (GObject *object)
{
	AthenaNavigationAction *action = ATHENA_NAVIGATION_ACTION (object);
	...
	(* G_OBJECT_CLASS (athena_navigation_action_parent_class)->finalize) (object);
}

static void
athena_navigation_action_set_property (GObject *object,
					 guint prop_id,
					 const GValue *value,
					 GParamSpec *pspec)
{
	AthenaNavigationAction *nav;

	nav = ATHENA_NAVIGATION_ACTION (object);
...
static void
athena_navigation_action_get_property (GObject *object,
					 guint prop_id,
					 GValue *value,
					 GParamSpec *pspec)
{
...
static void
athena_navigation_action_class_init (AthenaNavigationActionClass *class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (class);
	GtkActionClass *action_class = GTK_ACTION_CLASS (class);

	object_class->finalize = athena_navigation_action_finalize;
	object_class->set_property = athena_navigation_action_set_property;
	object_class->get_property = athena_navigation_action_get_property;
	...
	g_type_class_add_private (object_class, sizeof(AthenaNavigationActionPrivate));
}

static void
athena_navigation_action_init (AthenaNavigationAction *action)
{
        action->priv = ATHENA_NAVIGATION_ACTION_GET_PRIVATE (action);
}
