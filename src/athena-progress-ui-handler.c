...
/*
 * athena-progress-ui-handler.c: file operation progress user interface.
 *
...
#include "athena-progress-info-widget.h"
...
struct _AthenaProgressUIHandlerPriv {
...
G_DEFINE_TYPE (AthenaProgressUIHandler, athena_progress_ui_handler, G_TYPE_OBJECT);
...
notification_show_details_cb (NotifyNotification *notification,
			      ...
			      gpointer user_data)
{
	AthenaProgressUIHandler *self = user_data;
...
static void
progress_ui_handler_ensure_notification (AthenaProgressUIHandler *self)
{
...
static void
progress_ui_handler_ensure_status_icon (AthenaProgressUIHandler *self)
{
	GIcon *icon;
	GtkStatusIcon *status_icon;
...
static void
progress_ui_handler_update_notification (AthenaProgressUIHandler *self)
{
	gchar *body;
...
static void
progress_ui_handler_update_status_icon (AthenaProgressUIHandler *self)
{
	gchar *tooltip;
...
static void
progress_ui_handler_ensure_window (AthenaProgressUIHandler *self)
{
...
static void
progress_ui_handler_add_to_window (AthenaProgressUIHandler *self,
				   AthenaProgressInfo *info)
{
	GtkWidget *progress;

	progress = athena_progress_info_widget_new (info);
...
static void
progress_ui_handler_hide_notification_or_status (AthenaProgressUIHandler *self)
{
	if (self->priv->status_icon != NULL) {
...
static void
handle_new_progress_info (AthenaProgressUIHandler *self,
			  AthenaProgressInfo *info)
{
...
static TimeoutData *
timeout_data_new (AthenaProgressUIHandler *self,
		  AthenaProgressInfo *info)
{
	TimeoutData *retval;
...
static void
progress_info_started_cb (AthenaProgressInfo *info,
			  AthenaProgressUIHandler *self)
{
	AthenaApplication *app;
	TimeoutData *data;
...
static void
new_progress_info_cb (AthenaProgressInfoManager *manager,
		      AthenaProgressInfo *info,
		      AthenaProgressUIHandler *self)
{
...
static void
athena_progress_ui_handler_dispose (GObject *obj)
{
	AthenaProgressUIHandler *self = ATHENA_PROGRESS_UI_HANDLER (obj);

	g_clear_object (&self->priv->manager);

	G_OBJECT_CLASS (athena_progress_ui_handler_parent_class)->dispose (obj);
}
...
static void
athena_progress_ui_handler_init (AthenaProgressUIHandler *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ATHENA_TYPE_PROGRESS_UI_HANDLER,
						  AthenaProgressUIHandlerPriv);

	self->priv->manager = athena_progress_info_manager_new ();
	g_signal_connect (self->priv->manager, "new-progress-info",
			  G_CALLBACK (new_progress_info_cb), self);
}

static void
athena_progress_ui_handler_class_init (AthenaProgressUIHandlerClass *klass)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (klass);
	oclass->dispose = athena_progress_ui_handler_dispose;
	
	g_type_class_add_private (klass, sizeof (AthenaProgressUIHandlerPriv));
}

AthenaProgressUIHandler *
athena_progress_ui_handler_new (void)
{
	return g_object_new (ATHENA_TYPE_PROGRESS_UI_HANDLER, NULL);
}
