...
/* athena-mime-actions.c - uri-specific versions of mime action functions
...
#define DEBUG_FLAG ATHENA_DEBUG_MIME
...
typedef struct {
	...
	AthenaFileListHandle *files_handle;
...
{
	athena_file_unref (location->file);
	g_free (location->uri);
	g_free (location);
}
...
launch_location_from_file (AthenaFile *file)
{
	LaunchLocation *location;
	location = g_new (LaunchLocation, 1);
	location->file = athena_file_ref (file);
	location->uri = athena_file_get_uri (file);
				 ...
				 const char *uri)
{
	athena_file_unref (location->file);
	g_free (location->uri);
	location->file = athena_file_get_by_uri (uri);
...
static GList*
filter_athena_handler (GList *apps)
{
	GList *l, *next;
	GAppInfo *application;
	const char *id;
...
static gboolean
athena_mime_actions_check_if_required_attributes_ready (AthenaFile *file)
{
	AthenaFileAttributes attributes;
	gboolean ready;

	attributes = athena_mime_actions_get_required_file_attributes ();
	ready = athena_file_check_if_ready (file, attributes);

	return ready;
}

AthenaFileAttributes 
athena_mime_actions_get_required_file_attributes (void)
{
...
static gboolean
file_has_local_path (AthenaFile *file)
{
	GFile *location;
	char *path;
	gboolean res;
...
GAppInfo *
athena_mime_get_default_application_for_file (AthenaFile *file)
{
	GAppInfo *app;
	char *mime_type;
	char *uri_scheme;

	if (!athena_mime_actions_check_if_required_attributes_ready (file)) {
		return NULL;
	}
	...
	if (app == NULL) {
		uri_scheme = athena_file_get_uri_scheme (file);
		if (uri_scheme != NULL) {
...
static int
file_compare_by_mime_type (AthenaFile *file_a,
			   AthenaFile *file_b)
{
	char *mime_type_a, *mime_type_b;
	int ret;
	
	mime_type_a = athena_file_get_mime_type (file_a);
	mime_type_b = athena_file_get_mime_type (file_b);
...
static int
file_compare_by_parent_uri (AthenaFile *file_a,
			    AthenaFile *file_b) {
	char *parent_uri_a, *parent_uri_b;
	int ret;

	parent_uri_a = athena_file_get_parent_uri (file_a);
	parent_uri_b = athena_file_get_parent_uri (file_b);
...
GList *
athena_mime_get_applications_for_file (AthenaFile *file)
{
	char *mime_type;
	char *uri_scheme;
	GList *result;
	...
	return filter_athena_handler (result);
}

GAppInfo *
athena_mime_get_default_application_for_files (GList *files)
{
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_app = athena_mime_get_default_application_for_file (file);
...
GList *
athena_mime_get_applications_for_files (GList *files)
{
	GList *l, *sorted_files;
	AthenaFile *file;
	GList *one_ret, *ret;
	...
	for (l = sorted_files; l != NULL; l = l->next) {
		...
		one_ret = athena_mime_get_applications_for_file (file);
...
static void
report_broken_symbolic_link (GtkWindow *parent_window, AthenaFile *file)
{
	char *target_path;
	char *display_name;
	char *prompt;
	...
	target_path = athena_file_get_symbolic_link_target_path (file);
	if (target_path == NULL) {
...
get_executable_text_file_action (GtkWindow *parent_window, AthenaFile *file)
{
	GtkDialog *dialog;
	char *file_name;
	char *prompt;
	...
	g_assert (athena_file_contains_text (file));

	preferences_value = g_settings_get_enum	(athena_preferences,
						 ATHENA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
	switch (preferences_value) {
	case ATHENA_EXECUTABLE_TEXT_LAUNCH:
		return ACTIVATION_ACTION_LAUNCH;
	case ATHENA_EXECUTABLE_TEXT_DISPLAY:
		return ACTIVATION_ACTION_OPEN_IN_APPLICATION;
	case ATHENA_EXECUTABLE_TEXT_ASK:
		break;
	default:
		/* Complain non-fatally, since preference data can't be trusted */
		g_warning ("Unknown value %d for ATHENA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
			   preferences_value);
...
gboolean
athena_mime_file_opens_in_view (AthenaFile *file)
{
...
static ActivationAction
get_activation_action (AthenaFile *file)
{
	...
	if (activation_uri == NULL) {
		activation_uri = athena_file_get_uri (file);
	}
	...
	if (athena_file_is_launchable (file)) {
		...
		if (!executable_path) {
			action = ACTIVATION_ACTION_DO_NOTHING;
		} else if (athena_file_contains_text (file)) {
	...
	if (action == ACTIVATION_ACTION_DO_NOTHING) {
		if (athena_mime_file_opens_in_view (file)) {
...
gboolean
athena_mime_file_opens_in_external_app (AthenaFile *file)
{
 ...
 *
 * Construct a list of ApplicationLaunchParameters from a list of AthenaFiles,
			    ...
			    GList **unhandled_uris)
{
	...
	*unhandled_uris = NULL;
	...
	for (l = uris; l != NULL; l = l->next) {
		...
		app = athena_mime_get_default_application_for_file (file);
		if (app != NULL) {
			app_uris = NULL;
...
static gboolean
file_was_cancelled (AthenaFile *file)
{
	GError *error;
...
static gboolean
file_was_not_mounted (AthenaFile *file)
{
	GError *error;
...
activation_parameters_free (ActivateParameters *parameters)
{
	...
	athena_file_list_free (parameters->mountables);
	athena_file_list_free (parameters->start_mountables);
	athena_file_list_free (parameters->not_mounted);
...
cancel_activate_callback (gpointer callback_data)
{
	...
	if (parameters->files_handle) {
		athena_file_list_cancel_call_when_ready (parameters->files_handle);
...
activate_parameters_install_free (ActivateParametersInstall *parameters_install)
{
	...
	athena_file_unref (parameters_install->file);
	athena_file_list_free (parameters_install->files);
...
static char *
get_application_no_mime_type_handler_message (AthenaFile *file, char *uri)
{
...
choose_program (GtkDialog *message_dialog, int response, gpointer callback_data)
{
	...
	g_object_set_data_full (G_OBJECT (dialog), 
				"mime-action:file",
				athena_file_ref (file),
				(GDestroyNotify)athena_file_unref);
...
show_unhandled_type_error (ActivateParametersInstall *parameters)
{
	...
	char *mime_type = athena_file_get_mime_type (parameters->file);
	...
	g_object_set_data_full (G_OBJECT (dialog), 
				"mime-action:file",
				athena_file_ref (parameters->file),
				(GDestroyNotify)athena_file_unref);
					    ...
					    gpointer      user_data)
{
	...
	athena_mime_activate_files (parameters_install->parent_window,
                                    ...
                                    ActivateParametersInstall *parameters_install)
{
	...
	if (response_id == GTK_RESPONSE_YES) {
		mime_type = athena_file_get_mime_type (parameters_install->file);
...
application_unhandled_uri (ActivateParameters *parameters, char *uri)
{
	...
	show_install_mime = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
...
activate_parameters_desktop_free (ActivateParametersDesktop *parameters_desktop)
{
	...
	athena_file_unref (parameters_desktop->file);
				      ...
				      ActivateParametersDesktop *parameters)
{
	...
	case RESPONSE_RUN:
		...
		uri = athena_file_get_uri (parameters->file);
		DEBUG ("Launching untrusted launcher %s", uri);
		athena_launch_desktop_file (screen, uri, NULL,
					      parameters->parent_window);
		g_free (uri);
		break;
	case RESPONSE_MARK_TRUSTED:
		file = athena_file_get_location (parameters->file);
		athena_file_mark_desktop_file_trusted (file,
							 parameters->parent_window,
							 TRUE, 
							 NULL, NULL);
		g_object_unref (file);
		       ...
		       AthenaFile *file)
{
	...
	if (!athena_file_is_trusted_link (file)) {
		...
		parameters_desktop->file = athena_file_ref (file);
...
activate_files (ActivateParameters *parameters)
{
	...
	for (l = launch_files; l != NULL; l = l->next) {
		...
		athena_launch_application_from_command (screen, quoted_path, FALSE, NULL);
		g_free (quoted_path);
		g_free (executable_path);
		g_free (uri);
	...
	for (l = launch_in_terminal_files; l != NULL; l = l->next) {
		...
		athena_launch_application_from_command (screen, quoted_path, TRUE, NULL);
	...
	if (count > 1) {
		if ((parameters->flags & ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
	...
	if (parameters->slot != NULL &&
		...
		if ((flags & ATHENA_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
		    g_settings_get_enum (athena_preferences, ATHENA_PREFERENCES_NEW_TAB_POSITION) ==
		...
		for (l = open_in_view_files; l != NULL; l = l->next) {
			...
			athena_window_slot_open_location (parameters->slot,
							    f, flags, NULL);
			g_object_unref (f);
			g_free (uri);
		}
	...
	if (open_files) {
		for (l = open_in_app_parameters; l != NULL; l = l->next) {
			...
			athena_launch_application_by_uri (one_parameters->application,
							    one_parameters->uris,
							    parameters->parent_window);
	...
	window = NULL;
	if (parameters->slot != NULL) {
		window = athena_window_slot_get_window (parameters->slot);
	}

	if (open_in_app_parameters != NULL ||
	    unhandled_open_in_app_uris != NULL) {
		if ((parameters->flags & ATHENA_WINDOW_OPEN_FLAG_CLOSE_BEHIND) != 0 &&
		    window != NULL) {
...
activation_mount_not_mounted (ActivateParameters *parameters)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		loc = l->data;
		next = l->next;
		athena_file_invalidate_all_attributes (loc->file);
	}
	
	files = get_file_list_for_launch_locations (parameters->locations);
	athena_file_list_call_when_ready
		(files,
		 athena_mime_actions_get_required_file_attributes () | ATHENA_FILE_ATTRIBUTE_LINK_INFO,
					 ...
					 gpointer callback_data)
{
	...
	for (l = parameters->locations; l != NULL; l = next) {
		...
		if (athena_file_get_file_type (file) == G_FILE_TYPE_MOUNTABLE &&
		    !athena_file_has_activation_uri (file)) {
	...
	for (l = parameters->locations; l != NULL; l = l->next) {
		 ...
		 */
		uri = athena_file_get_activation_uri (location->file);
		if (uri != NULL) {
...
static void
activation_mountable_mounted (AthenaFile  *file,
			      ...
			      gpointer       callback_data)
{
	...
	if (error == NULL) {
		...
		target_file = athena_file_get (result_location);
...
static void
activation_mountable_started (AthenaFile  *file,
			      GFile         *gfile_of_file,
			      GError        *error,
			      gpointer       callback_data)
{
 ...
 * @files: A GList of AthenaFiles to activate.
 * 
 **/
void
athena_mime_activate_files (GtkWindow *parent_window,
			      AthenaWindowSlot *slot,
			      GList *files,
			      ...
			      gboolean user_confirmation)
{
	...
	if (file_count == 1) {
		file_name = athena_file_get_display_name (files->data);
 ...
 * @view: FMDirectoryView in question.
 * @file: A AthenaFile representing the file in this view to activate.
 * @use_new_window: Should this item be opened in a new window?
 * 
 **/

void
athena_mime_activate_file (GtkWindow *parent_window,
			     AthenaWindowSlot *slot,
			     AthenaFile *file,
			     const char *launch_directory,
			     AthenaWindowOpenFlags flags)
{
	...
	athena_mime_activate_files (parent_window, slot, files, launch_directory, flags, FALSE);
	g_list_free (files);
}
