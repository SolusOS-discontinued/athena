...
/*
 * athena-view-dnd.c: DnD helpers for AthenaView
 *
...
static inline void
view_widget_to_file_operation_position (AthenaView *view,
                                        GdkPoint *position)
{
	AthenaViewClass *class = ATHENA_VIEW_GET_CLASS (view);
...
static void
view_widget_to_file_operation_position_xy (AthenaView *view,
                                           int *x, int *y)
{
	GdkPoint position;
				  ...
				  gpointer user_data)
{
	...
	athena_link_local_create (data->target_uri,
				    link_name,
				    link_display_name,
				    icon_name,
				    data->url,
...
void
athena_view_handle_netscape_url_drop (AthenaView  *view,
                                        ...
                                        int            y)
{
	...
	if (action == GDK_ACTION_LINK) {
		...
		athena_view_move_copy_items (view, uri_list, points,
                                               target_uri,
                                               action, x, y);
...
void
athena_view_handle_uri_list_drop (AthenaView  *view,
                                    ...
                                    int            y)
{
	...
	container_uri = NULL;
	if (target_uri == NULL) {
		container_uri = athena_view_get_backing_uri (view);
		g_assert (container_uri != NULL);
	}

	if (action == GDK_ACTION_ASK) {
		action = athena_drag_drop_action_ask
			(GTK_WIDGET (view),
	...
	athena_view_move_copy_items (view, real_uri_list, points,
...
void
athena_view_handle_text_drop (AthenaView  *view,
                                const char    *text,
                                const char    *target_uri,
                                GdkDragAction  action,
                                int            x,
                                int            y)
{
	...
	athena_view_new_file_with_initial_contents (
...
void
athena_view_handle_raw_drop (AthenaView *view,
                               const char   *raw_data,
                               int           length,
                               const char   *target_uri,
...
void 
athena_view_drop_proxy_received_uris (AthenaView *view,
					...
					GdkDragAction action)
{
	...
	athena_view_move_copy_items (view, source_uri_list, NULL,
				       target_uri != NULL ? target_uri : container_uri,
				       action, 0, 0);
