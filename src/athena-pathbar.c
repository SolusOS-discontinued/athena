...
#define ATHENA_PATH_BAR_ICON_SIZE 16
...
G_DEFINE_TYPE (AthenaPathBar, athena_path_bar,
	       GTK_TYPE_CONTAINER);

static GFile* get_xdg_dir 				(GUserDirectory dir);
static void     athena_path_bar_scroll_up                (AthenaPathBar *path_bar);
static void     athena_path_bar_scroll_down              (AthenaPathBar *path_bar);
static void     athena_path_bar_stop_scrolling           (AthenaPathBar *path_bar);
static gboolean athena_path_bar_slider_button_press      (GtkWidget       *widget,
							    GdkEventButton  *event,
							    AthenaPathBar *path_bar);
static gboolean athena_path_bar_slider_button_release    (GtkWidget       *widget,
							    GdkEventButton  *event,
							    AthenaPathBar *path_bar);
static void     athena_path_bar_check_icon_theme         (AthenaPathBar *path_bar);
static void     athena_path_bar_update_button_appearance (ButtonData      *button_data);
static void     athena_path_bar_update_button_state      (ButtonData      *button_data,
							    gboolean         current_dir);
static gboolean athena_path_bar_update_path              (AthenaPathBar *path_bar,
							    GFile           *file_path,
							    gboolean         emit_signal);

static GtkWidget *
get_slider_button (AthenaPathBar  *path_bar,
...
static void
update_button_types (AthenaPathBar *path_bar)
{
	GList *list;
	GFile *path = NULL;
	...
	if (path != NULL) {
		athena_path_bar_update_path (path_bar, path, TRUE);
		g_object_unref (path);
	}
}
...
desktop_location_changed_callback (gpointer user_data)
{
	...
	path_bar = ATHENA_PATH_BAR (user_data);
	
	g_object_unref (path_bar->desktop_path);
	g_object_unref (path_bar->home_path);
	path_bar->desktop_path = athena_get_desktop_location ();
                        ...
                        AthenaPathBar *path_bar)
{
        ...
        for (list = path_bar->button_list; list; list = list->next) {
                ...
                if (g_file_equal (file, button_data->path)) {
                        ...
                        icon = athena_trash_monitor_get_icon ();
                        icon_info = athena_icon_info_lookup (icon, ATHENA_PATH_BAR_ICON_SIZE);                        
                        pixbuf = athena_icon_info_get_pixbuf_at_size (icon_info, ATHENA_PATH_BAR_ICON_SIZE);
...
slider_timeout (gpointer user_data)
{
	...
	if (gtk_widget_get_visible (GTK_WIDGET (path_bar))) {
		if (path_bar->drag_slider_timeout_for_up_button) {
			athena_path_bar_scroll_up (path_bar);
		} else {
			athena_path_bar_scroll_down (path_bar);
		}
	}
...
static void
athena_path_bar_slider_drag_motion (GtkWidget      *widget,
...
static void
athena_path_bar_slider_drag_leave (GtkWidget      *widget,
				     GdkDragContext *context,
				     unsigned int    time,
				     gpointer        user_data)
{
 ...
 * Ripped from athena-file.c (athena_file_is_user_special_directory) and slightly modified
 */
static GFile*
...
static void
athena_path_bar_init (AthenaPathBar *path_bar)
{
	char *p;
	...
	p = athena_get_desktop_directory ();
	...
	g_signal_connect_swapped (athena_preferences, "changed::" ATHENA_PREFERENCES_DESKTOP_IS_HOME_DIR,
        ...
        g_signal_connect_swapped (path_bar->up_slider_button, "clicked", G_CALLBACK (athena_path_bar_scroll_up), path_bar);
        g_signal_connect_swapped (path_bar->down_slider_button, "clicked", G_CALLBACK (athena_path_bar_scroll_down), path_bar);

        g_signal_connect (path_bar->up_slider_button, "button_press_event", G_CALLBACK (athena_path_bar_slider_button_press), path_bar);
        g_signal_connect (path_bar->up_slider_button, "button_release_event", G_CALLBACK (athena_path_bar_slider_button_release), path_bar);
        g_signal_connect (path_bar->down_slider_button, "button_press_event", G_CALLBACK (athena_path_bar_slider_button_press), path_bar);
        g_signal_connect (path_bar->down_slider_button, "button_release_event", G_CALLBACK (athena_path_bar_slider_button_release), path_bar);
	...
	g_signal_connect (path_bar->up_slider_button,
			  "drag-motion",
			  G_CALLBACK (athena_path_bar_slider_drag_motion),
			  path_bar);
	g_signal_connect (path_bar->up_slider_button,
			  "drag-leave",
			  G_CALLBACK (athena_path_bar_slider_drag_leave),
			  path_bar);
	...
	g_signal_connect (path_bar->down_slider_button,
        ...
        g_signal_connect (athena_trash_monitor_get (),
                          "trash_state_changed",
...
static void
athena_path_bar_finalize (GObject *object)
{
        AthenaPathBar *path_bar;

        path_bar = ATHENA_PATH_BAR (object);

	athena_path_bar_stop_scrolling (path_bar);
	...
	g_signal_handlers_disconnect_by_func (athena_trash_monitor_get (),
					      trash_state_changed_cb, path_bar);
	g_signal_handlers_disconnect_by_func (athena_preferences,
					      desktop_location_changed_callback,
					      path_bar);

        G_OBJECT_CLASS (athena_path_bar_parent_class)->finalize (object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void
remove_settings_signal (AthenaPathBar *path_bar,
			GdkScreen  *screen)
{
...
static void
athena_path_bar_dispose (GObject *object)
{
        remove_settings_signal (ATHENA_PATH_BAR (object), gtk_widget_get_screen (GTK_WIDGET (object)));

        G_OBJECT_CLASS (athena_path_bar_parent_class)->dispose (object);
}
 ...
 */
static void
athena_path_bar_get_preferred_width (GtkWidget *widget,
				       gint      *minimum,
				       gint      *natural)
{
	ButtonData *button_data;
	...
	path_bar = ATHENA_PATH_BAR (widget);
...
static void
athena_path_bar_get_preferred_height (GtkWidget *widget,
					gint      *minimum,
					gint      *natural)
{
	ButtonData *button_data;
...
static void
athena_path_bar_update_slider_buttons (AthenaPathBar *path_bar)
{
...
static void
athena_path_bar_unmap (GtkWidget *widget)
{
	athena_path_bar_stop_scrolling (ATHENA_PATH_BAR (widget));
	gdk_window_hide (ATHENA_PATH_BAR (widget)->event_window);

	GTK_WIDGET_CLASS (athena_path_bar_parent_class)->unmap (widget);
}

static void
athena_path_bar_map (GtkWidget *widget)
{
	gdk_window_show (ATHENA_PATH_BAR (widget)->event_window);

	GTK_WIDGET_CLASS (athena_path_bar_parent_class)->map (widget);
}

/* This is a tad complicated */
static void
athena_path_bar_size_allocate (GtkWidget     *widget,
			    	 GtkAllocation *allocation)
{
	...
	if (need_sliders) {
        	...
        	gtk_widget_size_allocate (path_bar->down_slider_button, &child_allocation);
      		...
      		athena_path_bar_update_slider_buttons (path_bar);
    	} else {
...
static void
athena_path_bar_style_updated (GtkWidget *widget)
{
	GTK_WIDGET_CLASS (athena_path_bar_parent_class)->style_updated (widget);

        athena_path_bar_check_icon_theme (ATHENA_PATH_BAR (widget));
}

static void
athena_path_bar_screen_changed (GtkWidget *widget,
			          GdkScreen *previous_screen)
{
        if (GTK_WIDGET_CLASS (athena_path_bar_parent_class)->screen_changed) {
                GTK_WIDGET_CLASS (athena_path_bar_parent_class)->screen_changed (widget, previous_screen);
	}
        /* We might nave a new settings, so we remove the old one */
        if (previous_screen) {
                remove_settings_signal (ATHENA_PATH_BAR (widget), previous_screen);
	}
        athena_path_bar_check_icon_theme (ATHENA_PATH_BAR (widget));
}

static gboolean
athena_path_bar_scroll (GtkWidget      *widget,
...
static void
athena_path_bar_realize (GtkWidget *widget)
{
	AthenaPathBar *path_bar;
	GtkAllocation allocation;
	GdkWindow *window;
...
static void
athena_path_bar_unrealize (GtkWidget *widget)
{
	AthenaPathBar *path_bar;
	...
	GTK_WIDGET_CLASS (athena_path_bar_parent_class)->unrealize (widget);
}

static void
athena_path_bar_add (GtkContainer *container,
		       GtkWidget    *widget)
{
...
static void
athena_path_bar_remove_1 (GtkContainer *container,
		       	    GtkWidget    *widget)
{
...
static void
athena_path_bar_remove (GtkContainer *container,
		          GtkWidget    *widget)
{
        AthenaPathBar *path_bar;
        GList *children;

        path_bar = ATHENA_PATH_BAR (container);

        if (widget == path_bar->up_slider_button) {
                athena_path_bar_remove_1 (container, widget);
...
static void
athena_path_bar_forall (GtkContainer *container,
...
static void
athena_path_bar_grab_notify (GtkWidget *widget,
...
static void
athena_path_bar_state_changed (GtkWidget    *widget,
...
static void
athena_path_bar_class_init (AthenaPathBarClass *path_bar_class)
{
        ...
        gobject_class->finalize = athena_path_bar_finalize;
        gobject_class->dispose = athena_path_bar_dispose;

	widget_class->get_preferred_height = athena_path_bar_get_preferred_height;
	widget_class->get_preferred_width = athena_path_bar_get_preferred_width;
	widget_class->realize = athena_path_bar_realize;
	widget_class->unrealize = athena_path_bar_unrealize;
	widget_class->unmap = athena_path_bar_unmap;
	widget_class->map = athena_path_bar_map;
        widget_class->size_allocate = athena_path_bar_size_allocate;
        widget_class->style_updated = athena_path_bar_style_updated;
        widget_class->screen_changed = athena_path_bar_screen_changed;
        widget_class->grab_notify = athena_path_bar_grab_notify;
        widget_class->state_changed = athena_path_bar_state_changed;
	widget_class->scroll_event = athena_path_bar_scroll;

        container_class->add = athena_path_bar_add;
        container_class->forall = athena_path_bar_forall;
        container_class->remove = athena_path_bar_remove;

        path_bar_signals [PATH_CLICKED] =
                g_signal_new ("path-clicked",
		  G_OBJECT_CLASS_TYPE (path_bar_class),
		  G_SIGNAL_RUN_FIRST,
		  G_STRUCT_OFFSET (AthenaPathBarClass, path_clicked),
		  NULL, NULL,
	 ...
	 path_bar_signals [PATH_SET] =
		g_signal_new ("path-set",
		  G_OBJECT_CLASS_TYPE (path_bar_class),
		  G_SIGNAL_RUN_FIRST,
		  G_STRUCT_OFFSET (AthenaPathBarClass, path_set),
		  NULL, NULL,
		  g_cclosure_marshal_VOID__OBJECT,
		  G_TYPE_NONE, 1,
		  G_TYPE_FILE);
...
static void
athena_path_bar_scroll_down (AthenaPathBar *path_bar)
{
        GList *list;
        GList *down_button;
        GList *up_button;
...
static void
athena_path_bar_scroll_up (AthenaPathBar *path_bar)
{
        GList *list;
...
static gboolean
athena_path_bar_scroll_timeout (AthenaPathBar *path_bar)
{
        ...
        if (path_bar->timer) {
			...
			path_bar->need_timer = FALSE;
	  		...
	  		path_bar->timer = g_timeout_add (SCROLL_TIMEOUT,
				   			 (GSourceFunc)athena_path_bar_scroll_timeout,
				   			 path_bar);
...
static void 
athena_path_bar_stop_scrolling (AthenaPathBar *path_bar)
{
        if (path_bar->timer) {
...
static gboolean
athena_path_bar_slider_button_press (GtkWidget       *widget, 
	   			       GdkEventButton  *event,
				       AthenaPathBar *path_bar)
{
...
static gboolean
athena_path_bar_slider_button_release (GtkWidget      *widget, 
  				         GdkEventButton *event,
				         AthenaPathBar     *path_bar)
{
...
static void
reload_icons (AthenaPathBar *path_bar)
{
        GList *list;
		...
		if (button_data->type != NORMAL_BUTTON || button_data->is_base_dir) {
                	athena_path_bar_update_button_appearance (button_data);
		}
...
static void
change_icon_theme (AthenaPathBar *path_bar)
{
	path_bar->icon_size = ATHENA_PATH_BAR_ICON_SIZE;
...
static void
athena_path_bar_check_icon_theme (AthenaPathBar *path_bar)
{
        GtkSettings *settings;
...
void
athena_path_bar_clear_buttons (AthenaPathBar *path_bar)
{
		   ...
		   gpointer   data)
{
        ...
        path_bar = ATHENA_PATH_BAR (gtk_widget_get_parent (button));
...
get_type_icon_info (ButtonData *button_data)
{
	switch (button_data->type)
        {
		case ROOT_BUTTON:
			return athena_icon_info_lookup_from_name (ATHENA_ICON_FILESYSTEM,
								    ATHENA_PATH_BAR_ICON_SIZE);

		case HOME_BUTTON:
			return athena_icon_info_lookup_from_name (ATHENA_ICON_HOME,
								    ATHENA_PATH_BAR_ICON_SIZE);

                case DESKTOP_BUTTON:
			return athena_icon_info_lookup_from_name (ATHENA_ICON_DESKTOP,
								    ATHENA_PATH_BAR_ICON_SIZE);
		case XDG_BUTTON:
			return athena_icon_info_lookup_from_name (button_data->xdg_icon,
							    ATHENA_PATH_BAR_ICON_SIZE);								
                case NORMAL_BUTTON:
			if (button_data->is_base_dir) {
				return athena_file_get_icon (button_data->file,
							       ATHENA_PATH_BAR_ICON_SIZE,
							       ATHENA_FILE_ICON_FLAGS_NONE);
			}
	    	default:
			return NULL;
        }
...
button_data_free (ButtonData *button_data)
{
	...
	if (button_data->file != NULL) {
		...
		athena_file_monitor_remove (button_data->file, button_data);
		athena_file_unref (button_data->file);
	}
...
static void
athena_path_bar_update_button_appearance (ButtonData *button_data)
{
	AthenaIconInfo *icon_info;
	GdkPixbuf *pixbuf;
        const gchar *dir_name = get_dir_name (button_data);
...
static void
athena_path_bar_update_button_state (ButtonData *button_data,
				       gboolean    current_dir)
{
	if (button_data->label != NULL) {
	...
	for (l = mounts; l != NULL; l = l->next) {
		...
		if (g_file_equal (location, root)) {
			...
			if (button_data) {
				...
				if (icon == NULL) {
					icon = g_themed_icon_new (ATHENA_ICON_FOLDER);
				}
				info = athena_icon_info_lookup (icon, ATHENA_PATH_BAR_ICON_SIZE);
				g_object_unref (icon);
				button_data->custom_icon = athena_icon_info_get_pixbuf_at_size (info, ATHENA_PATH_BAR_ICON_SIZE);
				g_object_unref (info);
...
static void
button_data_file_changed (AthenaFile *file,
			  ButtonData *button_data)
{
	...
	path_bar = (AthenaPathBar *) gtk_widget_get_ancestor (button_data->button,
								ATHENA_TYPE_PATH_BAR);
	if (path_bar == NULL) {
		return;
	}
	...
	if (!g_file_equal (button_data->path, location)) {
		...
		if (renamed) {
			...
			if (child) {
				 ...
				 */
				current_location = athena_file_get_location (current_button_data->file);
			}

        		athena_path_bar_update_path (path_bar, location, FALSE);
        		athena_path_bar_set_path (path_bar, current_location);
			g_object_unref (location);
			g_object_unref (current_location);
			return;
		}
	} else if (athena_file_is_gone (file)) {
		gint idx, position;
	...
	if (button_data->type != MOUNT_BUTTON) {
		display_name = athena_file_get_display_name (file);
...
static ButtonData *
make_directory_button (AthenaPathBar  *path_bar,
		       AthenaFile     *file,
		       gboolean          current_dir,
		       gboolean          base_dir)
{
	...
	path = athena_file_get_location (file);
	child = NULL;
	...
	if (button_data->dir_name == NULL) {
		button_data->dir_name = athena_file_get_display_name (file);
	}
	if (button_data->file == NULL) {
		button_data->file = athena_file_ref (file);
		athena_file_monitor_add (button_data->file, button_data,
        ...
        athena_path_bar_update_button_state (button_data, current_dir);
	...
	athena_drag_slot_proxy_init (button_data->button, button_data->file, NULL);
...
static gboolean
athena_path_bar_check_parent_path (AthenaPathBar *path_bar,
				     GFile *location,
				     ButtonData **current_button_data)
{
        GList *list;
		...
		*current_button_data = NULL;
		...
		if (need_new_fake_root) {
			path_bar->fake_root = NULL;
	  		...
	  		athena_path_bar_update_button_state (BUTTON_DATA (list->data),
...
static gboolean
athena_path_bar_update_path (AthenaPathBar *path_bar,
			       GFile *file_path,
			       gboolean emit_signal)
{
	AthenaFile *file, *parent_file;
        gboolean first_directory, last_directory;
        gboolean result;
        GList *new_buttons, *l, *fake_root;
	ButtonData *button_data, *current_button_data;

        g_return_val_if_fail (ATHENA_IS_PATH_BAR (path_bar), FALSE);
	...
	file = athena_file_get (file_path);
        ...
        athena_path_bar_clear_buttons (path_bar);
...
gboolean
athena_path_bar_set_path (AthenaPathBar *path_bar, GFile *file_path)
{
	ButtonData *button_data;
        ...
        if (athena_path_bar_check_parent_path (path_bar, file_path, &button_data)) {
		if (path_bar->current_path != NULL) {
			g_object_unref (path_bar->current_path);
		}
	...
	return athena_path_bar_update_path (path_bar, file_path, TRUE);
}

GFile *
athena_path_bar_get_path_for_button (AthenaPathBar *path_bar,
				       GtkWidget       *button)
{
	GList *list;
 
	g_return_val_if_fail (ATHENA_IS_PATH_BAR (path_bar), NULL);
