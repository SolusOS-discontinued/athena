...
#include "athena-desktop-item-properties.h"
...
#include <libathena-private/athena-mime-application-chooser.h>
#include <libathena-private/athena-module.h>
#include <libathena-private/athena-undo-signal-handlers.h>
...
struct AthenaPropertiesWindowDetails {	
	...
	AthenaFile *group_change_file;
	char         *group_change_group;
	unsigned int  group_change_timeout;
	AthenaFile *owner_change_file;
...
static void directory_contents_value_field_update (AthenaPropertiesWindow *window);
static void file_changed_callback                 (AthenaFile       *file,
						   gpointer            user_data);
static void permission_button_update              (AthenaPropertiesWindow *window,
						   GtkToggleButton    *button);
static void permission_combo_update               (AthenaPropertiesWindow *window,
						   GtkComboBox        *combo);
static void value_field_update                    (AthenaPropertiesWindow *window,
						   GtkLabel           *field);
static void properties_window_update              (AthenaPropertiesWindow *window,
						   GList              *files);
static void is_directory_ready_callback           (AthenaFile       *file,
						   gpointer            data);
static void cancel_group_change_callback          (AthenaPropertiesWindow *window);
static void cancel_owner_change_callback          (AthenaPropertiesWindow *window);
static void parent_widget_destroyed_callback      (GtkWidget          *widget,
						   gpointer            callback_data);
static void select_image_button_callback          (GtkWidget          *widget,
						   AthenaPropertiesWindow *properties_window);
...
static void append_extension_pages                (AthenaPropertiesWindow *window);

static gboolean name_field_focus_out              (AthenaEntry *name_field,
						   GdkEventFocus *event,
						   gpointer callback_data);
static void name_field_activate                   (AthenaEntry *name_field,
						   gpointer callback_data);
static GtkLabel *attach_ellipsizing_value_label   (GtkGrid *grid,
						   GtkWidget *sibling,
						   const char *initial_text);
						   
static GtkWidget* create_pie_widget 		  (AthenaPropertiesWindow *window);

G_DEFINE_TYPE (AthenaPropertiesWindow, athena_properties_window, GTK_TYPE_DIALOG);

static gboolean
is_multi_file_window (AthenaPropertiesWindow *window)
{
	GList *l;
	int count;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		if (!athena_file_is_gone (ATHENA_FILE (l->data))) {			
			count++;
			if (count > 1) {
				return TRUE;
			}	
...
static int
get_not_gone_original_file_count (AthenaPropertiesWindow *window)
{
	GList *l;
	int count;
...
static AthenaFile *
get_original_file (AthenaPropertiesWindow *window) 
{
	...
	return ATHENA_FILE (window->details->original_files->data);
}

static AthenaFile *
get_target_file_for_original_file (AthenaFile *file)
{
	...
	if (ATHENA_IS_DESKTOP_ICON_FILE (file)) {
		...
		if (link != NULL) {
			...
			if (location) {
				target_file = athena_file_get (location);
		...
		uri_to_display = athena_file_get_activation_uri (file);
		if (uri_to_display != NULL) {
			target_file = athena_file_get_by_uri (uri_to_display);
...
static AthenaFile *
get_target_file (AthenaPropertiesWindow *window)
{
	return ATHENA_FILE (window->details->target_files->data);
}
...
static void
get_image_for_properties_window (AthenaPropertiesWindow *window,
				 char **icon_name,
				 GdkPixbuf **icon_pixbuf)
{
	AthenaIconInfo *icon, *new_icon;
	GList *l;
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		...
		if (!icon) {
			icon = athena_file_get_icon (file, ATHENA_ICON_SIZE_STANDARD, ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS | ATHENA_FILE_ICON_FLAGS_IGNORE_VISITING);
		} else {
			new_icon = athena_file_get_icon (file, ATHENA_ICON_SIZE_STANDARD, ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS | ATHENA_FILE_ICON_FLAGS_IGNORE_VISITING);
	...
	if (!icon) {
		icon = athena_icon_info_lookup_from_name ("text-x-generic", ATHENA_ICON_SIZE_STANDARD);
	}

	if (icon_name != NULL) {
		*icon_name = g_strdup (athena_icon_info_get_used_name (icon));
	}

	if (icon_pixbuf != NULL) {
		*icon_pixbuf = athena_icon_info_get_pixbuf_at_size (icon, ATHENA_ICON_SIZE_STANDARD);
	}
...
static void
reset_icon (AthenaPropertiesWindow *properties_window)
{
	GList *l;
...
static void  
athena_properties_window_drag_data_received (GtkWidget *widget, GdkDragContext *context,
					       int x, int y,
					       ...
					       guint info, guint time)
{
	...
	if (!exactly_one) {
		...
		if (uri_is_local_image (uris[0])) {			
			set_icon (uris[0], ATHENA_PROPERTIES_WINDOW (window));
		} else {
			GFile *f;
...
static GtkWidget *
create_image_widget (AthenaPropertiesWindow *window,
		     gboolean is_customizable)
{
	...
	if (is_customizable) {
		...
		g_signal_connect (image, "drag_data_received",
				  G_CALLBACK (athena_properties_window_drag_data_received), NULL);
...
static void
set_name_field (AthenaPropertiesWindow *window,
		...
		const gchar *name)
{
	 ...
	 */
	use_label = is_multi_file_window (window) || !athena_file_can_rename (get_original_file (window));
	new_widget = !window->details->name_field || (use_label ? ATHENA_IS_ENTRY (window->details->name_field) : GTK_IS_LABEL (window->details->name_field));
	...
	if (new_widget) {
		...
		if (use_label) {
			...
			window->details->name_field = athena_entry_new ();
			 ...
			 * we've removed Undo from Athena for now, I'm just ifdeffing out this
			...
			athena_undo_set_up_athena_entry_for_undo ( ATHENA_ENTRY (window->details->name_field));
			athena_undo_editable_set_undo_key (GTK_EDITABLE (window->details->name_field), TRUE);
#endif
...
static void
update_name_field (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	...
	if (is_multi_file_window (window)) {
		...
		if (file == NULL || athena_file_is_gone (file)) {
			...
			current_name = athena_file_get_display_name (file);
		}
...
static void
name_field_restore_original_name (AthenaEntry *name_field)
{
	...
	athena_entry_select_all (name_field);

	g_free (displayed_name);
}

static void
rename_callback (AthenaFile *file, GFile *res_loc, GError *error, gpointer callback_data)
{
	AthenaPropertiesWindow *window;

	window = ATHENA_PROPERTIES_WINDOW (callback_data);

	/* Complain to user if rename failed. */
	if (error != NULL) {
		athena_report_error_renaming_file (file, 
						     window->details->pending_name, 
						     error,
						     GTK_WINDOW (window));
		if (window->details->name_field != NULL) {
			name_field_restore_original_name (ATHENA_ENTRY (window->details->name_field));
		}
	}
...
static void
set_pending_name (AthenaPropertiesWindow *window, const char *name)
{
...
static void
name_field_done_editing (AthenaEntry *name_field, AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *new_name;
	const char *original_name;
	
	g_return_if_fail (ATHENA_IS_ENTRY (name_field));
	...
	if (strlen (new_name) == 0) {
		name_field_restore_original_name (ATHENA_ENTRY (name_field));
	} else {
...
static gboolean
name_field_focus_out (AthenaEntry *name_field,
		      GdkEventFocus *event,
		      gpointer callback_data)
{
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (callback_data));

	if (gtk_widget_get_sensitive (GTK_WIDGET (name_field))) {
		name_field_done_editing (name_field, ATHENA_PROPERTIES_WINDOW (callback_data));
	}
...
static void
name_field_activate (AthenaEntry *name_field, gpointer callback_data)
{
	g_assert (ATHENA_IS_ENTRY (name_field));
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (callback_data));

	/* Accept changes. */
	name_field_done_editing (name_field, ATHENA_PROPERTIES_WINDOW (callback_data));

	athena_entry_select_all_at_idle (name_field);
}

static void
update_properties_window_title (AthenaPropertiesWindow *window)
{
	char *name, *title;
	AthenaFile *file;
...
static void
clear_extension_pages (AthenaPropertiesWindow *window)
{
	int i;
	int num_pages;
	GtkWidget *page;
...
static void
refresh_extension_pages (AthenaPropertiesWindow *window)
{
...
static void
remove_from_dialog (AthenaPropertiesWindow *window,
		    AthenaFile *file)
{
	int index;
	GList *original_link;
	...
	original_file = ATHENA_FILE (original_link->data);
	target_file = ATHENA_FILE (target_link->data);
	...
	athena_file_monitor_remove (original_file, &window->details->original_files);
	athena_file_monitor_remove (target_file, &window->details->target_files);
...
static GList *
get_mime_list (AthenaPropertiesWindow *window)
{
	GList *ret;
	GList *l;
	...
	ret = NULL;
	for (l = window->details->target_files; l != NULL; l = l->next) {
		ret = g_list_append (ret, athena_file_get_mime_type (ATHENA_FILE (l->data)));
	}
...
static void
properties_window_update (AthenaPropertiesWindow *window, 
			  GList *files)
{
	GList *l;
	GList *mime_list;
	GList *tmp;
	AthenaFile *changed_file;
	...
	for (tmp = files; tmp != NULL; tmp = tmp->next) {
		changed_file = ATHENA_FILE (tmp->data);

		if (changed_file && athena_file_is_gone (changed_file)) {
...
update_files_callback (gpointer data)
{
 	...
 	window = ATHENA_PROPERTIES_WINDOW (data);
	...
	if (window->details->original_files == NULL) {
		...
		athena_file_list_free (window->details->changed_files);
...
static void
schedule_files_update (AthenaPropertiesWindow *window)
 {
 	g_assert (ATHENA_IS_PROPERTIES_WINDOW (window));
...
file_list_attributes_identical (GList *file_list, const char *attribute_name)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (first_attr == NULL) {
			first_attr = athena_file_get_string_attribute_with_default (file, attribute_name);
		} else {
			char *attr;
			attr = athena_file_get_string_attribute_with_default (file, attribute_name);
				...
				const char *inconsistent_value)
{
	if (file_list_attributes_identical (file_list, attribute_name)) {
		...
		for (l = file_list; l != NULL; l = l->next) {
			...
			file = ATHENA_FILE (l->data);
			if (!athena_file_is_gone (file)) {
				return athena_file_get_string_attribute_with_default
					(file, 
					 attribute_name);
			}
		}
...
file_list_all_directories (GList *file_list)
{
	GList *l;
	for (l = file_list; l != NULL; l = l->next) {
		if (!athena_file_is_directory (ATHENA_FILE (l->data))) {
			return FALSE;
		}
	}
	return TRUE;
...
static void
value_field_update (AthenaPropertiesWindow *window, GtkLabel *label)
{
...
static GtkWidget*
attach_value_field_internal (AthenaPropertiesWindow *window,
			     GtkGrid *grid,
			     GtkWidget *sibling,
...
static GtkWidget*
attach_value_field (AthenaPropertiesWindow *window,
		    GtkGrid *grid,
		    GtkWidget *sibling,
		    const char *file_attribute_name,
		    const char *inconsistent_string,
...
static GtkWidget*
attach_ellipsizing_value_field (AthenaPropertiesWindow *window,
				GtkGrid *grid,
				GtkWidget *sibling,
		    		const char *file_attribute_name,
				const char *inconsistent_string,
...
static void
group_change_callback (AthenaFile *file,
		       GFile *res_loc,
		       GError *error,
		       AthenaPropertiesWindow *window)
{
	...
	athena_report_error_setting_group (file, error, GTK_WINDOW (window));
...
static void
cancel_group_change_callback (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *group;
	...
	athena_file_cancel (file, (AthenaFileOperationCallback) group_change_callback, window);
...
static gboolean
schedule_group_change_timeout (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *group;
	...
	athena_file_set_group
		(file,  group,
		 (AthenaFileOperationCallback) group_change_callback, window);
...
static void
schedule_group_change (AthenaPropertiesWindow *window,
		       AthenaFile       *file,
		       const char         *group)
{
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (window));
	...
	window->details->group_change_file = athena_file_ref (file);
...
static void
unschedule_or_cancel_group_change (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *group;
...
static void
changed_group_callback (GtkComboBox *combo_box, AthenaFile *file)
{
	AthenaPropertiesWindow *window;
	char *group;
	char *cur_group;
	...
	cur_group = athena_file_get_group_name (file);

	if (group != NULL && strcmp (group, cur_group) != 0) {
		/* Try to change file group. If this fails, complain to user. */
		window = ATHENA_PROPERTIES_WINDOW (gtk_widget_get_ancestor (GTK_WIDGET (combo_box), GTK_TYPE_WINDOW));
...
static void
synch_groups_combo_box (GtkComboBox *combo_box, AthenaFile *file)
{
	GList *groups;
	GList *node;
	GtkTreeModel *model;
	...
	groups = athena_file_get_settable_group_names (file);
	...
	current_group_name = athena_file_get_group_name (file);
		        ...
		        AthenaFile *file)
{
	...
	g_signal_connect_data (combo_box, "changed",
			       ...
			       athena_file_ref (file),
			       (GClosureNotify)athena_file_unref, 0);

	return combo_box;
}	

static void
owner_change_callback (AthenaFile *file,
		       ...
		       AthenaPropertiesWindow *window)
{
	...
	athena_report_error_setting_owner (file, error, GTK_WINDOW (window));
...
static void
cancel_owner_change_callback (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *owner;
	...
	athena_file_cancel (file, (AthenaFileOperationCallback) owner_change_callback, window);
...
static gboolean
schedule_owner_change_timeout (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *owner;
	...
	athena_file_set_owner
		(file,  owner,
		 (AthenaFileOperationCallback) owner_change_callback, window);
...
static void
schedule_owner_change (AthenaPropertiesWindow *window,
		       AthenaFile       *file,
		       const char         *owner)
{
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (window));
	...
	window->details->owner_change_file = athena_file_ref (file);
...
static void
unschedule_or_cancel_owner_change (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
	char *owner;
...
static void
changed_owner_callback (GtkComboBox *combo_box, AthenaFile* file)
{
	AthenaPropertiesWindow *window;
	char *owner_text;
	char **name_array;
	...
	g_free (owner_text);
	cur_owner = athena_file_get_owner_name (file);
...
static void
synch_user_menu (GtkComboBox *combo_box, AthenaFile *file)
{
	GList *users;
	GList *node;
	GtkTreeModel *model;
	...
	owner_name = athena_file_get_string_attribute (file, "owner");
...
static gboolean
file_has_prefix (AthenaFile *file,
		 GList *prefix_candidates)
{
	...
	for (p = prefix_candidates; p != NULL; p = p->next) {
		...
		candidate_location = athena_file_get_location (ATHENA_FILE (p->data));
...
static void
directory_contents_value_field_update (AthenaPropertiesWindow *window)
{
	AthenaRequestStatus file_status, status;
	char *text, *temp;
	...
	status = ATHENA_REQUEST_DONE;
	file_status = ATHENA_REQUEST_NOT_STARTED;
	...
	for (l = window->details->target_files; l; l = l->next) {
		...
		if (athena_file_is_directory (file)) {
			file_status = athena_file_get_deep_counts (file, 
					 &directory_count,
					 &file_count, 
					 &file_unreadable,
					 &file_size,
			...
			if (file_status != ATHENA_REQUEST_DONE) {
				status = file_status;
			}
		} else {
			++total_count;
			total_size += athena_file_get_size (file);
		}
	}
...
static void
schedule_directory_contents_update (AthenaPropertiesWindow *window)
{
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (window));
...
static GtkLabel *
attach_directory_contents_value_field (AthenaPropertiesWindow *window,
				       GtkGrid *grid,
				       GtkWidget *sibling)
{
	GtkLabel *value_field;
	...
	for (l = window->details->target_files; l; l = l->next) {
		file = ATHENA_FILE (l->data);
		athena_file_recompute_deep_counts (file);
...
static void
append_title_value_pair (AthenaPropertiesWindow *window,
			 GtkGrid *grid,
			 const char *title, 
 			 const char *file_attribute_name,
...
static void
append_title_and_ellipsizing_value (AthenaPropertiesWindow *window,
				    GtkGrid *grid,
				    const char *title,
				    const char *file_attribute_name,
				    const char *inconsistent_state,
...
static void
append_directory_contents_fields (AthenaPropertiesWindow *window,
				  GtkGrid *grid)
{
...
static gboolean
is_merged_trash_directory (AthenaFile *file) 
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_computer_directory (AthenaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_network_directory (AthenaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
is_burn_directory (AthenaFile *file)
{
	char *file_uri;
	gboolean result;
...
static gboolean
should_show_custom_icon_buttons (AthenaPropertiesWindow *window) 
{
	if (is_multi_file_window (window)) {
		return FALSE;
	}
...
static gboolean
should_show_file_type (AthenaPropertiesWindow *window) 
{
	if (!is_multi_file_window (window) 
...
static gboolean
should_show_location_info (AthenaPropertiesWindow *window) 
{
	if (!is_multi_file_window (window) 
	    && (is_merged_trash_directory (get_target_file (window)) ||
		is_computer_directory (get_target_file (window)) ||
...
static gboolean
should_show_accessed_date (AthenaPropertiesWindow *window) 
{
...
static gboolean
should_show_link_target (AthenaPropertiesWindow *window)
{
	if (!is_multi_file_window (window)
	    && athena_file_is_symbolic_link (get_target_file (window))) {
		return TRUE;
	}
...
static gboolean
should_show_free_space (AthenaPropertiesWindow *window)
{
...
static gboolean
should_show_volume_usage (AthenaPropertiesWindow *window)
{
	AthenaFile 		*file;
...
static GtkWidget* 
create_pie_widget (AthenaPropertiesWindow *window)
{
	AthenaFile		*file;
	GtkGrid                 *grid;
...
static GtkWidget*
create_volume_usage_widget (AthenaPropertiesWindow *window)
{
	GtkWidget *piewidget;
	gchar *uri;
	AthenaFile *file;
...
static void
create_basic_page (AthenaPropertiesWindow *window)
{
	GtkGrid *grid;
	...
	hbox = create_page_with_hbox (window->details->notebook, _("Basic"),
				      "help:gnome-help/athena-file-properties-basic");
	...
	if (ATHENA_IS_ENTRY (window->details->name_field)) {
		athena_entry_select_all (ATHENA_ENTRY (window->details->name_field));
		gtk_widget_grab_focus (GTK_WIDGET (window->details->name_field));
	}

	if (athena_desktop_item_properties_should_show (window->details->target_files)) {
		...
		gtk_size_group_add_widget (label_size_group,
					   GTK_WIDGET (window->details->name_label));
		box = athena_desktop_item_properties_make_box (label_size_group,
								 window->details->target_files);
	...
	if (is_multi_file_window (window) ||
	    athena_file_is_directory (get_target_file (window))) {
...
static gboolean 
files_has_directory (AthenaPropertiesWindow *window)
{
	GList *l;
...
static gboolean 
files_has_changable_permissions_directory (AthenaPropertiesWindow *window)
{
	GList *l;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (athena_file_is_directory (file) &&
		    ...
		    athena_file_can_set_permissions (file)) {
			return TRUE;
		}
...
static gboolean 
files_has_file (AthenaPropertiesWindow *window)
{
	GList *l;
...
static void
start_long_operation (AthenaPropertiesWindow *window)
{
...
static void
end_long_operation (AthenaPropertiesWindow *window)
{
...
static void
permission_change_callback (AthenaFile *file,
			    GFile *res_loc,
			    GError *error,
			    gpointer callback_data)
{
	...
	/* Report the error if it's an error. */
	athena_report_error_setting_permissions (file, error, NULL);

	g_object_unref (window);
}

static void
update_permissions (AthenaPropertiesWindow *window,
		    ...
		    gboolean use_original)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (!apply_to_both_folder_and_dir &&
		    ((athena_file_is_directory (file) && !is_folder) ||
		     (!athena_file_is_directory (file) && is_folder))) {
			continue;
		}

		permissions = athena_file_get_permissions (file);
		if (use_original) {
			gpointer ptr;
...
static gboolean
initial_permission_state_consistent (AthenaPropertiesWindow *window,
				     guint32 mask,
				     gboolean is_folder,
...
static void
permission_button_update (AthenaPropertiesWindow *window,
			  GtkToggleButton *button)
{
	GList *l;
	gboolean all_set;
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		file_permissions = athena_file_get_permissions (file);
...
static void
set_up_permissions_checkbox (AthenaPropertiesWindow *window,
...
static GtkWidget *
add_permissions_checkbox_with_label (AthenaPropertiesWindow *window,
				     GtkGrid *grid,
				     GtkWidget *sibling,
				     const char *label,
...
static GtkWidget *
add_permissions_checkbox (AthenaPropertiesWindow *window,
			  GtkGrid *grid,
			  GtkWidget *sibling,
			  CheckboxType type,
			  guint32 permission_to_check,
...
static void
permission_combo_changed (GtkWidget *combo, AthenaPropertiesWindow *window)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
permission_combo_update (AthenaPropertiesWindow *window,
			 GtkComboBox *combo)
{
	PermissionType type;
...
static void
add_permissions_combo_box (AthenaPropertiesWindow *window, GtkGrid *grid,
...
static GtkWidget *
append_special_execution_checkbox (AthenaPropertiesWindow *window,
				   GtkGrid *grid,
				   GtkWidget *sibling,
...
static void
append_special_execution_flags (AthenaPropertiesWindow *window, GtkGrid *grid)
{
	GtkWidget *title;
...
static void
create_simple_permissions (AthenaPropertiesWindow *window, GtkGrid *page_grid)
{
	...
	if (!is_multi_file_window (window) && athena_file_can_set_owner (get_target_file (window))) {
	...
	if (!is_multi_file_window (window) && athena_file_can_set_group (get_target_file (window))) {
...
static void
create_permission_checkboxes (AthenaPropertiesWindow *window,
			      GtkGrid *page_grid,
			      gboolean is_folder)
{
...
static void
create_advanced_permissions (AthenaPropertiesWindow *window, GtkGrid *page_grid)
{
	GtkLabel *group_label;
	GtkLabel *owner_label;
	GtkComboBox *group_combo_box;
			 ...
			 AthenaPropertiesWindow *window)
{
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		if (athena_file_is_directory (file) &&
			...
			start_long_operation (window);
			g_object_ref (window);
			athena_file_set_permissions_recursive (uri,
								 file_permission,
...
static void
create_permissions_page (AthenaPropertiesWindow *window)
{
	...
	vbox = create_page_with_vbox (window->details->notebook,
				      _("Permissions"),
				      "help:gnome-help/athena-file-properties-permissions");
	...
	if (all_can_get_permissions (file_list) && all_can_get_permissions (window->details->target_files)) {
		...
		if (g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_SHOW_ADVANCED_PERMISSIONS)) {
		...
		if (!is_multi_file_window (window)) {
			file_name = athena_file_get_display_name (get_target_file (window));
...
static void
append_extension_pages (AthenaPropertiesWindow *window)
{
	GList *providers;
	GList *p;
	
 	providers = athena_module_get_extensions_for_type (ATHENA_TYPE_PROPERTY_PAGE_PROVIDER);
	
	for (p = providers; p != NULL; p = p->next) {
		AthenaPropertyPageProvider *provider;
		GList *pages;
		GList *l;

		provider = ATHENA_PROPERTY_PAGE_PROVIDER (p->data);
		
		pages = athena_property_page_provider_get_pages 
			(provider, window->details->original_files);
		
		for (l = pages; l != NULL; l = l->next) {
			AthenaPropertyPage *page;
			GtkWidget *page_widget;
			GtkWidget *label;
			
			page = ATHENA_PROPERTY_PAGE (l->data);
...
static gboolean
should_show_permissions (AthenaPropertiesWindow *window) 
{
	AthenaFile *file;
...
get_pending_key (GList *file_list)
{
	...
	uris = NULL;
	for (l = file_list; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, athena_file_get_uri (ATHENA_FILE (l->data)));
	}
		  ...
		  const char *startup_id)
{
	...
	data->original_files = athena_file_list_copy (original_files);
	data->target_files = athena_file_list_copy (target_files);
...
{
	athena_file_list_free (data->original_files);
	athena_file_list_free (data->target_files);
...
static void
file_changed_callback (AthenaFile *file, gpointer user_data)
{
	AthenaPropertiesWindow *window = ATHENA_PROPERTIES_WINDOW (user_data);
...
static gboolean
is_a_special_file (AthenaFile *file)
{
	if (file == NULL ||
...
static gboolean
should_show_open_with (AthenaPropertiesWindow *window)
{
	AthenaFile *file;
...
static void
create_open_with_page (AthenaPropertiesWindow *window)
{
	GtkWidget *vbox;
	char *mime_type;
	char *uri = NULL;
	...
	mime_type = athena_file_get_mime_type (get_target_file (window));

	if (!is_multi_file_window (window)) {
		uri = athena_file_get_uri (get_target_file (window));
		if (uri == NULL) {
			return;
		}
	} else {
	...
	vbox = athena_mime_application_chooser_new (uri, uris, mime_type);
...
static AthenaPropertiesWindow *
create_properties_window (StartupData *startup_data)
{
	AthenaPropertiesWindow *window;
	GList *l;

	window = ATHENA_PROPERTIES_WINDOW (gtk_widget_new (ATHENA_TYPE_PROPERTIES_WINDOW, NULL));

	window->details->original_files = athena_file_list_copy (startup_data->original_files);
	
	window->details->target_files = athena_file_list_copy (startup_data->target_files);

	gtk_window_set_wmclass (GTK_WINDOW (window), "file_properties", "Athena");
	...
	for (l = window->details->target_files; l != NULL; l = l->next) {
		...
		attributes = 0;
		if (athena_file_is_directory (file)) {
			attributes |= ATHENA_FILE_ATTRIBUTE_DEEP_COUNTS;
		}
		
		attributes |= ATHENA_FILE_ATTRIBUTE_INFO;
		athena_file_monitor_add (file, &window->details->target_files, attributes);
	}	
		
	for (l = window->details->target_files; l != NULL; l = l->next) {
		g_signal_connect_object (ATHENA_FILE (l->data),
					 "changed",
					 G_CALLBACK (file_changed_callback),
					 G_OBJECT (window),
					 0);
...
get_target_file_list (GList *original_files)
{
	...
	for (l = original_files; l != NULL; l = l->next) {
		...
		target = get_target_file_for_original_file (ATHENA_FILE (l->data));
...
static void
add_window (AthenaPropertiesWindow *window)
{
	if (!is_multi_file_window (window)) {
...
static void
remove_window (AthenaPropertiesWindow *window)
{
	gpointer key;
...
static void
is_directory_ready_callback (AthenaFile *file,
			     gpointer data)
{
	...
	if (g_hash_table_size (startup_data->pending_files) == 0) {
		AthenaPropertiesWindow *new_window;
...
void
athena_properties_window_present (GList       *original_files,
				    GtkWidget   *parent_widget,
				    const gchar *startup_id) 
{
	...
	athena_file_list_free (target_files);
	g_free(pending_key);
...
{
	GError *error = NULL;
	AthenaPropertiesWindow *window = ATHENA_PROPERTIES_WINDOW (dialog);
	GtkWidget *curpage;
	const char *helpuri;
...
real_destroy (GtkWidget *object)
{
	...
	window = ATHENA_PROPERTIES_WINDOW (object);

	remove_window (window);

	for (l = window->details->original_files; l != NULL; l = l->next) {
		athena_file_monitor_remove (ATHENA_FILE (l->data), &window->details->original_files);
	}
	athena_file_list_free (window->details->original_files);
	window->details->original_files = NULL;
	
	for (l = window->details->target_files; l != NULL; l = l->next) {
		athena_file_monitor_remove (ATHENA_FILE (l->data), &window->details->target_files);
	}
	athena_file_list_free (window->details->target_files);
	...
	GTK_WIDGET_CLASS (athena_properties_window_parent_class)->destroy (object);
}
...
real_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (athena_properties_window_parent_class)->finalize (object);
}
...
static void
set_icon (const char* icon_uri, AthenaPropertiesWindow *properties_window)
{
	AthenaFile *file;
	char *file_uri;
	char *icon_path;
	...
	g_assert (ATHENA_IS_PROPERTIES_WINDOW (properties_window));
	...
	if (icon_path != NULL) {
		...
		for (l = properties_window->details->original_files; l != NULL; l = l->next) {
			...
			if (athena_file_is_mime_type (file, "application/x-desktop")) {
				if (athena_link_local_set_icon (file_uri, icon_path)) {
				...
				athena_file_set_metadata (file, ATHENA_METADATA_KEY_CUSTOM_ICON, NULL, real_icon_uri);
				athena_file_set_metadata (file, ATHENA_METADATA_KEY_ICON_SCALE, NULL, NULL);
			      ...
			      AthenaPropertiesWindow *window)
{
	...
	if (g_list_length (window->details->original_files) == 1) {
		file = ATHENA_FILE (window->details->original_files->data);
	...
	for (l = window->details->original_files; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		image_path = athena_file_get_metadata (file, ATHENA_METADATA_KEY_CUSTOM_ICON, NULL);
...
static void
athena_properties_window_class_init (AthenaPropertiesWindowClass *class)
{
	...
	g_type_class_add_private (class, sizeof (AthenaPropertiesWindowDetails));
}

static void
athena_properties_window_init (AthenaPropertiesWindow *window)
{
	window->details = G_TYPE_INSTANCE_GET_PRIVATE (window, ATHENA_TYPE_PROPERTIES_WINDOW,
						       AthenaPropertiesWindowDetails);
}
