...
#define ATHENA_DND_URI_LIST_TYPE 	  "text/uri-list"
#define ATHENA_DND_TEXT_PLAIN_TYPE 	  "text/plain"
...
struct AthenaLocationBarDetails {
	GtkLabel *label;
	AthenaEntry *entry;
...
enum {
	ATHENA_DND_MC_DESKTOP_ICON,
	ATHENA_DND_URI_LIST,
	ATHENA_DND_TEXT_PLAIN,
	ATHENA_DND_NTARGETS
};
...
static const GtkTargetEntry drag_types [] = {
	{ ATHENA_DND_URI_LIST_TYPE,   0, ATHENA_DND_URI_LIST },
	{ ATHENA_DND_TEXT_PLAIN_TYPE, 0, ATHENA_DND_TEXT_PLAIN },
};
...
G_DEFINE_TYPE (AthenaLocationBar, athena_location_bar,
	       GTK_TYPE_BOX);

static AthenaWindow *
athena_location_bar_get_window (GtkWidget *bar)
{
	return ATHENA_WINDOW (gtk_widget_get_ancestor (bar, ATHENA_TYPE_WINDOW));
}
...
static void
emit_location_changed (AthenaLocationBar *bar)
{
	char *location;

	location = athena_location_bar_get_location (bar);
	g_signal_emit (bar,
			     ...
			     gpointer callback_data)
{
	...
	int name_count;
	AthenaWindow *new_window, *window;
	GdkScreen      *screen;
	gboolean new_windows_for_extras;
	char *prompt;
	char *detail;
	GFile *location;
	AthenaLocationBar *self = ATHENA_LOCATION_BAR (widget);
	...
	window = athena_location_bar_get_window (widget);
	...
	athena_location_bar_set_location (self, names[0]);	
	...
	if (new_windows_for_extras) {
		...
		for (i = 1; names[i] != NULL; ++i) {
			new_window = athena_application_create_window (application, screen);
			location = g_file_new_for_uri (names[i]);
			athena_window_go_to (new_window, location);
			g_object_unref (location);
		}
	}
			       ...
			       GdkEventButton        *event)
{
	...
	window = athena_location_bar_get_window (gtk_widget_get_parent (widget));
	...
	athena_view_pop_up_location_context_menu (view, event, NULL);
...
editable_changed_callback (GtkEntry *entry,
			   gpointer user_data)
{
	athena_location_bar_update_label (ATHENA_LOCATION_BAR (user_data));
}

void
athena_location_bar_activate (AthenaLocationBar *bar)
{
	...
	athena_entry_select_all (bar->details->entry);
}

static void
athena_location_bar_cancel (AthenaLocationBar *bar)
{
	char *last_location;

	last_location = bar->details->last_location;
	athena_location_bar_set_location (bar, last_location);
}

static void
finalize (GObject *object)
{
	AthenaLocationBar *bar;

	bar = ATHENA_LOCATION_BAR (object);
	...
	G_OBJECT_CLASS (athena_location_bar_parent_class)->finalize (object);
}

static void
athena_location_bar_class_init (AthenaLocationBarClass *klass)
{
	...
	klass->cancel = athena_location_bar_cancel;

	signals[CANCEL] = g_signal_new
		("cancel",
		 G_TYPE_FROM_CLASS (klass),
		 G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		 G_STRUCT_OFFSET (AthenaLocationBarClass,
				  cancel),
		 NULL, NULL,
	...
	g_type_class_add_private (klass, sizeof (AthenaLocationBarDetails));
}

static void
athena_location_bar_init (AthenaLocationBar *bar)
{
	GtkWidget *label;
	GtkWidget *entry;
	GtkWidget *event_box;

	bar->details = G_TYPE_INSTANCE_GET_PRIVATE (bar, ATHENA_TYPE_LOCATION_BAR,
						    AthenaLocationBarDetails);
	...
	entry = athena_location_entry_new ();
	...
	bar->details->entry = ATHENA_ENTRY (entry);	
...
void
athena_location_bar_set_location (AthenaLocationBar *bar,
				    const char *location)
{
	...
	if (eel_uri_is_search (location)) {
		athena_location_entry_set_special_text (ATHENA_LOCATION_ENTRY (bar->details->entry),
							  "");
	} else {
		file = g_file_new_for_uri (location);
		formatted_location = g_file_get_parse_name (file);
		g_object_unref (file);
		athena_location_entry_update_current_location (ATHENA_LOCATION_ENTRY (bar->details->entry),
								 formatted_location);
	...
	athena_location_bar_update_label (bar);
}

AthenaEntry *
athena_location_bar_get_entry (AthenaLocationBar *location_bar)
{
