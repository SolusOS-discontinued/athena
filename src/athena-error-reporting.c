...
/* athena-error-reporting.h - implementation of file manager functions that report
 	                        errors to the user.
...
#define NEW_NAME_TAG "Athena: new name"
#define MAXIMUM_DISPLAYED_FILE_NAME_LENGTH	50

static void finish_rename (AthenaFile *file, gboolean stop_timer, GError *error);

void
athena_report_error_loading_directory (AthenaFile *file,
					 GError *error,
...
void
athena_report_error_setting_group (AthenaFile *file,
				     GError *error,
				     GtkWindow *parent_window)
{
	char *file_name;
	...
	if (message == NULL) {
		...
		g_warning ("Hit unhandled case %s:%d in athena_report_error_setting_group", 
...
void
athena_report_error_setting_permissions (AthenaFile *file,
					   GError *error,
					   GtkWindow *parent_window)
{
	char *file_name;
...
typedef struct _AthenaRenameData {
	char *name;
...
void
athena_report_error_renaming_file (AthenaFile *file,
				     const char *new_name,
				     GError *error,
				     GtkWindow *parent_window)
{
	 ...
	 */
	original_name = athena_file_get_display_name (file);
	...
	if (message == NULL) {
		/* We should invent decent error messages for every case we actually experience. */
		g_warning ("Hit unhandled case %s:%d in athena_report_error_renaming_file", 
			   g_quark_to_string (error->domain), error->code);
		/* fall through */
...
static void
athena_rename_data_free (AthenaRenameData *data)
{
	g_free (data->name);
	g_free (data);
}
...
static void
rename_callback (AthenaFile *file, GFile *result_location,
		 GError *error, gpointer callback_data)
{
	AthenaRenameData *data;
	...
	if (error &&
		...
		athena_report_error_renaming_file (file, data->name, error, NULL);
	}
...
cancel_rename_callback (gpointer callback_data)
{
	...
	finish_rename (ATHENA_FILE (callback_data), FALSE, error);
	g_error_free (error);
}

static void
finish_rename (AthenaFile *file, gboolean stop_timer, GError *error)
{
	AthenaRenameData *data;
	...
	athena_file_cancel (file, rename_callback, NULL);
	if (stop_timer) {
...
void
athena_rename_file (AthenaFile *file,
		      const char *new_name,
		      AthenaFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	data = g_new0 (AthenaRenameData, 1);
	...
	g_object_set_data_full (G_OBJECT (file),
				NEW_NAME_TAG,
				data, (GDestroyNotify)athena_rename_data_free);
