...
struct AthenaImagePropertiesPageDetails {
...
typedef struct {
        GObject parent;
} AthenaImagePropertiesPageProvider;

typedef struct {
        GObjectClass parent;
} AthenaImagePropertiesPageProviderClass;


static GType athena_image_properties_page_provider_get_type (void);
static void  property_page_provider_iface_init                (AthenaPropertyPageProviderIface *iface);


G_DEFINE_TYPE (AthenaImagePropertiesPage, athena_image_properties_page, GTK_TYPE_BOX);

G_DEFINE_TYPE_WITH_CODE (AthenaImagePropertiesPageProvider, athena_image_properties_page_provider, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (ATHENA_TYPE_PROPERTY_PAGE_PROVIDER,
						property_page_provider_iface_init));

static void
athena_image_properties_page_finalize (GObject *object)
{
	AthenaImagePropertiesPage *page;

	page = ATHENA_IMAGE_PROPERTIES_PAGE (object);
	...
	G_OBJECT_CLASS (athena_image_properties_page_parent_class)->finalize (object);
}
		     ...
		     gpointer      data)
{
	...
	page = ATHENA_IMAGE_PROPERTIES_PAGE (data);
...
static gboolean
append_tag_value_pair (AthenaImagePropertiesPage *page,
		       ExifData *data,
		       ExifTag   tag,
		       char     *description) 
{
...
static void
append_exifdata_string (ExifData *exifdata, AthenaImagePropertiesPage *page)
{
...
#ifdef HAVE_EXEMPI
static void
append_xmp_value_pair (AthenaImagePropertiesPage *page,
		       XmpPtr      xmp,
		       const char *ns,
		       const char *propname,
		       char       *descr)
...
static void
append_xmpdata_string (XmpPtr xmp, AthenaImagePropertiesPage *page)
{
	if (xmp != NULL) {
...
static void
load_finished (AthenaImagePropertiesPage *page)
{
			...
			gpointer         callback_data)
{
	...
	page = ATHENA_IMAGE_PROPERTIES_PAGE (callback_data);
...
static void
load_location (AthenaImagePropertiesPage *page,
	       const char                  *location)
{
	GFile *file;

	g_assert (ATHENA_IS_IMAGE_PROPERTIES_PAGE (page));
...
static void
athena_image_properties_page_class_init (AthenaImagePropertiesPageClass *class)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (class);

	object_class->finalize = athena_image_properties_page_finalize;

	g_type_class_add_private (object_class, sizeof(AthenaImagePropertiesPageDetails));
}

static void
athena_image_properties_page_init (AthenaImagePropertiesPage *page)
{
	page->details = G_TYPE_INSTANCE_GET_PRIVATE (page,
						     ATHENA_TYPE_IMAGE_PROPERTIES_PAGE,
						     AthenaImagePropertiesPageDetails);
...
static GList *
get_property_pages (AthenaPropertyPageProvider *provider,
                    GList *files)
{
	GList *pages;
	AthenaPropertyPage *real_page;
	AthenaFileInfo *file;
        char *uri;
	AthenaImagePropertiesPage *page;
	...
	file = ATHENA_FILE_INFO (files->data);
	
	if (!
	    (athena_file_info_is_mime_type (file, "image/x-bmp") ||
	     athena_file_info_is_mime_type (file, "image/x-ico") ||
	     athena_file_info_is_mime_type (file, "image/jpeg") ||
	     athena_file_info_is_mime_type (file, "image/gif") ||
	     athena_file_info_is_mime_type (file, "image/png") ||
	     athena_file_info_is_mime_type (file, "image/pnm") ||
	     athena_file_info_is_mime_type (file, "image/ras") ||
	     athena_file_info_is_mime_type (file, "image/tga") ||
	     athena_file_info_is_mime_type (file, "image/tiff") ||
	     athena_file_info_is_mime_type (file, "image/wbmp") ||
	     athena_file_info_is_mime_type (file, "image/x-xbitmap") ||
	     athena_file_info_is_mime_type (file, "image/x-xpixmap"))) {
		return NULL;
	}
	...
	page = g_object_new (athena_image_properties_page_get_type (), NULL);
	load_location (page, uri);

	g_free (uri);

        real_page = athena_property_page_new
                ("AthenaImagePropertiesPage::property_page", 
...
static void 
property_page_provider_iface_init (AthenaPropertyPageProviderIface *iface)
{
...
static void
athena_image_properties_page_provider_init (AthenaImagePropertiesPageProvider *sidebar)
{
}

static void
athena_image_properties_page_provider_class_init (AthenaImagePropertiesPageProviderClass *class)
{
}

void
athena_image_properties_page_register (void)
{
        athena_module_add_type (athena_image_properties_page_provider_get_type ());
}
