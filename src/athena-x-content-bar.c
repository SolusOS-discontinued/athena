...
#define ATHENA_X_CONTENT_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), ATHENA_TYPE_X_CONTENT_BAR, AthenaXContentBarPrivate))

struct AthenaXContentBarPrivate
{
	GtkWidget *label;
	GtkWidget *button;
...
G_DEFINE_TYPE (AthenaXContentBar, athena_x_content_bar, GTK_TYPE_INFO_BAR)
			 ...
			 gpointer user_data)
{
	...
	AthenaXContentBar *bar = user_data;
	...
	if (default_app != NULL) {
		athena_launch_application_for_mount (default_app, bar->priv->mount,
...
static void
athena_x_content_bar_set_x_content_type (AthenaXContentBar *bar, const char *x_content_type)
{					  
	char *message;
	char *description;
	GAppInfo *default_app;
...
static void
athena_x_content_bar_set_mount (AthenaXContentBar *bar, GMount *mount)
{
...
static void
athena_x_content_bar_set_property (GObject      *object,
				     guint         prop_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
	AthenaXContentBar *bar;

	bar = ATHENA_X_CONTENT_BAR (object);

	switch (prop_id) {
	case PROP_MOUNT:
		athena_x_content_bar_set_mount (bar, G_MOUNT (g_value_get_object (value)));
		break;
	case PROP_X_CONTENT_TYPE:
		athena_x_content_bar_set_x_content_type (bar, g_value_get_string (value));
		break;
	default:
...
static void
athena_x_content_bar_get_property (GObject    *object,
				     guint       prop_id,
				     GValue     *value,
				     GParamSpec *pspec)
{
...
static void
athena_x_content_bar_finalize (GObject *object)
{
	AthenaXContentBar *bar = ATHENA_X_CONTENT_BAR (object);

	g_free (bar->priv->x_content_type);
	if (bar->priv->mount != NULL)
		g_object_unref (bar->priv->mount);

        G_OBJECT_CLASS (athena_x_content_bar_parent_class)->finalize (object);
}

static void
athena_x_content_bar_class_init (AthenaXContentBarClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);
	object_class->get_property = athena_x_content_bar_get_property;
	object_class->set_property = athena_x_content_bar_set_property;
	object_class->finalize = athena_x_content_bar_finalize;

	g_type_class_add_private (klass, sizeof (AthenaXContentBarPrivate));
...
static void
athena_x_content_bar_init (AthenaXContentBar *bar)
{
	GtkWidget *content_area;

	bar->priv = ATHENA_X_CONTENT_BAR_GET_PRIVATE (bar);
...
GtkWidget *
athena_x_content_bar_new (GMount *mount, 
			    const char *x_content_type)
{
	return g_object_new (ATHENA_TYPE_X_CONTENT_BAR, 
			     "mount", mount,
