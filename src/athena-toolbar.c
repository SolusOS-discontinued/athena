...
athena_toolbar_constructed (GObject *obj)
{
	...
	gtk_ui_manager_add_ui_from_resource (self->priv->ui_manager, "/com/solusos/athena/athena-toolbar-ui.xml", NULL);
	...
	// Boxed chunky look for the search button
	gtk_style_context_add_class (gtk_widget_get_style_context (search_item),
				     GTK_STYLE_CLASS_RAISED);
	...
	gtk_container_add (GTK_CONTAINER (box), GTK_WIDGET (tool_button));
	tool_button = toolbar_create_toolbutton (self, TRUE, ATHENA_ACTION_VIEW_DETAILS, "view-list-symbolic");
	gtk_container_add (GTK_CONTAINER (box), GTK_WIDGET (tool_button));
