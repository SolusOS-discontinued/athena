...
typedef enum {
	ATHENA_QUERY_EDITOR_ROW_LOCATION,
	ATHENA_QUERY_EDITOR_ROW_TYPE,
	
	ATHENA_QUERY_EDITOR_ROW_LAST
} AthenaQueryEditorRowType;

typedef struct {
	AthenaQueryEditorRowType type;
	AthenaQueryEditor *editor;
	GtkWidget *hbox;
	GtkWidget *combo;
	...
	void *data;
} AthenaQueryEditorRow;


typedef struct {
	const char *name;
	GtkWidget * (*create_widgets)      (AthenaQueryEditorRow *row);
	void        (*add_to_query)        (AthenaQueryEditorRow *row,
					    AthenaQuery          *query);
	void        (*free_data)           (AthenaQueryEditorRow *row);
	void        (*add_rows_from_query) (AthenaQueryEditor *editor,
					    AthenaQuery *query);
} AthenaQueryEditorRowOps;

struct AthenaQueryEditorDetails {
	GtkWidget *entry;
...
static void entry_activate_cb (GtkWidget *entry, AthenaQueryEditor *editor);
static void entry_changed_cb  (GtkWidget *entry, AthenaQueryEditor *editor);
static void athena_query_editor_changed_force (AthenaQueryEditor *editor,
						 gboolean             force);
static void athena_query_editor_changed (AthenaQueryEditor *editor);
static AthenaQueryEditorRow * athena_query_editor_add_row (AthenaQueryEditor *editor,
							       AthenaQueryEditorRowType type);

static GtkWidget *location_row_create_widgets  (AthenaQueryEditorRow *row);
static void       location_row_add_to_query    (AthenaQueryEditorRow *row,
					        AthenaQuery          *query);
static void       location_row_free_data       (AthenaQueryEditorRow *row);
static void       location_add_rows_from_query (AthenaQueryEditor    *editor,
					        AthenaQuery          *query);
static GtkWidget *type_row_create_widgets      (AthenaQueryEditorRow *row);
static void       type_row_add_to_query        (AthenaQueryEditorRow *row,
					        AthenaQuery          *query);
static void       type_row_free_data           (AthenaQueryEditorRow *row);
static void       type_add_rows_from_query     (AthenaQueryEditor    *editor,
					        AthenaQuery          *query);



static AthenaQueryEditorRowOps row_type[] = {
	{ N_("Location"),
...
G_DEFINE_TYPE (AthenaQueryEditor, athena_query_editor, GTK_TYPE_BOX);

static void
athena_query_editor_dispose (GObject *object)
{
	AthenaQueryEditor *editor;

	editor = ATHENA_QUERY_EDITOR (object);
	...
	if (editor->details->bar != NULL) {
		...
		athena_search_bar_return_entry (editor->details->bar);
		eel_remove_weak_pointer (&editor->details->bar);
	}

	G_OBJECT_CLASS (athena_query_editor_parent_class)->dispose (object);
}

static gboolean
athena_query_editor_draw (GtkWidget *widget,
			    cairo_t *cr)
{
	...
	GTK_WIDGET_CLASS (athena_query_editor_parent_class)->draw (widget, cr);

	return FALSE;
}

static void
athena_query_editor_class_init (AthenaQueryEditorClass *class)
{
	...
	gobject_class = G_OBJECT_CLASS (class);
        gobject_class->dispose = athena_query_editor_dispose;

	widget_class = GTK_WIDGET_CLASS (class);
	widget_class->draw = athena_query_editor_draw;

	signals[CHANGED] =
		g_signal_new ("changed",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaQueryEditorClass, changed),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ATHENA_TYPE_QUERY, G_TYPE_BOOLEAN);

	signals[CANCEL] =
		g_signal_new ("cancel",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		              G_STRUCT_OFFSET (AthenaQueryEditorClass, cancel),
		              NULL, NULL,
	...
	g_type_class_add_private (class, sizeof (AthenaQueryEditorDetails));
}

static void
entry_activate_cb (GtkWidget *entry, AthenaQueryEditor *editor)
{
	if (editor->details->typing_timeout_id) {
		g_source_remove (editor->details->typing_timeout_id);
		editor->details->typing_timeout_id = 0;
	...
	athena_query_editor_changed_force (editor, TRUE);
}
...
typing_timeout_cb (gpointer user_data)
{
	...
	editor = ATHENA_QUERY_EDITOR (user_data);

	athena_query_editor_changed (editor);
...
static void
entry_changed_cb (GtkWidget *entry, AthenaQueryEditor *editor)
{
...
static void
edit_clicked (GtkButton *button, AthenaQueryEditor *editor)
{
	athena_query_editor_set_visible (editor, TRUE);
	athena_query_editor_grab_focus (editor);
}

/* Location */

static GtkWidget *
location_row_create_widgets (AthenaQueryEditorRow *row)
{
	GtkWidget *chooser;
	...
	g_signal_connect_swapped (chooser, "current-folder-changed",
				  G_CALLBACK (athena_query_editor_changed),
				  row->editor);
...
static void
location_row_add_to_query (AthenaQueryEditorRow *row,
			   AthenaQuery          *query)
{
	char *folder, *uri;
	...
	athena_query_set_location (query, uri);
	g_free (uri);
}

static void
location_row_free_data (AthenaQueryEditorRow *row)
{
}

static void
location_add_rows_from_query (AthenaQueryEditor    *editor,
			      AthenaQuery          *query)
{
	AthenaQueryEditorRow *row;
	char *uri, *folder;
	...
	row = athena_query_editor_add_row (editor,
					     ATHENA_QUERY_EDITOR_ROW_LOCATION);
...
static void
type_add_custom_type (AthenaQueryEditorRow *row,
...
static void
type_combo_changed (GtkComboBox *combo_box, AthenaQueryEditorRow *row)
{
	GtkTreeIter iter;
	gboolean other;
	GtkTreeModel *model;
	...
	athena_query_editor_changed (row->editor);
}

static GtkWidget *
type_row_create_widgets (AthenaQueryEditorRow *row)
{
	GtkWidget *combo;
...
static void
type_row_add_to_query (AthenaQueryEditorRow *row,
		       AthenaQuery          *query)
{
	GtkTreeIter iter;
	char **mimetypes;
	...
	if (mimetypes != NULL) {
		while (*mimetypes != NULL) {
			athena_query_add_mime_type (query, *mimetypes);
			mimetypes++;
		}
	}
	if (mimetype) {
		athena_query_add_mime_type (query, mimetype);
		g_free (mimetype);
	}
}

static void
type_row_free_data (AthenaQueryEditorRow *row)
{
}
...
static void
type_add_rows_from_query (AthenaQueryEditor    *editor,
			  AthenaQuery          *query)
{
	GList *mime_types;
	char *mime_type;
	...
	mime_types = athena_query_get_mime_types (query);
...
static AthenaQueryEditorRowType
get_next_free_type (AthenaQueryEditor *editor)
{
	AthenaQueryEditorRow *row;
	AthenaQueryEditorRowType type;
	gboolean found;
	...
	for (type = 0; type < ATHENA_QUERY_EDITOR_ROW_LAST; type++) {
		found = FALSE;
	...
	return ATHENA_QUERY_EDITOR_ROW_TYPE;
}

static void
remove_row_cb (GtkButton *clicked_button, AthenaQueryEditorRow *row)
{
	AthenaQueryEditor *editor;
...
static void
create_type_widgets (AthenaQueryEditorRow *row)
{
	row->type_widget = row_type[row->type].create_widgets (row);
}

static void
row_type_combo_changed_cb (GtkComboBox *combo_box, AthenaQueryEditorRow *row)
{
	AthenaQueryEditorRowType type;
...
static AthenaQueryEditorRow *
athena_query_editor_add_row (AthenaQueryEditor *editor,
			       AthenaQueryEditorRowType type)
{
	...
	row = g_new0 (AthenaQueryEditorRow, 1);
	row->editor = editor;
	row->type = type;
	...
	row->combo = combo;
	for (i = 0; i < ATHENA_QUERY_EDITOR_ROW_LAST; i++) {
...
static void
go_search_cb (GtkButton *clicked_button, AthenaQueryEditor *editor)
{
	athena_query_editor_changed_force (editor, TRUE);
}

static void
add_new_row_cb (GtkButton *clicked_button, AthenaQueryEditor *editor)
{
	athena_query_editor_add_row (editor, get_next_free_type (editor));
	athena_query_editor_changed (editor);
}

static void
athena_query_editor_init (AthenaQueryEditor *editor)
{
	GtkWidget *hbox, *label, *button;
	char *label_markup;

	editor->details = G_TYPE_INSTANCE_GET_PRIVATE (editor, ATHENA_TYPE_QUERY_EDITOR,
						       AthenaQueryEditorDetails);
...
void
athena_query_editor_set_default_query (AthenaQueryEditor *editor)
{
	athena_query_editor_add_row (editor, ATHENA_QUERY_EDITOR_ROW_LOCATION);
	athena_query_editor_changed (editor);
}

static void
finish_first_line (AthenaQueryEditor *editor, GtkWidget *hbox, gboolean use_go)
{
...
static void
setup_internal_entry (AthenaQueryEditor *editor)
{
...
static void
setup_external_entry (AthenaQueryEditor *editor, GtkWidget *entry)
{
	GtkWidget *hbox, *label;
	gchar *label_markup;
...
void
athena_query_editor_set_visible (AthenaQueryEditor *editor,
				   gboolean visible)
{
...
static gboolean
query_is_valid (AthenaQueryEditor *editor)
{
	const char *text;
...
static void
athena_query_editor_changed_force (AthenaQueryEditor *editor, gboolean force_reload)
{
	AthenaQuery *query;
	...
	if (query_is_valid (editor)) {
		query = athena_query_editor_get_query (editor);
...
static void
athena_query_editor_changed (AthenaQueryEditor *editor)
{
	athena_query_editor_changed_force (editor, FALSE);
}

void
athena_query_editor_grab_focus (AthenaQueryEditor *editor)
{
...
AthenaQuery *
athena_query_editor_get_query (AthenaQueryEditor *editor)
{
...
void
athena_query_editor_clear_query (AthenaQueryEditor *editor)
{
...
GtkWidget *
athena_query_editor_new (gboolean start_hidden)
{
	GtkWidget *editor;

	editor = g_object_new (ATHENA_TYPE_QUERY_EDITOR, NULL);
	athena_query_editor_set_visible (ATHENA_QUERY_EDITOR (editor),
					   !start_hidden);
	
	setup_internal_entry (ATHENA_QUERY_EDITOR (editor));
		
	return editor;
}

static void
detach_from_external_entry (AthenaQueryEditor *editor)
{
	if (editor->details->bar != NULL) {
		athena_search_bar_return_entry (editor->details->bar);
...
static void
attach_to_external_entry (AthenaQueryEditor *editor)
{
	if (editor->details->bar != NULL) {
		athena_search_bar_borrow_entry (editor->details->bar);
...
GtkWidget*
athena_query_editor_new_with_bar (gboolean start_hidden,
				    ...
				    AthenaWindowSlot *slot)
{
	...
	editor = ATHENA_QUERY_EDITOR (g_object_new (ATHENA_TYPE_QUERY_EDITOR, NULL));
	athena_query_editor_set_visible (editor, !start_hidden);
	...
	entry = athena_search_bar_borrow_entry (bar);
...
void
athena_query_editor_set_query (AthenaQueryEditor *editor, AthenaQuery *query)
{
	AthenaQueryEditorRowType type;
	char *text;

	if (!query) {
		athena_query_editor_clear_query (editor);
		return;
	}

	text = athena_query_get_text (query);
