...
static int athena_list_model_file_entry_compare_func (gconstpointer a,
							gconstpointer b,
							gpointer      user_data);
static void athena_list_model_tree_model_init (GtkTreeModelIface *iface);
static void athena_list_model_sortable_init (GtkTreeSortableIface *iface);
static void athena_list_model_multi_drag_source_init (EggTreeMultiDragSourceIface *iface);

struct AthenaListModelDetails {
	GSequence *files;
...
{
	athena_file_unref (file_entry->file);
	...
	if (file_entry->subdirectory != NULL) {
		athena_directory_unref (file_entry->subdirectory);
	}
...
athena_list_model_get_flags (GtkTreeModel *tree_model)
{
	return GTK_TREE_MODEL_ITERS_PERSIST;
}

static int
athena_list_model_get_n_columns (GtkTreeModel *tree_model)
{
	return ATHENA_LIST_MODEL_NUM_COLUMNS + ATHENA_LIST_MODEL (tree_model)->details->columns->len;
}

static GType
athena_list_model_get_column_type (GtkTreeModel *tree_model, int index)
{
	switch (index) {
	case ATHENA_LIST_MODEL_FILE_COLUMN:
		return ATHENA_TYPE_FILE;
	case ATHENA_LIST_MODEL_SUBDIRECTORY_COLUMN:
		return ATHENA_TYPE_DIRECTORY;
	...
	default:
		if (index < ATHENA_LIST_MODEL_NUM_COLUMNS + ATHENA_LIST_MODEL (tree_model)->details->columns->len) {
...
static void
athena_list_model_ptr_to_iter (AthenaListModel *model, GSequenceIter *ptr, GtkTreeIter *iter)
{
...
static gboolean
athena_list_model_get_iter (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreePath *path)
{
	AthenaListModel *model;
	GSequence *files;
	GSequenceIter *ptr;
	...
	athena_list_model_ptr_to_iter (model, ptr, iter);
	
	return TRUE;
}

static GtkTreePath *
athena_list_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	GtkTreePath *path;
	AthenaListModel *model;
	GSequenceIter *ptr;
...
static gboolean
athena_list_model_iter_next (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	AthenaListModel *model;
...
static gboolean
athena_list_model_iter_children (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent)
{
	AthenaListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
athena_list_model_iter_has_child (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	FileEntry *file_entry;
	
	if (iter == NULL) {
		return !athena_list_model_is_empty (ATHENA_LIST_MODEL (tree_model));
	}
...
static int
athena_list_model_iter_n_children (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
	AthenaListModel *model;
	GSequence *files;
	FileEntry *file_entry;
...
static gboolean
athena_list_model_iter_nth_child (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent, int n)
{
	AthenaListModel *model;
	GSequenceIter *child;
	GSequence *files;
...
static gboolean
athena_list_model_iter_parent (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child)
{
	AthenaListModel *model;
	FileEntry *file_entry;
...
lookup_file (AthenaListModel *model, AthenaFile *file,
	      ...
	      GHashTable *reverse_map)
{
	...
	if (ptr) {
		...
		athena_list_model_ptr_to_iter (data->model, ptr, iter);
...
GList *
athena_list_model_get_all_iters_for_file (AthenaListModel *model, AthenaFile *file)
{
	struct GetIters data;
...
gboolean
athena_list_model_get_first_iter_for_file (AthenaListModel          *model,
					     AthenaFile         *file,
					     GtkTreeIter          *iter)
{
	GList *list;
	...
	list = athena_list_model_get_all_iters_for_file (model, file);
	if (list != NULL) {
		res = TRUE;
...
gboolean
athena_list_model_get_tree_iter_from_file (AthenaListModel *model, AthenaFile *file,
					     AthenaDirectory *directory,
					     GtkTreeIter *iter)
{
	GSequenceIter *ptr;
...
static int
athena_list_model_file_entry_compare_func (gconstpointer a,
					     gconstpointer b,
					     gpointer      user_data)
{
	...
	model = (AthenaListModel *)user_data;

	file_entry1 = (FileEntry *)a;
	file_entry2 = (FileEntry *)b;
	
	if (file_entry1->file != NULL && file_entry2->file != NULL) {
		result = athena_file_compare_for_sort_by_attribute_q (file_entry1->file, file_entry2->file,
...
int
athena_list_model_compare_func (AthenaListModel *model,
				  AthenaFile *file1,
				  AthenaFile *file2)
{
	int result;

	result = athena_file_compare_for_sort_by_attribute_q (file1, file2,
								model->details->sort_attribute,
								model->details->sort_directories_first,
								(model->details->order == GTK_SORT_DESCENDING));
...
static void
athena_list_model_sort_file_entries (AthenaListModel *model, GSequence *files, GtkTreePath *path)
{
	...
	for (i = 0; i < length; ++i) {
		...
		if (file_entry->files != NULL) {
			...
			athena_list_model_sort_file_entries (model, file_entry->files, path);
	...
	/* sort */
	g_sequence_sort (files, athena_list_model_file_entry_compare_func, model);
...
static void
athena_list_model_sort (AthenaListModel *model)
{
	GtkTreePath *path;

	path = gtk_tree_path_new ();

	athena_list_model_sort_file_entries (model, model->details->files, path);

	gtk_tree_path_free (path);
}

static gboolean
athena_list_model_get_sort_column_id (GtkTreeSortable *sortable,
					gint            *sort_column_id,
					GtkSortType     *order)
{
	AthenaListModel *model;
	...
	model = (AthenaListModel *)sortable;
	
	id = athena_list_model_get_sort_column_id_from_attribute 
...
static void
athena_list_model_set_sort_column_id (GtkTreeSortable *sortable, gint sort_column_id, GtkSortType order)
{
	AthenaListModel *model;

	model = (AthenaListModel *)sortable;

	model->details->sort_attribute = athena_list_model_get_attribute_from_sort_column_id (model, sort_column_id);

	model->details->order = order;

	athena_list_model_sort (model);
	gtk_tree_sortable_sort_column_changed (sortable);
}

static gboolean
athena_list_model_has_default_sort_func (GtkTreeSortable *sortable)
{
	return FALSE;
}

static gboolean
athena_list_model_multi_row_draggable (EggTreeMultiDragSource *drag_source, GList *path_list)
{
	return TRUE;
}

static void
each_path_get_data_binder (AthenaDragEachSelectedItemDataGet data_get,
			   gpointer context,
			   gpointer data)
{
	...
	for (l = info->path_list; l != NULL; l = l->next) {
		...
		file = athena_list_model_file_for_path (info->model, path);
		if (file) {
...
static gboolean
athena_list_model_multi_drag_data_get (EggTreeMultiDragSource *drag_source, 
					 GList *path_list, 
					 GtkSelectionData *selection_data)
{
	...
	model = ATHENA_LIST_MODEL (drag_source);

	context.model = model;
	context.path_list = path_list;

	if (!drag_target_list) {
		drag_target_list = athena_list_model_get_drag_target_list ();
	}

	if (gtk_target_list_find (drag_target_list,
				  gtk_selection_data_get_target (selection_data),
				  &target_info)) {
		athena_drag_drag_data_get (NULL,
					     NULL,
					     selection_data,
					     target_info,
					     GDK_CURRENT_TIME,
...
static gboolean
athena_list_model_multi_drag_data_delete (EggTreeMultiDragSource *drag_source, GList *path_list)
{
	return TRUE;
}

static void
add_dummy_row (AthenaListModel *model, FileEntry *parent_entry)
{
	...
	dummy_file_entry->ptr = g_sequence_insert_sorted (parent_entry->files, dummy_file_entry,
							  athena_list_model_file_entry_compare_func, model);
	iter.stamp = model->details->stamp;
...
gboolean
athena_list_model_add_file (AthenaListModel *model, AthenaFile *file,
			      AthenaDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	...
	file_entry->file = athena_file_ref (file);
...
void
athena_list_model_file_changed (AthenaListModel *model, AthenaFile *file,
				  AthenaDirectory *directory)
{
	...
	g_sequence_sort_changed (ptr, athena_list_model_file_entry_compare_func, model);
	...
	if (pos_before != pos_after) {
		...
		if (parent_file_entry == NULL) {
			...
			has_iter = TRUE;
			athena_list_model_ptr_to_iter (model, parent_file_entry->ptr, &iter);
...
gboolean
athena_list_model_is_empty (AthenaListModel *model)
{
	return (g_sequence_get_length (model->details->files) == 0);
}

guint
athena_list_model_get_length (AthenaListModel *model)
{
	return g_sequence_get_length (model->details->files);
}

static void
athena_list_model_remove (AthenaListModel *model, GtkTreeIter *iter)
{
	...
	if (file_entry->files != NULL) {
		while (g_sequence_get_length (file_entry->files) > 0) {
			...
			if (child_file_entry->file != NULL) {
				athena_list_model_remove_file (model,
...
void
athena_list_model_remove_file (AthenaListModel *model, AthenaFile *file,
			   AthenaDirectory *directory)
{
	GtkTreeIter iter;

	if (athena_list_model_get_tree_iter_from_file (model, file, directory, &iter)) {
		athena_list_model_remove (model, &iter);
	}
}

static void
athena_list_model_clear_directory (AthenaListModel *model, GSequence *files)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	...
	while (g_sequence_get_length (files) > 0) {
		...
		file_entry = g_sequence_get (iter.user_data);
		if (file_entry->files != NULL) {
			athena_list_model_clear_directory (model, file_entry->files);
		}
...
void
athena_list_model_clear (AthenaListModel *model)
{
	g_return_if_fail (model != NULL);

	athena_list_model_clear_directory (model, model->details->files);
}

AthenaFile *
athena_list_model_file_for_path (AthenaListModel *model, GtkTreePath *path)
{
	AthenaFile *file;
	GtkTreeIter iter;
...
gboolean
athena_list_model_load_subdirectory (AthenaListModel *model, GtkTreePath *path, AthenaDirectory **directory)
{
	GtkTreeIter iter;
	FileEntry *file_entry;
	AthenaDirectory *subdirectory;
	...
	subdirectory = athena_directory_get_for_file (file_entry->file);

	if (g_hash_table_lookup (model->details->directory_reverse_map,
				 subdirectory) != NULL) {
		athena_directory_unref (subdirectory);
	...
	/* Return a ref too */
	athena_directory_ref (subdirectory);
...
void
athena_list_model_unload_subdirectory (AthenaListModel *model, GtkTreeIter *iter)
{
	...
	while (g_sequence_get_length (file_entry->files) > 0) {
		...
		if (child_file_entry->file == NULL) {
			...
			athena_list_model_ptr_to_iter (model, child_ptr, &child_iter);
			athena_list_model_remove (model, &child_iter);
		}
	}
...
void
athena_list_model_set_should_sort_directories_first (AthenaListModel *model, gboolean sort_directories_first)
{
...
int
athena_list_model_get_sort_column_id_from_attribute (AthenaListModel *model,
						       GQuark attribute)
{
	guint i;
	...
	for (i = 0; i < model->details->columns->len; i++) {
		...
		column = 
			ATHENA_COLUMN (model->details->columns->pdata[i]);
		g_object_get (G_OBJECT (column), 
			      "attribute_q", &column_attribute, 
			      NULL);
		if (column_attribute == attribute) {
			return ATHENA_LIST_MODEL_NUM_COLUMNS + i;
		}
	}
...
GQuark
athena_list_model_get_attribute_from_sort_column_id (AthenaListModel *model,
						       int sort_column_id)
{
	AthenaColumn *column;
	int index;
	...
	index = sort_column_id - ATHENA_LIST_MODEL_NUM_COLUMNS;
	...
	column = ATHENA_COLUMN (model->details->columns->pdata[index]);
...
AthenaZoomLevel
athena_list_model_get_zoom_level_from_column_id (int column)
{
	switch (column) {
	case ATHENA_LIST_MODEL_SMALLEST_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_SMALLEST;
	case ATHENA_LIST_MODEL_SMALLER_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_SMALLER;
	case ATHENA_LIST_MODEL_SMALL_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_SMALL;
	case ATHENA_LIST_MODEL_STANDARD_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_STANDARD;
	case ATHENA_LIST_MODEL_LARGE_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_LARGE;
	case ATHENA_LIST_MODEL_LARGER_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_LARGER;
	case ATHENA_LIST_MODEL_LARGEST_ICON_COLUMN:
		return ATHENA_ZOOM_LEVEL_LARGEST;
	}

	g_return_val_if_reached (ATHENA_ZOOM_LEVEL_STANDARD);
}

int
athena_list_model_get_column_id_from_zoom_level (AthenaZoomLevel zoom_level)
{
	switch (zoom_level) {
	case ATHENA_ZOOM_LEVEL_SMALLEST:
		return ATHENA_LIST_MODEL_SMALLEST_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_SMALLER:
		return ATHENA_LIST_MODEL_SMALLER_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_SMALL:
		return ATHENA_LIST_MODEL_SMALL_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_STANDARD:
		return ATHENA_LIST_MODEL_STANDARD_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_LARGE:
		return ATHENA_LIST_MODEL_LARGE_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_LARGER:
		return ATHENA_LIST_MODEL_LARGER_ICON_COLUMN;
	case ATHENA_ZOOM_LEVEL_LARGEST:
		return ATHENA_LIST_MODEL_LARGEST_ICON_COLUMN;
	}

	g_return_val_if_reached (ATHENA_LIST_MODEL_STANDARD_ICON_COLUMN);
}

void
athena_list_model_set_drag_view (AthenaListModel *model,
				   GtkTreeView *view,
				   int drag_begin_x,
				   int drag_begin_y)
{
...
GtkTargetList *
athena_list_model_get_drag_target_list ()
{
	GtkTargetList *target_list;

	target_list = gtk_target_list_new (drag_types, G_N_ELEMENTS (drag_types));
	gtk_target_list_add_text_targets (target_list, ATHENA_ICON_DND_TEXT);

	return target_list;
}

int               
athena_list_model_add_column (AthenaListModel *model,
				AthenaColumn *column)
{
	g_ptr_array_add (model->details->columns, column);
	g_object_ref (column);

	return ATHENA_LIST_MODEL_NUM_COLUMNS + (model->details->columns->len - 1);
}

int
athena_list_model_get_column_number (AthenaListModel *model,
...
static void
athena_list_model_dispose (GObject *object)
{
	AthenaListModel *model;
	int i;

	model = ATHENA_LIST_MODEL (object);
	...
	G_OBJECT_CLASS (athena_list_model_parent_class)->dispose (object);
}

static void
athena_list_model_finalize (GObject *object)
{
	AthenaListModel *model;
	...
	if (model->details->highlight_files != NULL) {
		athena_file_list_free (model->details->highlight_files);
		model->details->highlight_files = NULL;
	}

	g_free (model->details);

	G_OBJECT_CLASS (athena_list_model_parent_class)->finalize (object);
}

static void
athena_list_model_init (AthenaListModel *model)
{
	model->details = g_new0 (AthenaListModelDetails, 1);
...
static void
athena_list_model_class_init (AthenaListModelClass *klass)
{
	...
	object_class->finalize = athena_list_model_finalize;
	object_class->dispose = athena_list_model_dispose;

      list_model_signals[SUBDIRECTORY_UNLOADED] =
        g_signal_new ("subdirectory_unloaded",
                      ATHENA_TYPE_LIST_MODEL,
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (AthenaListModelClass, subdirectory_unloaded),
                      NULL, NULL,
...
static void
athena_list_model_tree_model_init (GtkTreeModelIface *iface)
{
	iface->get_flags = athena_list_model_get_flags;
	iface->get_n_columns = athena_list_model_get_n_columns;
	iface->get_column_type = athena_list_model_get_column_type;
	iface->get_iter = athena_list_model_get_iter;
	iface->get_path = athena_list_model_get_path;
	iface->get_value = athena_list_model_get_value;
	iface->iter_next = athena_list_model_iter_next;
	iface->iter_children = athena_list_model_iter_children;
	iface->iter_has_child = athena_list_model_iter_has_child;
	iface->iter_n_children = athena_list_model_iter_n_children;
	iface->iter_nth_child = athena_list_model_iter_nth_child;
	iface->iter_parent = athena_list_model_iter_parent;
}

static void
athena_list_model_sortable_init (GtkTreeSortableIface *iface)
{
	iface->get_sort_column_id = athena_list_model_get_sort_column_id;
	iface->set_sort_column_id = athena_list_model_set_sort_column_id;
	iface->has_default_sort_func = athena_list_model_has_default_sort_func;
}

static void
athena_list_model_multi_drag_source_init (EggTreeMultiDragSourceIface *iface)
{
	iface->row_draggable = athena_list_model_multi_row_draggable;
	iface->drag_data_get = athena_list_model_multi_drag_data_get;
	iface->drag_data_delete = athena_list_model_multi_drag_data_delete;
}

void
athena_list_model_subdirectory_done_loading (AthenaListModel *model, AthenaDirectory *directory)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	 ...
	 */
	if (!athena_directory_is_not_empty (directory) &&
             ...
             gpointer user_data)
{
	...
	iters = athena_list_model_get_all_iters_for_file (model, file);
...
void
athena_list_model_set_highlight_for_files (AthenaListModel *model,
					     GList *files)
{
	if (model->details->highlight_files != NULL) {
	...
	if (files != NULL) {
		model->details->highlight_files = athena_file_list_copy (files);
		g_list_foreach (model->details->highlight_files,
		                refresh_row, model);
