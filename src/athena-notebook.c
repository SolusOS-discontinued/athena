...
static void athena_notebook_init		 (AthenaNotebook *notebook);
static void athena_notebook_class_init	 (AthenaNotebookClass *klass);
static int  athena_notebook_insert_page	 (GtkNotebook *notebook,
					  GtkWidget *child,
					  GtkWidget *tab_label,
					  GtkWidget *menu_label,
					  int position);
static void athena_notebook_remove	 (GtkContainer *container,
...
G_DEFINE_TYPE (AthenaNotebook, athena_notebook, GTK_TYPE_NOTEBOOK);

static void
athena_notebook_class_init (AthenaNotebookClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);
	GtkNotebookClass *notebook_class = GTK_NOTEBOOK_CLASS (klass);

	container_class->remove = athena_notebook_remove;

	notebook_class->insert_page = athena_notebook_insert_page;

	signals[TAB_CLOSE_REQUEST] =
		g_signal_new ("tab-close-request",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AthenaNotebookClass, tab_close_request),
			      NULL, NULL,
...
find_notebook_at_pointer (gint abs_x, gint abs_y)
{
	...
	/* toplevel should be an AthenaWindow */
	if (toplevel != NULL && ATHENA_IS_WINDOW (toplevel))
	{
		return ATHENA_NOTEBOOK (ATHENA_WINDOW (toplevel)->details->active_pane->notebook);
	}
...
static gboolean
is_in_notebook_window (AthenaNotebook *notebook,
		       gint abs_x, gint abs_y)
{
	AthenaNotebook *nb_at_pointer;
...
static gint
find_tab_num_at_pos (AthenaNotebook *notebook, gint abs_x, gint abs_y)
{
...
static gboolean
button_press_cb (AthenaNotebook *notebook,
...
static void
athena_notebook_init (AthenaNotebook *notebook)
{
...
void
athena_notebook_sync_loading (AthenaNotebook *notebook,
				AthenaWindowSlot *slot)
{
	GtkWidget *tab_label, *spinner, *icon;
	gboolean active;

	g_return_if_fail (ATHENA_IS_NOTEBOOK (notebook));
...
void
athena_notebook_sync_tab_label (AthenaNotebook *notebook,
				  AthenaWindowSlot *slot)
{
			 ...
			 AthenaWindowSlot *slot)
{
	...
	notebook = gtk_widget_get_ancestor (GTK_WIDGET (slot), ATHENA_TYPE_NOTEBOOK);
	if (notebook != NULL) {
...
static GtkWidget *
build_tab_label (AthenaNotebook *nb, AthenaWindowSlot *slot)
{
	...
	gtk_widget_set_name (close_button, "athena-tab-close-button");
	...
	athena_drag_slot_proxy_init (hbox, NULL, slot);
...
static int
athena_notebook_insert_page (GtkNotebook *gnotebook,
			       GtkWidget *tab_widget,
			       GtkWidget *tab_label,
			       GtkWidget *menu_label,
			       int position)
{
	...
	position = GTK_NOTEBOOK_CLASS (athena_notebook_parent_class)->insert_page (gnotebook,
										     tab_widget,
										     tab_label,
										     menu_label,
										     position);
...
int
athena_notebook_add_tab (AthenaNotebook *notebook,
			   AthenaWindowSlot *slot,
			   int position,
			   gboolean jump_to)
{
	...
	g_return_val_if_fail (ATHENA_IS_NOTEBOOK (notebook), -1);
	g_return_val_if_fail (ATHENA_IS_WINDOW_SLOT (slot), -1);
...
static void
athena_notebook_remove (GtkContainer *container,
			  GtkWidget *tab_widget)
{
	GtkNotebook *gnotebook = GTK_NOTEBOOK (container);
	GTK_CONTAINER_CLASS (athena_notebook_parent_class)->remove (container, tab_widget);
...
void
athena_notebook_reorder_current_child_relative (AthenaNotebook *notebook,
						  int offset)
{
	...
	if (!athena_notebook_can_reorder_current_child_relative (notebook, offset)) {
		return;
	}
...
void
athena_notebook_set_current_page_relative (AthenaNotebook *notebook,
					     int offset)
{
	GtkNotebook *gnotebook;
	int page;
	...
	if (!athena_notebook_can_set_current_page_relative (notebook, offset)) {
		return;
	}
...
static gboolean
athena_notebook_is_valid_relative_position (AthenaNotebook *notebook,
					      int offset)
{
	GtkNotebook *gnotebook;
	int page;
...
gboolean
athena_notebook_can_reorder_current_child_relative (AthenaNotebook *notebook,
						      int offset)
{
	g_return_val_if_fail (ATHENA_IS_NOTEBOOK (notebook), FALSE);

	return athena_notebook_is_valid_relative_position (notebook, offset);
}

gboolean
athena_notebook_can_set_current_page_relative (AthenaNotebook *notebook,
						 int offset)
{
	g_return_val_if_fail (ATHENA_IS_NOTEBOOK (notebook), FALSE);
