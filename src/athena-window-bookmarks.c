...
static void refresh_bookmarks_menu (AthenaWindow *window);
...
remove_bookmarks_for_uri_if_yes (GtkDialog *dialog, int response, gpointer callback_data)
{
	...
	if (response == GTK_RESPONSE_YES) {
		...
		athena_bookmark_list_delete_items_with_uri (window->details->bookmark_list, uri);
	}
...
static GtkWindow *
get_or_create_bookmarks_window (AthenaWindow *window)
{
 ...
 **/
void
athena_bookmarks_exiting (void)
{
	if (bookmarks_window != NULL) {
		athena_bookmarks_window_save_geometry (bookmarks_window);
 ...
 */
void
athena_window_add_bookmark_for_current_location (AthenaWindow *window)
{
	AthenaBookmark *bookmark;
	...
	if (!athena_bookmark_list_contains (list, bookmark)) {
		athena_bookmark_list_append (list, bookmark); 
	}
}

void
athena_window_edit_bookmarks (AthenaWindow *window)
{
	GtkWindow *dialog;
...
bookmark_holder_new (AthenaBookmark *bookmark, 
		     AthenaWindow *window,
		     GCallback refresh_callback,
		     AthenaBookmarkFailedCallback failed_callback)
{
...
activate_bookmark_in_menu_item (GtkAction *action, gpointer user_data)
{
	...
	if (athena_bookmark_uri_known_not_to_exist (holder->bookmark)) {
		holder->failed_callback (holder->window, holder->bookmark);
	} else {
	        location = athena_bookmark_get_location (holder->bookmark);
		slot = athena_window_get_active_slot (holder->window);
...
static void
refresh_bookmarks_menu (AthenaWindow *window)
{
	g_assert (ATHENA_IS_WINDOW (window));
 ...
 */
void 
athena_window_initialize_bookmarks_menu (AthenaWindow *window)
{
	g_assert (ATHENA_IS_WINDOW (window));
