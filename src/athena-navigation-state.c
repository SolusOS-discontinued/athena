...
struct _AthenaNavigationStateDetails {
...
G_DEFINE_TYPE (AthenaNavigationState, athena_navigation_state, G_TYPE_OBJECT);

static void
clear_bindings (AthenaNavigationState *self)
{
...
static void
athena_navigation_state_init (AthenaNavigationState *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, ATHENA_TYPE_NAVIGATION_STATE,
						  AthenaNavigationStateDetails);
}
...
static void
athena_navigation_state_finalize (GObject *obj)
{
	AthenaNavigationState *self = ATHENA_NAVIGATION_STATE (obj);

	g_strfreev (self->priv->action_names);

	G_OBJECT_CLASS (athena_navigation_state_parent_class)->finalize (obj);	
}
...
static void
athena_navigation_state_class_init (AthenaNavigationStateClass *klass)
{
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->dispose = athena_navigation_state_dispose;
	oclass->finalize = athena_navigation_state_finalize;
	oclass->set_property = athena_navigation_state_set_property;
	...
	g_type_class_add_private (klass, sizeof (AthenaNavigationStateDetails));
}

AthenaNavigationState *
athena_navigation_state_new (GtkActionGroup *slave,
			       const gchar **action_names)
{
	return g_object_new (ATHENA_TYPE_NAVIGATION_STATE,
			     "slave", slave,
...
void
athena_navigation_state_add_group (AthenaNavigationState *self,
...
GtkActionGroup *
athena_navigation_state_get_master (AthenaNavigationState *self)
{
