...
#include "athena-tree-sidebar-model.h"
...
struct FMTreeViewDetails {
	...
	AthenaWindowOpenFlags activation_flags;
...
static void
notify_clipboard_info (AthenaClipboardMonitor *monitor,
...
static gboolean
show_iter_for_file (FMTreeView *view, AthenaFile *file, GtkTreeIter *iter)
{
	GtkTreeModel *model;
...
show_selection_idle_callback (gpointer callback_data)
{
	...
	file = athena_file_get_by_uri (view->details->selection_location);
	if (file == NULL) {
		return FALSE;
	}
	...
	if (!athena_file_is_directory (file)) {
		old_file = file;
		file = athena_file_get_parent (file);
...
row_loaded_callback (GtkTreeModel     *tree_model,
		     ...
		     FMTreeView *view)
{
	AthenaFile *file, *tmp_file, *selection_file;
	...
	selection_file = athena_file_get_by_uri (view->details->selection_location);
	while (selection_file != NULL) {
		if (file == selection_file) {
			athena_file_unref (file);
			athena_file_unref (selection_file);

			schedule_show_selection (view);
			return;
		}
		tmp_file = athena_file_get_parent (selection_file);
		athena_file_unref (selection_file);
...
static void
got_activation_uri_callback (AthenaFile *file, gpointer callback_data)
{
        char *uri, *file_uri;
        FMTreeView *view;
	GdkScreen *screen;
	...
	open_in_same_slot =
		(view->details->activation_flags &
		 (ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW |
		  ATHENA_WINDOW_OPEN_FLAG_NEW_TAB)) == 0;

	slot = athena_window_get_active_slot (view->details->window);
	...
	if (athena_file_is_launcher (file)) {
		...
		athena_launch_desktop_file (screen, file_uri, NULL, NULL);
		g_free (file_uri);
	} else if (uri != NULL
		...
		if (file_uri == NULL) {
			...
			athena_window_slot_open_location
				(slot,
				 location, 
				 view->details->activation_flags,
				 NULL);
			...
			athena_launch_application_from_command (screen, file_uri, FALSE, NULL);
			g_free (file_uri);
		}
	...
	g_free (uri);
	athena_file_unref (view->details->activation_file);
...
selection_changed_timer_callback(FMTreeView *view)
{
	...
	attributes = ATHENA_FILE_ATTRIBUTE_INFO | ATHENA_FILE_ATTRIBUTE_LINK_INFO;
	athena_file_call_when_ready (view->details->activation_file, attributes,
				       got_activation_uri_callback, view);
...
compare_rows (GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer callback_data)
{
	...
	if (file_a == file_b) {
		...
		result = athena_file_compare_for_sort (file_a, file_b,
							 ATHENA_FILE_SORT_BY_DISPLAY_NAME,
							 FALSE, FALSE);
	}

	athena_file_unref (file_a);
	athena_file_unref (file_b);
...
static gboolean
is_parent_writable (AthenaFile *file)
{
	AthenaFile *parent;
	gboolean result;
	...
	result = athena_file_can_write (parent);
			 ...
			 FMTreeView *view)
{
	...
	using_browser = g_settings_get_boolean (athena_preferences,
	...
	if (event->button == 3) {
		...
		if (using_browser) {
			gtk_widget_set_sensitive (view->details->popup_open_in_new_window,
						  athena_file_is_directory (view->details->popup_file));
		...
		gtk_widget_set_sensitive (view->details->popup_create_folder,
			athena_file_is_directory (view->details->popup_file) &&
			athena_file_can_write (view->details->popup_file));
		gtk_widget_set_sensitive (view->details->popup_paste, FALSE);
		if (athena_file_is_directory (view->details->popup_file) &&
			athena_file_can_write (view->details->popup_file)) {
			gtk_clipboard_request_contents (athena_clipboard_get (GTK_WIDGET (view->details->tree_widget)),
							copied_files_atom,
							clipboard_contents_received_callback, g_object_ref (view));
		}
		can_move_file_to_trash = athena_file_can_trash (view->details->popup_file);
		gtk_widget_set_sensitive (view->details->popup_trash, can_move_file_to_trash);
		
		if (g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_ENABLE_DELETE)) {
			parent_file_is_writable = is_parent_writable (view->details->popup_file);
			file_is_home_or_desktop = athena_file_is_home (view->details->popup_file)
				|| athena_file_is_desktop_directory (view->details->popup_file);
			file_is_special_link = ATHENA_IS_DESKTOP_ICON_FILE (view->details->popup_file);
			    ...
			    AthenaWindowOpenFlags flags)
{
	...
	view->details->activation_file = athena_file_ref (file);
...
fm_tree_view_open_in_new_tab_cb (GtkWidget *menu_item,
				    FMTreeView *view)
{
	fm_tree_view_activate_file (view, view->details->popup_file, ATHENA_WINDOW_OPEN_FLAG_NEW_TAB);
}
...
fm_tree_view_open_in_new_window_cb (GtkWidget *menu_item,
				    FMTreeView *view)
{
	fm_tree_view_activate_file (view, view->details->popup_file, ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
		 ...
		 gpointer data)
{
	 ...
	 */
	list = g_list_prepend (NULL, athena_file_get (new_folder));

	athena_properties_window_present (list, GTK_WIDGET (data), NULL);
			       ...
			       FMTreeView *view)
{
	...
	parent_uri = athena_file_get_uri (view->details->popup_file);
	athena_file_operations_new_folder (GTK_WIDGET (view->details->tree_widget),
					     NULL,
					     parent_uri,
		   ...
		   gboolean cut)
{
	...
	gtk_clipboard_set_with_data (athena_clipboard_get (GTK_WIDGET (view->details->tree_widget)),
				     targets, n_targets,
				     athena_get_clipboard_callback, athena_clear_clipboard_callback,
				     NULL);
        gtk_target_table_free (targets, n_targets);

	athena_clipboard_monitor_set_clipboard_info (athena_clipboard_monitor_get (),
	                                               &info);
	g_list_free (info.files);

	name = athena_file_get_display_name (view->details->popup_file);
	if (cut) {
	...
	athena_window_push_status (view->details->window,
					  status_string);
					...
					gpointer          data)
{
	...
	directory_uri = athena_file_get_uri (view->details->popup_file);
		       ...
		       FMTreeView *view)
{
	...
	if (!athena_file_can_trash (view->details->popup_file)) {
		return;
	}
	
	list = g_list_prepend (NULL,
			       athena_file_get_location (view->details->popup_file));
	
	athena_file_operations_trash_or_delete (list, 
		        ...
		        FMTreeView *view)
{
	...
	athena_file_operations_delete (location_list, fm_tree_view_get_containing_window (view), NULL, NULL);
			    ...
			    FMTreeView *view)
{
	...
	list = g_list_prepend (NULL, athena_file_ref (view->details->popup_file));

	athena_properties_window_present (list, GTK_WIDGET (view->details->tree_widget), NULL);
...
fm_tree_view_unmount_cb (GtkWidget *menu_item,
			 FMTreeView *view)
{
	AthenaFile *file = view->details->popup_file;
	GMount *mount;
	...
	if (mount != NULL) {
		athena_file_operations_unmount_mount (fm_tree_view_get_containing_window (view),
							mount, FALSE, TRUE);
	}
}
...
free_popup_file_in_idle_cb (gpointer data)
{
	...
	if (view->details->popup_file != NULL) {
		athena_file_unref (view->details->popup_file);
...
create_popup_menu (FMTreeView *view)
{
	...
	g_signal_connect (menu_item, "activate",
			  ...
			  view);
	g_settings_bind (athena_preferences,
			 ATHENA_PREFERENCES_ALWAYS_USE_BROWSER,
			 menu_item,
			 "visible",
			 G_SETTINGS_BIND_GET);
	...
	menu_image = gtk_image_new_from_icon_name (ATHENA_ICON_TRASH_FULL,
						   GTK_ICON_SIZE_MENU);
	gtk_widget_show (menu_image);
	...
	menu_image = gtk_image_new_from_icon_name (ATHENA_ICON_DELETE,
						   GTK_ICON_SIZE_MENU);
	gtk_widget_show (menu_image);
...
create_tree (FMTreeView *view)
{
	...
	icon = g_themed_icon_new (ATHENA_ICON_COMPUTER);
	fm_tree_model_add_root_uri (view->details->child_model, "computer:///", _("Computer"), icon, NULL);
	g_object_unref (icon);
#endif
 	home_uri = athena_get_home_directory_uri ();
	...
	view->details->drag_dest = 
		athena_tree_view_drag_dest_new (view->details->tree_widget);
...
update_filtering_from_preferences (FMTreeView *view)
{
	...
	fm_tree_model_set_show_only_directories
		(view->details->child_model,
		 g_settings_get_boolean (athena_tree_sidebar_preferences,
...
fm_tree_view_init (FMTreeView *view)
{
	...
	g_signal_connect_swapped (athena_tree_sidebar_preferences,
				  "changed::" ATHENA_PREFERENCES_TREE_SHOW_ONLY_DIRECTORIES,
...
fm_tree_view_dispose (GObject *object)
{
	...
	g_signal_handlers_disconnect_by_func (athena_preferences,
					      G_CALLBACK(filtering_changed_callback),
					      view);
	g_signal_handlers_disconnect_by_func (athena_tree_sidebar_preferences,
					      G_CALLBACK(filtering_changed_callback),
					      view);
...
static void 
hidden_files_mode_changed_callback (AthenaWindow *window,
				    FMTreeView *view)
{
	update_filtering_from_preferences (view);
}
...
GtkWidget *
athena_tree_sidebar_new (AthenaWindow *window)
{
	FMTreeView *sidebar;
