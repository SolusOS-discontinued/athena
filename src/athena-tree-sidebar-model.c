...
typedef gboolean (* FilePredicate) (AthenaFile *);
...
static TreeNode *
tree_node_new (AthenaFile *file, FMTreeModelRoot *root)
{
	TreeNode *node;

	node = g_new0 (TreeNode, 1);
	node->file = athena_file_ref (file);
	node->root = root;
	return node;
}
...
tree_node_destroy (FMTreeModel *model, TreeNode *node)
{
	...
	athena_directory_unref (node->directory);
			...
			AthenaFileIconFlags flags)
{
	...
	size = athena_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
	gicon = G_ICON (athena_file_get_icon_pixbuf (file, size, TRUE, flags));

	i = 0;
	emblems_to_ignore[i++] = ATHENA_FILE_EMBLEM_NAME_TRASH;

	if (node->parent && node->parent->file) {
		if (!athena_file_can_write (node->parent->file)) {
	...
	emblem = NULL;
	emblem_icons = athena_file_get_emblem_icons (node->file,
						       emblems_to_ignore);
	...
	info = athena_icon_info_lookup (gicon, size);
	retval = athena_icon_info_get_pixbuf_nodefault_at_size (info, size);
	model = node->root->model;

	g_object_unref (gicon);

	highlight = (g_list_find_custom (model->details->highlighted_files,
	                                 file, (GCompareFunc) athena_file_compare_location) != NULL);
...
{
	return tree_node_update_pixbuf (node, &node->open_pixbuf, ATHENA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
}
...
tree_node_update_display_name (TreeNode *node)
{
	...
	display_name = athena_file_get_display_name (node->file);
...
tree_node_get_open_pixbuf (TreeNode *node)
{
	if (node->open_pixbuf == NULL) {
		node->open_pixbuf = tree_node_get_pixbuf (node, ATHENA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
	}
...
{
	if (node->display_name == NULL) {
		node->display_name = athena_file_get_display_name (node->file);
	}
...
static TreeNode *
get_node_from_file (FMTreeModelRoot *root, AthenaFile *file)
{
	return g_hash_table_lookup (root->file_to_node_map, file);
}

static TreeNode *
get_parent_node_from_file (FMTreeModelRoot *root, AthenaFile *file)
{
...
static TreeNode *
create_node_for_file (FMTreeModelRoot *root, AthenaFile *file)
{
	TreeNode *node;
...
get_node_uri (GtkTreeIter *iter)
{
	...
	node = iter->user_data;
	if (node != NULL) {
		return athena_file_get_uri (node->file);
	}

	parent = iter->user_data2;
	parent_uri = athena_file_get_uri (parent->file);
...
abandon_node_ref_count (FMTreeModel *model, TreeNode *node)
{
	if (node->parent != NULL) {
		...
		if (node->ref_count != 0) {
			...
			uri = athena_file_get_uri (node->file);
...
stop_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
	...
	athena_directory_file_monitor_remove (node->directory, model);
}
...
update_node_without_reporting (FMTreeModel *model, TreeNode *node)
{
	...
	if (node->directory == NULL &&
	    (athena_file_is_directory (node->file) || node->parent == NULL)) {
		node->directory = athena_directory_get_for_file (node->file);
...
static gboolean
should_show_file (FMTreeModel *model, AthenaFile *file)
{
	gboolean should;
	TreeNode *node;

	should = athena_file_should_show (file,
	...
	if (should && athena_file_is_gone (file)) {
		should = FALSE;
	}
...
update_node (FMTreeModel *model, TreeNode *node)
{
	...
	if (node->parent != NULL && node->parent->directory != NULL
	    && !athena_directory_contains_file (node->parent->directory, node->file)) {
			...
			gpointer callback_data)
{
	...
	for (node = changed_files; node != NULL; node = node->next) {
		process_file_change (root, ATHENA_FILE (node->data));
	}
}
		       ...
		       FMTreeModelRoot *root)
{
	...
	if (node == NULL) {
		 ...
		 * broken due to athena_directory_remove_file()
...
start_monitoring_directory (FMTreeModel *model, TreeNode *node)
{
	...
	set_done_loading (model, node, athena_directory_are_all_files_seen (directory));

	attributes = get_tree_monitor_attributes ();
	athena_directory_file_monitor_add (directory, model,
					     model->details->show_hidden_files,
...
#if 0
	g_warning ("Node '%s' %s",
		   node && node->file ? athena_file_get_uri (node->file) : "no name",
...
fm_tree_model_add_root_uri (FMTreeModel *model, const char *root_uri, const char *display_name, GIcon *icon, GMount *mount)
{
	...
	file = athena_file_get_by_uri (root_uri);
...
GMount *
fm_tree_model_get_mount_for_root_node_file (FMTreeModel *model, AthenaFile *file)
{
	TreeNode *node;
...
fm_tree_model_remove_root_uri (FMTreeModel *model, const char *uri)
{
	...
	if (node) {
		...
		athena_file_monitor_remove (node->file, model);
		path = get_node_path (model, node);
					   ...
					   gboolean show_hidden_files)
{
	...
	if (!show_hidden_files) {
		destroy_by_function (model, athena_file_is_hidden_file);
	}
	schedule_monitoring_update (model);
}

static gboolean
file_is_not_directory (AthenaFile *file)
{
...
fm_tree_model_iter_get_file (FMTreeModel *model, GtkTreeIter *iter)
{
	...
	node = iter->user_data;
	return node == NULL ? NULL : athena_file_ref (node->file);
}
...
static void
do_update_node (AthenaFile *file,
                  FMTreeModel *model)
{
                                       ...
                                       GList *files)
{
	...
	if (model->details->highlighted_files != NULL) {
		...
		athena_file_list_free (old_files);
	}
...
fm_tree_model_finalize (GObject *object)
{
	...
	if (model->details->highlighted_files != NULL) {
		athena_file_list_free (model->details->highlighted_files);
	}
