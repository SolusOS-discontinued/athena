...
#include <libathena-private/athena-directory-notify.h>
...
struct AthenaDesktopIconViewDetails
{
...
static void     real_merge_menus                                  (AthenaView        *view);
static void     real_update_menus                                 (AthenaView        *view);
static void     athena_desktop_icon_view_update_icon_container_fonts  (AthenaDesktopIconView      *view);
static void     font_changed_callback                             (gpointer                callback_data);

G_DEFINE_TYPE (AthenaDesktopIconView, athena_desktop_icon_view, ATHENA_TYPE_ICON_VIEW)

static char *desktop_directory;
static time_t desktop_dir_modify_time;

#define get_icon_container(w) athena_icon_view_get_icon_container(ATHENA_ICON_VIEW (w))
...
static void
icon_container_set_workarea (AthenaIconContainer *icon_container,
			     GdkScreen             *screen,
			     long                  *workareas,
			     int                    n_items)
{
	...
	athena_icon_container_set_margins (icon_container,
					     left, right, top, bottom);
}

static void
net_workarea_changed (AthenaDesktopIconView *icon_view,
		      GdkWindow         *window)
{
	...
	g_return_if_fail (ATHENA_IS_DESKTOP_ICON_VIEW (icon_view));
				   ...
				   gpointer data)
{
	...
	AthenaDesktopIconView *icon_view;

	icon_view = ATHENA_DESKTOP_ICON_VIEW (data);
...
static void
real_begin_loading (AthenaView *object)
{
	AthenaIconContainer *icon_container;
	AthenaDesktopIconView *view;

	view = ATHENA_DESKTOP_ICON_VIEW (object);

	icon_container = get_icon_container (view);
	if (view->details->background == NULL) {
		view->details->background = athena_desktop_background_new (icon_container);
	}

	ATHENA_VIEW_CLASS (athena_desktop_icon_view_parent_class)->begin_loading (object);
}

static const char *
real_get_id (AthenaView *view)
{
	return ATHENA_DESKTOP_ICON_VIEW_ID;
}

static void
athena_desktop_icon_view_dispose (GObject *object)
{
	AthenaDesktopIconView *icon_view;
	GtkUIManager *ui_manager;

	icon_view = ATHENA_DESKTOP_ICON_VIEW (object);
	...
	G_OBJECT_CLASS (athena_desktop_icon_view_parent_class)->dispose (object);
}

static void
athena_desktop_icon_view_class_init (AthenaDesktopIconViewClass *class)
{
	AthenaViewClass *vclass;

	vclass = ATHENA_VIEW_CLASS (class);

	G_OBJECT_CLASS (class)->dispose = athena_desktop_icon_view_dispose;
	...
	g_type_class_add_private (class, sizeof (AthenaDesktopIconViewDetails));
}

static void
athena_desktop_icon_view_handle_middle_click (AthenaIconContainer *icon_container,
						GdkEventButton *event,
						AthenaDesktopIconView *desktop_icon_view)
{
...
static void
unrealized_callback (GtkWidget *widget, AthenaDesktopIconView *desktop_icon_view)
{
...
static void
realized_callback (GtkWidget *widget, AthenaDesktopIconView *desktop_icon_view)
{
	GdkWindow *root_window;
	GdkScreen *screen;
...
get_default_zoom_level (void)
{
	...
	return CLAMP (default_zoom_level, ATHENA_ZOOM_LEVEL_SMALLEST, ATHENA_ZOOM_LEVEL_LARGEST);
}
...
default_zoom_level_changed (gpointer user_data)
{
	...
	desktop_icon_view = ATHENA_DESKTOP_ICON_VIEW (user_data);
	new_level = get_default_zoom_level ();

	athena_icon_container_set_zoom_level (get_icon_container (desktop_icon_view),
						new_level);
}
...
do_desktop_rescan (gpointer data)
{
	...
	desktop_icon_view = ATHENA_DESKTOP_ICON_VIEW (data);
	...
	athena_directory_force_reload
		(athena_view_get_model (ATHENA_VIEW (desktop_icon_view)));

	return TRUE;
}

static void
done_loading (AthenaDirectory *model,
	      AthenaDesktopIconView *desktop_icon_view)
{
	struct stat buf;
...
/* This function is used because the AthenaDirectory model does not
 * exist always in the desktop_icon_view, so we wait until it has been
 * instantiated.
 */
static void
delayed_init (AthenaDesktopIconView *desktop_icon_view)
{
	/* Keep track of the load time. */
	g_signal_connect_object (athena_view_get_model (ATHENA_VIEW (desktop_icon_view)),
				 "done_loading",
...
{
 	g_return_if_fail (ATHENA_IS_DESKTOP_ICON_VIEW (callback_data));
	
	athena_desktop_icon_view_update_icon_container_fonts (ATHENA_DESKTOP_ICON_VIEW (callback_data));
}

static void
athena_desktop_icon_view_update_icon_container_fonts (AthenaDesktopIconView *icon_view)
{
	AthenaIconContainer *icon_container;
	char *font;
	...
	font = g_settings_get_string (athena_desktop_preferences,
				      ATHENA_PREFERENCES_DESKTOP_FONT);

	athena_icon_container_set_font (icon_container, font);

	g_free (font);
}

static void
athena_desktop_icon_view_init (AthenaDesktopIconView *desktop_icon_view)
{
	AthenaIconContainer *icon_container;
	GtkAllocation allocation;
	...
	desktop_icon_view->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_icon_view,
								  ATHENA_TYPE_DESKTOP_ICON_VIEW,
								  AthenaDesktopIconViewDetails);
	...
	athena_icon_view_filter_by_screen (ATHENA_ICON_VIEW (desktop_icon_view), TRUE);
	icon_container = get_icon_container (desktop_icon_view);
	athena_icon_container_set_use_drop_shadows (icon_container, TRUE);
	athena_icon_view_container_set_sort_desktop (ATHENA_ICON_VIEW_CONTAINER (icon_container), TRUE);

	/* Do a reload on the desktop if we don't have FAM, a smarter
	 * way to keep track of the items on the desktop.
	 */
	if (!athena_monitor_active ()) {
	...
	athena_icon_container_set_is_fixed_size (icon_container, TRUE);
	athena_icon_container_set_is_desktop (icon_container, TRUE);
	athena_icon_container_set_store_layout_timestamps (icon_container, TRUE);
	...
	athena_view_ignore_hidden_file_preferences
		(ATHENA_VIEW (desktop_icon_view));

	athena_view_set_show_foreign (ATHENA_VIEW (desktop_icon_view),
					FALSE);
	...
	g_signal_connect_object (icon_container, "middle_click",
				 G_CALLBACK (athena_desktop_icon_view_handle_middle_click), desktop_icon_view, 0);
	...
	g_signal_connect_swapped (athena_desktop_preferences,
				  "changed::" ATHENA_PREFERENCES_DESKTOP_FONT,
				  G_CALLBACK (font_changed_callback),
				  desktop_icon_view);

	default_zoom_level_changed (desktop_icon_view);
	athena_desktop_icon_view_update_icon_container_fonts (desktop_icon_view);

	g_signal_connect_swapped (gnome_lockdown_preferences,
				  "changed::" ATHENA_PREFERENCES_LOCKDOWN_COMMAND_LINE,
				  G_CALLBACK (athena_view_update_menus),
				  desktop_icon_view);
}
...
action_change_background_callback (GtkAction *action, 
				   gpointer data)
{
        g_assert (ATHENA_VIEW (data));

	athena_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (data)),
...
action_empty_trash_conditional_callback (GtkAction *action,
					 gpointer data)
{
        g_assert (ATHENA_IS_VIEW (data));

	athena_file_operations_empty_trash (GTK_WIDGET (data));
}
...
static gboolean
trash_link_is_selection (AthenaView *view)
{
	GList *selection;
	...
	if (eel_g_list_exactly_one_item (selection) &&
	    ATHENA_IS_DESKTOP_ICON_FILE (selection->data)) {
		link = athena_desktop_icon_file_get_link (ATHENA_DESKTOP_ICON_FILE (selection->data));
		/* link may be NULL if the link was recently removed (unmounted) */
		if (link != NULL &&
		    athena_desktop_link_get_link_type (link) == ATHENA_DESKTOP_LINK_TRASH) {
			result = TRUE;
		}
		if (link) {
			g_object_unref (link);
...
{
	AthenaDesktopIconView *desktop_view;
	char *label;
	gboolean include_empty_trash;
	GtkAction *action;

	g_assert (ATHENA_IS_DESKTOP_ICON_VIEW (view));

	ATHENA_VIEW_CLASS (athena_desktop_icon_view_parent_class)->update_menus (view);

	desktop_view = ATHENA_DESKTOP_ICON_VIEW (view);


	/* New Launcher */
	action = gtk_action_group_get_action (desktop_view->details->desktop_action_group,
					      ATHENA_ACTION_NEW_LAUNCHER_DESKTOP);
	...
	action = gtk_action_group_get_action (desktop_view->details->desktop_action_group,
					      ATHENA_ACTION_EMPTY_TRASH_CONDITIONAL);
	gtk_action_set_visible (action,
				include_empty_trash);
...
real_merge_menus (AthenaView *view)
{
	...
	ATHENA_VIEW_CLASS (athena_desktop_icon_view_parent_class)->merge_menus (view);
	...
	desktop_view->details->desktop_merge_id =
		gtk_ui_manager_add_ui_from_resource (ui_manager, "/com/solusos/athena/athena-desktop-icon-view-ui.xml", NULL);
}

static AthenaView *
athena_desktop_icon_view_create (AthenaWindowSlot *slot)
{
	AthenaIconView *view;

	view = g_object_new (ATHENA_TYPE_DESKTOP_ICON_VIEW,
			     "window-slot", slot,
...
static gboolean
athena_desktop_icon_view_supports_uri (const char *uri,
				   GFileType file_type,
...
static AthenaViewInfo athena_desktop_icon_view = {
	ATHENA_DESKTOP_ICON_VIEW_ID,
	"Desktop View",
	"_Desktop",
	...
	athena_desktop_icon_view_create,
	athena_desktop_icon_view_supports_uri
};

void
athena_desktop_icon_view_register (void)
{
	athena_desktop_icon_view.error_label = _(athena_desktop_icon_view.error_label);
	athena_desktop_icon_view.startup_error_label = _(athena_desktop_icon_view.startup_error_label);
	
	athena_view_factory_register (&athena_desktop_icon_view);
}
