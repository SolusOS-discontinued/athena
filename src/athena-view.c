...
#include <libathena-private/athena-desktop-directory.h>
...
#include <libathena-private/athena-file-changes-queue.h>
...
#include <libathena-private/athena-file-private.h>
...
#include <libathena-private/athena-recent.h>
...
#include <libathena-private/athena-file-undo-manager.h>

#define DEBUG_FLAG ATHENA_DEBUG_DIRECTORY_VIEW
...
#define ATHENA_VIEW_MENU_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER  "/MenuBar/File/Open Placeholder/Open With/Applications Placeholder"
#define ATHENA_VIEW_MENU_PATH_APPLICATIONS_PLACEHOLDER    	  "/MenuBar/File/Open Placeholder/Applications Placeholder"
#define ATHENA_VIEW_MENU_PATH_SCRIPTS_PLACEHOLDER               "/MenuBar/File/Open Placeholder/Scripts/Scripts Placeholder"
#define ATHENA_VIEW_MENU_PATH_EXTENSION_ACTIONS_PLACEHOLDER     "/MenuBar/Edit/Extension Actions"
#define ATHENA_VIEW_MENU_PATH_NEW_DOCUMENTS_PLACEHOLDER  	  "/MenuBar/File/New Items Placeholder/New Documents/New Documents Placeholder"
#define ATHENA_VIEW_MENU_PATH_OPEN				  "/MenuBar/File/Open Placeholder/Open"

#define ATHENA_VIEW_POPUP_PATH_SELECTION			  "/selection"
#define ATHENA_VIEW_POPUP_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER "/selection/Open Placeholder/Open With/Applications Placeholder"
#define ATHENA_VIEW_POPUP_PATH_APPLICATIONS_PLACEHOLDER    	  "/selection/Open Placeholder/Applications Placeholder"
#define ATHENA_VIEW_POPUP_PATH_SCRIPTS_PLACEHOLDER    	  "/selection/Open Placeholder/Scripts/Scripts Placeholder"
#define ATHENA_VIEW_POPUP_PATH_EXTENSION_ACTIONS		  "/selection/Extension Actions"
#define ATHENA_VIEW_POPUP_PATH_OPEN				  "/selection/Open Placeholder/Open"

#define ATHENA_VIEW_POPUP_PATH_BACKGROUND			  "/background"
#define ATHENA_VIEW_POPUP_PATH_BACKGROUND_SCRIPTS_PLACEHOLDER	  "/background/Before Zoom Items/New Object Items/Scripts/Scripts Placeholder"
#define ATHENA_VIEW_POPUP_PATH_BACKGROUND_NEW_DOCUMENTS_PLACEHOLDER "/background/Before Zoom Items/New Object Items/New Documents/New Documents Placeholder"

#define ATHENA_VIEW_POPUP_PATH_LOCATION			  "/location"
...
struct AthenaViewDetails
{
	AthenaWindow *window;
	AthenaWindowSlot *slot;
	AthenaDirectory *model;
	AthenaFile *directory_as_file;
	AthenaFile *location_popup_directory_as_file;
	 ...
	 * This flag should need not be set inside subclasses. AthenaView automatically
...
static void     athena_view_duplicate_selection              (AthenaView      *view,
							        GList                *files,
							        GArray               *item_locations);
static void     athena_view_create_links_for_files           (AthenaView      *view,
							        GList                *files,
							        GArray               *item_locations);
static void     trash_or_delete_files                          (GtkWindow            *parent_window,
								...
								AthenaView      *view);
static void     load_directory                                 (AthenaView      *view,
								AthenaDirectory    *directory);
static void     athena_view_merge_menus                      (AthenaView      *view);
static void     athena_view_unmerge_menus                    (AthenaView      *view);
static void     athena_view_init_show_hidden_files           (AthenaView      *view);
static void     clipboard_changed_callback                     (AthenaClipboardMonitor *monitor,
								AthenaView      *view);
static void     open_one_in_new_window                         (gpointer              data,
								gpointer              callback_data);
static void     schedule_update_menus                          (AthenaView      *view);
static void     schedule_update_menus_callback                 (gpointer              callback_data);
static void     remove_update_menus_timeout_callback           (AthenaView      *view);
static void     schedule_update_status                          (AthenaView      *view);
static void     remove_update_status_idle_callback             (AthenaView *view); 
static void     reset_update_interval                          (AthenaView      *view);
static void     schedule_idle_display_of_pending_files         (AthenaView      *view);
static void     unschedule_display_of_pending_files            (AthenaView      *view);
static void     disconnect_model_handlers                      (AthenaView      *view);
static void     metadata_for_directory_as_file_ready_callback  (AthenaFile         *file,
								gpointer              callback_data);
static void     metadata_for_files_in_directory_ready_callback (AthenaDirectory    *directory,
								GList                *files,
								gpointer              callback_data);
static void     athena_view_trash_state_changed_callback     (AthenaTrashMonitor *trash,
							        gboolean              state,
							        gpointer              callback_data);
static void     athena_view_select_file                      (AthenaView      *view,
							        AthenaFile         *file);

static void     update_templates_directory                     (AthenaView *view);
static void     user_dirs_changed                              (AthenaView *view);

static gboolean file_list_all_are_folders                      (GList *file_list);

static void unschedule_pop_up_location_context_menu (AthenaView *view);

G_DEFINE_TYPE (AthenaView, athena_view, GTK_TYPE_SCROLLED_WINDOW);
#define parent_class athena_view_parent_class

/* virtual methods (public and non-public) */

/**
 * athena_view_merge_menus:
 * 
 * Add this view's menus to the window's menu bar.
 * @view: AthenaView in question.
 */
static void
athena_view_merge_menus (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->merge_menus (view);
}

static void
athena_view_unmerge_menus (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->unmerge_menus (view);}

static char *
real_get_backing_uri (AthenaView *view)
{
	AthenaDirectory *directory;
	char *uri;
       
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);
	...
	if (ATHENA_IS_DESKTOP_DIRECTORY (directory)) {
		directory = athena_desktop_directory_get_real_directory (ATHENA_DESKTOP_DIRECTORY (directory));
	} else {
...
char *
athena_view_get_backing_uri (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);

	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_backing_uri (view);
}

/**
 * athena_view_select_all:
 *
 * select all the items in the view
 * 
 **/
static void
athena_view_select_all (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->select_all (view);
}

static void
athena_view_call_set_selection (AthenaView *view, GList *selection)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->set_selection (view, selection);
}

static GList *
athena_view_get_selection_for_file_transfer (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);

	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_selection_for_file_transfer (view);
}

/**
 * athena_view_get_selected_icon_locations:
 *
 ...
 **/
static GArray *
athena_view_get_selected_icon_locations (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);

	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_selected_icon_locations (view);
}

static void
athena_view_invert_selection (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->invert_selection (view);
}

/**
 * athena_view_reveal_selection:
 *
 * Scroll as necessary to reveal the selected items.
 **/
static void
athena_view_reveal_selection (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->reveal_selection (view);
}

/**
 * athena_view_reset_to_defaults:
 *
 * set sorting order, zoom level, etc. to match defaults
 * 
 **/
static void
athena_view_reset_to_defaults (AthenaView *view)
{
	AthenaWindowShowHiddenFilesMode mode;

	g_return_if_fail (ATHENA_IS_VIEW (view));

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->reset_to_defaults (view);

	mode = athena_window_get_hidden_files_mode (view->details->window);
	if (mode != ATHENA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
		athena_window_set_hidden_files_mode (view->details->window,
						       ATHENA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT);
	}
}

static gboolean
athena_view_using_manual_layout (AthenaView  *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);

	return 	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->using_manual_layout (view);
}

static guint
athena_view_get_item_count (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), 0);

	return 	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_item_count (view);
}

/**
 * athena_view_can_rename_file
 *
 ...
 **/
static gboolean
athena_view_can_rename_file (AthenaView *view, AthenaFile *file)
{
	return 	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_rename_file (view, file);
}

static gboolean
athena_view_is_read_only (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);

	return 	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->is_read_only (view);
}

static gboolean
showing_trash_directory (AthenaView *view)
{
	AthenaFile *file;
...
static gboolean
athena_view_supports_creating_files (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);

	return !athena_view_is_read_only (view) && !showing_trash_directory (view);
}

static gboolean
athena_view_is_empty (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);

	return 	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->is_empty (view);
}

/**
 * athena_view_bump_zoom_level:
 *
 * bump the current zoom level by invoking the relevant subclass through the slot
 * 
 **/
void
athena_view_bump_zoom_level (AthenaView *view,
			       int zoom_increment)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));

	if (!athena_view_supports_zooming (view)) {
		return;
	}

	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->bump_zoom_level (view, zoom_increment);
}
 ...
 **/
void
athena_view_zoom_to_level (AthenaView *view,
			     AthenaZoomLevel zoom_level)
{
	...
	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->zoom_to_level (view, zoom_level);
}

AthenaZoomLevel
athena_view_get_zoom_level (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), ATHENA_ZOOM_LEVEL_STANDARD);

	if (!athena_view_supports_zooming (view)) {
		return ATHENA_ZOOM_LEVEL_STANDARD;
	}

	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_zoom_level (view);
}

/**
 * athena_view_can_zoom_in:
 *
 * Determine whether the view can be zoomed any closer.
 * @view: The zoomable AthenaView.
 * 
 * Return value: TRUE if @view can be zoomed any closer, FALSE otherwise.
 * 
 **/
gboolean
athena_view_can_zoom_in (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);
	...
	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_zoom_in (view);
}

/**
 * athena_view_can_zoom_out:
 *
 ...
 **/
gboolean
athena_view_can_zoom_out (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);
	...
	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->can_zoom_out (view);
}

gboolean
athena_view_supports_zooming (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);
...
/**
 * athena_view_restore_default_zoom_level:
 *
 * restore to the default zoom level by invoking the relevant subclass through the slot
 * 
 **/
void
athena_view_restore_default_zoom_level (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));
	...
	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->restore_default_zoom_level (view);
}

const char *
athena_view_get_view_id (AthenaView *view)
{
	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_view_id (view);
}

char *
athena_view_get_first_visible_file (AthenaView *view)
{
	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_first_visible_file (view);
}

void
athena_view_scroll_to_file (AthenaView *view,
			      const char *uri)
{
	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->scroll_to_file (view, uri);
}

char **
athena_view_get_emblem_names_to_exclude (AthenaView *view)
{
	char **excludes;
	int i;
	...
	i = 0;
	excludes[i++] = g_strdup (ATHENA_FILE_EMBLEM_NAME_TRASH);

	if (!athena_file_can_write (view->details->directory_as_file)) {
		excludes[i++] = g_strdup (ATHENA_FILE_EMBLEM_NAME_CANT_WRITE);
	}
...
/**
 * athena_view_get_selection:
 *
 * Get a list of AthenaFile pointers that represents the
 ...
 * @view: AthenaView whose selected items are of interest.
 * 
 * Return value: GList of AthenaFile pointers representing the selection.
 * 
 **/
GList *
athena_view_get_selection (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);

	return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->get_selection (view);
}

/**
 * athena_view_update_menus:
 * 
 * Update the sensitivity and wording of dynamic menu items.
 * @view: AthenaView in question.
 */
void
athena_view_update_menus (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));
	...
	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->update_menus (view);
			           ...
			           AthenaView *directory_view)
{
	...
	result->files = athena_file_list_copy (files);
...
application_launch_parameters_free (ApplicationLaunchParameters *parameters)
{
	...
	athena_file_list_free (parameters->files);
...
file_and_directory_list_to_files (GList *fad_list)
{
	...
	for (l = fad_list; l != NULL; l = l->next) {
		fad = l->data;
		res = g_list_prepend (res, athena_file_ref (fad->file));
	}
...
static GList *
file_and_directory_list_from_files (AthenaDirectory *directory, GList *files)
{
	GList *res, *l;
	FileAndDirectory *fad;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		fad->directory = athena_directory_ref (directory);
		fad->file = athena_file_ref (l->data);
...
{
	athena_directory_unref (fad->directory);
	athena_file_unref (fad->file);
	g_free (fad);
}
...
script_launch_parameters_new (AthenaFile *file,
			      AthenaView *directory_view)
{
...
{
	g_object_unref (parameters->directory_view);
	athena_file_unref (parameters->file);
	g_free (parameters);
}			      

static CreateTemplateParameters *
create_template_parameters_new (AthenaFile *file,
				AthenaView *directory_view)
{
...
AthenaWindow *
athena_view_get_athena_window (AthenaView  *view)
{
...
AthenaWindowSlot *
athena_view_get_athena_window_slot (AthenaView  *view)
{
 ...
 */
static GtkWindow *
athena_view_get_containing_window (AthenaView *view)
{
	GtkWidget *window;
...
static gboolean
athena_view_confirm_multiple (GtkWindow *parent_window,
				int count,
				gboolean tabs)
{
	GtkDialog *dialog;
...
static gboolean
selection_contains_one_item_in_menu_callback (AthenaView *view, GList *selection)
{
...
static gboolean
selection_not_empty_in_menu_callback (AthenaView *view, GList *selection)
{
	if (selection != NULL) {
		return TRUE;
	}
...
static char *
get_view_directory (AthenaView *view)
{
	char *uri, *path;
	GFile *f;
	
	uri = athena_directory_get_uri (view->details->model);
	if (eel_uri_is_desktop (uri)) {
		g_free (uri);
		uri = athena_get_desktop_directory_uri ();
...
void
athena_view_preview_files (AthenaView *view,
			     GList *files,
			     GArray *locations)
{
	...
	uri = athena_file_get_uri (files->data);
	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));

	xid = gdk_x11_window_get_xid (gtk_widget_get_window (toplevel));
	athena_previewer_call_show_file (previewer, uri, xid, TRUE);
}

void
athena_view_activate_files (AthenaView *view,
			      GList *files,
			      ...
			      gboolean confirm_multiple)
{
	...
	athena_mime_activate_files (athena_view_get_containing_window (view),
				      view->details->slot,
				      files,
				      path,
				      flags,
...
static void
athena_view_activate_file (AthenaView *view,
			     AthenaFile *file,
			     AthenaWindowOpenFlags flags)
{
	char *path;
	...
	path = get_view_directory (view);
	athena_mime_activate_file (athena_view_get_containing_window (view),
				     view->details->slot,
				     file,
				     path,
				     flags);
		      ...
		      gpointer callback_data)
{
	...
	athena_view_activate_files (view,
				      selection,
				      0,
				      TRUE);
				   ...
				   gpointer callback_data)
{
	...
	athena_view_activate_files (view,
				      selection,
				      ATHENA_WINDOW_OPEN_FLAG_CLOSE_BEHIND,
				      TRUE);
	athena_file_list_free (selection);
}
				...
				gpointer callback_data)
{
	...
	window = athena_view_get_containing_window (view);

	if (athena_view_confirm_multiple (window, g_list_length (selection), FALSE)) {
			      ...
			      gpointer callback_data)
{
	...
	if (athena_view_confirm_multiple (window, g_list_length (selection), TRUE)) {
		athena_view_activate_files (view,
					      selection,
				...
				gpointer user_data)
{
	...
	g_signal_emit_by_name (athena_signaller_get_current (), "mime_data_changed");

	files.next = NULL;
	files.prev = NULL;
	files.data = file;
	athena_launch_application (info, &files, parent_window);
...
static void
choose_program (AthenaView *view,
		AthenaFile *file)
{
	GtkWidget *dialog;
	GFile *location;
	...
	parent_window = athena_view_get_containing_window (view);
...
static void
open_with_other_program (AthenaView *view)
{
        GList *selection;
	...
	if (selection_contains_one_item_in_menu_callback (view, selection)) {
		choose_program (view, ATHENA_FILE (selection->data));
	}
...
action_other_application_callback (GtkAction *action,
				   gpointer callback_data)
{
	g_assert (ATHENA_IS_VIEW (callback_data));

	open_with_other_program (ATHENA_VIEW (callback_data));
}

static void
trash_or_delete_selected_files (AthenaView *view)
{
        GList *selection;
	...
	if (!view->details->selection_was_removed) {
		selection = athena_view_get_selection_for_file_transfer (view);
		trash_or_delete_files (athena_view_get_containing_window (view),
				       selection, TRUE,
				       view);
		athena_file_list_free (selection);
...
action_trash_callback (GtkAction *action,
		       gpointer callback_data)
{
        trash_or_delete_selected_files (ATHENA_VIEW (callback_data));
}

static void
delete_selected_files (AthenaView *view)
{
        GList *selection;
	GList *node;
	GList *locations;
	...
	for (node = selection; node != NULL; node = node->next) {
		locations = g_list_prepend (locations,
					    athena_file_get_location ((AthenaFile *) node->data));
	}
	locations = g_list_reverse (locations);

	athena_file_operations_delete (locations, athena_view_get_containing_window (view), NULL, NULL);
...
action_delete_callback (GtkAction *action,
			gpointer callback_data)
{
        delete_selected_files (ATHENA_VIEW (callback_data));
}
				    ...
				    gpointer callback_data)
{
	...
	selection = athena_view_get_selection_for_file_transfer (view);
	athena_restore_files_from_trash (selection,
					   athena_view_get_containing_window (view));
...
static gboolean
real_delete (AthenaView *view)
{
	GtkAction *action;

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_DELETE);
	if (gtk_action_get_sensitive (action) &&
	    gtk_action_get_visible (action)) {
			   ...
			   gpointer callback_data)
{
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		 ...
		 */
		selected_item_locations = athena_view_get_selected_icon_locations (view);
	        athena_view_duplicate_selection (view, selection, selected_item_locations);
			     ...
			     gpointer callback_data)
{
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		selected_item_locations = athena_view_get_selected_icon_locations (view);
	        athena_view_create_links_for_files (view, selection, selected_item_locations);
	        g_array_free (selected_item_locations, TRUE);
	}
			    ...
			    gpointer callback_data)
{
	...
	athena_view_select_all (callback_data);
}
				  ...
				  gpointer callback_data)
{
	...
	athena_view_invert_selection (callback_data);
}
...
pattern_select_response_cb (GtkWidget *dialog, int response, gpointer user_data)
{
	...
	case GTK_RESPONSE_OK :
		...
		selection = athena_directory_match_pattern (directory,
							      gtk_entry_get_text (GTK_ENTRY (entry)));
			
		if (selection) {
			athena_view_call_set_selection (view, selection);
			athena_file_list_free (selection);

			athena_view_reveal_selection(view);
		}
		/* fall through */
...
static void
select_pattern (AthenaView *view)
{
	GtkWidget *dialog;
	GtkWidget *label;
	GtkWidget *example;
				   ...
				   gpointer callback_data)
{
	...
	athena_view_reset_to_defaults (callback_data);
}


static void
hidden_files_mode_changed (AthenaWindow *window,
			   gpointer callback_data)
{
	AthenaView *directory_view;
	...
	athena_view_init_show_hidden_files (directory_view);
}
...
action_save_search_callback (GtkAction *action,
			     gpointer callback_data)
{                
	AthenaSearchDirectory *search;
	AthenaView	*directory_view;
	
        directory_view = ATHENA_VIEW (callback_data);

	if (directory_view->details->model &&
	    ATHENA_IS_SEARCH_DIRECTORY (directory_view->details->model)) {
		search = ATHENA_SEARCH_DIRECTORY (directory_view->details->model);
		athena_search_directory_save_search (search);
				...
				gpointer callback_data)
{
	...
	if (directory_view->details->model &&
		...
		dialog = gtk_dialog_new_with_buttons (_("Save Search as"),
						      athena_view_get_containing_window (directory_view),
						      0,
		...
		if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK) {
			...
			if (g_str_has_suffix (entry_text, ATHENA_SAVED_SEARCH_EXTENSION)) {
				filename_utf8 = g_strdup (entry_text);
			} else {
				filename_utf8 = g_strconcat (entry_text, ATHENA_SAVED_SEARCH_EXTENSION, NULL);
			}
			...
			athena_search_directory_save_to_file (search, uri);
			location = g_file_new_for_uri (uri);
			athena_file_changes_queue_file_added (location);
			g_object_unref (location);
			athena_file_changes_consume_changes (TRUE);
			g_free (uri);
		}
...
{                
	...
	athena_file_operations_empty_trash (GTK_WIDGET (callback_data));
}
...
delayed_rename_file_hack_callback (RenameData *data)
{
	...
	if (view->details->window != NULL &&
	    view->details->active) {
		ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->start_renaming_file (view, new_file, FALSE);
		athena_view_reveal_selection (view);
	}
...
delayed_rename_file_hack_removed (RenameData *data)
{
	...
	athena_file_unref (data->new_file);
	g_free (data);
}


static void
rename_file (AthenaView *view, AthenaFile *new_file)
{
	RenameData *data;
	...
	*/
	if (ATHENA_IS_LIST_VIEW (view)) {
		athena_view_select_file (view, new_file);
		
		data = g_new (RenameData, 1);
		data->view = g_object_ref (view);
		data->new_file = athena_file_ref (new_file);
...
static void
reveal_newly_added_folder (AthenaView *view, AthenaFile *new_file,
			   AthenaDirectory *directory, GFile *target_location)
{
	GFile *location;

	location = athena_file_get_location (new_file);
...
static void
track_newly_added_locations (AthenaView *view, AthenaFile *new_file,
			     AthenaDirectory *directory, gpointer user_data)
{
	NewFolderData *data;

	data = user_data;

	g_hash_table_insert (data->added_locations, athena_file_get_location (new_file), NULL);
}
		 ...
		 gpointer user_data)
{
	...
	file = athena_file_get (new_folder);
	athena_file_set_metadata
		(file, ATHENA_METADATA_KEY_SCREEN,
		 NULL,
		 screen_string);
...
new_folder_data_new (AthenaView *directory_view)
{
	NewFolderData *data;
...
static GdkPoint *
context_menu_to_file_operation_position (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);

	if (athena_view_using_manual_layout (view)
	    && view->details->context_menu_position.x >= 0
	    && view->details->context_menu_position.y >= 0) {
		ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->widget_to_file_operation_position
...
static void
athena_view_new_folder (AthenaView *directory_view)
{
	char *parent_uri;
	NewFolderData *data;
	GdkPoint *pos;
	...
	parent_uri = athena_view_get_backing_uri (directory_view);
	athena_file_operations_new_folder (GTK_WIDGET (directory_view),
					     pos, parent_uri,
...
static NewFolderData *
setup_new_folder_data (AthenaView *directory_view)
{
	NewFolderData *data;
...
void
athena_view_new_file_with_initial_contents (AthenaView *view,
					      ...
					      GdkPoint *pos)
{
	...
	athena_file_operations_new_file (GTK_WIDGET (view),
...
static void
athena_view_new_file (AthenaView *directory_view,
			const char *parent_uri,
			AthenaFile *source)
{
	GdkPoint *pos;
	...
	container_uri = NULL;
	if (parent_uri == NULL) {
		container_uri = athena_view_get_backing_uri (directory_view);
		g_assert (container_uri != NULL);
	}

	if (source == NULL) {
		athena_view_new_file_with_initial_contents (directory_view,
	...
	g_return_if_fail (athena_file_is_local (source));

	pos = context_menu_to_file_operation_position (directory_view);

	data = setup_new_folder_data (directory_view);

	source_uri = athena_file_get_uri (source);

	athena_file_operations_new_file_from_template (GTK_WIDGET (directory_view),
							 pos,
							 parent_uri != NULL ? parent_uri : container_uri,
							 NULL,
							 source_uri,
...
{                
	...
	athena_view_new_folder (ATHENA_VIEW (callback_data));
}
...
{                
	...
	athena_view_new_file (ATHENA_VIEW (callback_data), NULL, NULL);
}
			      ...
			      gpointer callback_data)
{
	...
	parent_uri = athena_view_get_backing_uri (view);

	window = athena_view_get_containing_window (view);
	athena_launch_application_from_command (gtk_widget_get_screen (GTK_WIDGET (view)),
			    ...
			    gpointer callback_data)
{
	...
	if (g_list_length (selection) == 0) {
		if (view->details->directory_as_file != NULL) {
			files = g_list_append (NULL, athena_file_ref (view->details->directory_as_file));

			athena_properties_window_present (files, GTK_WIDGET (view), NULL);

			athena_file_list_free (files);
		}
	} else {
		athena_properties_window_present (selection, GTK_WIDGET (view), NULL);
	}
        athena_file_list_free (selection);
}
				     ...
				     gpointer   callback_data)
{
	...
	view = ATHENA_VIEW (callback_data);
	g_assert (ATHENA_IS_FILE (view->details->location_popup_directory_as_file));

	files = g_list_append (NULL, athena_file_ref (view->details->location_popup_directory_as_file));
...
all_files_in_trash (GList *files)
{
	...
	for (node = files; node != NULL; node = node->next) {
		if (!athena_file_is_in_trash (ATHENA_FILE (node->data))) {
			return FALSE;
		}
	}
...
static gboolean
all_selected_items_in_trash (AthenaView *view)
{
	GList *selection;
	gboolean result;
...
click_policy_changed_callback (gpointer callback_data)
{
	...
	ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->click_policy_changed (view);
}

gboolean
athena_view_should_sort_directories_first (AthenaView *view)
{
...
sort_directories_first_changed_callback (gpointer callback_data)
{
	...
	preference_value =
		g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_SORT_DIRECTORIES_FIRST);

	if (preference_value != view->details->sort_directories_first) {
		view->details->sort_directories_first = preference_value;
		return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->sort_directories_first_changed (view);
	}
}
...
static void
scripts_added_or_changed_callback (AthenaDirectory *directory,
				   GList *files,
				   gpointer callback_data)
{
	AthenaView *view;
...
static void
templates_added_or_changed_callback (AthenaDirectory *directory,
				     GList *files,
				     gpointer callback_data)
{
	AthenaView *view;
...
static void
add_directory_to_directory_list (AthenaView *view,
				 AthenaDirectory *directory,
				 ...
				 GCallback changed_callback)
{
	...
	if (g_list_find (*directory_list, directory) == NULL) {
		...
		athena_directory_file_monitor_add (directory, directory_list,
						     FALSE, attributes,
						     (AthenaDirectoryCallback)changed_callback, view);
...
static void
remove_directory_from_directory_list (AthenaView *view,
				      AthenaDirectory *directory,
				      GList **directory_list,
				      GCallback changed_callback)
{
	*directory_list = g_list_remove	(*directory_list, directory);
	...
	athena_directory_file_monitor_remove (directory, directory_list);
...
static void
add_directory_to_scripts_directory_list (AthenaView *view,
					 AthenaDirectory *directory)
{
...
static void
remove_directory_from_scripts_directory_list (AthenaView *view,
					      AthenaDirectory *directory)
{
...
static void
add_directory_to_templates_directory_list (AthenaView *view,
					   AthenaDirectory *directory)
{
	add_directory_to_directory_list (view, directory,
...
static void
remove_directory_from_templates_directory_list (AthenaView *view,
						AthenaDirectory *directory)
{
	remove_directory_from_directory_list (view, directory,
					      &view->details->templates_directory_list,
...
static void
slot_active (AthenaWindowSlot *slot,
	     AthenaView *view)
{
	if (view->details->active) {
		return;
	...
	athena_view_merge_menus (view);
	schedule_update_menus (view);
}

static void
slot_inactive (AthenaWindowSlot *slot,
	       AthenaView *view)
{
	if (!view->details->active) {
		return;
	...
	athena_view_unmerge_menus (view);
	remove_update_menus_timeout_callback (view);
}

void
athena_view_grab_focus (AthenaView *view)
{
...
int
athena_view_get_selection_count (AthenaView *view)
{
	/* FIXME: This could be faster if we special cased it in subclasses */
	GList *files;
	int len;

	files = athena_view_get_selection (ATHENA_VIEW (view));
...
static void
update_undo_actions (AthenaView *view)
{
	AthenaFileUndoInfo *info;
	AthenaFileUndoManagerState undo_state;
	GtkAction *action;
	...
	info = athena_file_undo_manager_get_action ();
	undo_state = athena_file_undo_manager_get_state ();

	if (info != NULL && 
	    (undo_state > ATHENA_FILE_UNDO_MANAGER_STATE_NONE)) {
		is_undo = (undo_state == ATHENA_FILE_UNDO_MANAGER_STATE_UNDO);
		...
		athena_file_undo_info_get_strings (info,
...
static void
undo_manager_changed_cb (AthenaFileUndoManager* manager,
			 AthenaView *view)
{
...
void
athena_view_set_selection (AthenaView *athena_view,
			     GList *selection)
{
	AthenaView *view;

	view = ATHENA_VIEW (athena_view);
...
get_bulk_rename_tool ()
{
	...
	g_settings_get (athena_preferences, ATHENA_PREFERENCES_BULK_RENAME_TOOL, "^ay", &bulk_rename_tool);
...
static void
athena_view_init (AthenaView *view)
{
	AtkObject *atk_object;
	AthenaDirectory *scripts_directory;
	AthenaDirectory *templates_directory;
	char *templates_uri;
	AthenaFileUndoManager* manager;

	view->details = G_TYPE_INSTANCE_GET_PRIVATE (view, ATHENA_TYPE_VIEW,
						     AthenaViewDetails);
	...
	if (set_up_scripts_directory_global ()) {
		scripts_directory = athena_directory_get_by_uri (scripts_directory_uri);
		add_directory_to_scripts_directory_list (view, scripts_directory);
		athena_directory_unref (scripts_directory);
	} else {
		g_warning ("Ignoring scripts directory, it may be a broken link\n");
	}

	if (athena_should_use_templates_directory ()) {
		templates_uri = athena_get_templates_directory_uri ();
		templates_directory = athena_directory_get_by_uri (templates_uri);
		g_free (templates_uri);
		add_directory_to_templates_directory_list (view, templates_directory);
		athena_directory_unref (templates_directory);
	}
	update_templates_directory (view);
	g_signal_connect_object (athena_signaller_get_current (),
				 "user_dirs_changed",
	...
	g_signal_connect_object (athena_trash_monitor_get (), "trash_state_changed",
				 G_CALLBACK (athena_view_trash_state_changed_callback), view, 0);

	/* React to clipboard changes */
	g_signal_connect_object (athena_clipboard_monitor_get (), "clipboard_changed",
	...
	g_signal_connect_object (athena_signaller_get_current (), "popup_menu_changed",
				 G_CALLBACK (athena_view_update_menus), view, G_CONNECT_SWAPPED);

	gtk_widget_show (GTK_WIDGET (view));

	g_signal_connect_swapped (athena_preferences,
				  "changed::" ATHENA_PREFERENCES_ENABLE_DELETE,
				  G_CALLBACK (schedule_update_menus_callback), view);
	g_signal_connect_swapped (athena_preferences,
				  "changed::" ATHENA_PREFERENCES_CLICK_POLICY,
				  G_CALLBACK(click_policy_changed_callback),
				  view);
	g_signal_connect_swapped (athena_preferences,
				  "changed::" ATHENA_PREFERENCES_SORT_DIRECTORIES_FIRST, 
				  G_CALLBACK(sort_directories_first_changed_callback), view);
	g_signal_connect_swapped (gnome_lockdown_preferences,
				  "changed::" ATHENA_PREFERENCES_LOCKDOWN_COMMAND_LINE,
				  G_CALLBACK (schedule_update_menus), view);

	g_signal_connect_swapped (athena_window_state,
				  "changed::" ATHENA_WINDOW_STATE_START_WITH_STATUS_BAR,
				  G_CALLBACK (athena_view_display_selection_info), view);
				  
	manager = athena_file_undo_manager_get ();
...
static void
real_unmerge_menus (AthenaView *view)
{
	...
	ui_manager = athena_window_get_ui_manager (view->details->window);
...
static void
athena_view_destroy (GtkWidget *object)
{
	AthenaView *view;
	GList *node, *next;

	view = ATHENA_VIEW (object);
	...
	while (view->details->subdirectory_list != NULL) {
		athena_view_remove_subdirectory (view,
	...
	if (view->details->model) {
		athena_directory_unref (view->details->model);
		view->details->model = NULL;
	}
	
	if (view->details->directory_as_file) {
		athena_file_unref (view->details->directory_as_file);
		view->details->directory_as_file = NULL;
	}

	GTK_WIDGET_CLASS (athena_view_parent_class)->destroy (object);
}

static void
athena_view_finalize (GObject *object)
{
	AthenaView *view;
	...
	g_signal_handlers_disconnect_by_func (athena_window_state,
					      athena_view_display_selection_info, view);
	...
	G_OBJECT_CLASS (athena_view_parent_class)->finalize (object);
}

/**
 * athena_view_display_selection_info:
 *
 * Display information about the current selection, and notify the view frame of the changed selection.
 * @view: AthenaView for which to display selection info.
 *
 **/
void
athena_view_display_selection_info (AthenaView *view)
{
	GList *selection;
	...
	for (p = selection; p != NULL; p = p->next) {
		...
		if (athena_file_is_directory (file)) {
			folder_count++;
			if (athena_file_get_directory_item_count (file, &file_item_count, NULL)) {
			...
			non_folder_count++;
			if (!athena_file_can_get_size (file)) {
				non_folder_size_known = TRUE;
				non_folder_size += athena_file_get_size (file);
			}
		}

		if (first_item_name == NULL) {
			first_item_name = athena_file_get_display_name (file);
		}
	}
	...
	free_space_str = athena_file_get_volume_free_space (view->details->directory_as_file);
	...
	if (folder_count == 0 && non_folder_count == 0)	{
		...
		item_count = athena_view_get_item_count (view);
	...
	athena_window_slot_set_status (view->details->slot,
					 status_string,
					 view_status_string);
...
static void
athena_view_send_selection_change (AthenaView *view)
{
...
void
athena_view_load_location (AthenaView *athena_view,
			     GFile        *location)
{
	AthenaDirectory *directory;
	AthenaView *directory_view;

	directory_view = ATHENA_VIEW (athena_view);
	      ...
	      gboolean all_files_seen)
{
	 ...
	 * is no AthenaWindow any more.
	 */
	if (view->details->window != NULL) {
		if (all_files_seen) {
			athena_window_report_load_complete (view->details->window, ATHENA_VIEW (view));
		}
...
debuting_files_data_free (DebutingFilesData *data)
{
	...
	athena_file_list_free (data->added_files);
	g_free (data);
}
 ...
 */
static void
debuting_files_add_file_callback (AthenaView *view,
				  AthenaFile *new_file,
				  AthenaDirectory *directory,
...
static void
pre_copy_move_add_file_callback (AthenaView *view,
				 AthenaFile *new_file,
				 AthenaDirectory *directory,
...
/* This needs to be called prior to athena_file_operations_copy_move.
...
pre_copy_move (AthenaView *directory_view)
{
...
copy_move_done_partition_func (gpointer data, gpointer callback_data)
{
	...
	location = athena_file_get_location (ATHENA_FILE (data));
			 ...
			 gpointer data)
{
	...
	if (directory_view != NULL) {
		g_assert (ATHENA_IS_VIEW (directory_view));
		...
		if (g_hash_table_size (debuting_files) == 0) {
			...
			if (debuting_files_data->added_files != NULL) {
				athena_view_call_set_selection (directory_view,
								  debuting_files_data->added_files);
				athena_view_reveal_selection (directory_view);
			}
...
static gboolean
view_file_still_belongs (AthenaView *view,
			 AthenaFile *file,
			 AthenaDirectory *directory)
{
	...
	return athena_directory_contains_file (directory, file);
}

static gboolean
still_should_show_file (AthenaView *view, AthenaFile *file, AthenaDirectory *directory)
{
	return athena_view_should_show_file (view, file) &&
		view_file_still_belongs (view, file, directory);
}

static gboolean
ready_to_load (AthenaFile *file)
{
	return athena_file_check_if_ready (file,
					     ATHENA_FILE_ATTRIBUTES_FOR_ICON);
}
...
compare_files_cover (gconstpointer a, gconstpointer b, gpointer callback_data)
{
	...
	if (fad1->directory < fad2->directory) {
		...
		return ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->compare_files (view, fad1->file, fad2->file);
	}
}
static void
sort_files (AthenaView *view, GList **list)
{
 ...
 */
static void
process_new_files (AthenaView *view)
{
	...
	for (node = new_added_files; node != NULL; node = next) {
		...
		if (athena_view_should_show_file (view, pending->file)) {
	...
	for (node = new_changed_files; node != NULL; node = next) {
		...
		if (!still_should_show_file (view, pending->file, pending->directory) || ready_to_load (pending->file)) {
			...
			} else if (athena_view_should_show_file (view, pending->file)) {
				new_changed_files = g_list_delete_link (new_changed_files, node);
...
static void
process_old_files (AthenaView *view)
{
	...
	if (send_selection_change) {
		 ...
		 * have changed.
		 */
		athena_view_send_selection_change (view);
	}
}

static void
display_pending_files (AthenaView *view)
{
	...
	if (view->details->model != NULL
	    && athena_directory_are_all_files_seen (view->details->model)
...
void
athena_view_freeze_updates (AthenaView *view)
{
...
void
athena_view_unfreeze_updates (AthenaView *view)
{
...
static void
remove_update_menus_timeout_callback (AthenaView *view) 
{
...
static void
update_menus_if_pending (AthenaView *view)
{
	if (!view->details->menu_states_untrustworthy) {
		return;
	}
...
static void
schedule_idle_display_of_pending_files (AthenaView *view)
{
...
static void
schedule_timeout_display_of_pending_files (AthenaView *view, guint interval)
{
...
static void
unschedule_display_of_pending_files (AthenaView *view)
{
...
static void
queue_pending_files (AthenaView *view,
		     AthenaDirectory *directory,
		     GList *files,
		     GList **pending_list)
{
	...
	*pending_list = g_list_concat (file_and_directory_list_from_files (directory, files),
	...
	if (! view->details->loading || athena_directory_are_all_files_seen (directory)) {
		schedule_timeout_display_of_pending_files (view, view->details->update_interval);
	}
}

static void
remove_changes_timeout_callback (AthenaView *view) 
{
...
static void
reset_update_interval (AthenaView *view)
{
...
static void
schedule_changes (AthenaView *view)
{
...
static void
files_added_callback (AthenaDirectory *directory,
		      GList *files,
		      gpointer callback_data)
{
	AthenaView *view;
...
static void
files_changed_callback (AthenaDirectory *directory,
			GList *files,
			gpointer callback_data)
{
	AthenaView *view;
...
static void
done_loading_callback (AthenaDirectory *directory,
		       gpointer callback_data)
{
	AthenaView *view;
...
static void
load_error_callback (AthenaDirectory *directory,
		     GError *error,
		     gpointer callback_data)
{
	AthenaView *view;
...
static void
real_load_error (AthenaView *view, GError *error)
{
	/* Report only one error per failed directory load (from the UI
	 * point of view, not from the AthenaDirectory point of view).
	...
	if (!view->details->reported_load_error) {
		athena_report_error_loading_directory 
			(athena_view_get_directory_as_file (view),
			 error,
			 athena_view_get_containing_window (view));
	}
...
void
athena_view_add_subdirectory (AthenaView  *view,
				AthenaDirectory*directory)
{
	AthenaFileAttributes attributes;
	...
	attributes =
		...
		ATHENA_FILE_ATTRIBUTE_EXTENSION_INFO;

	athena_directory_file_monitor_add (directory,
					     &view->details->model,
...
void
athena_view_remove_subdirectory (AthenaView  *view,
				   AthenaDirectory*directory)
{
	g_assert (g_list_find (view->details->subdirectory_list, directory));
	...
	athena_directory_file_monitor_remove (directory, &view->details->model);

	athena_directory_unref (directory);
}

/**
 * athena_view_get_loading:
 * @view: an #AthenaView.
 *
 ...
 **/
gboolean
athena_view_get_loading (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);
...
GtkUIManager *
athena_view_get_ui_manager (AthenaView  *view)
{
	if (view->details->window == NULL) {
		return NULL;
	}
	return athena_window_get_ui_manager (view->details->window);	
}

/**
 * athena_view_get_model:
 *
 * Get the model for this AthenaView.
 * @view: AthenaView of interest.
 * 
 * Return value: AthenaDirectory for this view.
 * 
 **/
AthenaDirectory *
athena_view_get_model (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);
...
GdkAtom
athena_view_get_copied_files_atom (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), GDK_NONE);
...
prepend_uri_one (gpointer data, gpointer callback_data)
{
	...
	g_assert (ATHENA_IS_FILE (data));
	g_assert (callback_data != NULL);

	result = (GList **) callback_data;
	file = (AthenaFile *) data;
	*result = g_list_prepend (*result, athena_file_get_uri (file));
}
...
static void
athena_view_create_links_for_files (AthenaView *view, GList *files,
				      GArray *relative_item_points)
{
	...
	dir_uri = athena_view_get_backing_uri (view);
	athena_file_operations_copy_move (uris, relative_item_points, dir_uri, GDK_ACTION_LINK, 
...
static void
athena_view_duplicate_selection (AthenaView *view, GList *files,
				   GArray *relative_item_points)
{
	GList *uris;
	CopyMoveDoneData *copy_move_done_data;
	...
	offset_drop_points (relative_item_points,
        ...
        copy_move_done_data = pre_copy_move (view);
	athena_file_operations_copy_move (uris, relative_item_points, NULL, GDK_ACTION_COPY,
					    GTK_WIDGET (view), copy_move_done_callback, copy_move_done_data);
	g_list_free_full (uris, g_free);
}
 ...
 *	 ATHENA_DESKTOP_LINK_TRASH, ATHENA_DESKTOP_LINK_HOME, ATHENA_DESKTOP_LINK_MOUNT
 */
 
static gboolean
special_link_in_selection (AthenaView *view)
{
	gboolean saw_link;
	...
	selection = athena_view_get_selection (ATHENA_VIEW (view));

	for (node = selection; node != NULL; node = node->next) {
		file = ATHENA_FILE (node->data);

		saw_link = ATHENA_IS_DESKTOP_ICON_FILE (file);
...
static gboolean
desktop_or_home_dir_in_selection (AthenaView *view)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		saw_desktop_or_home_dir =
			athena_file_is_home (file)
			|| athena_file_is_desktop_directory (file);
		       ...
		       AthenaView *view)
{
	...
	athena_file_operations_trash_or_delete (locations,
						  parent_window,
						  (AthenaDeleteCallback) trash_or_delete_done_cb,
						  view);
	g_list_free_full (locations, g_object_unref);
}

static gboolean
can_rename_file (AthenaView *view, AthenaFile *file)
{
	return athena_file_can_rename (file);
}

gboolean
athena_view_get_is_renaming (AthenaView *view)
{
	return view->details->is_renaming;
}

void
athena_view_set_is_renaming (AthenaView *view,
			       gboolean      is_renaming)
{
...
static void
start_renaming_file (AthenaView *view,
		     AthenaFile *file,
		     gboolean select_all)
{
	...
	if (file !=  NULL) {
		athena_view_select_file (view, file);
	}
}

static void
update_context_menu_position_from_event (AthenaView *view,
...
open_one_in_new_window (gpointer data, gpointer callback_data)
{
	...
	athena_view_activate_file (ATHENA_VIEW (callback_data),
				     ATHENA_FILE (data),
				     ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}

AthenaFile *
athena_view_get_directory_as_file (AthenaView *view)
{
	g_assert (ATHENA_IS_VIEW (view));
				       ...
				       gpointer callback_data)
{
	...
	athena_launch_application 
		(launch_parameters->application,
		 ...
		 athena_view_get_containing_window (launch_parameters->directory_view));
}
...
static void
add_application_to_open_with_menu (AthenaView *view,
				   GAppInfo *application, 
				   GList *files,
				   int index,
				   ...
				   const gboolean submenu)
{
	...
	gtk_ui_manager_add_ui (athena_window_get_ui_manager (view->details->window),
			       view->details->open_with_merge_id,
			       menu_placeholder,
			       action_name,
			       action_name,
	...
	menuitem = gtk_ui_manager_get_widget (
					      athena_window_get_ui_manager (view->details->window),
					      path);
...
static void
add_x_content_apps (AthenaView *view, AthenaFile *file, GList **applications)
{
	GMount *mount;
	...
	mount = athena_file_get_mount (file);

	if (mount == NULL) {
		return;
	}
	
	x_content_types = athena_get_cached_x_content_types_for_mount (mount);
...
static void
reset_open_with_menu (AthenaView *view, GList *selection)
{
	...
	athena_ui_prepare_merge_ui (ui_manager,
				      "OpenWithGroup",
				      &view->details->open_with_merge_id,
				      &view->details->open_with_action_group);
	...
	for (node = selection; node != NULL; node = node->next) {
		...
		other_applications_visible &= ((!athena_mime_file_opens_in_view (file) &&
						!athena_file_is_athena_link (file)) ||
					       athena_file_is_directory (file));
	}

	default_app = NULL;
	if (filter_default) {
		default_app = athena_mime_get_default_application_for_files (selection);
	}

	applications = NULL;
	if (other_applications_visible) {
		applications = athena_mime_get_applications_for_files (selection);
	}

	if (g_list_length (selection) == 1) {
		add_x_content_apps (view, ATHENA_FILE (selection->data), &applications);
	}
	...
	for (node = applications, index = 0; node != NULL; node = node->next, index++) {
		...
		if (submenu_visible) {
			menu_path = ATHENA_VIEW_MENU_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER;
			popup_path = ATHENA_VIEW_POPUP_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER;
		} else {
			menu_path = ATHENA_VIEW_MENU_PATH_APPLICATIONS_PLACEHOLDER;
			popup_path = ATHENA_VIEW_POPUP_PATH_APPLICATIONS_PLACEHOLDER;
		}
	...
	if (submenu_visible) {
		action = gtk_action_group_get_action (view->details->dir_action_group,
						      ATHENA_ACTION_OTHER_APPLICATION1);
		gtk_action_set_visible (action, open_with_chooser_visible);
		action = gtk_action_group_get_action (view->details->dir_action_group,
						      ATHENA_ACTION_OTHER_APPLICATION2);
			      ...
			      GList *selection)
{
	...
	for (l = providers; l != NULL; l = l->next) {
		...
		file_items = athena_menu_provider_get_file_items (provider,
								    window,
								    selection);
...
extension_action_callback_data_free (ExtensionActionCallbackData *data)
{
	...
	athena_file_list_free (data->selection);
...
search_in_menu_items (GList* items, const char *item_name)
{
	...
	for (list = items; list != NULL; list = list->next) {
		...
		if (menu != NULL) {
			...
			submenus = athena_menu_get_items (menu);
			ret = search_in_menu_items (submenus, item_name);
			athena_menu_item_list_free (submenus);
			g_object_unref (menu);
			if (ret) {
				return TRUE;
			}
			   ...
			   gpointer callback_data)
{
	...
	if (is_valid) {
		athena_menu_item_activate (data->item);
	}
}

static GdkPixbuf *
get_menu_icon_for_file (AthenaFile *file)
{
	AthenaIconInfo *info;
	GdkPixbuf *pixbuf;
	int size;

	size = athena_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
	
	info = athena_file_get_icon (file, size, 0);
	pixbuf = athena_icon_info_get_pixbuf_nodefault_at_size (info, size);
	g_object_unref (info);
...
static GtkAction *
add_extension_action_for_files (AthenaView *view, 
				AthenaMenuItem *item,
				GList *files)
{
	...
	if (icon != NULL) {
		pixbuf = athena_ui_get_menu_icon (icon);
		if (pixbuf != NULL) {
	...
	data->view = view;
	data->selection = athena_file_list_copy (files);
	data->action = action;
...
static void
add_extension_menu_items (AthenaView *view,
			  GList *files,
			  GList *menu_items,
			  const char *subdirectory)
{
	...
	for (l = menu_items; l; l = l->next) {
		...
		path = g_build_path ("/", ATHENA_VIEW_POPUP_PATH_EXTENSION_ACTIONS, subdirectory, NULL);
		gtk_ui_manager_add_ui (ui_manager,
				       view->details->extensions_menu_merge_id,
				       path,
		...
		path = g_build_path ("/", ATHENA_VIEW_MENU_PATH_EXTENSION_ACTIONS_PLACEHOLDER, subdirectory, NULL);
		gtk_ui_manager_add_ui (ui_manager,
				       view->details->extensions_menu_merge_id,
				       path,
				       gtk_action_get_name (action),
...
static void
reset_extension_actions_menu (AthenaView *view, GList *selection)
{
	GList *items;
	GtkUIManager *ui_manager;
...
static char *
change_to_view_directory (AthenaView *view)
{
	char *path;
	char *old_path;
				   ...
				   AthenaDirectory *model)
{
	...
	model_location = athena_directory_get_location (model);

	for (node = selection, i = 0; node != NULL; node = node->next, i++) {
		file = ATHENA_FILE (node->data);

		if (!athena_file_is_local (file)) {
			parameters[i] = NULL;
			g_strfreev (parameters);
			return NULL;
		}

		file_location = athena_file_get_location (ATHENA_FILE (node->data));
...
get_file_paths_or_uris_as_newline_delimited_string (GList *selection, gboolean get_paths)
{
	...
	for (node = selection; node != NULL; node = node->next) {
		uri = NULL;
		if (ATHENA_IS_DESKTOP_ICON_FILE (node->data)) {
			link = athena_desktop_icon_file_get_link (ATHENA_DESKTOP_ICON_FILE (node->data));
			if (link != NULL) {
				location = athena_desktop_link_get_activation_location (link);
				uri = g_file_get_uri (location);
				g_object_unref (location);
				g_object_unref (G_OBJECT (link));
			}
		} else {
			uri = athena_file_get_uri (ATHENA_FILE (node->data));
		}
		if (uri == NULL) {
			continue;
		}
...
static void
get_strings_for_environment_variables (AthenaView *view, GList *selected_files,
				       char **file_paths, char **uris, char **uri)
{
	char *directory_uri;

	/* We need to check that the directory uri starts with "file:" since
	 * athena_directory_is_local returns FALSE for nfs.
	 */
	directory_uri = athena_directory_get_uri (view->details->model);
...
static AthenaView *
get_directory_view_of_extra_pane (AthenaView *view)
{
	AthenaWindowSlot *slot;
	AthenaView *next_view;

	slot = athena_window_get_extra_slot (athena_view_get_athena_window (view));
	if (slot != NULL) {
		next_view = athena_window_slot_get_current_view (slot);

		if (ATHENA_IS_VIEW (next_view)) {
			return ATHENA_VIEW (next_view);
		}
	}
	return NULL;
}
 ...
 * to take advantage of the current Athena state.
 */
static void
set_script_environment_variables (AthenaView *view, GList *selected_files)
{
	char *file_paths;
	char *uris;
	char *uri;
	...
	g_setenv ("ATHENA_SCRIPT_SELECTED_FILE_PATHS", file_paths, TRUE);
	g_free (file_paths);

	g_setenv ("ATHENA_SCRIPT_SELECTED_URIS", uris, TRUE);
	g_free (uris);

	g_setenv ("ATHENA_SCRIPT_CURRENT_URI", uri, TRUE);
	g_free (uri);

	geometry_string = eel_gtk_window_get_geometry_string
		(GTK_WINDOW (athena_view_get_containing_window (view)));
	g_setenv ("ATHENA_SCRIPT_WINDOW_GEOMETRY", geometry_string, TRUE);
	...
	if (next_view) {
		...
		next_pane_selected_files = athena_view_get_selection (next_view);

		get_strings_for_environment_variables (next_view, next_pane_selected_files,
						       &file_paths, &uris, &uri);
		athena_file_list_free (next_pane_selected_files);
	} else {
		file_paths = g_strdup("");
		uris = g_strdup("");
		uri = g_strdup("");
	...
	g_setenv ("ATHENA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS", file_paths, TRUE);
	g_free (file_paths);

	g_setenv ("ATHENA_SCRIPT_NEXT_PANE_SELECTED_URIS", uris, TRUE);
	g_free (uris);

	g_setenv ("ATHENA_SCRIPT_NEXT_PANE_CURRENT_URI", uri, TRUE);
	g_free (uri);
}
...
{
	g_unsetenv ("ATHENA_SCRIPT_SELECTED_FILE_PATHS");
	g_unsetenv ("ATHENA_SCRIPT_SELECTED_URIS");
	g_unsetenv ("ATHENA_SCRIPT_CURRENT_URI");
	g_unsetenv ("ATHENA_SCRIPT_WINDOW_GEOMETRY");
	g_unsetenv ("ATHENA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS");
	g_unsetenv ("ATHENA_SCRIPT_NEXT_PANE_SELECTED_URIS");
	g_unsetenv ("ATHENA_SCRIPT_NEXT_PANE_CURRENT_URI");
}
...
run_script_callback (GtkAction *action, gpointer callback_data)
{
	...
	file_uri = athena_file_get_uri (launch_parameters->file);
	...
	selected_files = athena_view_get_selection (launch_parameters->directory_view);
	...
	athena_launch_application_from_command_array (screen, quoted_path, FALSE,
							(const char * const *) parameters);
	g_strfreev (parameters);

	athena_file_list_free (selected_files);
...
static void
add_script_to_scripts_menus (AthenaView *directory_view,
			     AthenaFile *file,
			     ...
			     const char *popup_bg_path)
{
	...
	ui_manager = athena_window_get_ui_manager (directory_view->details->window);
...
static void
add_submenu_to_directory_menus (AthenaView *directory_view,
				GtkActionGroup *action_group,
				guint merge_id,
				AthenaFile *file,
				const char *menu_path,
...
static gboolean
update_directory_in_scripts_menu (AthenaView *view, AthenaDirectory *directory)
{
	...
	g_free (uri);
	menu_path = g_strconcat (ATHENA_VIEW_MENU_PATH_SCRIPTS_PLACEHOLDER,
				 escaped_path,
				 NULL);
	popup_path = g_strconcat (ATHENA_VIEW_POPUP_PATH_SCRIPTS_PLACEHOLDER,
				  escaped_path,
				  NULL);
	popup_bg_path = g_strconcat (ATHENA_VIEW_POPUP_PATH_BACKGROUND_SCRIPTS_PLACEHOLDER,
				     escaped_path,
				     NULL);
	g_free (escaped_path);

	file_list = athena_directory_get_file_list (directory);
	filtered = athena_file_list_filter_hidden (file_list, FALSE);
	athena_file_list_free (file_list);

	file_list = athena_file_list_sort_by_display_name (filtered);

	any_scripts = FALSE;
	for (node = file_list; node != NULL; node = node->next) {
		file = node->data;

		if (athena_file_is_launchable (file)) {
			add_script_to_scripts_menus (view, file, menu_path, popup_path, popup_bg_path);
			any_scripts = TRUE;
		} else if (athena_file_is_directory (file)) {
			uri = athena_file_get_uri (file);
			if (directory_belongs_in_scripts_menu (uri)) {
				dir = athena_directory_get_by_uri (uri);
				add_directory_to_scripts_directory_list (view, dir);
				athena_directory_unref (dir);
...
static void
update_scripts_menu (AthenaView *view)
{
	gboolean any_scripts;
	...
	any_scripts = FALSE;
	sorted_copy = athena_directory_list_sort_by_uri
		(athena_directory_list_copy (view->details->scripts_directory_list));
	...
	athena_directory_list_free (sorted_copy);

	action = gtk_action_group_get_action (view->details->dir_action_group, ATHENA_ACTION_SCRIPTS);
...
create_template_callback (GtkAction *action, gpointer callback_data)
{
	...
	athena_view_new_file (parameters->directory_view, NULL, parameters->file);
}

static void
add_template_to_templates_menus (AthenaView *directory_view,
				 AthenaFile *file,
				 const char *menu_path,
				 const char *popup_bg_path)
{
	...
	tmp = athena_file_get_display_name (file);
...
static void
update_templates_directory (AthenaView *view)
{
	AthenaDirectory *templates_directory;
	GList *node, *next;
	char *templates_uri;
...
static void
user_dirs_changed (AthenaView *view)
{
	update_templates_directory (view);
	view->details->templates_invalid = TRUE;
	schedule_update_menus (view);
...
static gboolean
update_directory_in_templates_menu (AthenaView *view,
				    ...
				    AthenaDirectory *directory)
{
	...
	g_free (uri);
	menu_path = g_strconcat (ATHENA_VIEW_MENU_PATH_NEW_DOCUMENTS_PLACEHOLDER,
				 escaped_path,
				 NULL);
	popup_bg_path = g_strconcat (ATHENA_VIEW_POPUP_PATH_BACKGROUND_NEW_DOCUMENTS_PLACEHOLDER,
				     escaped_path,
				     NULL);
	g_free (escaped_path);
	...
	for (node = file_list; num < TEMPLATE_LIMIT && node != NULL; node = node->next, num++) {
		...
		if (athena_file_is_directory (file)) {
			...
			g_free (uri);
		} else if (athena_file_can_read (file)) {
...
static void
update_templates_menu (AthenaView *view)
{
	gboolean any_templates;
	GList *sorted_copy, *node;
	AthenaDirectory *directory;
	...
	if (athena_should_use_templates_directory ()) {
		templates_directory_uri = athena_get_templates_directory_uri ();
	} else {
	...
	any_templates = FALSE;
	sorted_copy = athena_directory_list_sort_by_uri
		(athena_directory_list_copy (view->details->templates_directory_list));
	for (node = sorted_copy; node != NULL; node = node->next) {
		directory = node->data;
	...
	action = gtk_action_group_get_action (view->details->dir_action_group, ATHENA_ACTION_NO_TEMPLATES);
...
{      
	...
	view = ATHENA_VIEW (callback_data);
	athena_window_slot_go_to (view->details->slot, location, FALSE);
	...
	eel_show_info_dialog_with_details 
		(_("All executable files in this folder will appear in the "
		 ...
		 _("All executable files in this folder will appear in the "
		   ...
		   "set by Athena, which the scripts may use:\n\n"
		   "ATHENA_SCRIPT_SELECTED_FILE_PATHS: newline-delimited paths for selected files (only if local)\n\n"
		   "ATHENA_SCRIPT_SELECTED_URIS: newline-delimited URIs for selected files\n\n"
		   "ATHENA_SCRIPT_CURRENT_URI: URI for current location\n\n"
		   "ATHENA_SCRIPT_WINDOW_GEOMETRY: position and size of current window\n\n"
		   "ATHENA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS: newline-delimited paths for selected files in the inactive pane of a split-view window (only if local)\n\n"
		   "ATHENA_SCRIPT_NEXT_PANE_SELECTED_URIS: newline-delimited URIs for selected files in the inactive pane of a split-view window\n\n"
		   "ATHENA_SCRIPT_NEXT_PANE_CURRENT_URI: URI for current location in the inactive pane of a split-view window"),
		 athena_view_get_containing_window (view));
}

static GtkMenu *
create_popup_menu (AthenaView *view, const char *popup_path)
{
	GtkWidget *menu;
	
	menu = gtk_ui_manager_get_widget (athena_window_get_ui_manager (view->details->window),
					  popup_path);
...
static void
copy_or_cut_files (AthenaView *view,
		   ...
		   gboolean         cut)
{
	...
	gtk_clipboard_set_with_data (athena_clipboard_get (GTK_WIDGET (view)),
				     targets, n_targets,
				     athena_get_clipboard_callback, athena_clear_clipboard_callback,
				     NULL);
        gtk_target_table_free (targets, n_targets);

	athena_clipboard_monitor_set_clipboard_info (athena_clipboard_monitor_get (), &info);

	count = g_list_length (clipboard_contents);
	if (count == 1) {
		name = athena_file_get_display_name (clipboard_contents->data);
		if (cut) {
...
static void
move_copy_selection_to_location (AthenaView *view,
				 int copy_action,
				 char *target_uri)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		uris = g_list_prepend (uris,
				       athena_file_get_uri ((AthenaFile *) l->data));
	}
	uris = g_list_reverse (uris);

	athena_view_move_copy_items (view, uris, NULL, target_uri,
				       copy_action,
				       0, 0);
...
static void
move_copy_selection_to_next_pane (AthenaView *view,
				  int copy_action)
{
	AthenaWindowSlot *slot;
	char *dest_location;
	...
	dest_location = athena_window_slot_get_current_uri (slot);
...
action_copy_to_home_callback (GtkAction *action, gpointer callback_data)
{
	...
	dest_location = athena_get_home_directory_uri ();
...
action_copy_to_desktop_callback (GtkAction *action, gpointer callback_data)
{
	...
	dest_location = athena_get_desktop_directory_uri ();
	move_copy_selection_to_location (view, GDK_ACTION_COPY, dest_location);
	g_free (dest_location);
}
...
static void
paste_clipboard_data (AthenaView *view,
		      ...
		      char *destination_uri)
{
	...
	cut = FALSE;
	item_uris = athena_clipboard_get_uri_list_from_selection_data (selection_data, &cut,
									 copied_files_atom);
	...
	if (item_uris == NULL|| destination_uri == NULL) {
		...
		athena_view_move_copy_items (view, item_uris, NULL, destination_uri,
					       cut ? GDK_ACTION_MOVE : GDK_ACTION_COPY,
					       0, 0);

		/* If items are cut then remove from clipboard */
		if (cut) {
			gtk_clipboard_clear (athena_clipboard_get (GTK_WIDGET (view)));
		}
				   ...
				   gpointer          data)
{
	...
	view_uri = athena_view_get_backing_uri (view);
					...
					gpointer          callback_data)
{
	...
	view = ATHENA_VIEW (data->view);

	if (view->details->window != NULL) {
		directory_uri = athena_file_get_activation_uri (data->target);
			     ...
			     gpointer callback_data)
{
	...
	g_object_ref (view);
	gtk_clipboard_request_contents (athena_clipboard_get (GTK_WIDGET (view)),
					copied_files_atom,
...
static void
paste_into (AthenaView *view,
	    AthenaFile *target)
{
	PasteIntoData *data;

	g_assert (ATHENA_IS_VIEW (view));
	g_assert (ATHENA_IS_FILE (target));

	data = g_new (PasteIntoData, 1);

	data->view = g_object_ref (view);
	data->target = athena_file_ref (target);
				  ...
				  gpointer callback_data)
{
	...
	selection = athena_view_get_selection (view);
	if (selection != NULL) {
		paste_into (view, ATHENA_FILE (selection->data));
		athena_file_list_free (selection);
	}
...
static void
invoke_external_bulk_rename_utility (AthenaView *view,
				     GList *selection)
{
	GString *cmd;
	char *parameter;
	...
	for (walk = selection; walk; walk = walk->next) {
		file = walk->data;
		parameter = athena_file_get_uri (file);
...
static void
real_action_undo (AthenaView *view)
{
	GtkWidget *toplevel;

	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));
	athena_file_undo_manager_undo (GTK_WINDOW (toplevel));
}

static void
real_action_redo (AthenaView *view)
{
	GtkWidget *toplevel;

	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (view));
	athena_file_undo_manager_redo (GTK_WINDOW (toplevel));
}
...
action_undo_callback (GtkAction *action,
		      gpointer callback_data)
{
	real_action_undo (ATHENA_VIEW (callback_data));
}
...
action_redo_callback (GtkAction *action,
		      gpointer callback_data)
{
	real_action_redo (ATHENA_VIEW (callback_data));
}

static void
real_action_rename (AthenaView *view,
		    gboolean select_all)
{
	AthenaFile *file;
	GList *selection;
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		...
		if (selection->next != NULL) {
			...
			file = ATHENA_FILE (selection->data);
			if (!select_all) {
				/* directories don't have a file extension, so
				 * they are always pre-selected as a whole */
				select_all = athena_file_is_directory (file);
			}
			ATHENA_VIEW_CLASS (G_OBJECT_GET_CLASS (view))->start_renaming_file (view, file, select_all);
		}
	}
...
action_rename_callback (GtkAction *action,
			gpointer callback_data)
{
	real_action_rename (ATHENA_VIEW (callback_data), FALSE);
}
...
action_rename_select_all_callback (GtkAction *action,
				   gpointer callback_data)
{
	real_action_rename (ATHENA_VIEW (callback_data), TRUE);
}

static void
file_mount_callback (AthenaFile  *file,
...
static void
file_unmount_callback (AthenaFile  *file,
		       GFile         *result_location,
		       GError        *error,
		       gpointer       callback_data)
{
...
static void
file_eject_callback (AthenaFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
...
static void
file_stop_callback (AthenaFile  *file,
		    GFile         *result_location,
		    GError        *error,
		    gpointer       callback_data)
{
			      ...
			      gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (athena_file_can_mount (file)) {
			mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
			g_mount_operation_set_password_save (mount_op, G_PASSWORD_SAVE_FOR_SESSION);
			athena_file_mount (file, mount_op, NULL,
				...
				gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		if (athena_file_can_unmount (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
			athena_file_unmount (file, mount_op, NULL,
			      ...
			      gpointer data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (athena_file_can_eject (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
			athena_file_eject (file, mount_op, NULL,
...
static void
file_start_callback (AthenaFile  *file,
		     GFile         *result_location,
		     GError        *error,
		     gpointer       callback_data)
{
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (athena_file_can_start (file) || athena_file_can_start_degraded (file)) {
			mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
			athena_file_start (file, mount_op, NULL,
			     ...
			     gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (athena_file_can_stop (file)) {
			GMountOperation *mount_op;
			mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
			athena_file_stop (file, mount_op, NULL,
			      ...
			      gpointer   data)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (athena_file_can_poll_for_media (file) && !athena_file_is_media_check_automatic (file)) {
			athena_file_poll_for_media (file);
		}
	}
	athena_file_list_free (selection);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
	g_mount_operation_set_password_save (mount_op, G_PASSWORD_SAVE_FOR_SESSION);
	athena_file_mount (file, mount_op, NULL, file_mount_callback, NULL);
	g_object_unref (mount_op);
}
				     ...
				     gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
	athena_file_unmount (file, mount_op, NULL, file_unmount_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer data)
{
	...
	mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
	athena_file_eject (file, mount_op, NULL, file_eject_callback, g_object_ref (view));
	g_object_unref (mount_op);
}
				   ...
				   gpointer   data)
{
	...
	mount_op = gtk_mount_operation_new (athena_view_get_containing_window (view));
	athena_file_start (file, mount_op, NULL, file_start_callback, NULL);
	g_object_unref (mount_op);
}
					...
					gpointer data)
{
	...
	uri = athena_file_get_activation_uri (file);
	icon = athena_file_get_icon (file, ATHENA_ICON_SIZE_STANDARD, 0);
	icon_name = athena_icon_info_get_used_name (icon);
	name = athena_file_get_display_name (file);
					 ...
					 gpointer   callback_data)
{
	...
	athena_view_activate_file (view,
				     file,
				     ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
				 ...
				 gpointer   callback_data)
{
	...
	athena_file_operations_delete (files, athena_view_get_containing_window (view),
					 NULL, NULL);
					     ...
					     gpointer callback_data)
{
	...
	l.next = NULL;
	l.data = file;
	athena_restore_files_from_trash (&l,
					   athena_view_get_containing_window (view));
}

static void
athena_view_init_show_hidden_files (AthenaView *view)
{
	AthenaWindowShowHiddenFilesMode mode;
	...
	if (mode == ATHENA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
		show_hidden_default_setting = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_SHOW_HIDDEN_FILES);
		...
		if (mode == ATHENA_WINDOW_SHOW_HIDDEN_FILES_ENABLE) {
...
static const GtkActionEntry directory_view_entries[] = {
 ...
 /* tooltip */                   N_("Redo the last undone action"),
  ...
  /* name, stock id */         { ATHENA_ACTION_LOCATION_OPEN_ALTERNATE, NULL,
  ...
  /* name, stock id */         { ATHENA_ACTION_LOCATION_OPEN_IN_NEW_TAB, NULL,
  ...
  /* name, stock id */         { ATHENA_ACTION_LOCATION_CUT, GTK_STOCK_CUT,
  ...
  /* name, stock id */         { ATHENA_ACTION_LOCATION_COPY, GTK_STOCK_COPY,
  /* label, accelerator */       NULL, "",
  /* tooltip */                  N_("Prepare this folder to be copied with a Paste command"),
				 G_CALLBACK (action_location_copy_callback) },
  /* name, stock id */         { ATHENA_ACTION_LOCATION_PASTE_FILES_INTO, GTK_STOCK_PASTE,
  /* label, accelerator */       N_("_Paste Into Folder"), "",
  ...
  /* name, stock id */         { ATHENA_ACTION_LOCATION_RESTORE_FROM_TRASH, NULL,
	       ...
	       GtkActionGroup *action_group)
{
	...
	if (strcmp (gtk_action_get_name (action), ATHENA_ACTION_NEW_EMPTY_DOCUMENT) == 0 &&
	    GTK_IS_IMAGE_MENU_ITEM (proxy)) {
		pixbuf = athena_ui_get_menu_icon ("text-x-generic");
		if (pixbuf != NULL) {
...
static void
real_merge_menus (AthenaView *view)
{
	GtkActionGroup *action_group;
	GtkUIManager *ui_manager;
	GtkAction *action;
	...
	tooltip = g_strdup_printf (_("Run or manage scripts from %s"), "~/.gnome2/athena-scripts");
	...
	action = gtk_action_group_get_action (action_group, ATHENA_ACTION_NO_TEMPLATES);
	...
	view->details->dir_merge_id = gtk_ui_manager_add_ui_from_resource (ui_manager, "/com/solusos/athena/athena-directory-view-ui.xml", NULL);
...
static gboolean
can_paste_into_file (AthenaFile *file)
{
	if (athena_file_is_directory (file) &&
	    athena_file_can_write (file)) {
		return TRUE;
	}
	if (athena_file_has_activation_uri (file)) {
		GFile *location;
		AthenaFile *activation_file;
		gboolean res;
		
		location = athena_file_get_activation_location (file);
		activation_file = athena_file_get (location);
		g_object_unref (location);
	
		/* The target location might not have data for it read yet,
		   and we can't want to do sync I/O, so treat the unknown
		   case as can-write */
		res = (athena_file_get_file_type (activation_file) == G_FILE_TYPE_UNKNOWN) ||
			(athena_file_get_file_type (activation_file) == G_FILE_TYPE_DIRECTORY &&
			 athena_file_can_write (activation_file));

		athena_file_unref (activation_file);
			    ...
			    gpointer          user_data)
{
	...
	gtk_action_set_sensitive (action,
				  can_paste && !athena_view_is_read_only (view));

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_PASTE_FILES_INTO);
	gtk_action_set_sensitive (action,
	                          can_paste && count == 1 &&
	                          can_paste_into_file (ATHENA_FILE (selection->data)));

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_PASTE_FILES_INTO);
...
static gboolean
should_show_empty_trash (AthenaView *view)
{
...
{
	GList *l;
	AthenaFile *file, *linked_file;
	char *activation_uri;
	gboolean is_dir;
	
	for (l = file_list; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		if (athena_file_is_athena_link (file) &&
		    !ATHENA_IS_DESKTOP_ICON_FILE (file)) {
			if (athena_file_is_launcher (file)) {
				return FALSE;
			}
				
			activation_uri = athena_file_get_activation_uri (file);
			...
			linked_file = athena_file_get_existing_by_uri (activation_uri);
			...
			is_dir =
				(linked_file != NULL &&
				 athena_file_is_directory (linked_file)) ||
				(activation_uri != NULL &&
				 activation_uri[strlen (activation_uri) - 1] == '/');
			
			athena_file_unref (linked_file);
			g_free (activation_uri);
...
static void
file_should_show_foreach (AthenaFile        *file,
			  gboolean            *show_mount,
			  ...
			  GDriveStartStopType *start_stop_type)
{
	...
	*show_poll = FALSE;
	...
	if (athena_file_can_unmount (file) && !*show_eject && !*show_stop) {
		*show_unmount = TRUE;
	}
	...
	*start_stop_type = athena_file_get_start_stop_type (file);
...
static void
file_should_show_self (AthenaFile        *file,
		       gboolean            *show_mount,
		       gboolean            *show_unmount,
		       gboolean            *show_eject,
		       gboolean            *show_start,
...
files_are_all_directories (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		all_directories &= athena_file_is_directory (file);
	}
...
files_is_none_directory (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		no_directory &= !athena_file_is_directory (file);
	}
...
update_restore_from_trash_action (GtkAction *action,
				  ...
				  gboolean is_self)
{
	AthenaFile *original_file;
	AthenaFile *original_dir;
	...
	if (files != NULL) {
		if (g_list_length (files) == 1) {
			original_file = athena_file_get_trash_original_file (files->data);
		} else {
			original_dirs_hash = athena_trashed_files_get_original_directories (files, NULL);
			if (original_dirs_hash != NULL) {
				original_dirs = g_hash_table_get_keys (original_dirs_hash);
				if (g_list_length (original_dirs) == 1) {
					original_dir = athena_file_ref (ATHENA_FILE (original_dirs->data));
				}
			}
		}
	}
	...
	if (original_file != NULL || original_dirs != NULL) {
		...
		if (original_file != NULL) {
			original_location = athena_file_get_location (original_file);
		} else if (original_dir != NULL) {
			original_location = athena_file_get_location (original_dir);
		}
	...
	athena_file_unref (original_file);
	athena_file_unref (original_dir);
...
static void
real_update_menus_volumes (AthenaView *view,
			   GList *selection,
			   gint selection_count)
{
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SELF_STOP_VOLUME);
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SELF_POLL);
	gtk_action_set_visible (action, show_self_poll);

}

static void
real_update_location_menu_volumes (AthenaView *view)
{
	GtkAction *action;
	AthenaFile *file;
	gboolean show_mount;
	...
	file = ATHENA_FILE (view->details->location_popup_directory_as_file);
	file_should_show_foreach (file,
				  &show_mount,
				  &show_unmount,
				  &show_eject,
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_MOUNT_VOLUME);
	gtk_action_set_visible (action, show_mount);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_UNMOUNT_VOLUME);
	gtk_action_set_visible (action, show_unmount);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_EJECT_VOLUME);
	gtk_action_set_visible (action, show_eject);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_START_VOLUME);
	gtk_action_set_visible (action, show_start);
	if (show_start) {
		switch (start_stop_type) {
		default:
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_STOP_VOLUME);
	gtk_action_set_visible (action, show_stop);
	if (show_stop) {
		switch (start_stop_type) {
		default:
 ...
 */
static void
real_update_paste_menu (AthenaView *view,
			GList *selection,
			gint selection_count)
{
	...
	selection_is_read_only = selection_count == 1 &&
		(!athena_file_can_write (ATHENA_FILE (selection->data)) &&
		 !athena_file_has_activation_uri (ATHENA_FILE (selection->data)));
		 
	is_read_only = athena_view_is_read_only (view);
	...
	gtk_clipboard_request_targets (athena_clipboard_get (GTK_WIDGET (view)),
				       clipboard_targets_received,
				       view);
}

static void
real_update_location_menu (AthenaView *view)
{
	GtkAction *action;
	AthenaFile *file;
	...
	show_open_in_new_tab = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_ALWAYS_USE_BROWSER);
	show_open_alternate = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_ALWAYS_USE_BROWSER);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_OPEN_ALTERNATE);
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_OPEN_IN_NEW_TAB);
	...
	file = view->details->location_popup_directory_as_file;
	g_assert (ATHENA_IS_FILE (file));
	g_assert (athena_file_check_if_ready (file, ATHENA_FILE_ATTRIBUTE_INFO |
						ATHENA_FILE_ATTRIBUTE_MOUNT |
						ATHENA_FILE_ATTRIBUTE_FILESYSTEM_INFO));

	is_special_link = ATHENA_IS_DESKTOP_ICON_FILE (file);
	is_desktop_or_home_dir = athena_file_is_home (file)
		|| athena_file_is_desktop_directory (file);

	can_delete_file =
		athena_file_can_delete (file) &&
		!is_special_link &&
		!is_desktop_or_home_dir;

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_CUT);
	...
	if (file != NULL &&
		...
		if (athena_file_is_self_owned (file)) {
			show_delete = FALSE;
		}
		...
		show_separate_delete_command = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_ENABLE_DELETE);
	}

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_TRASH);
	g_object_set (action,
		      "label", label,
		      "tooltip", tip,
		      "icon-name", (file != NULL &&
				    athena_file_is_in_trash (file)) ?
		      ATHENA_ICON_DELETE : ATHENA_ICON_TRASH_FULL,
		      NULL);
	gtk_action_set_sensitive (action, can_delete_file);
	gtk_action_set_visible (action, show_delete);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_DELETE);
	gtk_action_set_visible (action, show_separate_delete_command);
	if (show_separate_delete_command) {
		gtk_action_set_sensitive (action, can_delete_file);
		g_object_set (action,
			      "icon-name", ATHENA_ICON_DELETE,
			      "sensitive", can_delete_file,
			      NULL);
	}

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_LOCATION_RESTORE_FROM_TRASH);
	l.prev = NULL;
	l.next = NULL;
	l.data = file;
...
static void
clipboard_changed_callback (AthenaClipboardMonitor *monitor, AthenaView *view)
{
	GList *selection;
	gint selection_count;
...
can_delete_all (GList *files)
{
	...
	for (l = files; l != NULL; l = l->next) {
		file = l->data;
		if (!athena_file_can_delete (file)) {
			return FALSE;
		}
	}
	return TRUE;
...
static gboolean
has_writable_extra_pane (AthenaView *view)
{
	AthenaView *other_view;

	other_view = get_directory_view_of_extra_pane (view);
	if (other_view != NULL) {
		return !athena_view_is_read_only (other_view);
	}
	return FALSE;
}

static void
real_update_menus (AthenaView *view)
{
	GList *selection, *l;
	gint selection_count;
	...
	can_create_files = athena_view_supports_creating_files (view);
	can_delete_files =
	...
	if (selection_count > 1) {
		...
		gtk_action_set_sensitive (action,
					  selection_count == 1 &&
					  athena_view_can_rename_file (view, selection->data));
	}

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_NEW_FOLDER);
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (!athena_mime_file_opens_in_external_app (file)) {
			show_app = FALSE;
		}
	...
	if (can_open && show_app) {
		app = athena_mime_get_default_application_for_files (selection);
	}
	...
	show_open_alternate = file_list_all_are_folders (selection) &&
		selection_count > 0 &&
		g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_ALWAYS_USE_BROWSER) &&
		!ATHENA_IS_DESKTOP_ICON_VIEW (view);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_OPEN_ALTERNATE);
	...
	show_properties = (!ATHENA_IS_DESKTOP_ICON_VIEW (view) || selection_count > 0);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_PROPERTIES);
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_PROPERTIES_ACCEL);

	gtk_action_set_sensitive (action, show_properties);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_EMPTY_TRASH);
	g_object_set (action,
		      "label", _("E_mpty Trash"),
		      NULL);
	gtk_action_set_sensitive (action, !athena_trash_monitor_is_empty ());
	...
	if (view->details->model &&
	    ATHENA_IS_SEARCH_DIRECTORY (view->details->model)) {
		AthenaSearchDirectory *search;

		search = ATHENA_SEARCH_DIRECTORY (view->details->model);
		if (athena_search_directory_is_saved_search (search)) {
			show_save_search = TRUE;
			save_search_sensitive = athena_search_directory_is_modified (search);
		} else {
			show_save_search_as = TRUE;
		}
	} 
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SAVE_SEARCH);
	gtk_action_set_visible (action, show_save_search);
	gtk_action_set_sensitive (action, save_search_sensitive);
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SAVE_SEARCH_AS);
	gtk_action_set_visible (action, show_save_search_as);


	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SELECT_ALL);
	gtk_action_set_sensitive (action, !athena_view_is_empty (view));

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_SELECT_PATTERN);
	gtk_action_set_sensitive (action, !athena_view_is_empty (view));

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_INVERT_SELECTION);
	gtk_action_set_sensitive (action, !athena_view_is_empty (view));
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_COPY_TO_NEXT_PANE);
	...
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_MOVE_TO_NEXT_PANE);
	gtk_action_set_sensitive (action, can_delete_files && next_pane_is_writable);


	show_desktop_target =
		g_settings_get_boolean (gnome_background_preferences, ATHENA_PREFERENCES_SHOW_DESKTOP) &&
		!g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_DESKTOP_IS_HOME_DIR);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_COPY_TO_HOME);
	gtk_action_set_sensitive (action, can_copy_files);
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_COPY_TO_DESKTOP);
	gtk_action_set_sensitive (action, can_copy_files);
	gtk_action_set_visible (action, show_desktop_target);

	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_MOVE_TO_HOME);
	gtk_action_set_sensitive (action, can_delete_files);
	action = gtk_action_group_get_action (view->details->dir_action_group,
					      ATHENA_ACTION_MOVE_TO_DESKTOP);
	gtk_action_set_sensitive (action, can_delete_files);
	gtk_action_set_visible (action, show_desktop_target);
 ...
 **/
void 
athena_view_pop_up_selection_context_menu  (AthenaView *view, 
					      GdkEventButton  *event)
{
	g_assert (ATHENA_IS_VIEW (view));
	...
	eel_pop_up_context_menu (create_popup_menu 
				 (view, ATHENA_VIEW_POPUP_PATH_SELECTION),
				 event);
}
 ...
 **/
void 
athena_view_pop_up_background_context_menu (AthenaView *view, 
					      GdkEventButton  *event)
{
	g_assert (ATHENA_IS_VIEW (view));
	...
	eel_pop_up_context_menu (create_popup_menu 
				 (view, ATHENA_VIEW_POPUP_PATH_BACKGROUND),
				 event);
}

static void
real_pop_up_location_context_menu (AthenaView *view)
{
	...
	eel_pop_up_context_menu (create_popup_menu 
				 (view, ATHENA_VIEW_POPUP_PATH_LOCATION),
				 view->details->location_popup_event);
}

static void
location_popup_file_attributes_ready (AthenaFile *file,
				      gpointer      data)
{
	AthenaView *view;
...
static void
unschedule_pop_up_location_context_menu (AthenaView *view)
{
	if (view->details->location_popup_directory_as_file != NULL) {
		g_assert (ATHENA_IS_FILE (view->details->location_popup_directory_as_file));
		athena_file_cancel_call_when_ready (view->details->location_popup_directory_as_file,
						      location_popup_file_attributes_ready,
						      view);
		athena_file_unref (view->details->location_popup_directory_as_file);
		view->details->location_popup_directory_as_file = NULL;
	}
}

static void
schedule_pop_up_location_context_menu (AthenaView *view,
				       GdkEventButton  *event,
				       AthenaFile    *file)
{
	g_assert (ATHENA_IS_FILE (file));
	...
	if (file == view->details->location_popup_directory_as_file) {
		if (athena_file_check_if_ready (file, ATHENA_FILE_ATTRIBUTE_INFO |
						  ATHENA_FILE_ATTRIBUTE_MOUNT |
						  ATHENA_FILE_ATTRIBUTE_FILESYSTEM_INFO)) {
			real_pop_up_location_context_menu (view);
		}
		...
		view->details->location_popup_directory_as_file = athena_file_ref (file);
		athena_file_call_when_ready (view->details->location_popup_directory_as_file,
					       ATHENA_FILE_ATTRIBUTE_INFO |
					       ATHENA_FILE_ATTRIBUTE_MOUNT |
					       ATHENA_FILE_ATTRIBUTE_FILESYSTEM_INFO,
					       location_popup_file_attributes_ready,
 ...
 **/
void 
athena_view_pop_up_location_context_menu (AthenaView *view, 
					    GdkEventButton  *event,
					    const char      *location)
{
	AthenaFile *file;
	...
	if (location != NULL) {
		file = athena_file_get_by_uri (location);
	} else {
		file = athena_file_ref (view->details->directory_as_file);
	}
...
static void
schedule_update_menus (AthenaView *view) 
{
	g_assert (ATHENA_IS_VIEW (view));
...
static void
remove_update_status_idle_callback (AthenaView *view) 
{
...
static void
schedule_update_status (AthenaView *view) 
{
	g_assert (ATHENA_IS_VIEW (view));
...
/**
 * athena_view_notify_selection_changed:
 * 
 * Notify this view that the selection has changed. This is normally
 * called only by subclasses.
 * @view: AthenaView whose selection has changed.
 * 
 **/
void
athena_view_notify_selection_changed (AthenaView *view)
{
	GtkWindow *window;
	GList *selection;
...
static void
file_changed_callback (AthenaFile *file, gpointer callback_data)
{
	AthenaView *view = ATHENA_VIEW (callback_data);
 ...
 * @view: AthenaView whose location will be changed.
 * @uri: A string representing the uri to switch to.
 * 
 **/
static void
load_directory (AthenaView *view,
		AthenaDirectory *directory)
{
	AthenaDirectory *old_directory;
	AthenaFile *old_file;
	AthenaFileAttributes attributes;
	...
	athena_directory_unref (old_directory);

	old_file = view->details->directory_as_file;
	view->details->directory_as_file =
		athena_directory_get_corresponding_file (directory);
	athena_file_unref (old_file);
	...
	attributes = 
		ATHENA_FILE_ATTRIBUTE_INFO |
		ATHENA_FILE_ATTRIBUTE_FILESYSTEM_INFO;
	athena_file_monitor_add (view->details->directory_as_file,
				   &view->details->directory_as_file,
				   attributes);
...
static void
finish_loading (AthenaView *view)
{
	AthenaFileAttributes attributes;

	athena_window_report_load_underway (view->details->window,
					      ATHENA_VIEW (view));
	...
	athena_window_view_visible  (view->details->window, ATHENA_VIEW (view));

	if (athena_directory_are_all_files_seen (view->details->model)) {
		/* Unschedule a pending update and schedule a new one with the minimal
		 * update interval. This gives the view a short chance at gathering the
		 * (cached) deep counts.
		 */
	...
	athena_directory_file_monitor_add (view->details->model,
					     &view->details->model,
					     view->details->show_hidden_files,
					     attributes,
					     files_added_callback, view);
...
static void
finish_loading_if_all_metadata_loaded (AthenaView *view)
{
...
static void
metadata_for_directory_as_file_ready_callback (AthenaFile *file,
			      		       gpointer callback_data)
{
	AthenaView *view;
...
static void
metadata_for_files_in_directory_ready_callback (AthenaDirectory *directory,
				   		GList *files,
			           		gpointer callback_data)
{
	AthenaView *view;
...
static void
disconnect_directory_handler (AthenaView *view, int *id)
{
	disconnect_handler (G_OBJECT (view->details->model), id);
}

static void
disconnect_directory_as_file_handler (AthenaView *view, int *id)
{
	disconnect_handler (G_OBJECT (view->details->directory_as_file), id);
}

static void
disconnect_model_handlers (AthenaView *view)
{
	if (view->details->model == NULL) {
		return;
	}
	...
	athena_file_cancel_call_when_ready (view->details->directory_as_file,
					      metadata_for_directory_as_file_ready_callback,
					      view);
	athena_directory_cancel_callback (view->details->model,
					    metadata_for_files_in_directory_ready_callback,
					    view);
	athena_directory_file_monitor_remove (view->details->model,
						&view->details->model);
	athena_file_monitor_remove (view->details->directory_as_file,
				      &view->details->directory_as_file);
}

static void
athena_view_select_file (AthenaView *view, AthenaFile *file)
{
	GList file_list;
	...
	file_list.next = NULL;
	file_list.prev = NULL;
	athena_view_call_set_selection (view, &file_list);
}
 ...
 **/
void
athena_view_stop_loading (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));
	...
	if (view->details->model != NULL) {
		athena_directory_file_monitor_remove (view->details->model, view);
	}
	done_loading (view, FALSE);
}

gboolean
athena_view_is_editable (AthenaView *view)
{
	AthenaDirectory *directory;
	...
	if (directory != NULL) {
		return athena_directory_is_editable (directory);
	}
...
static gboolean
real_is_read_only (AthenaView *view)
{
	AthenaFile *file;
	
	if (!athena_view_is_editable (view)) {
		return TRUE;
	}
	
	file = athena_view_get_directory_as_file (view);
	if (file != NULL) {
		return !athena_file_can_write (file);
	}
	return FALSE;
}

/**
 * athena_view_should_show_file
 * 
 * Returns whether or not this file should be displayed based on
 * current filtering options.
 */
gboolean
athena_view_should_show_file (AthenaView *view, AthenaFile *file)
{
	return athena_file_should_show (file,
					  view->details->show_hidden_files,
					  view->details->show_foreign_files);
}

static gboolean
real_using_manual_layout (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), FALSE);
...
{
	schedule_update_menus (ATHENA_VIEW (callback_data));
}

void
athena_view_ignore_hidden_file_preferences (AthenaView *view)
{
...
void
athena_view_set_show_foreign (AthenaView *view,
...
char *
athena_view_get_uri (AthenaView *view)
{
	g_return_val_if_fail (ATHENA_IS_VIEW (view), NULL);
	if (view->details->model == NULL) {
		return NULL;
	}
	return athena_directory_get_uri (view->details->model);
}

void
athena_view_move_copy_items (AthenaView *view,
			       ...
			       int x, int y)
{
	...
	target_file = athena_file_get_existing_by_uri (target_uri);
	/* special-case "command:" here instead of starting a move/copy */
	if (target_file != NULL && athena_file_is_launcher (target_file)) {
		...
		return;
		   ...
		   target_file != NULL &&
		   athena_file_is_archive (target_file)) {
		...
		athena_launch_application_from_command (screen, command, FALSE, NULL);
		g_free (command);
...
static void
athena_view_trash_state_changed_callback (AthenaTrashMonitor *trash_monitor,
					    gboolean state, gpointer callback_data)
{
	AthenaView *view;

	view = (AthenaView *) callback_data;
	g_assert (ATHENA_IS_VIEW (view));
...
void
athena_view_start_batching_selection_changes (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));
...
void
athena_view_stop_batching_selection_changes (AthenaView *view)
{
	g_return_if_fail (ATHENA_IS_VIEW (view));
...
gboolean
athena_view_get_active (AthenaView *view)
{
	g_assert (ATHENA_IS_VIEW (view));
...
static GArray *
real_get_selected_icon_locations (AthenaView *view)
{
...
static void
athena_view_set_property (GObject         *object,
			    guint            prop_id,
			    const GValue    *value,
			    GParamSpec      *pspec)
{
	...
	directory_view = ATHENA_VIEW (object);

	switch (prop_id)  {
	case PROP_WINDOW_SLOT:
		g_assert (directory_view->details->slot == NULL);

		slot = ATHENA_WINDOW_SLOT (g_value_get_object (value));
...
gboolean
athena_view_handle_scroll_event (AthenaView *directory_view,
				   GdkEventScroll *event)
{
	...
	if (event->state & GDK_CONTROL_MASK) {
		...
		case GDK_SCROLL_UP:
			/* Zoom In */
			athena_view_bump_zoom_level (directory_view, 1);
			return TRUE;
...
static gboolean
athena_view_scroll_event (GtkWidget *widget,
			    GdkEventScroll *event)
{
	AthenaView *directory_view;

	directory_view = ATHENA_VIEW (widget);
	if (athena_view_handle_scroll_event (directory_view, event)) {
		return TRUE;
	}
...
static void
athena_view_parent_set (GtkWidget *widget,
			  GtkWidget *old_parent)
{
	...
	if (parent != NULL) {
		...
		if (view->details->slot == 
		    athena_window_get_active_slot (view->details->window)) {
			view->details->active = TRUE;
...
static void
athena_view_class_init (AthenaViewClass *klass)
{
	GObjectClass *oclass;
	...
	oclass->finalize = athena_view_finalize;
	oclass->set_property = athena_view_set_property;

	widget_class->destroy = athena_view_destroy;
	widget_class->scroll_event = athena_view_scroll_event;
	widget_class->parent_set = athena_view_parent_set;

	g_type_class_add_private (klass, sizeof (AthenaViewDetails));
	...
	signals[ADD_FILE] =
		g_signal_new ("add_file",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, add_file),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ATHENA_TYPE_FILE, ATHENA_TYPE_DIRECTORY);
	signals[BEGIN_FILE_CHANGES] =
		g_signal_new ("begin_file_changes",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, begin_file_changes),
		              NULL, NULL,
	...
	signals[BEGIN_LOADING] =
		g_signal_new ("begin_loading",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, begin_loading),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[CLEAR] =
		g_signal_new ("clear",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, clear),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[END_FILE_CHANGES] =
		g_signal_new ("end_file_changes",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, end_file_changes),
		              NULL, NULL,
		              g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);
	signals[END_LOADING] =
		g_signal_new ("end_loading",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, end_loading),
		              NULL, NULL,
	...
	signals[FILE_CHANGED] =
		g_signal_new ("file_changed",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, file_changed),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ATHENA_TYPE_FILE, ATHENA_TYPE_DIRECTORY);
	signals[LOAD_ERROR] =
		g_signal_new ("load_error",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, load_error),
		              NULL, NULL,
	...
	signals[REMOVE_FILE] =
		g_signal_new ("remove_file",
		              G_TYPE_FROM_CLASS (klass),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaViewClass, remove_file),
		              NULL, NULL,
		              g_cclosure_marshal_generic,
		              G_TYPE_NONE, 2, ATHENA_TYPE_FILE, ATHENA_TYPE_DIRECTORY);
	...
	signals[TRASH] =
		g_signal_new ("trash",
			      ...
			      G_STRUCT_OFFSET (AthenaViewClass, trash),
	...
	signals[DELETE] =
		g_signal_new ("delete",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (AthenaViewClass, delete),
			      g_signal_accumulator_true_handled, NULL,
			      g_cclosure_marshal_generic,
			      G_TYPE_BOOLEAN, 0);
