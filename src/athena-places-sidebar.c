...
#define DEBUG_FLAG ATHENA_DEBUG_PLACES
...
typedef struct {
	...
	gboolean mounting;
	AthenaWindowSlot *go_to_after_mount_slot;
	AthenaWindowOpenFlags go_to_after_mount_flags;
...
} AthenaPlacesSidebarClass;

typedef struct {
        GObject parent;
} AthenaPlacesSidebarProvider;

typedef struct {
        GObjectClass parent;
} AthenaPlacesSidebarProviderClass;
...
static void  open_selected_bookmark                    (AthenaPlacesSidebar        *sidebar,
							GtkTreeModel                 *model,
							GtkTreeIter                  *iter,
							AthenaWindowOpenFlags flags);
static void  athena_places_sidebar_style_set         (GtkWidget                    *widget,
							GtkStyle                     *previous_style);
static gboolean eject_or_unmount_bookmark              (AthenaPlacesSidebar *sidebar,
							GtkTreePath *path);
static gboolean eject_or_unmount_selection             (AthenaPlacesSidebar *sidebar);
...
static void bookmarks_check_popup_sensitivity          (AthenaPlacesSidebar *sidebar);
...
static const GtkTargetEntry athena_shortcuts_source_targets[] = {
	{ "GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW }
};

/* Target types for dropping into the shortcuts list */
static const GtkTargetEntry athena_shortcuts_drop_targets [] = {
	{ "GTK_TREE_MODEL_ROW", GTK_TARGET_SAME_WIDGET, GTK_TREE_MODEL_ROW },
...
typedef struct {
  ...
  AthenaPlacesSidebar *sidebar;
} AthenaShortcutsModel;

typedef struct {
  GtkListStoreClass parent_class;
} AthenaShortcutsModelClass;

#define ATHENA_TYPE_SHORTCUTS_MODEL (_athena_shortcuts_model_get_type ())
#define ATHENA_SHORTCUTS_MODEL(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), ATHENA_SHORTCUTS_MODEL_TYPE, AthenaShortcutsModel))

GType _athena_shortcuts_model_get_type (void);
static void _athena_shortcuts_model_drag_source_init (GtkTreeDragSourceIface *iface);
G_DEFINE_TYPE_WITH_CODE (AthenaShortcutsModel, _athena_shortcuts_model, GTK_TYPE_LIST_STORE,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_DRAG_SOURCE,
						_athena_shortcuts_model_drag_source_init));
static GtkListStore *athena_shortcuts_model_new (AthenaPlacesSidebar *sidebar);

G_DEFINE_TYPE (AthenaPlacesSidebar, athena_places_sidebar, GTK_TYPE_SCROLLED_WINDOW);

static GdkPixbuf *
get_eject_icon (AthenaPlacesSidebar *sidebar,
		gboolean highlighted)
{
	...
	icon_size = athena_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
...
{
	return g_settings_get_boolean (gnome_background_preferences, ATHENA_PREFERENCES_SHOW_DESKTOP) &&
	       !g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_DESKTOP_IS_HOME_DIR);
}

static gboolean
is_built_in_bookmark (AthenaFile *file)
{
	gboolean built_in;
	gint idx;

	if (athena_file_is_home (file)) {
		return TRUE;
	}

	if (athena_file_is_desktop_directory (file) && !should_show_desktop ()) {
		return FALSE;
	}
	...
	for (idx = 0; idx < G_USER_N_DIRECTORIES; idx++) {
		...
		if (athena_file_is_user_special_directory (file, idx)) {
...
static GtkTreeIter
add_heading (AthenaPlacesSidebar *sidebar,
...
static void
check_heading_for_section (AthenaPlacesSidebar *sidebar,
			   SectionType section_type)
{
	switch (section_type) {
...
static void
add_place (AthenaPlacesSidebar *sidebar,
	   ...
	   const char *tooltip)
{
	...
	icon_size = athena_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
	icon_info = athena_icon_info_lookup (icon, icon_size);
...
static void
sidebar_update_restore_selection (AthenaPlacesSidebar *sidebar,
				  const gchar *location,
				  const gchar *last_uri)
{
...
static void
update_places (AthenaPlacesSidebar *sidebar)
{
	...
	slot = athena_window_get_active_slot (sidebar->window);
	location = athena_window_slot_get_current_uri (slot);
	...
	/* add bookmarks */
	bookmark_count = athena_bookmark_list_length (sidebar->bookmarks);

	for (index = 0; index < bookmark_count; ++index) {
		bookmark = athena_bookmark_list_item_at (sidebar->bookmarks, index);

		if (athena_bookmark_uri_known_not_to_exist (bookmark)) {
			continue;
		}

		root = athena_bookmark_get_location (bookmark);
		file = athena_file_get (root);
		...
		bookmark_name = athena_bookmark_get_name (bookmark);
		icon = athena_bookmark_get_icon (bookmark);
		mount_uri = athena_bookmark_get_uri (bookmark);
		tooltip = g_file_get_parse_name (root);
	...
	/* home folder */
	mount_uri = athena_get_home_directory_uri ();
	icon = g_themed_icon_new (ATHENA_ICON_HOME);
	add_place (sidebar, PLACES_BUILT_IN,
		   SECTION_COMPUTER,
		   _("Home"), icon,
	...
	if (should_show_desktop ()) {
		...
		icon = g_themed_icon_new (ATHENA_ICON_DESKTOP);
		add_place (sidebar, PLACES_BUILT_IN,
			   SECTION_COMPUTER,
			   _("Desktop"), icon,
			   mount_uri, NULL, NULL, NULL, 0,
	...
	for (index = 0; index < G_USER_N_DIRECTORIES; index++) {
		...
		icon = athena_user_special_directory_get_gicon (index);
		mount_uri = g_file_get_uri (root);
		tooltip = g_file_get_parse_name (root);
	...
	icon = g_themed_icon_new (ATHENA_ICON_FILESYSTEM);
	add_place (sidebar, PLACES_BUILT_IN,
		   SECTION_COMPUTER,
		   _("File System"), icon,
		   mount_uri, NULL, NULL, NULL, 0,
	...
	icon = g_themed_icon_new (ATHENA_ICON_NETWORK);
	add_place (sidebar, PLACES_BUILT_IN,
		   SECTION_NETWORK,
...
static gboolean
over_eject_button (AthenaPlacesSidebar *sidebar,
		   gint x,
		   gint y,
		   GtkTreePath **path)
{
	...
	*path = NULL;
	...
	if (gtk_tree_view_get_path_at_pos (sidebar->tree_view,
		...
		eject_button_size = athena_get_icon_size_for_stock_size (GTK_ICON_SIZE_MENU);
...
static gboolean
clicked_eject_button (AthenaPlacesSidebar *sidebar,
		      GtkTreePath **path)
{
...
desktop_setting_changed_callback (gpointer user_data)
{
	...
	sidebar = ATHENA_PLACES_SIDEBAR (user_data);

	update_places (sidebar);
}

static void
loading_uri_callback (AthenaWindow *window,
		      char *location,
		      AthenaPlacesSidebar *sidebar)
{
	GtkTreeSelection *selection;
...
static void
free_drag_data (AthenaPlacesSidebar *sidebar)
{
	sidebar->drag_data_received = FALSE;

	if (sidebar->drag_list) {
		athena_drag_destroy_selection_list (sidebar->drag_list);
		sidebar->drag_list = NULL;
	}
}

static gboolean
can_accept_file_as_bookmark (AthenaFile *file)
{
	return (athena_file_is_directory (file) &&
...
can_accept_items_as_bookmarks (const GList *items)
{
	...
	for (max = 100; items != NULL && max >= 0; items = items->next, max--) {
		uri = ((AthenaDragSelectionItem *)items->data)->uri;
	...
	if (pos == GTK_TREE_VIEW_DROP_BEFORE ||
		...
		if (sidebar->drag_list == NULL) {
			...
			gtk_tree_model_get (GTK_TREE_MODEL (sidebar->store),
					    ...
					    PLACES_SIDEBAR_COLUMN_URI, &uri,
					    -1);
			athena_drag_default_drop_action_for_icons (context, uri,
								     sidebar->drag_list,
								     &action);
			g_free (uri);
		}
...
static void
bookmarks_drop_uris (AthenaPlacesSidebar *sidebar,
		     ...
		     int                    position)
{
	...
	for (i = 0; uris[i]; i++) {
		...
		uri = athena_file_get_drop_target_uri (file);
		location = g_file_new_for_uri (uri);
		athena_file_unref (file);

		bookmark = athena_bookmark_new (location, NULL, NULL);

		if (!athena_bookmark_list_contains (sidebar->bookmarks, bookmark)) {
			athena_bookmark_list_insert_item (sidebar->bookmarks, bookmark, position++);
		}
...
build_selection_list (const char *data)
{
	...
	for (i = 0; uris[i]; i++) {
		uri = uris[i];
		item = athena_drag_selection_item_new ();
...
static gboolean
get_selected_iter (AthenaPlacesSidebar *sidebar,
		   GtkTreeIter *iter)
{
	GtkTreeSelection *selection;
...
static void
reorder_bookmarks (AthenaPlacesSidebar *sidebar,
		   int                   new_position)
{
	GtkTreeIter iter;
	PlaceType type; 
	...
	if (type != PLACES_BOOKMARK ||
	    old_position < 0 ||
	    old_position >= athena_bookmark_list_length (sidebar->bookmarks)) {
		return;
	}

	athena_bookmark_list_move_item (sidebar->bookmarks, old_position,
					  new_position);
}
			     ...
			     AthenaPlacesSidebar *sidebar)
{
	...
	if (tree_pos == GTK_TREE_VIEW_DROP_BEFORE ||
		...
		if (real_action == GDK_ACTION_ASK) {
			real_action =
				athena_drag_drop_action_ask (GTK_WIDGET (tree_view),
		...
		if (real_action > 0) {
			...
			case TEXT_URI_LIST:
				...
				athena_file_operations_copy_move (uris, NULL, drop_uri,
								    real_action, GTK_WIDGET (tree_view),
								    NULL, NULL);
				athena_drag_destroy_selection_list (selection_list);
				g_list_free (uris);
				success = TRUE;
				break;
			case GTK_TREE_MODEL_ROW:
				...
				GtkMenu   *menu)
{
	...
	sidebar = ATHENA_PLACES_SIDEBAR (attach_widget);
	g_assert (ATHENA_IS_PLACES_SIDEBAR (sidebar));
...
static void
bookmarks_check_popup_sensitivity (AthenaPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	PlaceType type; 
	GDrive *drive = NULL;
	...
	gtk_widget_set_sensitive (sidebar->popup_menu_empty_trash_item, !athena_trash_monitor_is_empty ());
	...
	if (mount != NULL) {
		...
		show_properties = athena_directory_is_local (directory);
		   ...
		   GObject *user_data)
{
	...
	if (mount != NULL) {
		...
		if (sidebar->go_to_after_mount_slot != NULL) {
			if ((sidebar->go_to_after_mount_flags & ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
				athena_window_slot_open_location (sidebar->go_to_after_mount_slot, location,
								    sidebar->go_to_after_mount_flags, NULL);
			} else {
				AthenaWindow *new, *cur;

				cur = ATHENA_WINDOW (sidebar->window);
				new = athena_application_create_window (athena_application_get_singleton (),
									  gtk_window_get_screen (GTK_WINDOW (cur)));
				athena_window_go_to (new, location);
			}
		}
...
static void
open_selected_bookmark (AthenaPlacesSidebar *sidebar,
			GtkTreeModel	      *model,
			GtkTreeIter	      *iter,
			AthenaWindowOpenFlags	      flags)
{
	...
	if (uri != NULL) {
		...
		if (volume != NULL && !sidebar->mounting) {
			...
			athena_file_operations_mount_volume_full (NULL, volume,
								    volume_mounted_cb,
								    G_OBJECT (sidebar));
...
static void
open_shortcut_from_menu (AthenaPlacesSidebar *sidebar,
			 AthenaWindowOpenFlags	       flags)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
open_shortcut_in_new_window_cb (GtkMenuItem	      *item,
				AthenaPlacesSidebar *sidebar)
{
	open_shortcut_from_menu (sidebar, ATHENA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}
...
open_shortcut_in_new_tab_cb (GtkMenuItem	      *item,
				AthenaPlacesSidebar *sidebar)
{
	open_shortcut_from_menu (sidebar, ATHENA_WINDOW_OPEN_FLAG_NEW_TAB);
}

/* Add bookmark for the selected item */
static void
add_bookmark (AthenaPlacesSidebar *sidebar)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	char *uri;
	...
	if (get_selected_iter (sidebar, &iter)) {
		...
		if (!athena_bookmark_list_contains (sidebar->bookmarks, bookmark)) {
			athena_bookmark_list_append (sidebar->bookmarks, bookmark);
		}
...
static void
rename_selected_bookmark (AthenaPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	GtkTreeViewColumn *column;
...
static void
remove_selected_bookmarks (AthenaPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	PlaceType type; 
	int index;
	...
	athena_bookmark_list_delete_item_at (sidebar->bookmarks, index);
}
		   ...
		   AthenaPlacesSidebar *sidebar)
{
	...
	if (volume != NULL) {
		athena_file_operations_mount_volume (NULL, volume);
		g_object_unref (volume);
	}
}
...
{
	if (mount != NULL) {
		athena_file_operations_unmount_mount_full (NULL, mount, FALSE, TRUE,
							     unmount_done,
...
static void
do_unmount_selection (AthenaPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GMount *mount;
...
static gboolean
eject_or_unmount_bookmark (AthenaPlacesSidebar *sidebar,
			   GtkTreePath *path)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
static gboolean
eject_or_unmount_selection (AthenaPlacesSidebar *sidebar)
{
	GtkTreeIter iter;
	GtkTreePath *path;
	gboolean ret;
...
empty_trash_cb (GtkMenuItem           *item,
		AthenaPlacesSidebar *sidebar)
{
	athena_file_operations_empty_trash (GTK_WIDGET (sidebar->window));
}

static gboolean
find_prev_or_next_row (AthenaPlacesSidebar *sidebar,
		       GtkTreeIter *iter,
		       gboolean go_up)
{
...
static gboolean
find_prev_row (AthenaPlacesSidebar *sidebar, GtkTreeIter *iter)
{
	return find_prev_or_next_row (sidebar, iter, TRUE);
}

static gboolean
find_next_row (AthenaPlacesSidebar *sidebar, GtkTreeIter *iter)
{
	       ...
	       AthenaPlacesSidebar *sidebar)
{
	...
	if (uri != NULL) {
		...
		file = athena_file_get_by_uri (uri);
		list = g_list_prepend (NULL, athena_file_ref (file));

		athena_properties_window_present (list, GTK_WIDGET (sidebar), NULL);
...
static gboolean
athena_places_sidebar_focus (GtkWidget *widget,
			       GtkDirectionType direction)
{
	AthenaPlacesSidebar *sidebar = ATHENA_PLACES_SIDEBAR (widget);
	GtkTreePath *path;
	GtkTreeIter iter;
	gboolean res;
	...
	return GTK_WIDGET_CLASS (athena_places_sidebar_parent_class)->focus (widget, direction);
}
...
static void
bookmarks_build_popup_menu (AthenaPlacesSidebar *sidebar)
{
	GtkWidget *item;
	...
	use_browser = g_settings_get_boolean (athena_preferences,
...
static void
bookmarks_update_popup_menu (AthenaPlacesSidebar *sidebar)
{
	bookmarks_build_popup_menu (sidebar);  
}

static void
bookmarks_popup_menu (AthenaPlacesSidebar *sidebar,
...
static void
update_eject_buttons (AthenaPlacesSidebar *sidebar,
		      GtkTreePath 	    *path)
{
	GtkTreeIter iter;
		  ...
		  AthenaPlacesSidebar *sidebar)
{
	...
	if (bookmark != NULL) {
		athena_bookmark_set_custom_name (bookmark, new_text);
	}
}
...
static void
trash_state_changed_cb (AthenaTrashMonitor *trash_monitor,
			gboolean             state,
			gpointer             data)
{
	AthenaPlacesSidebar *sidebar;

	sidebar = ATHENA_PLACES_SIDEBAR (data);
...
static void
athena_places_sidebar_init (AthenaPlacesSidebar *sidebar)
{
	GtkTreeView       *tree_view;
	...
	gtk_tree_view_column_set_max_width (GTK_TREE_VIEW_COLUMN (col), ATHENA_ICON_SIZE_SMALLER);
	gtk_tree_view_append_column (tree_view, col);

	sidebar->store = athena_shortcuts_model_new (sidebar);
	...
	gtk_tree_view_enable_model_drag_source (GTK_TREE_VIEW (tree_view),
						GDK_BUTTON1_MASK,
						athena_shortcuts_source_targets,
						G_N_ELEMENTS (athena_shortcuts_source_targets),
						GDK_ACTION_MOVE);
	gtk_drag_dest_set (GTK_WIDGET (tree_view),
			   0,
			   athena_shortcuts_drop_targets, G_N_ELEMENTS (athena_shortcuts_drop_targets),
...
static void
athena_places_sidebar_dispose (GObject *object)
{
	AthenaPlacesSidebar *sidebar;

	sidebar = ATHENA_PLACES_SIDEBAR (object);
	...
	G_OBJECT_CLASS (athena_places_sidebar_parent_class)->dispose (object);
}

static void
athena_places_sidebar_class_init (AthenaPlacesSidebarClass *class)
{
	G_OBJECT_CLASS (class)->dispose = athena_places_sidebar_dispose;

	GTK_WIDGET_CLASS (class)->style_set = athena_places_sidebar_style_set;
	GTK_WIDGET_CLASS (class)->focus = athena_places_sidebar_focus;
}

static void
athena_places_sidebar_set_parent_window (AthenaPlacesSidebar *sidebar,
					   AthenaWindow *window)
{
	AthenaWindowSlot *slot;
	...
	sidebar->bookmarks = athena_bookmark_list_new ();
	sidebar->uri = athena_window_slot_get_current_uri (slot);
	...
	g_signal_connect_swapped (athena_preferences, "changed::" ATHENA_PREFERENCES_ALWAYS_USE_BROWSER,
...
static void
athena_places_sidebar_style_set (GtkWidget *widget,
				   GtkStyle  *previous_style)
{
	AthenaPlacesSidebar *sidebar;

	sidebar = ATHENA_PLACES_SIDEBAR (widget);

	update_places (sidebar);
}

GtkWidget *
athena_places_sidebar_new (AthenaWindow *window)
{
	AthenaPlacesSidebar *sidebar;
	
	sidebar = g_object_new (athena_places_sidebar_get_type (), NULL);
	athena_places_sidebar_set_parent_window (sidebar, window);
...
static gboolean
athena_shortcuts_model_row_draggable (GtkTreeDragSource *drag_source,
					GtkTreePath       *path)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
static void
_athena_shortcuts_model_class_init (AthenaShortcutsModelClass *klass)
{

}

static void
_athena_shortcuts_model_init (AthenaShortcutsModel *model)
{
	model->sidebar = NULL;
}

static void
_athena_shortcuts_model_drag_source_init (GtkTreeDragSourceIface *iface)
{
	iface->row_draggable = athena_shortcuts_model_row_draggable;
}

static GtkListStore *
athena_shortcuts_model_new (AthenaPlacesSidebar *sidebar)
{
	AthenaShortcutsModel *model;
	...
	model = g_object_new (ATHENA_TYPE_SHORTCUTS_MODEL, NULL);
