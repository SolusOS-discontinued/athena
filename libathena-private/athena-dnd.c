...
/* athena-dnd.c - Common Drag & drop handling code shared by the icon container
   and the list view.
...
#include "athena-program-choosing.h"
...
void
athena_drag_finalize (AthenaDragInfo *drag_info)
{
	gtk_target_list_unref (drag_info->target_list);
	athena_drag_destroy_selection_list (drag_info->selection_list);

	g_free (drag_info);
}


/* Functions to deal with AthenaDragSelectionItems.  */

AthenaDragSelectionItem *
athena_drag_selection_item_new (void)
{
	return g_new0 (AthenaDragSelectionItem, 1);
}

static void
drag_selection_item_destroy (AthenaDragSelectionItem *item)
{
	g_free (item->uri);
	g_free (item);
}
...
void
athena_drag_destroy_selection_list (GList *list)
{
	GList *p;
...
char **
athena_drag_uri_array_from_selection_list (const GList *selection_list)
{
	GList *uri_list;
	char **uris;

	uri_list = athena_drag_uri_list_from_selection_list (selection_list);
	uris = athena_drag_uri_array_from_list (uri_list);
...
GList *
athena_drag_uri_list_from_selection_list (const GList *selection_list)
{
	AthenaDragSelectionItem *selection_item;
	GList *uri_list;
	const GList *l;

	uri_list = NULL;
	for (l = selection_list; l != NULL; l = l->next) {
		selection_item = (AthenaDragSelectionItem *) l->data;
...
char **
athena_drag_uri_array_from_list (const GList *uri_list)
{
	const GList *l;
	char **uris;
	int i;
...
GList *
athena_drag_uri_list_from_array (const char **uris)
{
	GList *uri_list;
	int i;
...
GList *
athena_drag_build_selection_list (GtkSelectionData *data)
{
	GList *result;
	const guchar *p, *oldp;
	int size;
...
static gboolean
athena_drag_file_local_internal (const char *target_uri_string,
...
gboolean
athena_drag_uris_local (const char *target_uri,
			  const GList *source_uri_list)
{
	/* must have at least one item */
	g_assert (source_uri_list);
	
	return athena_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
athena_drag_items_local (const char *target_uri_string,
			   const GList *selection_list)
{
	/* must have at least one item */
	g_assert (selection_list);

	return athena_drag_file_local_internal (target_uri_string,
						  ((AthenaDragSelectionItem *)selection_list->data)->uri);
}

gboolean
athena_drag_items_in_trash (const GList *selection_list)
{
	 ...
	 * and currently dropped items can only be from the same container
	 */
	return eel_uri_is_trash (((AthenaDragSelectionItem *)selection_list->data)->uri);
}

gboolean
athena_drag_items_on_desktop (const GList *selection_list)
{
	char *uri;
	 ...
	 * and currently dropped items can only be from the same container
	 */
	uri = ((AthenaDragSelectionItem *)selection_list->data)->uri;
	if (eel_uri_is_desktop (uri)) {
		return TRUE;
	}
	
	desktop = athena_get_desktop_location ();
...
static gboolean
check_same_fs (AthenaFile *file1,
	       AthenaFile *file2)
{
	char *id1, *id2;
	gboolean result;
	...
	if (file1 != NULL && file2 != NULL) {
		id1 = athena_file_get_filesystem_id (file1);
		id2 = athena_file_get_filesystem_id (file2);
...
{
	AthenaFile *naut_file;
	gboolean ret;

	/* if there's no a cached AthenaFile, it returns NULL */
	naut_file = athena_file_get_existing (file);
	if (naut_file == NULL) {
		return FALSE;
	}
	
	ret = athena_file_can_delete (naut_file);
	athena_file_unref (naut_file);
...
gboolean
athena_drag_drag_data_get (GtkWidget *widget,
...
gboolean
athena_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
...
void
athena_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
athena_drag_autoscroll_start (AthenaDragInfo *drag_info,
				GtkWidget        *widget,
				GSourceFunc       callback,
				gpointer          user_data)
{
	if (athena_drag_autoscroll_in_scroll_region (widget)) {
...
void
athena_drag_autoscroll_stop (AthenaDragInfo *drag_info)
{
	if (drag_info->auto_scroll_timeout_id != 0) {
		g_source_remove (drag_info->auto_scroll_timeout_id);
		drag_info->auto_scroll_timeout_id = 0;
...
gboolean
athena_drag_selection_includes_special_link (GList *selection_list)
{
	GList *node;
	char *uri;

	for (node = selection_list; node != NULL; node = node->next) {
		uri = ((AthenaDragSelectionItem *) node->data)->uri;
