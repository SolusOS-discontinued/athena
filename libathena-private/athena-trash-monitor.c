...
/* 
   athena-trash-monitor.c: Athena trash state watcher.
...
struct AthenaTrashMonitorDetails {
	gboolean empty;
	GIcon *icon;
...
static AthenaTrashMonitor *athena_trash_monitor = NULL;

G_DEFINE_TYPE(AthenaTrashMonitor, athena_trash_monitor, G_TYPE_OBJECT)

static void
athena_trash_monitor_finalize (GObject *object)
{
	AthenaTrashMonitor *trash_monitor;

	trash_monitor = ATHENA_TRASH_MONITOR (object);
	...
	G_OBJECT_CLASS (athena_trash_monitor_parent_class)->finalize (object);
}

static void
athena_trash_monitor_class_init (AthenaTrashMonitorClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = athena_trash_monitor_finalize;

	signals[TRASH_STATE_CHANGED] = g_signal_new
		("trash_state_changed",
		 G_TYPE_FROM_CLASS (object_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AthenaTrashMonitorClass, trash_state_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__BOOLEAN,
		 G_TYPE_NONE, 1,
		 G_TYPE_BOOLEAN);

	g_type_class_add_private (object_class, sizeof(AthenaTrashMonitorDetails));
}
		...
		gpointer user_data)
{
	...
	trash_monitor = ATHENA_TRASH_MONITOR (user_data);
		...
		if (icon) {
			...
			if (G_IS_THEMED_ICON (icon)) {
				...
				for (i = 0; names[i] != NULL; i++) {
					if (strcmp (names[i], ATHENA_ICON_TRASH_FULL) == 0) {
						empty = FALSE;
						break;
					}
				}
...
static void
schedule_update_info (AthenaTrashMonitor *trash_monitor)
{
	GFile *location;
...
static void
athena_trash_monitor_init (AthenaTrashMonitor *trash_monitor)
{
	GFile *location;

	trash_monitor->details = G_TYPE_INSTANCE_GET_PRIVATE (trash_monitor,
							      ATHENA_TYPE_TRASH_MONITOR,
							      AthenaTrashMonitorDetails);

	trash_monitor->details->empty = TRUE;
	trash_monitor->details->icon = g_themed_icon_new (ATHENA_ICON_TRASH);
...
{
	g_object_unref (athena_trash_monitor);
}

AthenaTrashMonitor *
athena_trash_monitor_get (void)
{
	if (athena_trash_monitor == NULL) {
		/* not running yet, start it up */

		athena_trash_monitor = ATHENA_TRASH_MONITOR
			(g_object_new (ATHENA_TYPE_TRASH_MONITOR, NULL));
		eel_debug_call_at_shutdown (unref_trash_monitor);
	}

	return athena_trash_monitor;
}

gboolean
athena_trash_monitor_is_empty (void)
{
...
GIcon *
athena_trash_monitor_get_icon (void)
{
	AthenaTrashMonitor *monitor;
...
void
athena_trash_monitor_add_new_trash_directories (void)
{
