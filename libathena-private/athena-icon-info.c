...
#include "athena-default-file-icon.h"
#include <gtk/gtk.h>
#include <gio/gio.h>

struct _AthenaIconInfo
{
	GObject parent;
...
G_DEFINE_TYPE (AthenaIconInfo,
	       athena_icon_info,
	       G_TYPE_OBJECT);

static void
athena_icon_info_init (AthenaIconInfo *icon)
{
...
gboolean
athena_icon_info_is_fallback (AthenaIconInfo  *icon)
{
...
static void
athena_icon_info_finalize (GObject *object)
{
        AthenaIconInfo *icon;

        icon = ATHENA_ICON_INFO (object);
	...
	g_free (icon->icon_name);
        ...
        G_OBJECT_CLASS (athena_icon_info_parent_class)->finalize (object);
}

static void
athena_icon_info_class_init (AthenaIconInfoClass *icon_info_class)
{
        GObjectClass *gobject_class;

        gobject_class = (GObjectClass *) icon_info_class;

        gobject_class->finalize = athena_icon_info_finalize;

}

AthenaIconInfo *
athena_icon_info_new_for_pixbuf (GdkPixbuf *pixbuf)
{
	AthenaIconInfo *icon;

	icon = g_object_new (ATHENA_TYPE_ICON_INFO, NULL);
...
athena_icon_info_new_for_icon_info (GtkIconInfo *icon_info)
{
	AthenaIconInfo *icon;
	GdkPoint *points;
	gint n_points;
...
void
athena_icon_info_clear_caches (void)
{
	if (loadable_icon_cache) {
...
AthenaIconInfo *
athena_icon_info_lookup (GIcon *icon,
			   int size)
{
	...
	if (G_IS_LOADABLE_ICON (icon)) {
		...
		icon_info = athena_icon_info_new_for_pixbuf (pixbuf);
		...
		if (gtkicon_info == NULL) {
			return athena_icon_info_new_for_pixbuf (NULL);
		}
		...
		icon_info = athena_icon_info_new_for_icon_info (gtkicon_info);
...
AthenaIconInfo *
athena_icon_info_lookup_from_name (const char *name,
				     int size)
{
	GIcon *icon;
	AthenaIconInfo *info;
	...
	info = athena_icon_info_lookup (icon, size);
	g_object_unref (icon);
	return info;
}

AthenaIconInfo *
athena_icon_info_lookup_from_path (const char *path,
				     int size)
{
	GFile *icon_file;
	GIcon *icon;
...
GdkPixbuf *
athena_icon_info_get_pixbuf_nodefault (AthenaIconInfo  *icon)
{
	GdkPixbuf *res;
...
GdkPixbuf *
athena_icon_info_get_pixbuf (AthenaIconInfo *icon)
{
	GdkPixbuf *res;

	res = athena_icon_info_get_pixbuf_nodefault (icon);
	if (res == NULL) {
		res = gdk_pixbuf_new_from_data (athena_default_file_icon,
						GDK_COLORSPACE_RGB,
						TRUE,
						8,
						athena_default_file_icon_width,
						athena_default_file_icon_height,
						athena_default_file_icon_width * 4, /* stride */
...
GdkPixbuf *
athena_icon_info_get_pixbuf_nodefault_at_size (AthenaIconInfo  *icon,
						 gsize              forced_size)
{
	...
	pixbuf = athena_icon_info_get_pixbuf_nodefault (icon);
...
GdkPixbuf *
athena_icon_info_get_pixbuf_at_size (AthenaIconInfo  *icon,
				       gsize              forced_size)
{
	GdkPixbuf *pixbuf, *scaled_pixbuf;
	int w, h, s;
	...
	pixbuf = athena_icon_info_get_pixbuf (icon);
...
gboolean
athena_icon_info_get_embedded_rect (AthenaIconInfo  *icon,
...
gboolean
athena_icon_info_get_attach_points (AthenaIconInfo  *icon,
				      GdkPoint         **points,
				      gint              *n_points)
{
...
const char *
athena_icon_info_get_display_name   (AthenaIconInfo  *icon)
{
	return icon->display_name;
}

const char *
athena_icon_info_get_used_name (AthenaIconInfo  *icon)
{
 ...
 * 
 * Return value: icon size between ATHENA_ICON_SIZE_SMALLEST and
 * ATHENA_ICON_SIZE_LARGEST, inclusive.
 */
guint
athena_get_icon_size_for_zoom_level (AthenaZoomLevel zoom_level)
{
	switch (zoom_level) {
	case ATHENA_ZOOM_LEVEL_SMALLEST:
		return ATHENA_ICON_SIZE_SMALLEST;
	case ATHENA_ZOOM_LEVEL_SMALLER:
		return ATHENA_ICON_SIZE_SMALLER;
	case ATHENA_ZOOM_LEVEL_SMALL:
		return ATHENA_ICON_SIZE_SMALL;
	case ATHENA_ZOOM_LEVEL_STANDARD:
		return ATHENA_ICON_SIZE_STANDARD;
	case ATHENA_ZOOM_LEVEL_LARGE:
		return ATHENA_ICON_SIZE_LARGE;
	case ATHENA_ZOOM_LEVEL_LARGER:
		return ATHENA_ICON_SIZE_LARGER;
	case ATHENA_ZOOM_LEVEL_LARGEST:
		return ATHENA_ICON_SIZE_LARGEST;
	}
	g_return_val_if_reached (ATHENA_ICON_SIZE_STANDARD);
}

gint
athena_get_icon_size_for_stock_size (GtkIconSize size)
{
  gint w, h;
...
guint
athena_icon_get_emblem_size_for_icon_size (guint size)
{
	if (size >= 96)
		return 48;
	if (size >= 64)
...
gboolean
athena_icon_theme_can_render (GThemedIcon *icon)
{
	GtkIconTheme *icon_theme;
...
GIcon *
athena_user_special_directory_get_gicon (GUserDirectory directory)
{

	#define ICON_CASE(x) \
		case G_USER_DIRECTORY_ ## x:\
			return g_themed_icon_new (ATHENA_ICON_FOLDER_ ## x);
