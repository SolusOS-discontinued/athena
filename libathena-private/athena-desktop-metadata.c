...
void
athena_desktop_set_metadata_string (AthenaFile *file,
                                      const gchar *name,
                                      const gchar *key,
                                      const gchar *string)
{
	...
	if (athena_desktop_update_metadata_from_keyfile (file, name)) {
		athena_file_changed (file);
	}	
}

#define STRV_TERMINATOR "@x-athena-desktop-metadata-term@"

void
athena_desktop_set_metadata_stringv (AthenaFile *file,
                                       const char *name,
                                       const char *key,
...
gboolean
athena_desktop_update_metadata_from_keyfile (AthenaFile *file,
					       const gchar *name)
{
	gchar **keys, **values;
	...
	res = athena_file_update_metadata_from_info (file, info);
