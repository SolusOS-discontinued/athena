...
/* athena-file-utilities.c - implementation of file manipulation routines.
...
#define ATHENA_USER_DIRECTORY_NAME "athena"
#define DEFAULT_ATHENA_DIRECTORY_MODE (0755)
...
static GFile *athena_find_file_insensitive_next (GFile *parent, const gchar *name);

char *
athena_compute_title_for_location (GFile *location)
{
	AthenaFile *file;
	char *title;
	...
	title = NULL;
	if (location) {
		file = athena_file_get (location);
		title = athena_file_get_description (file);
		if (title == NULL) {
			title = athena_file_get_display_name (file);
		}
 ...
 * 
 * Get the path for the directory containing athena settings.
 *
 * Return value: the directory path.
 **/
char *
athena_get_user_directory (void)
{
	char *user_directory = NULL;

	user_directory = g_build_filename (g_get_user_config_dir (),
					   ATHENA_USER_DIRECTORY_NAME,
					   NULL);
	
	if (!g_file_test (user_directory, G_FILE_TEST_EXISTS)) {
		g_mkdir (user_directory, DEFAULT_ATHENA_DIRECTORY_MODE);
		/* FIXME bugzilla.gnome.org 41286: 
		 * How should we handle the case where this mkdir fails? 
		 * Note that athena_application_startup will refuse to launch if this 
		 * directory doesn't get created, so that case is OK. But the directory 
		 * could be deleted after Athena was launched, and perhaps
...
/**
 * athena_get_accel_map_file:
 * 
 * Get the path for the filename containing athena accelerator map.
 * The filename need not exist.
 *
 * Return value: the filename path, or NULL if the home directory could not be found
 **/
char *
athena_get_accel_map_file (void)
{
	...
	if (override) {
		return g_build_filename (override, "accels/athena", NULL);
	} else {
		return g_build_filename (g_get_home_dir (), ".gnome2/accels/athena", NULL);
	}
}
...
static void
xdg_dir_changed (AthenaFile *file,
		 XdgDirEntry *dir)
{
	...
	if (cached_xdg_dirs != NULL) {
		for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
			if (cached_xdg_dirs[i].file != NULL) {
				athena_file_monitor_remove (cached_xdg_dirs[i].file,
							      &cached_xdg_dirs[i]);
				g_signal_handlers_disconnect_by_func (cached_xdg_dirs[i].file,
								      G_CALLBACK (xdg_dir_changed),
								      &cached_xdg_dirs[i]);
				athena_file_unref (cached_xdg_dirs[i].file);
			}
...
update_xdg_dir_cache (void)
{
	...
	for (i = 0 ; cached_xdg_dirs[i].type != NULL; i++) {
		...
		if (strcmp (cached_xdg_dirs[i].path, g_get_home_dir ()) != 0) {
			...
			cached_xdg_dirs[i].file = athena_file_get_by_uri (uri);
			athena_file_monitor_add (cached_xdg_dirs[i].file,
						   &cached_xdg_dirs[i],
...
char *
athena_get_xdg_dir (const char *type)
{
	int i;
...
{
	if (g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_DESKTOP_IS_HOME_DIR)) {
		return g_strdup (g_get_home_dir());
	} else {
		return athena_get_xdg_dir ("DESKTOP");
	}
}
 ...
 **/
char *
athena_get_desktop_directory (void)
{
...
GFile *
athena_get_desktop_location (void)
{
	char *desktop_directory;
	GFile *res;
 ...
 **/
char *
athena_get_desktop_directory_uri (void)
{
	char *desktop_path;
	char *desktop_uri;
...
char *
athena_get_desktop_directory_uri_no_create (void)
{
	char *desktop_path;
	char *desktop_uri;
...
gboolean
athena_should_use_templates_directory (void)
{
	char *dir;
	gboolean res;
	
	dir = athena_get_xdg_dir ("TEMPLATES");
...
char *
athena_get_templates_directory (void)
{
	return athena_get_xdg_dir ("TEMPLATES");
}

void
athena_create_templates_directory (void)
{
	char *dir;

	dir = athena_get_templates_directory ();
	if (!g_file_test (dir, G_FILE_TEST_EXISTS)) {
		g_mkdir (dir, DEFAULT_ATHENA_DIRECTORY_MODE);
	}
	g_free (dir);
}

char *
athena_get_templates_directory_uri (void)
{
	char *directory, *uri;

	directory = athena_get_templates_directory ();
...
char *
athena_get_searches_directory (void)
{
	char *user_dir;
	char *searches_dir;

	user_dir = athena_get_user_directory ();
	searches_dir = g_build_filename (user_dir, "searches", NULL);
	g_free (user_dir);
	
	if (!g_file_test (searches_dir, G_FILE_TEST_EXISTS))
		g_mkdir (searches_dir, DEFAULT_ATHENA_DIRECTORY_MODE);
...
gboolean
athena_is_home_directory_file (GFile *dir,
				 const char *filename)
{
	char *dirname;
...
gboolean
athena_is_home_directory (GFile *dir)
{
...
gboolean
athena_is_root_directory (GFile *dir)
{
...
gboolean
athena_is_desktop_directory_file (GFile *dir,
				    const char *file)
{
...
gboolean
athena_is_desktop_directory (GFile *dir)
{
 ...
 **/
char *
athena_get_gmc_desktop_directory (void)
{
	return g_build_filename (g_get_home_dir (), LEGACY_DESKTOP_DIRECTORY_NAME, NULL);
}

char *
athena_get_data_file_path (const char *partial_path)
{
	char *path;
	char *user_directory;
	...
	path = g_build_filename (ATHENA_DATADIR, partial_path, NULL);
	if (g_file_test (path, G_FILE_TEST_EXISTS)) {
		return path;
	}
	g_free (path);
...
char *
athena_ensure_unique_file_name (const char *directory_uri,
...
char *
athena_unique_temporary_file_name (void)
{
	const char *prefix = "/tmp/athena-temp-file";
	char *file_name;
	int fd;
...
GFile *
athena_find_existing_uri_in_hierarchy (GFile *location)
{
	GFileInfo *info;
	GFile *tmp;
...
athena_find_file_insensitive (GFile *parent, const gchar *name)
{
	...
	for (i = 0; (component = split_path[i]) != NULL; i++) {
		if (!(next = athena_find_file_insensitive_next (file,
		                                                  component))) {
			/* File does not exist */
			g_object_unref (file);
			file = NULL;
...
static GFile *
athena_find_file_insensitive_next (GFile *parent, const gchar *name)
{
...
gboolean
athena_is_file_roller_installed (void)
{
 ...
 * (e.g. whenever Athena is doing file operations).
 *
 * Returns: an integer cookie, which must be passed to
 *    athena_uninhibit_power_manager() to resume
 *    normal power management.
 */
int
athena_inhibit_power_manager (const char *message)
{
...
/**
 * athena_uninhibit_power_manager:
 * @cookie: the cookie value returned by athena_inhibit_power_manager()
 *
 ...
 */
void
athena_uninhibit_power_manager (gint cookie)
{
	GDBusConnection *connection;
	GVariant *result;
	GError *error = NULL;
...
gboolean
athena_is_in_system_dir (GFile *file)
{
...
GHashTable *
athena_trashed_files_get_original_directories (GList *files,
						 GList **unhandled_files)
{
	GHashTable *directories;
	AthenaFile *file, *original_file, *original_dir;
	GList *l, *m;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		original_file = athena_file_get_trash_original_file (file);

		original_dir = NULL;
		if (original_file != NULL) {
			original_dir = athena_file_get_parent (original_file);
		}
...
locations_from_file_list (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		file = ATHENA_FILE (l->data);
		ret = g_list_prepend (ret, athena_file_get_location (file));
	}
...
void
athena_restore_files_from_trash (GList *files,
				   GtkWindow *parent_window)
{
	AthenaFile *file, *original_dir;
	...
	original_dirs_hash = athena_trashed_files_get_original_directories (files, &unhandled_files);
	...
	if (original_dirs_hash != NULL) {
		...
		for (l = original_dirs; l != NULL; l = l->next) {
			original_dir = ATHENA_FILE (l->data);
			original_dir_location = athena_file_get_location (original_dir);

			files = g_hash_table_lookup (original_dirs_hash, original_dir);
			locations = locations_from_file_list (files);

			athena_file_operations_move
				(locations, NULL, 
	...
	athena_file_list_unref (unhandled_files);
}
...
void
athena_get_x_content_types_for_mount_async (GMount *mount,
					      AthenaMountGetContent callback,
					      ...
					      gpointer user_data)
{
	...
	cached = g_object_get_data (G_OBJECT (mount), "athena-content-type-cache");
	if (cached != NULL) {
		if (callback) {
...
char **
athena_get_cached_x_content_types_for_mount (GMount *mount)
{
	char **cached;
...
void
athena_self_check_file_utilities (void)
{
}
