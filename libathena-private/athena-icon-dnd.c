...
/* athena-icon-dnd.c - Drag & drop handling for the icon container widget.
...
#include <libathena-private/athena-desktop-background.h>
...
static const GtkTargetEntry drop_types [] = {
	...
	{ ATHENA_ICON_DND_URI_LIST_TYPE, 0, ATHENA_ICON_DND_URI_LIST },
	{ ATHENA_ICON_DND_XDNDDIRECTSAVE_TYPE, 0, ATHENA_ICON_DND_XDNDDIRECTSAVE }, /* XDS Protocol Type */
	{ ATHENA_ICON_DND_RAW_TYPE, 0, ATHENA_ICON_DND_RAW },
	/* Must be last: */
	{ ATHENA_ICON_DND_ROOTWINDOW_DROP_TYPE,  0, ATHENA_ICON_DND_ROOTWINDOW_DROP }
};
...
static char * athena_icon_container_find_drop_target (AthenaIconContainer *container,
							GdkDragContext *context,
							int x, int y, gboolean *icon_hit,
							gboolean rewrite_desktop);

static EelCanvasItem *
create_selection_shadow (AthenaIconContainer *container,
			 GList *list)
{
...
typedef struct {
	...
	AthenaDragEachSelectedItemDataGet iteratee;
...
static gboolean
icon_get_data_binder (AthenaIcon *icon, gpointer data)
{
	...
	g_assert (ATHENA_IS_ICON_CONTAINER (context->iterator_context));

	container = ATHENA_ICON_CONTAINER (context->iterator_context);
	...
	uri = athena_icon_container_get_icon_uri (container, icon);
	if (uri == NULL) {
...
/* Iterate over each selected icon in a AthenaIconContainer,
 * calling each_function on each.
 */
static void
athena_icon_container_each_selected_icon (AthenaIconContainer *container,
	gboolean (*each_function) (AthenaIcon *, gpointer), gpointer data)
{
	GList *p;
	AthenaIcon *icon;
...
/* Adaptor function used with athena_icon_container_each_selected_icon
 * to help iterate over all selected items, passing uris, x, y, w and h
 * values to the iteratee
 */
static void
each_icon_get_data_binder (AthenaDragEachSelectedItemDataGet iteratee, 
	gpointer iterator_context, gpointer data)
{
	IconGetDataBinderContext context;
	AthenaIconContainer *container;

	g_assert (ATHENA_IS_ICON_CONTAINER (iterator_context));
	container = ATHENA_ICON_CONTAINER (iterator_context);

	context.iterator_context = iterator_context;
	context.iteratee = iteratee;
	context.iteratee_data = data;
	athena_icon_container_each_selected_icon (container, icon_get_data_binder, &context);
}
			...
			gpointer data)
{
	...
	g_assert (ATHENA_IS_ICON_CONTAINER (widget));
	g_return_if_fail (context != NULL);

	/* Call common function from athena-drag that set's up
	 * the selection data in the right format. Pass it means to
	 * iterate all the selected icons.
	 */
	athena_drag_drag_data_get (widget, context, selection_data,
...
static void
athena_icon_container_position_shadow (AthenaIconContainer *container,
					 int x, int y)
{
...
static void
athena_icon_container_dropped_icon_feedback (GtkWidget *widget,
					       GtkSelectionData *data,
					       int x, int y)
{
	AthenaIconContainer *container;
	AthenaIconDndInfo *dnd_info;

	container = ATHENA_ICON_CONTAINER (widget);
	dnd_info = container->details->dnd_info;
	
	/* Delete old selection list. */
	athena_drag_destroy_selection_list (dnd_info->drag_info.selection_list);
	...
	dnd_info->drag_info.selection_list = athena_drag_build_selection_list (data);
	dnd_info->shadow = create_selection_shadow (container, dnd_info->drag_info.selection_list);
	athena_icon_container_position_shadow (container, x, y);
}
...
get_direct_save_filename (GdkDragContext *context)
{
	...
	if (!gdk_property_get (gdk_drag_context_get_source_window (context), gdk_atom_intern (ATHENA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
static void
set_direct_save_uri (GtkWidget *widget, GdkDragContext *context, AthenaDragInfo *drag_info, int x, int y)
{
	GFile *base, *child;
	...
	drag_info->data_type = ATHENA_ICON_DND_XDNDDIRECTSAVE;
	
	uri = NULL;
	
	filename = get_direct_save_filename (context);
	drop_target = athena_icon_container_find_drop_target (ATHENA_ICON_CONTAINER (widget), 
	...
	if (filename != NULL && drop_target != NULL) {
		...
		gdk_property_change (gdk_drag_context_get_source_window (context),
				     gdk_atom_intern (ATHENA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE),
...
get_data_on_first_target_we_support (GtkWidget *widget, GdkDragContext *context, guint32 time, int x, int y)
{
	...
	if (drop_types_list == NULL) {
		...
		gtk_target_list_add_text_targets (drop_types_list, ATHENA_ICON_DND_TEXT);
	}
	if (drop_types_list_root == NULL) {
		drop_types_list_root = gtk_target_list_new (drop_types,
							    G_N_ELEMENTS (drop_types));
		gtk_target_list_add_text_targets (drop_types_list_root, ATHENA_ICON_DND_TEXT);
	}
	
	if (athena_icon_container_get_is_desktop (ATHENA_ICON_CONTAINER (widget))) {
	...
	if (target != GDK_NONE) {
		...
		drag_info = &(ATHENA_ICON_CONTAINER (widget)->details->dnd_info->drag_info);

		found = gtk_target_list_find (list, target, &info);
		g_assert (found);

		/* Don't get_data for destructive ops */
		if ((info == ATHENA_ICON_DND_ROOTWINDOW_DROP ||
		     info == ATHENA_ICON_DND_XDNDDIRECTSAVE) &&
			...
			if (info == ATHENA_ICON_DND_XDNDDIRECTSAVE) {
...
static void
athena_icon_container_ensure_drag_data (AthenaIconContainer *container,
					  GdkDragContext *context,
					  guint32 time)
{
	AthenaIconDndInfo *dnd_info;
...
static AthenaIcon *
athena_icon_container_item_at (AthenaIconContainer *container,
				 int x, int y)
{
	GList *p;
	int size;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		eel_canvas_w2c (EEL_CANVAS (container),
				...
				&canvas_point.x1,
				&canvas_point.y1);
		if (athena_icon_canvas_item_hit_test_rectangle (icon->item, canvas_point)) {
			return icon;
		}
	}
...
static char *
get_container_uri (AthenaIconContainer *container)
{
	char *uri;
...
static gboolean
athena_icon_container_selection_items_local (AthenaIconContainer *container,
					       GList *items)
{
	...
	if (eel_uri_is_desktop (container_uri_string)) {
		result = athena_drag_items_on_desktop (items);
	} else {
		result = athena_drag_items_local (container_uri_string, items);
	}
...
/* handle dropped url */
static void
receive_dropped_netscape_url (AthenaIconContainer *container, const char *encoded_url, GdkDragContext *context, int x, int y)
{
	char *drop_target;
	...
	drop_target = athena_icon_container_find_drop_target (container, context, x, y, NULL, TRUE);
...
static void
receive_dropped_uri_list (AthenaIconContainer *container, const char *uri_list, GdkDragContext *context, int x, int y)
{	
	char *drop_target;
...
/* handle dropped text */
static void
receive_dropped_text (AthenaIconContainer *container, const char *text, GdkDragContext *context, int x, int y)
{	
	char *drop_target;
...
static void
receive_dropped_raw (AthenaIconContainer *container, const char *raw_data, int length, const char *direct_save_uri, GdkDragContext *context, int x, int y)
{
	char *drop_target;
...
auto_scroll_timeout_callback (gpointer data)
{
	...
	g_assert (ATHENA_IS_ICON_CONTAINER (data));
	...
	athena_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
	...
	if (!athena_icon_container_scroll (container, (int)x_scroll_delta, (int)y_scroll_delta)) {
...
static void
set_up_auto_scroll_if_needed (AthenaIconContainer *container)
{
	athena_drag_autoscroll_start (&container->details->dnd_info->drag_info,
...
static void
stop_auto_scroll (AthenaIconContainer *container)
{
	athena_drag_autoscroll_stop (&container->details->dnd_info->drag_info);
}

static void
handle_local_move (AthenaIconContainer *container,
		   double world_x, double world_y)
{
	GList *moved_icons, *p;
	AthenaDragSelectionItem *item;
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
		...
		icon = athena_icon_container_get_icon_by_uri
			(container, item->uri);
		...
		if (icon == NULL) {
			...
			file = athena_file_get_by_uri (item->uri);
			...
			athena_file_set_time_metadata (file,
							 ATHENA_METADATA_KEY_ICON_POSITION_TIMESTAMP, now);

			athena_icon_container_add (container, ATHENA_ICON_CONTAINER_ICON_DATA (file));
		...
		if (item->got_icon_position) {
			athena_icon_container_move_icon
				(container, icon,
...
static void
handle_nonlocal_move (AthenaIconContainer *container,
		      ...
		      gboolean icon_hit)
{
	...
	for (p = container->details->dnd_info->drag_info.selection_list; p != NULL; p = p->next) {
		...
		source_uris = g_list_prepend (source_uris, ((AthenaDragSelectionItem *)p->data)->uri);
	}
	...
	if (!icon_hit) {
		...
		for (index = 0, p = container->details->dnd_info->drag_info.selection_list;
			p != NULL; index++, p = p->next) {
		    	item_x = ((AthenaDragSelectionItem *)p->data)->icon_x;
			if (is_rtl)
				item_x = -item_x - ((AthenaDragSelectionItem *)p->data)->icon_width;
		     	g_array_index (source_item_locations, GdkPoint, index).x = item_x;
		     	g_array_index (source_item_locations, GdkPoint, index).y =
				((AthenaDragSelectionItem *)p->data)->icon_y;
		}
	}
	...
	if (eel_uri_is_desktop (target_uri)) {
		target_uri = athena_get_desktop_directory_uri ();
...
static char *
athena_icon_container_find_drop_target (AthenaIconContainer *container,
					  GdkDragContext *context,
					  int x, int y,
					  gboolean *icon_hit,
					  gboolean rewrite_desktop)
{
	AthenaIcon *drop_target_icon;
	double world_x, world_y;
	AthenaFile *file;
	char *icon_uri;
	char *container_uri;
	...
	drop_target_icon = athena_icon_container_item_at (container, world_x, world_y);
	if (drop_target_icon != NULL) {
		icon_uri = athena_icon_container_get_icon_uri (container, drop_target_icon);
		if (icon_uri != NULL) {
			file = athena_file_get_by_uri (icon_uri);

			if (!athena_drag_can_accept_info (file,
	...
	if (drop_target_icon == NULL) {
		...
		if (rewrite_desktop &&
			...
			container_uri = athena_get_desktop_directory_uri ();
		}
	...
	return athena_icon_container_get_icon_drop_target_uri (container, drop_target_icon);
}
...
selection_is_image_file (GList *selection_list)
{
	...
	AthenaDragSelectionItem *selected_item;
	gboolean result;
	GFile *location;
	GFileInfo *info;
...
static void
athena_icon_container_receive_dropped_icons (AthenaIconContainer *container,
					       GdkDragContext *context,
					       int x, int y)
{
	...
	if (real_action == GDK_ACTION_ASK) {
		...
		if (athena_drag_selection_includes_special_link (container->details->dnd_info->drag_info.selection_list)) {
			...
			if (eel_uri_is_desktop (container_uri) &&
				...
				action |= ATHENA_DND_ACTION_SET_AS_BACKGROUND;
			}

			g_free (container_uri);
		}
		real_action = athena_drag_drop_action_ask
			(GTK_WIDGET (container), action);
	}
	
	if (real_action == (GdkDragAction) ATHENA_DND_ACTION_SET_AS_BACKGROUND) {
		AthenaDesktopBackground *background;

		background = athena_desktop_background_new (container);
		selected_item = container->details->dnd_info->drag_info.selection_list->data;

		athena_desktop_background_receive_dropped_background_image (background,
									      selected_item->uri);
	...
	if (real_action > 0) {
		...
		drop_target = athena_icon_container_find_drop_target (container, 
		...
		if (!icon_hit && real_action == GDK_ACTION_MOVE) {
			 ...
			 */
			local_move_only = athena_icon_container_selection_items_local
	...
	g_free (drop_target);
	athena_drag_destroy_selection_list (container->details->dnd_info->drag_info.selection_list);
	container->details->dnd_info->drag_info.selection_list = NULL;
}

static void
athena_icon_container_get_drop_action (AthenaIconContainer *container,
					 GdkDragContext *context,
					 int x, int y,
					 int *action)
{
	...
	*action = 0;
	...
	case ATHENA_ICON_DND_GNOME_ICON_LIST:
		...
		athena_drag_default_drop_action_for_icons (context, drop_target, 
							     container->details->dnd_info->drag_info.selection_list, 
							     action);
		g_free (drop_target);
		break;
	case ATHENA_ICON_DND_URI_LIST:
		drop_target = athena_icon_container_find_drop_target (container,
									context, x, y, &icon_hit, FALSE);
		*action = athena_drag_default_drop_action_for_uri_list (context, drop_target);
...
static void
set_drop_target (AthenaIconContainer *container,
		 AthenaIcon *icon)
{
	AthenaIcon *old_icon;
	...
	athena_icon_container_update_icon (container, old_icon);
	athena_icon_container_update_icon (container, icon);
}

static void
athena_icon_dnd_update_drop_target (AthenaIconContainer *container,
				      GdkDragContext *context,
				      int x, int y)
{
	AthenaIcon *icon;
	...
	/* Find the item we hit with our drop, if any. */
	icon = athena_icon_container_item_at (container, world_x, world_y);
...
static void
athena_icon_container_free_drag_data (AthenaIconContainer *container)
{
	AthenaIconDndInfo *dnd_info;
		     ...
		     gpointer data)
{
	...
	dnd_info = ATHENA_ICON_CONTAINER (widget)->details->dnd_info;
	...
	set_drop_target (ATHENA_ICON_CONTAINER (widget), NULL);
	stop_auto_scroll (ATHENA_ICON_CONTAINER (widget));
	athena_icon_container_free_drag_data(ATHENA_ICON_CONTAINER (widget));
}
		     ...
		     gpointer        data)
{
	...
	start_y = container->details->dnd_info->drag_info.start_y +
        ...
        surface = athena_icon_canvas_item_get_drag_surface (container->details->drag_icon->item);
...
void
athena_icon_dnd_begin_drag (AthenaIconContainer *container,
		      ...
		      guint32 time)
{
	...
	athena_icon_container_ensure_drag_data (ATHENA_ICON_CONTAINER (widget), context, time);
	athena_icon_container_position_shadow (ATHENA_ICON_CONTAINER (widget), x, y);
	athena_icon_dnd_update_drop_target (ATHENA_ICON_CONTAINER (widget), context, x, y);
	set_up_auto_scroll_if_needed (ATHENA_ICON_CONTAINER (widget));
	/* Find out what the drop actions are based on our drag selection and
	 * the drop target.
	 */
	action = 0;
	athena_icon_container_get_drop_action (ATHENA_ICON_CONTAINER (widget), context, x, y,
						 &action);
	if (action != 0) {
...
void
athena_icon_dnd_end_drag (AthenaIconContainer *container)
{
	AthenaIconDndInfo *dnd_info;
			     ...
			     gpointer user_data)
{
	...
	switch (info) {
	case ATHENA_ICON_DND_GNOME_ICON_LIST:
		athena_icon_container_dropped_icon_feedback (widget, data, x, y);
		break;
	case ATHENA_ICON_DND_URI_LIST:
	case ATHENA_ICON_DND_TEXT:
	case ATHENA_ICON_DND_XDNDDIRECTSAVE:
		...
		case ATHENA_ICON_DND_XDNDDIRECTSAVE:
		{
			...
			if (selection_format == 8 &&
				...
				gtk_drag_get_data (widget, context,
				                  gdk_atom_intern (ATHENA_ICON_DND_RAW_TYPE,
				                                   FALSE),
				                  time);
				return;
...
void
athena_icon_dnd_init (AthenaIconContainer *container)
{
	...
	container->details->dnd_info = g_new0 (AthenaIconDndInfo, 1);
	athena_drag_init (&container->details->dnd_info->drag_info,
	...
	gtk_target_list_add_text_targets (targets, ATHENA_ICON_DND_TEXT);
...
void
athena_icon_dnd_fini (AthenaIconContainer *container)
{
	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));
	...
	if (container->details->dnd_info != NULL) {
		...
		athena_drag_finalize (&container->details->dnd_info->drag_info);
