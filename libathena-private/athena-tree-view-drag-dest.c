...
/* athena-tree-view-drag-dest.c: Handles drag and drop for treeviews which 
 *                                 contain a hierarchy of files
 */

#include <config.h>

#include "athena-tree-view-drag-dest.h"
...
struct _AthenaTreeViewDragDestDetails {
...
G_DEFINE_TYPE (AthenaTreeViewDragDest, athena_tree_view_drag_dest,
	       G_TYPE_OBJECT);
...
static void
remove_scroll_timeout (AthenaTreeViewDragDest *dest)
{
...
static void
remove_expand_timeout (AthenaTreeViewDragDest *dest)
{
...
static void
set_widget_highlight (AthenaTreeViewDragDest *dest, gboolean highlight)
{
...
static void
set_drag_dest_row (AthenaTreeViewDragDest *dest,
		   GtkTreePath *path)
{
	if (path) {
...
static void
clear_drag_dest_row (AthenaTreeViewDragDest *dest)
{
...
static gboolean
get_drag_data (AthenaTreeViewDragDest *dest,
	       ...
	       guint32 time)
{
	...
	if (target == gdk_atom_intern (ATHENA_ICON_DND_XDNDDIRECTSAVE_TYPE, FALSE) &&
	    !dest->details->drop_occurred) {
		dest->details->drag_type = ATHENA_ICON_DND_XDNDDIRECTSAVE;
...
static void
free_drag_data (AthenaTreeViewDragDest *dest)
{
	...
	if (dest->details->drag_list) {
		athena_drag_destroy_selection_list (dest->details->drag_list);
...
static char *
get_root_uri (AthenaTreeViewDragDest *dest)
{
	char *uri;
...
static AthenaFile *
file_for_path (AthenaTreeViewDragDest *dest, GtkTreePath *path)
{
	AthenaFile *file;
	char *uri;
...
static GtkTreePath *
get_drop_path (AthenaTreeViewDragDest *dest,
	       GtkTreePath *path)
{
	AthenaFile *file;
	GtkTreePath *ret;
	...
	while (file == NULL /* dummy row */ ||
	       !athena_drag_can_accept_info (file,
...
static char *
get_drop_target_uri_for_path (AthenaTreeViewDragDest *dest,
			      GtkTreePath *path)
{
	AthenaFile *file;
	char *target;
	...
	target = athena_file_get_drop_target_uri (file);
	athena_file_unref (file);
...
static guint
get_drop_action (AthenaTreeViewDragDest *dest, 
		 GdkDragContext *context,
		 GtkTreePath *path)
{
	char *drop_target;
	...
	if (!dest->details->have_drag_data ||
	    (dest->details->drag_type == ATHENA_ICON_DND_GNOME_ICON_LIST &&
	     dest->details->drag_list == NULL)) {
		return 0;
	}
		      ...
		      gpointer data)
{
	...
	dest = ATHENA_TREE_VIEW_DRAG_DEST (data);
...
static char *
get_drop_target_uri_at_pos (AthenaTreeViewDragDest *dest, int x, int y)
{
	char *drop_target;
	GtkTreePath *path;
	GtkTreePath *drop_path;
...
static void
receive_uris (AthenaTreeViewDragDest *dest,
	      GdkDragContext *context,
	      GList *source_uris,
	      int x, int y)
{
	...
	if (real_action == GDK_ACTION_ASK) {
		if (athena_drag_selection_includes_special_link (dest->details->drag_list)) {
	...
	if (dest->details->drag_type == ATHENA_ICON_DND_URI_LIST) {
		action = GDK_ACTION_COPY;
	}

	if (real_action > 0) {
		if (!athena_drag_uris_local (drop_target, source_uris)
...
static void
receive_dropped_icons (AthenaTreeViewDragDest *dest,
		       GdkDragContext *context,
		       int x, int y)
{
	GList *source_uris;
	...
	for (l = dest->details->drag_list; l != NULL; l = l->next) {
		source_uris = g_list_prepend (source_uris,
					      ((AthenaDragSelectionItem *)l->data)->uri);
	}
...
static void
receive_dropped_uri_list (AthenaTreeViewDragDest *dest,
			  GdkDragContext *context,
			  int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_text (AthenaTreeViewDragDest *dest,
		      GdkDragContext *context,
		      int x, int y)
{
	char *drop_target;
...
static void
receive_dropped_raw (AthenaTreeViewDragDest *dest,
...
static void
receive_dropped_netscape_url (AthenaTreeViewDragDest *dest,
			      GdkDragContext *context,
			      int x, int y)
{
	char *drop_target;
...
static gboolean
receive_xds (AthenaTreeViewDragDest *dest,
	     GtkWidget *widget,
	     guint32 time,
	     GdkDragContext *context,
	     int x, int y)
			     ...
			     gpointer data)
{
	...
	if (!dest->details->have_drag_data) {
		...
		if (info == ATHENA_ICON_DND_GNOME_ICON_LIST) {
			dest->details->drag_list = 
				athena_drag_build_selection_list (selection_data);
		}
	}
...
static gboolean
set_direct_save_uri (AthenaTreeViewDragDest *dest,
		     GdkDragContext *context,
		     int x, int y)
{
	GFile *base, *child;
		       ...
		       GObject *object)
{
	...
	dest = ATHENA_TREE_VIEW_DRAG_DEST (user_data);
...
static void
athena_tree_view_drag_dest_dispose (GObject *object)
{
	AthenaTreeViewDragDest *dest;
	
	dest = ATHENA_TREE_VIEW_DRAG_DEST (object);
	...
	G_OBJECT_CLASS (athena_tree_view_drag_dest_parent_class)->dispose (object);
}

static void
athena_tree_view_drag_dest_finalize (GObject *object)
{
	AthenaTreeViewDragDest *dest;
	...
	G_OBJECT_CLASS (athena_tree_view_drag_dest_parent_class)->finalize (object);
}

static void
athena_tree_view_drag_dest_init (AthenaTreeViewDragDest *dest)
{
	dest->details = G_TYPE_INSTANCE_GET_PRIVATE (dest, ATHENA_TYPE_TREE_VIEW_DRAG_DEST,
						     AthenaTreeViewDragDestDetails);
}

static void
athena_tree_view_drag_dest_class_init (AthenaTreeViewDragDestClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	
	gobject_class->dispose = athena_tree_view_drag_dest_dispose;
	gobject_class->finalize = athena_tree_view_drag_dest_finalize;

	g_type_class_add_private (class, sizeof (AthenaTreeViewDragDestDetails));

	signals[GET_ROOT_URI] = 
		g_signal_new ("get_root_uri",
			      G_TYPE_FROM_CLASS (class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (AthenaTreeViewDragDestClass,
					       get_root_uri),
			      NULL, NULL,
...
AthenaTreeViewDragDest *
athena_tree_view_drag_dest_new (GtkTreeView *tree_view)
{
	AthenaTreeViewDragDest *dest;
	GtkTargetList *targets;
	
	dest = g_object_new (ATHENA_TYPE_TREE_VIEW_DRAG_DEST, NULL);
