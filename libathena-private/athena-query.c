...
#include "athena-query.h"
#include <eel/eel-glib-extensions.h>
#include <glib/gi18n.h>
#include <libathena-private/athena-file-utilities.h>

struct AthenaQueryDetails {
	char *text;
	char *location_uri;
	GList *mime_types;
};

static void  athena_query_class_init       (AthenaQueryClass *class);
static void  athena_query_init             (AthenaQuery      *query);

G_DEFINE_TYPE (AthenaQuery, athena_query, G_TYPE_OBJECT);
...
finalize (GObject *object)
{
	...
	query = ATHENA_QUERY (object);
	g_free (query->details->text);

	G_OBJECT_CLASS (athena_query_parent_class)->finalize (object);
}

static void
athena_query_class_init (AthenaQueryClass *class)
{
	...
	g_type_class_add_private (class, sizeof (AthenaQueryDetails));
}

static void
athena_query_init (AthenaQuery *query)
{
	query->details = G_TYPE_INSTANCE_GET_PRIVATE (query, ATHENA_TYPE_QUERY,
						      AthenaQueryDetails);
}

AthenaQuery *
athena_query_new (void)
{
	return g_object_new (ATHENA_TYPE_QUERY,  NULL);
}


char *
athena_query_get_text (AthenaQuery *query)
{
	return g_strdup (query->details->text);
}

void 
athena_query_set_text (AthenaQuery *query, const char *text)
{
	g_free (query->details->text);
...
char *
athena_query_get_location (AthenaQuery *query)
{
	return g_strdup (query->details->location_uri);
}
	
void
athena_query_set_location (AthenaQuery *query, const char *uri)
{
...
GList *
athena_query_get_mime_types (AthenaQuery *query)
{
	return eel_g_str_list_copy (query->details->mime_types);
}

void
athena_query_set_mime_types (AthenaQuery *query, GList *mime_types)
{
...
void
athena_query_add_mime_type (AthenaQuery *query, const char *mime_type)
{
...
char *
athena_query_to_readable_string (AthenaQuery *query)
{
	 ...
	 GError **err)
{
	...
	if (info->in_text) {
		athena_query_set_text (info->query, t);
	} else if (info->in_location) {
		uri = decode_home_uri (t);
		athena_query_set_location (info->query, uri);
		g_free (uri);
	} else if (info->in_mimetypes && info->in_mimetype) {
		athena_query_add_mime_type (info->query, t);
	}
...
static AthenaQuery *
athena_query_parse_xml (char *xml, gsize xml_len)
{
...
athena_query_load (char *file)
{
	...
	query = athena_query_parse_xml (xml, xml_len);
	g_free (xml);
...
static char *
athena_query_to_xml (AthenaQuery *query)
{
	GString *xml;
	char *text;
	char *uri;
...
gboolean
athena_query_save (AthenaQuery *query, char *file)
{
	char *xml;
	GError *err = NULL;
	gboolean res;
