...
#include <config.h>
#include "athena-file-queue.h"

#include <glib.h>

struct AthenaFileQueue {
	GList *head;
	GList *tail;
...
AthenaFileQueue *
athena_file_queue_new (void)
{
	AthenaFileQueue *queue;
	
	queue = g_new0 (AthenaFileQueue, 1);
...
void
athena_file_queue_destroy (AthenaFileQueue *queue)
{
	g_hash_table_destroy (queue->item_to_link_map);
	athena_file_list_free (queue->head);
	g_free (queue);
}

void
athena_file_queue_enqueue (AthenaFileQueue *queue,
			     AthenaFile      *file)
{
...
AthenaFile *
athena_file_queue_dequeue (AthenaFileQueue *queue)
{
	AthenaFile *file;

	file = athena_file_queue_head (queue);
	athena_file_queue_remove (queue, file);
...
void
athena_file_queue_remove (AthenaFileQueue *queue,
			    AthenaFile *file)
{
	GList *link;
...
AthenaFile *
athena_file_queue_head (AthenaFileQueue *queue)
{
	if (queue->head == NULL) {
		return NULL;
	}

	return ATHENA_FILE (queue->head->data);
}

gboolean
athena_file_queue_is_empty (AthenaFileQueue *queue)
{
