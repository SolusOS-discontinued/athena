...
struct _AthenaProgressInfoClass
{
...
G_DEFINE_TYPE (AthenaProgressInfo, athena_progress_info, G_TYPE_OBJECT)

static void
athena_progress_info_finalize (GObject *object)
{
	AthenaProgressInfo *info;
	
	info = ATHENA_PROGRESS_INFO (object);

	g_free (info->status);
	g_free (info->details);
	g_object_unref (info->cancellable);
	
	if (G_OBJECT_CLASS (athena_progress_info_parent_class)->finalize) {
		(*G_OBJECT_CLASS (athena_progress_info_parent_class)->finalize) (object);
	}
}

static void
athena_progress_info_dispose (GObject *object)
{
	AthenaProgressInfo *info;
...
static void
athena_progress_info_class_init (AthenaProgressInfoClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	
	gobject_class->finalize = athena_progress_info_finalize;
	gobject_class->dispose = athena_progress_info_dispose;
...
static void
athena_progress_info_init (AthenaProgressInfo *info)
{
	...
	athena_progress_info_manager_add_new_info (manager, info);
	g_object_unref (manager);
}

AthenaProgressInfo *
athena_progress_info_new (void)
{
	AthenaProgressInfo *info;
	
	info = g_object_new (ATHENA_TYPE_PROGRESS_INFO, NULL);
	
	return info;
}

char *
athena_progress_info_get_status (AthenaProgressInfo *info)
{
	char *res;
...
char *
athena_progress_info_get_details (AthenaProgressInfo *info)
{
	char *res;
...
double
athena_progress_info_get_progress (AthenaProgressInfo *info)
{
	double res;
...
void
athena_progress_info_cancel (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
GCancellable *
athena_progress_info_get_cancellable (AthenaProgressInfo *info)
{
	GCancellable *c;
...
gboolean
athena_progress_info_get_is_started (AthenaProgressInfo *info)
{
	gboolean res;
...
gboolean
athena_progress_info_get_is_finished (AthenaProgressInfo *info)
{
	gboolean res;
...
gboolean
athena_progress_info_get_is_paused (AthenaProgressInfo *info)
{
	gboolean res;
...
static void
queue_idle (AthenaProgressInfo *info, gboolean now)
{
...
void
athena_progress_info_pause (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_resume (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_start (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_finish (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_take_status (AthenaProgressInfo *info,
				    char *status)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_set_status (AthenaProgressInfo *info,
				   const char *status)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_take_details (AthenaProgressInfo *info,
				     char           *details)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_set_details (AthenaProgressInfo *info,
				    const char           *details)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_pulse_progress (AthenaProgressInfo *info)
{
	G_LOCK (progress_info);
...
void
athena_progress_info_set_progress (AthenaProgressInfo *info,
				     double                current,
				     double                total)
{
