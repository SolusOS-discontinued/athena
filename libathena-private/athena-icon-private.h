...
#ifndef ATHENA_ICON_CONTAINER_PRIVATE_H
#define ATHENA_ICON_CONTAINER_PRIVATE_H

#include <eel/eel-glib-extensions.h>
#include <libathena-private/athena-icon-canvas-item.h>
...
/* Private AthenaIconContainer members. */
...
struct AthenaIconContainerDetails {
	/* List of icons. */
	GList *icons;
	GList *new_icons;
	GHashTable *icon_set;
	...
	AthenaIcon *keyboard_icon_to_reveal;
	...
	AthenaIcon *double_click_icon[2]; /* Both clicks in a double click need to be on the same icon */
	guint double_click_button[2];

	AthenaIcon *range_selection_base_icon;
	...
	int font_size_table[ATHENA_ZOOM_LEVEL_LARGEST + 1];
	...
	/* Layout mode */
	AthenaIconLayoutMode layout_mode;

	/* Label position */
	AthenaIconLabelPosition label_position;
...
AthenaIcon *athena_icon_container_get_icon_by_uri             (AthenaIconContainer *container,
								   const char            *uri);
void          athena_icon_container_move_icon                   (AthenaIconContainer *container,
								   AthenaIcon          *icon,
								   int                    x,
								   int                    y,
								   double                 scale,
...
void          athena_icon_container_select_list_unselect_others (AthenaIconContainer *container,
								   GList                 *icons);
char *        athena_icon_container_get_icon_uri                (AthenaIconContainer *container,
								   AthenaIcon          *icon);
char *        athena_icon_container_get_icon_drop_target_uri    (AthenaIconContainer *container,
								   AthenaIcon          *icon);
void          athena_icon_container_update_icon                 (AthenaIconContainer *container,
								   AthenaIcon          *icon);
gboolean      athena_icon_container_has_stored_icon_positions   (AthenaIconContainer *container);
gboolean      athena_icon_container_scroll                      (AthenaIconContainer *container,
								   int                    delta_x,
								   int                    delta_y);
void          athena_icon_container_update_scroll_region        (AthenaIconContainer *container);

#endif /* ATHENA_ICON_CONTAINER_PRIVATE_H */
