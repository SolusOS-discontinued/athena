...
/*
 *  athena-mime-application-chooser.c: an mime-application chooser
 *
...
#include <config.h>
#include "athena-mime-application-chooser.h"
...
struct _AthenaMimeApplicationChooserDetails {
	GList *files;
	char *uri;
...
G_DEFINE_TYPE (AthenaMimeApplicationChooser, athena_mime_application_chooser, GTK_TYPE_BOX);

static void
add_clicked_cb (GtkButton *button,
		gpointer user_data)
{
	AthenaMimeApplicationChooser *chooser = user_data;
	GAppInfo *info;
	gchar *message;
	GError *error = NULL;
...
reset_clicked_cb (GtkButton *button,
                  gpointer   user_data)
{
	AthenaMimeApplicationChooser *chooser;
	
	chooser = ATHENA_MIME_APPLICATION_CHOOSER (user_data);
...
static gchar *
get_extension_from_file (AthenaFile *nfile)
{
	char *name;
	char *extension;

	name = athena_file_get_name (nfile);
...
static void
athena_mime_application_chooser_apply_labels (AthenaMimeApplicationChooser *chooser)
{
	...
	if (chooser->details->files != NULL) {
		...
		if (g_content_type_is_unknown (chooser->details->content_type)) {
			extension = get_extension_from_file (ATHENA_FILE (chooser->details->files->data));
...
static void
athena_mime_application_chooser_build_ui (AthenaMimeApplicationChooser *chooser)
{
...
static void
athena_mime_application_chooser_init (AthenaMimeApplicationChooser *chooser)
{
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE (chooser, ATHENA_TYPE_MIME_APPLICATION_CHOOSER,
							AthenaMimeApplicationChooserDetails);
...
static void
athena_mime_application_chooser_constructed (GObject *object)
{
	AthenaMimeApplicationChooser *chooser = ATHENA_MIME_APPLICATION_CHOOSER (object);

	if (G_OBJECT_CLASS (athena_mime_application_chooser_parent_class)->constructed != NULL)
		G_OBJECT_CLASS (athena_mime_application_chooser_parent_class)->constructed (object);

	athena_mime_application_chooser_build_ui (chooser);
	athena_mime_application_chooser_apply_labels (chooser);
}

static void
athena_mime_application_chooser_finalize (GObject *object)
{
	AthenaMimeApplicationChooser *chooser;

	chooser = ATHENA_MIME_APPLICATION_CHOOSER (object);

	g_free (chooser->details->uri);
	g_free (chooser->details->content_type);

	G_OBJECT_CLASS (athena_mime_application_chooser_parent_class)->finalize (object);
}

static void
athena_mime_application_chooser_get_property (GObject *object,
						guint property_id,
						GValue *value,
						GParamSpec *pspec)
{
...
static void
athena_mime_application_chooser_set_property (GObject *object,
						guint property_id,
						const GValue *value,
						GParamSpec *pspec)
{
...
static void
athena_mime_application_chooser_class_init (AthenaMimeApplicationChooserClass *class)
{
	GObjectClass *gobject_class;

	gobject_class = G_OBJECT_CLASS (class);
	gobject_class->set_property = athena_mime_application_chooser_set_property;
	gobject_class->get_property = athena_mime_application_chooser_get_property;
	gobject_class->finalize = athena_mime_application_chooser_finalize;
	gobject_class->constructed = athena_mime_application_chooser_constructed;
	...
	g_type_class_add_private (class, sizeof (AthenaMimeApplicationChooserDetails));
}

GtkWidget *
athena_mime_application_chooser_new (const char *uri,
				       GList *files,
				       const char *mime_type)
{
	GtkWidget *chooser;

	chooser = g_object_new (ATHENA_TYPE_MIME_APPLICATION_CHOOSER,
				"uri", uri,
				"files", files,
