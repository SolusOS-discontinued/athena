...
static void 
athena_entry_user_changed_callback (AthenaEntry *entry)
{		
...
void
athena_undo_set_up_athena_entry_for_undo (AthenaEntry *entry)
{
	EditableUndoObjectData *data;
	
	if (!ATHENA_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_connect (entry, "user_changed",
			  G_CALLBACK (athena_entry_user_changed_callback),
			  NULL);
}

void
athena_undo_tear_down_athena_entry_for_undo (AthenaEntry *entry)
{
	if (!ATHENA_IS_ENTRY (entry) ) {
		return;
	}
	...
	g_signal_handlers_disconnect_by_func
		(entry, G_CALLBACK (athena_entry_user_changed_callback), NULL);
...
void
athena_undo_set_up_editable_for_undo (GtkEditable *editable)
{
	EditableUndoObjectData *data;
...
void
athena_undo_tear_down_editable_for_undo (GtkEditable *editable)
{
	if (!GTK_IS_EDITABLE (editable) ) {
		return;
	}
