/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   athena-directory-async.c: Athena directory model state machine.
...
struct DirectoryLoadState {
	...
	AthenaFile *load_directory_file;
	int load_file_count;
};

struct MimeListState {
	AthenaDirectory *directory;
	AthenaFile *mime_list_file;
	GCancellable *cancellable;
	GFileEnumerator *enumerator;
...
typedef struct {
	AthenaFile *file; /* Which file, NULL means all. */
	union {
		AthenaDirectoryCallback directory;
		AthenaFileCallback file;
	} callback;
...
typedef gboolean (* FileCheck) (AthenaFile *);
...
static void     deep_count_load                               (DeepCountState         *state,
							       GFile                  *location);
static gboolean request_is_satisfied                          (AthenaDirectory      *directory,
							       AthenaFile           *file,
							       Request                 request);
static void     cancel_loading_attributes                     (AthenaDirectory      *directory,
							       AthenaFileAttributes  file_attributes);
static void     add_all_files_to_work_queue                   (AthenaDirectory      *directory);
static void     link_info_done                                (AthenaDirectory      *directory,
							       AthenaFile           *file,
							       const char             *uri,
							       const char             *name,
							       GIcon                  *icon,
							       ...
							       gboolean                is_foreign);
static void     move_file_to_low_priority_queue               (AthenaDirectory      *directory,
							       AthenaFile           *file);
static void     move_file_to_extension_queue                  (AthenaDirectory      *directory,
							       AthenaFile           *file);
static void     athena_directory_invalidate_file_attributes (AthenaDirectory      *directory,
							       AthenaFileAttributes  file_attributes);

void
athena_set_kde_trash_name (const char *trash_dir)
{
 ...
 * Move to athena-glib-extensions?
 */
...
#if 0
static void
athena_directory_verify_request_counts (AthenaDirectory *directory)
{
	GList *l;
 ...
 */
static gboolean
async_job_start (AthenaDirectory *directory,
		 const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
/* End a job. */
static void
async_job_end (AthenaDirectory *directory,
	       const char *job)
{
#ifdef DEBUG_ASYNC_JOBS
	char *key;
...
async_job_wake_up (void)
{
	...
	while (async_job_count < MAX_ASYNC_JOBS) {
		...
		athena_directory_async_state_changed
			(ATHENA_DIRECTORY (value));
	}
	already_waking_up = FALSE;
}

static void
directory_count_cancel (AthenaDirectory *directory)
{
...
static void
deep_count_cancel (AthenaDirectory *directory)
{
	if (directory->details->deep_count_in_progress != NULL) {
		g_assert (ATHENA_IS_FILE (directory->details->deep_count_file));
		
		g_cancellable_cancel (directory->details->deep_count_in_progress->cancellable);

		directory->details->deep_count_file->details->deep_counts_status = ATHENA_REQUEST_NOT_STARTED;
...
static void
mime_list_cancel (AthenaDirectory *directory)
{
...
static void
top_left_cancel (AthenaDirectory *directory)
{
...
static void
link_info_cancel (AthenaDirectory *directory)
{
...
static void
thumbnail_cancel (AthenaDirectory *directory)
{
...
static void
mount_cancel (AthenaDirectory *directory)
{
...
static void
file_info_cancel (AthenaDirectory *directory)
{
...
static void
new_files_cancel (AthenaDirectory *directory)
{
	GList *l;
	NewFilesState *state;
...
static GList *
find_monitor (AthenaDirectory *directory,
	      AthenaFile *file,
	      gconstpointer client)
{
	Monitor monitor;
...
static void
remove_monitor_link (AthenaDirectory *directory,
		     GList *link)
{
	Monitor *monitor;
...
static void
remove_monitor (AthenaDirectory *directory,
		AthenaFile *file,
		gconstpointer client)
{
...
Request
athena_directory_set_up_request (AthenaFileAttributes file_attributes)
{
	Request request;

	request = 0;
	
	if ((file_attributes & ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DIRECTORY_COUNT);
	}
	
	if ((file_attributes & ATHENA_FILE_ATTRIBUTE_DEEP_COUNTS) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_DEEP_COUNT);
	}

	if ((file_attributes & ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_MIME_LIST);
	}
	if ((file_attributes & ATHENA_FILE_ATTRIBUTE_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}
	
	if (file_attributes & ATHENA_FILE_ATTRIBUTE_LINK_INFO) {
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
		REQUEST_SET_TYPE (request, REQUEST_LINK_INFO);
	}
	
	if (file_attributes & ATHENA_FILE_ATTRIBUTE_TOP_LEFT_TEXT) {
		REQUEST_SET_TYPE (request, REQUEST_TOP_LEFT_TEXT);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}
	
	if (file_attributes & ATHENA_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT) {
		REQUEST_SET_TYPE (request, REQUEST_LARGE_TOP_LEFT_TEXT);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if ((file_attributes & ATHENA_FILE_ATTRIBUTE_EXTENSION_INFO) != 0) {
		REQUEST_SET_TYPE (request, REQUEST_EXTENSION_INFO);
	}

	if (file_attributes & ATHENA_FILE_ATTRIBUTE_THUMBNAIL) {
		REQUEST_SET_TYPE (request, REQUEST_THUMBNAIL);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & ATHENA_FILE_ATTRIBUTE_MOUNT) {
		REQUEST_SET_TYPE (request, REQUEST_MOUNT);
		REQUEST_SET_TYPE (request, REQUEST_FILE_INFO);
	}

	if (file_attributes & ATHENA_FILE_ATTRIBUTE_FILESYSTEM_INFO) {
...
static void
mime_db_changed_callback (GObject *ignore, AthenaDirectory *dir)
{
	AthenaFileAttributes attrs;

	g_assert (dir != NULL);
	g_assert (dir->details != NULL);

	attrs = ATHENA_FILE_ATTRIBUTE_INFO |
		ATHENA_FILE_ATTRIBUTE_LINK_INFO |
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;

	athena_directory_force_reload_internal (dir, attrs);
}

void
athena_directory_monitor_add_internal (AthenaDirectory *directory,
					 AthenaFile *file,
					 gconstpointer client,
					 ...
					 gpointer callback_data)
{
	...
	monitor->client = client;
	monitor->request = athena_directory_set_up_request (file_attributes);
	 ...
	 * athena almost always shows the whole directory anyway, and
	 * it allows us to avoid one file monitor per file in a directory.
	 */
	if (directory->details->monitor == NULL) {
		directory->details->monitor = athena_monitor_directory (directory->details->location);
	}
...
static void
set_file_unconfirmed (AthenaFile *file, gboolean unconfirmed)
{
	AthenaDirectory *directory;
...
{
	show_hidden_files = g_settings_get_boolean (athena_preferences, ATHENA_PREFERENCES_SHOW_HIDDEN_FILES);
}

static gboolean
should_skip_file (AthenaDirectory *directory, GFileInfo *info)
{
...
dequeue_pending_idle_callback (gpointer callback_data)
{
	...
	directory = ATHENA_DIRECTORY (callback_data);
	...
	/* Build a list of AthenaFile objects. */
	for (node = pending_file_info; node != NULL; node = node->next) {
		...
		file = athena_directory_find_file_by_name (directory, name);
		if (file != NULL) {
			...
			if (!file->details->is_added) {
				 ...
				 * This can happen if someone called athena_file_get_by_uri()
			...
			} else if (athena_file_update_info (file, file_info)) {
				/* File changed, notify about the change. */
				athena_file_ref (file);
				changed_files = g_list_prepend (changed_files, file);
			}
		} else {
			/* new file, create a athena file object and add it to the list */
			file = athena_file_new_from_info (directory, file_info);
	...
	athena_directory_emit_change_signals (directory, changed_files);
	athena_file_list_free (changed_files);
	athena_directory_emit_files_added (directory, added_files);
	athena_file_list_free (added_files);

	if (directory->details->directory_loaded &&
	    !directory->details->directory_loaded_sent_notification) {
		/* Send the done_loading signal. */
		athena_directory_emit_done_loading (directory);
...
void
athena_directory_schedule_dequeue_pending (AthenaDirectory *directory)
{
...
static void
directory_load_one (AthenaDirectory *directory,
		    GFileInfo *info)
{
	if (info == NULL) {
		return;
	...
	athena_directory_schedule_dequeue_pending (directory);
}

static void
directory_load_cancel (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
file_list_cancel (AthenaDirectory *directory)
{
...
static void
directory_load_done (AthenaDirectory *directory,
		     GError *error)
{
	GList *node;
	...
	if (error != NULL) {
		...
		for (node = directory->details->file_list;
			...
			set_file_unconfirmed (ATHENA_FILE (node->data), FALSE);
		}

		athena_directory_emit_load_error (directory, error);
	}
...
void
athena_directory_monitor_remove_internal (AthenaDirectory *directory,
					    AthenaFile *file,
					    gconstpointer client)
{
	g_assert (ATHENA_IS_DIRECTORY (directory));
	g_assert (file == NULL || ATHENA_IS_FILE (file));
...
FileMonitors *
athena_directory_remove_file_monitors (AthenaDirectory *directory,
					 AthenaFile *file)
{
...
void
athena_directory_add_file_monitors (AthenaDirectory *directory,
				      AthenaFile *file,
...
static void
ready_callback_call (AthenaDirectory *directory,
...
void
athena_directory_call_when_ready_internal (AthenaDirectory *directory,
					     AthenaFile *file,
					     AthenaFileAttributes file_attributes,
					     ...
					     gpointer callback_data)
{
	...
	g_assert (directory == NULL || ATHENA_IS_DIRECTORY (directory));
	g_assert (file == NULL || ATHENA_IS_FILE (file));
	...
	callback.request = athena_directory_set_up_request (file_attributes);
...
gboolean      
athena_directory_check_if_ready_internal (AthenaDirectory *directory,
					    AthenaFile *file,
					    AthenaFileAttributes file_attributes)
{
	Request request;
...
static void
remove_callback_link_keep_data (AthenaDirectory *directory,
				GList *link)
{
	ReadyCallback *callback;
...
static void
remove_callback_link (AthenaDirectory *directory,
		      GList *link)
{
	ReadyCallback *callback;
...
void
athena_directory_cancel_callback_internal (AthenaDirectory *directory,
					     AthenaFile *file,
					     AthenaDirectoryCallback directory_callback,
					     AthenaFileCallback file_callback,
					     gpointer callback_data)
		    ...
		    gpointer user_data)
{
	...
	directory = athena_directory_ref (state->directory);
...
void
athena_directory_get_info_for_new_files (AthenaDirectory *directory,
					   GList *location_list)
{
	NewFilesState *state;
	GFile *location;
...
void
athena_async_destroying_file (AthenaFile *file)
{
	AthenaDirectory *directory;
	gboolean changed;
	GList *node, *next;
...
static gboolean
lacks_directory_count (AthenaFile *file)
{
	return !file->details->directory_count_is_up_to_date
		&& athena_file_should_show_directory_item_count (file);
}

static gboolean
should_get_directory_count_now (AthenaFile *file)
{
...
static gboolean
lacks_top_left (AthenaFile *file)
{
	return file->details->file_info_is_up_to_date &&
		!file->details->top_left_text_is_up_to_date 
		&& athena_file_should_get_top_left_text (file);
}

static gboolean
lacks_large_top_left (AthenaFile *file)
{
	return file->details->file_info_is_up_to_date &&
		(!file->details->top_left_text_is_up_to_date ||
...
static gboolean
lacks_filesystem_info (AthenaFile *file)
{
	return !file->details->filesystem_info_is_up_to_date;
}

static gboolean
lacks_deep_count (AthenaFile *file)
{
	return file->details->deep_counts_status != ATHENA_REQUEST_DONE;
}

static gboolean
lacks_mime_list (AthenaFile *file)
{
	return !file->details->mime_list_is_up_to_date;
}

static gboolean
should_get_mime_list (AthenaFile *file)
{
...
static gboolean
lacks_link_info (AthenaFile *file)
{
...
static gboolean
lacks_extension_info (AthenaFile *file)
{
	return file->details->pending_info_providers != NULL;
}

static gboolean
lacks_thumbnail (AthenaFile *file)
{
	return athena_file_should_show_thumbnail (file) &&
		file->details->thumbnail_path != NULL &&
		!file->details->thumbnail_is_up_to_date;
}

static gboolean
lacks_mount (AthenaFile *file)
{
	return (!file->details->mount_is_up_to_date &&
		(
		 ...
		 (file->details->type == G_FILE_TYPE_DIRECTORY &&
		  athena_file_is_self_owned (file)) ||
...
static gboolean
has_problem (AthenaDirectory *directory, AthenaFile *file, FileCheck problem)
{
	GList *node;
...
static gboolean
request_is_satisfied (AthenaDirectory *directory,
		      AthenaFile *file,
		      Request request)
{
...
static void
schedule_call_ready_callbacks (AthenaDirectory *directory)
{
 ...
 * before then */
static gboolean
call_ready_callbacks (AthenaDirectory *directory)
{
	gboolean found_any;
	GList *node, *next;
	ReadyCallback *callback;
...
gboolean
athena_directory_has_active_request_for_file (AthenaDirectory *directory,
						AthenaFile *file)
{
	GList *node;
	ReadyCallback *callback;
...
gboolean
athena_directory_is_anyone_monitoring_file_list (AthenaDirectory *directory)
{
...
gboolean
athena_directory_is_file_list_monitored (AthenaDirectory *directory) 
{
	return directory->details->file_list_monitored;
}

static void
mark_all_files_unconfirmed (AthenaDirectory *directory)
{
	GList *node;
	AthenaFile *file;
...
static void
read_dot_hidden_file (AthenaDirectory *directory)
{
	gsize file_size;
	char *file_contents;
	GFile *child;
...
directory_load_state_free (DirectoryLoadState *state)
{
	...
	athena_file_unref (state->load_directory_file);
	g_object_unref (state->cancellable);
	g_free (state);
}
...
static void
start_monitoring_file_list (AthenaDirectory *directory)
{
	DirectoryLoadState *state;
	...
	if (!directory->details->file_list_monitored) {
		...
		athena_file_list_ref (directory->details->file_list);
	}
	...
	if (kde_trash_dir_name != NULL && athena_directory_is_desktop_directory (directory)) {
		char *fn;
...
void
athena_directory_stop_monitoring_file_list (AthenaDirectory *directory)
{
	if (!directory->details->file_list_monitored) {
		g_assert (directory->details->directory_load_in_progress == NULL);
		return;
	...
	athena_file_list_unref (directory->details->file_list);
	directory->details->directory_loaded = FALSE;
}

static void
file_list_start_or_stop (AthenaDirectory *directory)
{
	if (athena_directory_is_anyone_monitoring_file_list (directory)) {
		start_monitoring_file_list (directory);
	} else {
		athena_directory_stop_monitoring_file_list (directory);
	}
}

void
athena_file_invalidate_count_and_mime_list (AthenaFile *file)
{
	AthenaFileAttributes attributes;
	
	attributes = ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
	
	athena_file_invalidate_attributes (file, attributes);
}
 ...
 */
void
athena_directory_invalidate_count_and_mime_list (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
athena_directory_invalidate_file_attributes (AthenaDirectory      *directory,
					       AthenaFileAttributes  file_attributes)
{
	GList *node;
	...
	for (node = directory->details->file_list; node != NULL; node = node->next) {
		athena_file_invalidate_attributes_internal (ATHENA_FILE (node->data),
							      file_attributes);
	}

	if (directory->details->as_file != NULL) {
		athena_file_invalidate_attributes_internal (directory->details->as_file,
							      file_attributes);
	}
}

void
athena_directory_force_reload_internal (AthenaDirectory     *directory,
					  AthenaFileAttributes file_attributes)
{
	/* invalidate attributes that are getting reloaded for all files */
	athena_directory_invalidate_file_attributes (directory, file_attributes);
...
static void
directory_count_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
count_children_done (AthenaDirectory *directory,
		     AthenaFile *count_file,
		     gboolean succeeded,
		     int count)
{
	g_assert (ATHENA_IS_FILE (count_file));
	 ...
	 */
	athena_file_changed (count_file);
...
directory_count_state_free (DirectoryCountState *state)
{
	...
	g_object_unref (state->cancellable);
	athena_directory_unref (state->directory);
	g_free (state);
}
...
static void
directory_count_start (AthenaDirectory *directory,
		       AthenaFile *file,
		       gboolean *doing_io)
{
	DirectoryCountState *state;
	...
	*doing_io = TRUE;
	...
	state->directory = athena_directory_ref (directory);
	state->cancellable = g_cancellable_new ();
...
deep_count_next_dir (DeepCountState *state)
{
	...
	if (state->deep_count_subdirectories != NULL) {
		...
		file->details->deep_counts_status = ATHENA_REQUEST_DONE;
		directory->details->deep_count_file = NULL;
		directory->details->deep_count_in_progress = NULL;
	...
	athena_file_updated_deep_count_in_progress (file);
...
static void
deep_count_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
deep_count_start (AthenaDirectory *directory,
		  AthenaFile *file,
		  gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	/* Start counting. */
	file->details->deep_counts_status = ATHENA_REQUEST_IN_PROGRESS;
...
static void
mime_list_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
mime_list_start (AthenaDirectory *directory,
		 AthenaFile *file,
		 gboolean *doing_io)
{
	MimeListState *state;
...
static void
top_left_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
			...
			gpointer callback_data)
{
	...
	AthenaDirectory *directory;
	AthenaFileDetails *file_details;
	gsize file_size;
	char *file_contents;
	...
	if (g_file_load_partial_contents_finish (G_FILE (source_object),
						 ...
						 NULL, NULL)) {
		file_details->top_left_text = athena_extract_top_left_text (file_contents, state->large, file_size);
	...
	athena_file_changed (state->file);
			     ...
			     gpointer callback_data)
{
	...
	if (state->large) {
		return bytes_read < ATHENA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
			count_lines (file_contents, bytes_read) <= ATHENA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
	} else {
		return bytes_read < ATHENA_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
			count_lines (file_contents, bytes_read) <= ATHENA_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
	}
}

static void
top_left_start (AthenaDirectory *directory,
		AthenaFile *file,
		gboolean *doing_io)
{
	GFile *location;
		     ...
		     gpointer user_data)
{
	...
	g_assert (ATHENA_IS_FILE (get_info_file));
	...
	if (info == NULL) {
		if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_NOT_FOUND) {
			/* mark file as gone */
			athena_file_mark_gone (get_info_file);
		}
		get_info_file->details->file_info_is_up_to_date = TRUE;
		athena_file_clear_info (get_info_file);
		get_info_file->details->get_info_failed = TRUE;
		get_info_file->details->get_info_error = error;
	} else {
		athena_file_update_info (get_info_file, info);
		g_object_unref (info);
	}

	athena_file_changed (get_info_file);
	athena_file_unref (get_info_file);
...
static void
file_info_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
file_info_start (AthenaDirectory *directory,
		 AthenaFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static gboolean
is_link_trusted (AthenaFile *file,
		 gboolean is_launcher)
{
	GFile *location;
	gboolean res;
	...
	if (athena_file_can_execute (file)) {
		return TRUE;
	}
	...
	if (athena_file_is_local (file)) {
		location = athena_file_get_location (file);
		res = athena_is_in_system_dir (location);
		g_object_unref (location);
	}
...
static void
link_info_done (AthenaDirectory *directory,
		AthenaFile *file,
		const char *uri,
		const char *name, 
		GIcon *icon,
		...
		gboolean is_foreign)
{
	...
	if (is_trusted) {
		athena_file_set_display_name (file, name, name, TRUE);
	} else {
		athena_file_set_display_name (file, NULL, NULL, TRUE);
	}
...
static void
link_info_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
link_info_got_data (AthenaDirectory *directory,
		    AthenaFile *file,
		    gboolean result,
		    goffset bytes_read,
		    char *file_contents)
{
	...
	/* Handle the case where we read the Athena link. */
	if (result) {
		link_uri = athena_file_get_uri (file);
		athena_link_get_link_info_given_file_contents (file_contents, bytes_read, link_uri,
...
static void
link_info_athena_link_read_callback (GObject *source_object,
				       GAsyncResult *res,
				       gpointer user_data)
{
...
static void
link_info_start (AthenaDirectory *directory,
		 AthenaFile *file,
		 gboolean *doing_io)
{
	GFile *location;
	gboolean athena_style_link;
	LinkInfoReadState *state;
	...
	*doing_io = TRUE;
	...
	athena_style_link = athena_file_is_athena_link (file);
	location = athena_file_get_location (file);
	
	/* If it's not a link we are done. If it is, we need to read it. */
	if (!athena_style_link) {
...
static void
thumbnail_done (AthenaDirectory *directory,
		AthenaFile *file,
		GdkPixbuf *pixbuf,
		gboolean tried_original)
{
...
static void
thumbnail_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
thumbnail_got_pixbuf (AthenaDirectory *directory,
		      AthenaFile *file,
		      GdkPixbuf *pixbuf,
		      gboolean tried_original)
{
				...
				gpointer user_data)
{
	...
	/* cf. athena_file_get_icon() */
	max_thumbnail_size = ATHENA_ICON_SIZE_LARGEST * cached_thumbnail_size / ATHENA_ICON_SIZE_STANDARD;
...
static void
thumbnail_start (AthenaDirectory *directory,
		 AthenaFile *file,
		 gboolean *doing_io)
{
	GFile *location;
...
static void
mount_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
got_mount (MountState *state, GMount *mount)
{
	...
	file = athena_file_ref (state->file);

	file->details->mount_is_up_to_date = TRUE;
	athena_file_set_mount (file, mount);
			       ...
			       gpointer user_data)
{
	...
	if (mount) {
		...
		location = athena_file_get_location (state->file);
...
static void
mount_start (AthenaDirectory *directory,
	     AthenaFile *file,
	     gboolean *doing_io)
{
	GFile *location;
	...
	*doing_io = TRUE;
	...
	if (file->details->type == G_FILE_TYPE_MOUNTABLE) {
		...
		mount = NULL;
		target = athena_file_get_activation_location (file);
		if (target != NULL) {
...
static void
filesystem_info_cancel (AthenaDirectory *directory)
{
	if (directory->details->filesystem_info_state != NULL) {
...
static void
filesystem_info_stop (AthenaDirectory *directory)
{
	AthenaFile *file;
...
static void
filesystem_info_start (AthenaDirectory *directory,
		       AthenaFile *file,
		       gboolean *doing_io)
{
	GFile *location;
...
static void
extension_info_cancel (AthenaDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		if (directory->details->extension_info_idle) {
			...
			athena_info_provider_cancel_update 
...
static void
extension_info_stop (AthenaDirectory *directory)
{
	if (directory->details->extension_info_in_progress != NULL) {
		AthenaFile *file;
...
static void
finish_info_provider (AthenaDirectory *directory,
		      AthenaFile *file,
		      AthenaInfoProvider *provider)
{
	...
	if (file->details->pending_info_providers == NULL) {
		athena_file_info_providers_done (file);
	}
}
...
info_provider_idle_callback (gpointer user_data)
{
	...
	if (response->handle != directory->details->extension_info_in_progress
		...
		g_warning ("Unexpected plugin response.  This probably indicates a bug in a Athena extension: handle=%p", response->handle);
	} else {
		AthenaFile *file;
		async_job_end (directory, "extension info");
...
static void
info_provider_callback (AthenaInfoProvider *provider,
			AthenaOperationHandle *handle,
			AthenaOperationResult result,
			gpointer user_data)
{
	...
	response->directory = ATHENA_DIRECTORY (user_data);
...
static void
extension_info_start (AthenaDirectory *directory,
		      AthenaFile *file,
		      gboolean *doing_io)
{
	AthenaInfoProvider *provider;
	...
	*doing_io = TRUE;
	...
	result = athena_info_provider_update_file_info
		(provider, 
		 ATHENA_FILE_INFO (file), 
		 update_complete, 
		 &handle);

	g_closure_unref (update_complete);

	if (result == ATHENA_OPERATION_COMPLETE ||
	    result == ATHENA_OPERATION_FAILED) {
...
static void
start_or_stop_io (AthenaDirectory *directory)
{
	AthenaFile *file;
	gboolean doing_io;
	...
	while (!athena_file_queue_is_empty (directory->details->high_priority_queue)) {
		file = athena_file_queue_head (directory->details->high_priority_queue);
	...
	while (!athena_file_queue_is_empty (directory->details->low_priority_queue)) {
		file = athena_file_queue_head (directory->details->low_priority_queue);
	...
	while (!athena_file_queue_is_empty (directory->details->extension_queue)) {
		file = athena_file_queue_head (directory->details->extension_queue);
 ...
 */
void
athena_directory_async_state_changed (AthenaDirectory *directory)
{
...
void
athena_directory_cancel (AthenaDirectory *directory)
{
...
static void
cancel_directory_count_for_file (AthenaDirectory *directory,
				 AthenaFile      *file)
{
	if (directory->details->count_in_progress != NULL &&
	    directory->details->count_in_progress->count_file == file) {
...
static void
cancel_deep_counts_for_file (AthenaDirectory *directory,
			     AthenaFile      *file)
{
	if (directory->details->deep_count_file == file) {
		deep_count_cancel (directory);
...
static void
cancel_mime_list_for_file (AthenaDirectory *directory,
			   AthenaFile      *file)
{
	if (directory->details->mime_list_in_progress != NULL &&
	    directory->details->mime_list_in_progress->mime_list_file == file) {
...
static void
cancel_top_left_text_for_file (AthenaDirectory *directory,
			       AthenaFile      *file)
{
	if (directory->details->top_left_read_state != NULL &&
	    directory->details->top_left_read_state->file == file) {
...
static void
cancel_file_info_for_file (AthenaDirectory *directory,
			   AthenaFile      *file)
{
	if (directory->details->get_info_file == file) {
		file_info_cancel (directory);
...
static void
cancel_thumbnail_for_file (AthenaDirectory *directory,
			   AthenaFile      *file)
{
	if (directory->details->thumbnail_state != NULL &&
	    directory->details->thumbnail_state->file == file) {
...
static void
cancel_mount_for_file (AthenaDirectory *directory,
			   AthenaFile      *file)
{
	if (directory->details->mount_state != NULL &&
	    directory->details->mount_state->file == file) {
...
static void
cancel_filesystem_info_for_file (AthenaDirectory *directory,
				 AthenaFile      *file)
{
	if (directory->details->filesystem_info_state != NULL &&
	    directory->details->filesystem_info_state->file == file) {
...
static void
cancel_link_info_for_file (AthenaDirectory *directory,
			   AthenaFile      *file)
{
	if (directory->details->link_info_read_state != NULL &&
	    directory->details->link_info_read_state->file == file) {
...
static void
cancel_loading_attributes (AthenaDirectory *directory,
			   AthenaFileAttributes file_attributes)
{
	Request request;
...
void
athena_directory_cancel_loading_file_attributes (AthenaDirectory      *directory,
						   AthenaFile           *file,
						   AthenaFileAttributes  file_attributes)
{
	Request request;
...
void
athena_directory_add_file_to_work_queue (AthenaDirectory *directory,
					   AthenaFile *file)
{
	g_return_if_fail (file->details->directory == directory);

	athena_file_queue_enqueue (directory->details->high_priority_queue,
				     file);
}


static void
add_all_files_to_work_queue (AthenaDirectory *directory)
{
	GList *node;
	AthenaFile *file;
...
void
athena_directory_remove_file_from_work_queue (AthenaDirectory *directory,
						AthenaFile *file)
{
	athena_file_queue_remove (directory->details->high_priority_queue,
				    file);
	athena_file_queue_remove (directory->details->low_priority_queue,
				    file);
	athena_file_queue_remove (directory->details->extension_queue,
				    file);
}


static void
move_file_to_low_priority_queue (AthenaDirectory *directory,
				 AthenaFile *file)
{
	/* Must add before removing to avoid ref underflow */
	athena_file_queue_enqueue (directory->details->low_priority_queue,
				     file);
	athena_file_queue_remove (directory->details->high_priority_queue,
				    file);
}
...
static void
move_file_to_extension_queue (AthenaDirectory *directory,
			      AthenaFile *file)
{
	/* Must add before removing to avoid ref underflow */
	athena_file_queue_enqueue (directory->details->extension_queue,
				     file);
	athena_file_queue_remove (directory->details->low_priority_queue,
				    file);
}
