/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   athena-link.c: .desktop link files.
...
#define ATHENA_LINK_GENERIC_TAG	"Link"
#define ATHENA_LINK_TRASH_TAG 	"X-athena-trash"
#define ATHENA_LINK_MOUNT_TAG 	"FSDevice"
#define ATHENA_LINK_HOME_TAG 		"X-athena-home"
...
static gboolean
athena_link_local_set_key (const char *uri,
			     const char *key,
			     const char *value,
			     gboolean    localize)
{
...
gboolean
athena_link_local_set_text (const char *uri,
			      const char *text)
{
	return athena_link_local_set_key (uri, "Name", text, TRUE);
}


gboolean
athena_link_local_set_icon (const char        *uri,
			      const char        *icon)
{
	return athena_link_local_set_key (uri, "Icon", icon, FALSE);
}

char *
athena_link_local_get_text (const char *path)
{
	return slurp_key_string (path, "Name", TRUE);
}

static char *
athena_link_get_link_uri_from_desktop (GKeyFile *key_file, const char *desktop_file_uri)
{
	GFile *file, *parent;
	char *type;
	char *retval;
	...
	} else if ((strcmp (type, ATHENA_LINK_GENERIC_TAG) == 0) ||
		   (strcmp (type, ATHENA_LINK_MOUNT_TAG) == 0) ||
		   (strcmp (type, ATHENA_LINK_TRASH_TAG) == 0) ||
		   (strcmp (type, ATHENA_LINK_HOME_TAG) == 0)) {
...
static char *
athena_link_get_link_name_from_desktop (GKeyFile *key_file)
{
	return g_key_file_get_locale_string (key_file, MAIN_GROUP, "Name", NULL, NULL);
}

static GIcon *
athena_link_get_link_icon_from_desktop (GKeyFile *key_file)
{
	...
	if (icon_str == NULL) {
		if (g_strcmp0 (type, "Application") == 0) {
			...
			icon_str = g_strdup (ATHENA_ICON_FOLDER);
...
char *
athena_link_local_get_link_uri (const char *uri)
{
	GKeyFile *key_file;
	char *retval;
	...
	retval = athena_link_get_link_uri_from_desktop (key_file, uri);
	g_key_file_free (key_file);
...
void
athena_link_get_link_info_given_file_contents (const char  *file_contents,
						 int          link_file_size,
						 const char  *file_uri,
						 char       **uri,
						 char       **name,
						 ...
						 gboolean    *is_foreign)
{
	...
	*uri = athena_link_get_link_uri_from_desktop (key_file, file_uri);
	*name = athena_link_get_link_name_from_desktop (key_file);
	*icon = athena_link_get_link_icon_from_desktop (key_file);
