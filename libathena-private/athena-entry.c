...
#include <config.h>
#include "athena-entry.h"

#include <string.h>
#include "athena-global-preferences.h"
#include "athena-undo-signal-handlers.h"
#include <gdk/gdkkeysyms.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

struct AthenaEntryDetails {
	gboolean user_edit;
...
static void athena_entry_editable_init (GtkEditableInterface *iface);

G_DEFINE_TYPE_WITH_CODE (AthenaEntry, athena_entry, GTK_TYPE_ENTRY,
			 G_IMPLEMENT_INTERFACE (GTK_TYPE_EDITABLE,
						athena_entry_editable_init));

static GtkEditableInterface *parent_editable_interface = NULL;

static void
athena_entry_init (AthenaEntry *entry)
{
	entry->details = g_new0 (AthenaEntryDetails, 1);
	
	entry->details->user_edit = TRUE;

	athena_undo_set_up_athena_entry_for_undo (entry);
}

GtkWidget *
athena_entry_new (void)
{
	return gtk_widget_new (ATHENA_TYPE_ENTRY, NULL);
}
...
static void
athena_entry_finalize (GObject *object)
{
	AthenaEntry *entry;

	entry = ATHENA_ENTRY (object);
	...
	G_OBJECT_CLASS (athena_entry_parent_class)->finalize (object);
}

static gboolean
athena_entry_key_press (GtkWidget *widget, GdkEventKey *event)
{
	AthenaEntry *entry;
	...
	result = GTK_WIDGET_CLASS (athena_entry_parent_class)->key_press_event (widget, event);
 ...
 **/
void
athena_entry_select_all (AthenaEntry *entry)
{
	g_return_if_fail (ATHENA_IS_ENTRY (entry));
...
select_all_at_idle (gpointer callback_data)
{
	...
	entry = ATHENA_ENTRY (callback_data);

	athena_entry_select_all (entry);
 ...
 **/
void
athena_entry_select_all_at_idle (AthenaEntry *entry)
{
	g_return_if_fail (ATHENA_IS_ENTRY (entry));
...
void
athena_entry_set_text (AthenaEntry *entry, const gchar *text)
{
	g_return_if_fail (ATHENA_IS_ENTRY (entry));
...
static void
athena_entry_set_selection_bounds (GtkEditable *editable,
				     int start_pos,
				     int end_pos)
{
...
static gboolean
athena_entry_button_press (GtkWidget *widget,
			     GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (athena_entry_parent_class)->button_press_event (widget, event);
...
static gboolean
athena_entry_button_release (GtkWidget *widget,
			       GdkEventButton *event)
{
	gboolean result;

	result = GTK_WIDGET_CLASS (athena_entry_parent_class)->button_release_event (widget, event);
...
static void
athena_entry_insert_text (GtkEditable *editable, const gchar *text,
			    int length, int *position)
{
	AthenaEntry *entry;

	entry = ATHENA_ENTRY(editable);
...
static void 
athena_entry_delete_text (GtkEditable *editable, int start_pos, int end_pos)
{
	AthenaEntry *entry;
 ...
 * this with type-completion behavior in AthenaLocationBar (see bug 5313).
 * There's a FIXME comment that seems to be about this same issue in
 * gtk+/gtkselection.c, gtk_selection_clear.
 */
static gboolean
athena_entry_selection_clear (GtkWidget *widget,
			        GdkEventSelection *event)
{
	...
	return GTK_WIDGET_CLASS (athena_entry_parent_class)->selection_clear_event (widget, event);
}

static void
athena_entry_editable_init (GtkEditableInterface *iface)
{
	parent_editable_interface = g_type_interface_peek_parent (iface);

	iface->insert_text = athena_entry_insert_text;
	iface->delete_text = athena_entry_delete_text;
	iface->set_selection_bounds = athena_entry_set_selection_bounds;
...
static void
athena_entry_class_init (AthenaEntryClass *class)
{
	...
	widget_class->button_press_event = athena_entry_button_press;
	widget_class->button_release_event = athena_entry_button_release;
	widget_class->key_press_event = athena_entry_key_press;
	widget_class->motion_notify_event = athena_entry_motion_notify;
	widget_class->selection_clear_event = athena_entry_selection_clear;
	
	gobject_class->finalize = athena_entry_finalize;
	...
	signals[USER_CHANGED] = g_signal_new
		("user_changed",
		 ...
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AthenaEntryClass, user_changed),
		 NULL, NULL,
	...
	signals[SELECTION_CHANGED] = g_signal_new
		("selection_changed",
		 G_TYPE_FROM_CLASS (class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AthenaEntryClass, selection_changed),
		 NULL, NULL,
		 g_cclosure_marshal_VOID__VOID,
		 G_TYPE_NONE, 0);
}
...
void
athena_entry_set_special_tab_handling (AthenaEntry *entry,
					 gboolean special_tab_handling)
{
	g_return_if_fail (ATHENA_IS_ENTRY (entry));
