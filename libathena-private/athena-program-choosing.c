...
/* athena-program-choosing.c - functions for selecting and activating
...
void
athena_launch_application_for_mount (GAppInfo *app_info,
				       GMount *mount,
				       GtkWindow *parent_window)
{
	...
	athena_launch_application (app_info,
				     files,
				     parent_window);

	g_list_free_full (files, (GDestroyNotify) athena_file_unref);
}
 ...
 */
void
athena_launch_application (GAppInfo *application, 
			     GList *files,
			     GtkWindow *parent_window)
{
	GList *uris, *l;
	...
	for (l = files; l != NULL; l = l->next) {
		uris = g_list_prepend (uris, athena_file_get_activation_uri (l->data));
	}
	uris = g_list_reverse (uris);
	athena_launch_application_by_uri (application, uris,
					    parent_window);
	g_list_free_full (uris, g_free);
}

void
athena_launch_application_by_uri (GAppInfo *application, 
				    GList *uris,
				    GtkWindow *parent_window)
{
	char *uri;
	...
	file = athena_file_get_by_uri (uris->data);
	icon = athena_file_get_icon (file, 48, 0);
	athena_file_unref (file);
	if (icon) {
		gdk_app_launch_context_set_icon_name (launch_context,
							athena_icon_info_get_used_name (icon));
		g_object_unref (icon);
	}
	...
	if (result) {
		for (l = uris; l != NULL; l = l->next) {
			file = athena_file_get_by_uri (l->data);
			athena_recent_add_file (file, application);
			athena_file_unref (file);
		}
	}
 ...
 */
void
athena_launch_application_from_command (GdkScreen  *screen,
 ...
 */
void
athena_launch_application_from_command_array (GdkScreen  *screen,
						const char *command_string,
						gboolean use_terminal,
...
void
athena_launch_desktop_file (GdkScreen   *screen,
