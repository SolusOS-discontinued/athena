...
struct AthenaSearchDirectoryDetails {
	AthenaQuery *query;
...
G_DEFINE_TYPE (AthenaSearchDirectory, athena_search_directory,
	       ATHENA_TYPE_DIRECTORY);

static void search_engine_hits_added (AthenaSearchEngine *engine, GList *hits, AthenaSearchDirectory *search);
static void search_engine_hits_subtracted (AthenaSearchEngine *engine, GList *hits, AthenaSearchDirectory *search);
static void search_engine_finished (AthenaSearchEngine *engine, AthenaSearchDirectory *search);
static void search_engine_error (AthenaSearchEngine *engine, const char *error, AthenaSearchDirectory *search);
static void search_callback_file_ready_callback (AthenaFile *file, gpointer data);
static void file_changed (AthenaFile *file, AthenaSearchDirectory *search);

static void
ensure_search_engine (AthenaSearchDirectory *search)
{
	if (!search->details->engine) {
		search->details->engine = athena_search_engine_new ();
...
static void
reset_file_list (AthenaSearchDirectory *search)
{
	...
	for (list = search->details->files; list != NULL; list = list->next) {
		...
		for (monitor_list = search->details->monitor_list; monitor_list; 
			...
			athena_file_monitor_remove (file, monitor);
		}
	}
	
	athena_file_list_free (search->details->files);
	search->details->files = NULL;
}

static void
start_or_stop_search_engine (AthenaSearchDirectory *search, gboolean adding)
{
	if (adding && (search->details->monitor_list ||
		...
		athena_search_engine_set_query (search->details->engine, search->details->query);

		reset_file_list (search);

		athena_search_engine_start (search->details->engine);
	} else if (!adding && (!search->details->monitor_list ||
		   !search->details->pending_callback_list) &&
		   search->details->engine &&
		   search->details->search_running) {
		search->details->search_running = FALSE;
		athena_search_engine_stop (search->details->engine);
...
static void
file_changed (AthenaFile *file, AthenaSearchDirectory *search)
{
	GList list;
	...
	athena_directory_emit_files_changed (ATHENA_DIRECTORY (search), &list);
}

static void
search_monitor_add (AthenaDirectory *directory,
		    gconstpointer client,
		    gboolean monitor_hidden_files,
		    ...
		    gpointer callback_data)
{
	...
	search = ATHENA_SEARCH_DIRECTORY (directory);
	...
	for (list = search->details->files; list != NULL; list = list->next) {
		...
		/* Add monitors */
		athena_file_monitor_add (file, monitor, file_attributes);
	}
...
static void
search_monitor_remove_file_monitors (SearchMonitor *monitor, AthenaSearchDirectory *search)
{
	GList *list;
	AthenaFile *file;
...
static void
search_monitor_destroy (SearchMonitor *monitor, AthenaSearchDirectory *search)
{
...
static void
search_monitor_remove (AthenaDirectory *directory,
		       gconstpointer client)
{
	AthenaSearchDirectory *search;
	SearchMonitor *monitor;
...
cancel_call_when_ready (gpointer key, gpointer value, gpointer user_data)
{
	...
	athena_file_cancel_call_when_ready (file, search_callback_file_ready_callback,
					      search_callback);
}
...
search_callback_destroy (SearchCallback *search_callback)
{
	...
	athena_file_list_free (search_callback->file_list);
...
{
	search_callback->callback (ATHENA_DIRECTORY (search_callback->search_directory),
...
static void
search_callback_file_ready_callback (AthenaFile *file, gpointer data)
{
...
search_callback_find (AthenaSearchDirectory *search, AthenaDirectoryCallback callback, gpointer callback_data)
{
	SearchCallback *search_callback;
	GList *list;
...
static SearchCallback *
search_callback_find_pending (AthenaSearchDirectory *search, AthenaDirectoryCallback callback, gpointer callback_data)
{
	SearchCallback *search_callback;
	GList *list;
...
static void
search_call_when_ready (AthenaDirectory *directory,
			AthenaFileAttributes file_attributes,
			gboolean wait_for_file_list,
			AthenaDirectoryCallback callback,
			gpointer callback_data)
{
	...
	if (wait_for_file_list && !search->details->search_finished) {
		...
		search_callback->file_list = athena_file_list_copy (search->details->files);
...
static void
search_cancel_callback (AthenaDirectory *directory,
			AthenaDirectoryCallback callback,
			gpointer callback_data)
{
	AthenaSearchDirectory *search;
...
static void
search_engine_hits_added (AthenaSearchEngine *engine, GList *hits, 
			  AthenaSearchDirectory *search)
{
	GList *hit_list;
	GList *file_list;
	...
	for (hit_list = hits; hit_list != NULL; hit_list = hit_list->next) {
		...
		if (g_str_has_suffix (uri, ATHENA_SAVED_SEARCH_EXTENSION)) {
		...
		for (monitor_list = search->details->monitor_list; monitor_list; monitor_list = monitor_list->next) {
			...
			/* Add monitors */
			athena_file_monitor_add (file, monitor, monitor->monitor_attributes);
		}
	...
	athena_directory_emit_files_added (ATHENA_DIRECTORY (search), file_list);

	file = athena_directory_get_corresponding_file (ATHENA_DIRECTORY (search));
	athena_file_emit_changed (file);
	athena_file_unref (file);
}

static void
search_engine_hits_subtracted (AthenaSearchEngine *engine, GList *hits, 
			       AthenaSearchDirectory *search)
{
	GList *hit_list;
	GList *monitor_list;
	...
	athena_directory_emit_files_changed (ATHENA_DIRECTORY (search), file_list);
...
{
	callback->file_list = athena_file_list_copy (callback->search_directory->details->files);
...
static void
search_engine_error (AthenaSearchEngine *engine, const char *error_message, AthenaSearchDirectory *search)
{
	GError *error;

	error = g_error_new_literal (G_IO_ERROR, G_IO_ERROR_FAILED,
				     error_message);
	athena_directory_emit_load_error (ATHENA_DIRECTORY (search),
					    error);
	g_error_free (error);
}

static void
search_engine_finished (AthenaSearchEngine *engine, AthenaSearchDirectory *search)
{
	search->details->search_finished = TRUE;

	athena_directory_emit_done_loading (ATHENA_DIRECTORY (search));
...
static void
search_force_reload (AthenaDirectory *directory)
{
	AthenaSearchDirectory *search;
...
static gboolean
search_are_all_files_seen (AthenaDirectory *directory)
{
	AthenaSearchDirectory *search;
...
static gboolean
search_contains_file (AthenaDirectory *directory,
		      AthenaFile *file)
{
	AthenaSearchDirectory *search;
...
static GList *
search_get_file_list (AthenaDirectory *directory)
{
	AthenaSearchDirectory *search;

	search = ATHENA_SEARCH_DIRECTORY (directory);

	return athena_file_list_copy (search->details->files);
}


static gboolean
search_is_editable (AthenaDirectory *directory)
{
	return FALSE;
}
...
search_dispose (GObject *object)
{
	...
	search = ATHENA_SEARCH_DIRECTORY (object);
	...
	G_OBJECT_CLASS (athena_search_directory_parent_class)->dispose (object);
}
...
search_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (athena_search_directory_parent_class)->finalize (object);
}

static void
athena_search_directory_init (AthenaSearchDirectory *search)
{
	search->details = g_new0 (AthenaSearchDirectoryDetails, 1);
}

static void
athena_search_directory_class_init (AthenaSearchDirectoryClass *class)
{
...
char *
athena_search_directory_generate_new_uri (void)
{
...
void
athena_search_directory_set_query (AthenaSearchDirectory *search,
				     AthenaQuery *query)
{
	AthenaDirectory *dir;
	AthenaFile *as_file;
	...
	dir = ATHENA_DIRECTORY (search);
	as_file = dir->details->as_file;
	if (as_file != NULL) {
		athena_search_directory_file_update_display_name (ATHENA_SEARCH_DIRECTORY_FILE (as_file));
	}
}

AthenaQuery *
athena_search_directory_get_query (AthenaSearchDirectory *search)
{
...
athena_search_directory_new_from_saved_search (const char *uri)
{
	AthenaSearchDirectory *search;
	AthenaQuery *query;
	char *file;
	
	search = ATHENA_SEARCH_DIRECTORY (g_object_new (ATHENA_TYPE_SEARCH_DIRECTORY, NULL));

	search->details->saved_search_uri = g_strdup (uri);
	
	file = g_filename_from_uri (uri, NULL, NULL);
	if (file != NULL) {
		query = athena_query_load (file);
		if (query != NULL) {
			athena_search_directory_set_query (search, query);
...
gboolean
athena_search_directory_is_saved_search (AthenaSearchDirectory *search)
{
	return search->details->saved_search_uri != NULL;
}

gboolean
athena_search_directory_is_modified (AthenaSearchDirectory *search)
{
	return search->details->modified;
}

void
athena_search_directory_save_to_file (AthenaSearchDirectory *search,
					const char              *save_file_uri)
{
	...
	if (search->details->query != NULL) {
		athena_query_save (search->details->query, file);
	}
...
void
athena_search_directory_save_search (AthenaSearchDirectory *search)
{
	if (search->details->saved_search_uri == NULL) {
		return;
	}

	athena_search_directory_save_to_file (search,
