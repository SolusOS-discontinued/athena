...
#include "athena-icon-info.h"

struct _AthenaFileConflictDialogDetails
{
	/* conflicting objects */
	AthenaFile *source;
	AthenaFile *destination;
	AthenaFile *dest_dir;
...
G_DEFINE_TYPE (AthenaFileConflictDialog,
	       athena_file_conflict_dialog,
	       GTK_TYPE_DIALOG);

#define ATHENA_FILE_CONFLICT_DIALOG_GET_PRIVATE(object)		\
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), ATHENA_TYPE_FILE_CONFLICT_DIALOG, \
				      AthenaFileConflictDialogDetails))

static void
file_icons_changed (AthenaFile *file,
		    AthenaFileConflictDialog *fcd)
{
	GdkPixbuf *pixbuf;

	pixbuf = athena_file_get_icon_pixbuf (fcd->details->destination,
						ATHENA_ICON_SIZE_LARGE,
						TRUE,
						ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS);

	gtk_image_set_from_pixbuf (GTK_IMAGE (fcd->details->dest_image), pixbuf);
	g_object_unref (pixbuf);

	pixbuf = athena_file_get_icon_pixbuf (fcd->details->source,
						ATHENA_ICON_SIZE_LARGE,
						TRUE,
						ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS);
...
file_list_ready_cb (GList *files,
		    gpointer user_data)
{
	AthenaFileConflictDialog *fcd = user_data;
	AthenaFile *src, *dest, *dest_dir;
	...
	src_mtime = athena_file_get_mtime (src);
	dest_mtime = athena_file_get_mtime (dest);

	dest_name = athena_file_get_display_name (dest);
	dest_dir_name = athena_file_get_display_name (dest_dir);

	source_is_dir = athena_file_is_directory (src);
	dest_is_dir = athena_file_is_directory (dest);

	type = athena_file_get_mime_type (dest);
	should_show_type = !athena_file_is_mime_type (src, type);
	...
	/* Set up file icons */
	pixbuf = athena_file_get_icon_pixbuf (dest,
						ATHENA_ICON_SIZE_LARGE,
						TRUE,
						ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS);
	...
	pixbuf = athena_file_get_icon_pixbuf (src,
						ATHENA_ICON_SIZE_LARGE,
						TRUE,
						ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS);
	...
	date = athena_file_get_string_attribute (dest,
						   "date_modified");
	size = athena_file_get_string_attribute (dest, "size");

	if (should_show_type) {
		type = athena_file_get_string_attribute (dest, "type");
	}
	...
	/* Second label */
	label = gtk_label_new (NULL);
	date = athena_file_get_string_attribute (src,
						   "date_modified");
	size = athena_file_get_string_attribute (src, "size");

	if (should_show_type) {
		type = athena_file_get_string_attribute (src, "type");
	}
	...
	/* Populate the entry */
	edit_name = athena_file_get_edit_name (dest);
	...
	athena_file_monitor_add (src, fcd, ATHENA_FILE_ATTRIBUTES_FOR_ICON);
	athena_file_monitor_add (dest, fcd, ATHENA_FILE_ATTRIBUTES_FOR_ICON);
...
static void
build_dialog_appearance (AthenaFileConflictDialog *fcd)
{
	GList *files = NULL;
	AthenaFileConflictDialogDetails *details = fcd->details;
	...
	athena_file_list_call_when_ready (files,
...
set_source_and_destination (GtkWidget *w,
			    ...
			    GFile *dest_dir)
{
	AthenaFileConflictDialog *dialog;
	AthenaFileConflictDialogDetails *details;

	dialog = ATHENA_FILE_CONFLICT_DIALOG (w);
	details = dialog->details;

	details->source = athena_file_get (source);
	details->destination = athena_file_get (destination);
	details->dest_dir = athena_file_get (dest_dir);
...
static void
athena_file_conflict_dialog_init (AthenaFileConflictDialog *fcd)
{
	...
	details = fcd->details = ATHENA_FILE_CONFLICT_DIALOG_GET_PRIVATE (fcd);
...
{
	AthenaFileConflictDialogDetails *details =
		ATHENA_FILE_CONFLICT_DIALOG (self)->details;

	g_free (details->conflict_name);

	if (details->handle != NULL) {
		athena_file_list_cancel_call_when_ready (details->handle);
	}

	if (details->src_handler_id) {
		g_signal_handler_disconnect (details->source, details->src_handler_id);
		athena_file_monitor_remove (details->source, self);
	}

	if (details->dest_handler_id) {
		g_signal_handler_disconnect (details->destination, details->dest_handler_id);
		athena_file_monitor_remove (details->destination, self);
	}

	athena_file_unref (details->source);
	athena_file_unref (details->destination);
	athena_file_unref (details->dest_dir);

	G_OBJECT_CLASS (athena_file_conflict_dialog_parent_class)->finalize (self);
}

static void
athena_file_conflict_dialog_class_init (AthenaFileConflictDialogClass *klass)
{
	G_OBJECT_CLASS (klass)->finalize = do_finalize;

	g_type_class_add_private (klass, sizeof (AthenaFileConflictDialogDetails));
}

char *
athena_file_conflict_dialog_get_new_name (AthenaFileConflictDialog *dialog)
{
...
gboolean
athena_file_conflict_dialog_get_apply_to_all (AthenaFileConflictDialog *dialog)
{
...
GtkWidget *
athena_file_conflict_dialog_new (GtkWindow *parent,
				   GFile *source,
				   GFile *destination,
				   GFile *dest_dir)
{
	...
	dialog = GTK_WIDGET (g_object_new (ATHENA_TYPE_FILE_CONFLICT_DIALOG,
