...
/* athena-column-chooser.h - A column chooser widget
...
#include <config.h>
#include "athena-column-chooser.h"

#include <string.h>
#include <gtk/gtk.h>
#include <glib/gi18n.h>

#include "athena-column-utilities.h"

struct _AthenaColumnChooserDetails
{
	GtkTreeView *view;
	GtkListStore *store;
...
G_DEFINE_TYPE(AthenaColumnChooser, athena_column_chooser, GTK_TYPE_BOX);

static void athena_column_chooser_constructed (GObject *object);

static void
athena_column_chooser_set_property (GObject *object,
				      guint param_id,
				      const GValue *value,
				      GParamSpec *pspec)
{
	AthenaColumnChooser *chooser;

	chooser = ATHENA_COLUMN_CHOOSER (object);
...
static void
athena_column_chooser_class_init (AthenaColumnChooserClass *chooser_class)
{
	GObjectClass *oclass;

	oclass = G_OBJECT_CLASS (chooser_class);

	oclass->set_property = athena_column_chooser_set_property;
	oclass->constructed = athena_column_chooser_constructed;

	signals[CHANGED] = g_signal_new
		("changed",
		 G_TYPE_FROM_CLASS (chooser_class),
		 G_SIGNAL_RUN_LAST,
		 G_STRUCT_OFFSET (AthenaColumnChooserClass,
				  changed),
		 NULL, NULL,
	...
	g_type_class_add_private (chooser_class, sizeof (AthenaColumnChooserDetails));
}

static void 
update_buttons (AthenaColumnChooser *chooser)
{
...
static void
list_changed (AthenaColumnChooser *chooser) 
{
			  ...
			  gpointer user_data)
{
	...
	chooser = ATHENA_COLUMN_CHOOSER (user_data);
...
{
	update_buttons (ATHENA_COLUMN_CHOOSER (user_data));
}
...
row_deleted_callback (GtkTreeModel *model, 
		       ...
		       gpointer user_data)
{
	list_changed (ATHENA_COLUMN_CHOOSER (user_data));
}

static void
add_tree_view (AthenaColumnChooser *chooser)
{
	GtkWidget *scrolled;
	GtkWidget *view;
	GtkListStore *store;
...
{
	g_signal_emit (ATHENA_COLUMN_CHOOSER (user_data), 
...
static void
add_buttons (AthenaColumnChooser *chooser)
{
	GtkWidget *box;
	GtkWidget *separator;
...
static void
populate_tree (AthenaColumnChooser *chooser)
{
	GList *columns;
	GList *l;

	columns = athena_get_columns_for_file (chooser->details->file);
...
static void
athena_column_chooser_constructed (GObject *object)
{
	AthenaColumnChooser *chooser;
...
static void
athena_column_chooser_init (AthenaColumnChooser *chooser)
{	
	chooser->details = G_TYPE_INSTANCE_GET_PRIVATE ((chooser), ATHENA_TYPE_COLUMN_CHOOSER, AthenaColumnChooserDetails);
...
static void 
set_visible_columns (AthenaColumnChooser *chooser,
...
static char **
get_column_names (AthenaColumnChooser *chooser, gboolean only_visible)
{
	GPtrArray *ret;
	GtkTreeIter iter;
...
static gboolean
get_column_iter (AthenaColumnChooser *chooser, 
		 AthenaColumn *column,
		 GtkTreeIter *iter)
{
	char *column_name;

	g_object_get (ATHENA_COLUMN (column), "name", &column_name, NULL);
...
static void
set_column_order (AthenaColumnChooser *chooser,
		  char **column_order)

{
	...
	columns = athena_get_columns_for_file (chooser->details->file);	
	columns = athena_sort_columns (columns, column_order);
	...
	for (l = columns; l != NULL; l = l->next) {
		...
		if (get_column_iter (chooser, ATHENA_COLUMN (l->data), &iter)) {
			GtkTreeIter before;
			if (path) {
				gtk_tree_model_get_iter (GTK_TREE_MODEL (chooser->details->store),
							 &before, path);
...
void
athena_column_chooser_set_settings (AthenaColumnChooser *chooser,
				      char **visible_columns,
				      char **column_order)
{
	g_return_if_fail (ATHENA_IS_COLUMN_CHOOSER (chooser));
...
void
athena_column_chooser_get_settings (AthenaColumnChooser *chooser,
				      char ***visible_columns,
				      char ***column_order)
{
	g_return_if_fail (ATHENA_IS_COLUMN_CHOOSER (chooser));
...
GtkWidget *
athena_column_chooser_new (AthenaFile *file)
{
	return g_object_new (ATHENA_TYPE_COLUMN_CHOOSER, "file", file, NULL);
}
