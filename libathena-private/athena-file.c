/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   athena-file.c: Athena file model.
...
#include <libathena-extension/athena-extension-private.h>
...
#define DEBUG_FLAG ATHENA_DEBUG_FILE
...
#undef ATHENA_FILE_DEBUG_REF
#undef ATHENA_FILE_DEBUG_REF_VALGRIND

#ifdef ATHENA_FILE_DEBUG_REF_VALGRIND
...
/* Name of Athena trash directories */
...
typedef void (* ModifyListFunction) (GList **list, AthenaFile *file);
...
static void     athena_file_info_iface_init                (AthenaFileInfoIface *iface);
static char *   athena_file_get_owner_as_string            (AthenaFile          *file,
							      gboolean               include_real_name);
static char *   athena_file_get_type_as_string             (AthenaFile          *file);
static gboolean update_info_and_name                         (AthenaFile          *file,
							      GFileInfo             *info);
static const char * athena_file_peek_display_name (AthenaFile *file);
static const char * athena_file_peek_display_name_collation_key (AthenaFile *file);
static void file_mount_unmounted (GMount *mount,  gpointer data);
static void metadata_hash_free (GHashTable *hash);

G_DEFINE_TYPE_WITH_CODE (AthenaFile, athena_file, G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (ATHENA_TYPE_FILE_INFO,
						athena_file_info_iface_init));

static void
athena_file_init (AthenaFile *file)
{
	file->details = G_TYPE_INSTANCE_GET_PRIVATE ((file), ATHENA_TYPE_FILE, AthenaFileDetails);

	athena_file_clear_info (file);
...
static GObject*
athena_file_constructor (GType                  type,
			   ...
			   GObjectConstructParam *construct_params)
{
  ...
  object = (* G_OBJECT_CLASS (athena_file_parent_class)->constructor) (type,
									 n_construct_properties,
									 construct_params);

  file = ATHENA_FILE (object);

  /* Set to default type after full construction */
  if (ATHENA_FILE_GET_CLASS (file)->default_file_type != G_FILE_TYPE_UNKNOWN) {
	  file->details->type = ATHENA_FILE_GET_CLASS (file)->default_file_type;
  }
...
gboolean
athena_file_set_display_name (AthenaFile *file,
				...
				gboolean custom)
{
	...
	if (custom && display_name == NULL) {
		...
		if (file->details->got_custom_display_name) {
			...
			athena_file_invalidate_attributes (file,
...
static void
athena_file_clear_display_name (AthenaFile *file)
{
	eel_ref_str_unref (file->details->display_name);
...
static void
clear_metadata (AthenaFile *file)
{
...
get_metadata_from_info (GFileInfo *info)
{
	...
	for (i = 0; attrs[i] != NULL; i++) {
		id = athena_metadata_get_id (attrs[i] + strlen ("metadata::"));
		if (id == 0) {
			continue;
		}
...
gboolean
athena_file_update_metadata_from_info (AthenaFile *file,
					 GFileInfo *info)
{
...
void
athena_file_clear_info (AthenaFile *file)
{
	...
	if (!file->details->got_custom_display_name) {
		athena_file_clear_display_name (file);
	}
...
static AthenaFile *
athena_file_new_from_filename (AthenaDirectory *directory,
				 const char *filename,
				 gboolean self_owned)
{
	AthenaFile *file;
	...
	if (ATHENA_IS_DESKTOP_DIRECTORY (directory)) {
		if (self_owned) {
			file = ATHENA_FILE (g_object_new (ATHENA_TYPE_DESKTOP_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = ATHENA_FILE (g_object_new (ATHENA_TYPE_VFS_FILE, NULL));
		}
	} else if (ATHENA_IS_SEARCH_DIRECTORY (directory)) {
		if (self_owned) {
			file = ATHENA_FILE (g_object_new (ATHENA_TYPE_SEARCH_DIRECTORY_FILE, NULL));
		} else {
			/* This doesn't normally happen, unless the user somehow types in a uri
			 * that references a file like this. (See #349840) */
			file = ATHENA_FILE (g_object_new (ATHENA_TYPE_VFS_FILE, NULL));
		}
	} else if (g_str_has_suffix (filename, ATHENA_SAVED_SEARCH_EXTENSION)) {
		file = ATHENA_FILE (g_object_new (ATHENA_TYPE_SAVED_SEARCH_FILE, NULL));
	} else {
		file = ATHENA_FILE (g_object_new (ATHENA_TYPE_VFS_FILE, NULL));
	}

	file->details->directory = athena_directory_ref (directory);
...
static void
modify_link_hash_table (AthenaFile *file,
			ModifyListFunction modify_function)
{
	...
	target_uri = athena_file_get_symbolic_link_target_uri (file);
...
static void
add_to_link_hash_table_list (GList **list, AthenaFile *file)
{
...
static void
add_to_link_hash_table (AthenaFile *file)
{
	modify_link_hash_table (file, add_to_link_hash_table_list);
}

static void
remove_from_link_hash_table_list (GList **list, AthenaFile *file)
{
	if (g_list_find (*list, file) != NULL) {
...
static void
remove_from_link_hash_table (AthenaFile *file)
{
	modify_link_hash_table (file, remove_from_link_hash_table_list);
}

AthenaFile *
athena_file_new_from_info (AthenaDirectory *directory,
			     GFileInfo *info)
{
	AthenaFile *file;
	...
	if (mime_type &&
	    strcmp (mime_type, ATHENA_SAVED_SEARCH_MIMETYPE) == 0) {
...
static AthenaFile *
athena_file_get_internal (GFile *location, gboolean create)
{
	gboolean self_owned;
	...
	directory = athena_directory_get_internal (parent, create);

	g_object_unref (parent);

	/* Get the name for the file. */
	if (self_owned && directory != NULL) {
		basename = athena_directory_get_name_for_self_as_new_file (directory);
	} else {
	...
	if (directory == NULL) {
		...
		file = athena_directory_find_file_by_name (directory, basename);
	}
	...
	if (file != NULL) {
		...
		file = athena_file_new_from_filename (directory, basename, self_owned);
		if (self_owned) {
...
AthenaFile *
athena_file_get (GFile *location)
{
	return athena_file_get_internal (location, TRUE);
}

AthenaFile *
athena_file_get_existing (GFile *location)
{
	return athena_file_get_internal (location, FALSE);
}

AthenaFile *
athena_file_get_existing_by_uri (const char *uri)
{
	GFile *location;
	AthenaFile *file;
	
	location = g_file_new_for_uri (uri);
	file = athena_file_get_internal (location, FALSE);
...
AthenaFile *
athena_file_get_by_uri (const char *uri)
{
	GFile *location;
	AthenaFile *file;
	
	location = g_file_new_for_uri (uri);
	file = athena_file_get_internal (location, TRUE);
	g_object_unref (location);
...
gboolean
athena_file_is_self_owned (AthenaFile *file)
{
...
finalize (GObject *object)
{
	...
	if (file->details->is_thumbnailing) {
		...
		athena_thumbnail_remove_from_queue (uri);
		g_free (uri);
	}
	
	athena_async_destroying_file (file);
	...
	if (athena_file_is_self_owned (file)) {
		...
		if (!file->details->is_gone) {
			athena_directory_remove_file (directory, file);
		}
	}
	...
	G_OBJECT_CLASS (athena_file_parent_class)->finalize (object);
}

AthenaFile *
athena_file_ref (AthenaFile *file)
{
	if (file == NULL) {
		return NULL;
	}
...
void
athena_file_unref (AthenaFile *file)
{
	if (file == NULL) {
		return;
	}

	g_return_if_fail (ATHENA_IS_FILE (file));
 ...
 */ 
char *
athena_file_get_parent_uri_for_display (AthenaFile *file) 
{
	GFile *parent;
	char *result;
	...
	parent = athena_file_get_parent_location (file);
	if (parent) {
 ...
 * Use athena_file_get_parent_uri_for_display instead if the
 * result is to be displayed on-screen.
 * If the parent is NULL, returns the empty string.
 */ 
char *
athena_file_get_parent_uri (AthenaFile *file) 
{
	g_assert (ATHENA_IS_FILE (file));
	...
	return athena_directory_get_uri (file->details->directory);
}

GFile *
athena_file_get_parent_location (AthenaFile *file) 
{
	g_assert (ATHENA_IS_FILE (file));
	...
	return athena_directory_get_location (file->details->directory);
}

AthenaFile *
athena_file_get_parent (AthenaFile *file)
{
	g_assert (ATHENA_IS_FILE (file));
	...
	return athena_directory_get_corresponding_file (file->details->directory);
}

/**
 * athena_file_can_read:
 * 
 ...
 */
gboolean
athena_file_can_read (AthenaFile *file)
{
 ...
 */
gboolean
athena_file_can_write (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
 ...
 */
gboolean
athena_file_can_execute (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_mount (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_unmount (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_eject (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_start (AthenaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
athena_file_can_start_degraded (AthenaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
athena_file_can_poll_for_media (AthenaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
athena_file_is_media_check_automatic (AthenaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
gboolean
athena_file_can_stop (AthenaFile *file)
{
	gboolean ret;
	GDrive *drive;
...
GDriveStartStopType
athena_file_get_start_stop_type (AthenaFile *file)
{
...
void
athena_file_mount (AthenaFile                   *file,
		     ...
		     gpointer                        callback_data)
{
	...
	if (ATHENA_FILE_GET_CLASS (file)->mount == NULL) {
		if (callback) {
			error = NULL;
		...
		ATHENA_FILE_GET_CLASS (file)->mount (file, mount_op, cancellable, callback, callback_data);
	}
}
...
unmount_done (void *callback_data)
{
	...
	athena_file_unref (data->file);
	g_free (data);
}

void
athena_file_unmount (AthenaFile                   *file,
		       GMountOperation                *mount_op,
		       GCancellable                   *cancellable,
		       AthenaFileOperationCallback   callback,
		       gpointer                        callback_data)
{
	...
	if (file->details->can_unmount) {
		if (ATHENA_FILE_GET_CLASS (file)->unmount != NULL) {
			ATHENA_FILE_GET_CLASS (file)->unmount (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->file = athena_file_ref (file);
		data->callback = callback;
		data->callback_data = callback_data;
		athena_file_operations_unmount_mount_full (NULL, file->details->mount, FALSE, TRUE, unmount_done, data);
	} else if (callback) {
...
void
athena_file_eject (AthenaFile                   *file,
		     GMountOperation                *mount_op,
		     GCancellable                   *cancellable,
		     AthenaFileOperationCallback   callback,
		     gpointer                        callback_data)
{
	...
	if (file->details->can_eject) {
		if (ATHENA_FILE_GET_CLASS (file)->eject != NULL) {
			ATHENA_FILE_GET_CLASS (file)->eject (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		...
		data->callback = callback;
		data->callback_data = callback_data;
		athena_file_operations_unmount_mount_full (NULL, file->details->mount, TRUE, TRUE, unmount_done, data);
	} else if (callback) {
		callback (file, NULL, NULL, callback_data);
	}
}
...
void
athena_file_start (AthenaFile                   *file,
		     ...
		     gpointer                        callback_data)
{
	...
	if ((file->details->can_start || file->details->can_start_degraded) &&
	    ATHENA_FILE_GET_CLASS (file)->start != NULL) {
		ATHENA_FILE_GET_CLASS (file)->start (file, start_op, cancellable, callback, callback_data);
	} else {
		if (callback) {
			error = NULL;
			g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
		    ...
		    gpointer callback_data)
{
	...
	athena_file_operation_complete (op, NULL, error);
	if (error) {
		g_error_free (error);
	}
}
...
void
athena_file_stop (AthenaFile                   *file,
		    GMountOperation                *mount_op,
		    GCancellable                   *cancellable,
		    AthenaFileOperationCallback   callback,
		    gpointer                        callback_data)
{
	...
	if (ATHENA_FILE_GET_CLASS (file)->stop != NULL) {
		if (file->details->can_stop) {
			ATHENA_FILE_GET_CLASS (file)->stop (file, mount_op, cancellable, callback, callback_data);
		} else {
			if (callback) {
				error = NULL;
				g_set_error_literal (&error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
...
void
athena_file_poll_for_media (AthenaFile *file)
{
	if (file->details->can_poll_for_media) {
		if (ATHENA_FILE_GET_CLASS (file)->stop != NULL) {
			ATHENA_FILE_GET_CLASS (file)->poll_for_media (file);
		}
	} else if (file->details->mount != NULL) {
		GDrive *drive;
		drive = g_mount_get_drive (file->details->mount);
 ...
 */
gboolean
athena_file_is_desktop_directory (AthenaFile *file)
{
	GFile *dir;
	...
	return athena_is_desktop_directory_file (dir, eel_ref_str_peek (file->details->name));
}

static gboolean
is_desktop_file (AthenaFile *file)
{
	return athena_file_is_mime_type (file, "application/x-desktop");
}

static gboolean
can_rename_desktop_file (AthenaFile *file)
{
	GFile *location;
	gboolean res;
 ...
 */
gboolean
athena_file_can_rename (AthenaFile *file)
{
	gboolean can_rename;
	...
	if (ATHENA_IS_DESKTOP_ICON_FILE (file)) {
		AthenaDesktopLink *link;

		link = athena_desktop_icon_file_get_link (ATHENA_DESKTOP_ICON_FILE (file));

		if (link != NULL) {
			can_rename = athena_desktop_link_can_rename (link);
			g_object_unref (link);
		}
	}
...
gboolean
athena_file_can_delete (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_trash (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
GFile *
athena_file_get_location (AthenaFile *file)
{
	GFile *dir;
...
char *
athena_file_get_uri (AthenaFile *file)
{
	char *uri;
	GFile *loc;
	...
	loc = athena_file_get_location (file);
...
char *
athena_file_get_uri_scheme (AthenaFile *file)
{
	GFile *loc;
	char *scheme;
	...
	loc = athena_directory_get_location (file->details->directory);
...
AthenaFileOperation *
athena_file_operation_new (AthenaFile *file,
			     AthenaFileOperationCallback callback,
			     gpointer callback_data)
{
	AthenaFileOperation *op;

	op = g_new0 (AthenaFileOperation, 1);
	op->file = athena_file_ref (file);
...
static void
athena_file_operation_remove (AthenaFileOperation *op)
{
...
void
athena_file_operation_free (AthenaFileOperation *op)
{
	athena_file_operation_remove (op);
	athena_file_unref (op->file);
	g_object_unref (op->cancellable);
	if (op->free_data) {
	...
	if (op->undo_info != NULL) {
		athena_file_undo_manager_set_action (op->undo_info);
	}
...
void
athena_file_operation_complete (AthenaFileOperation *op, GFile *result_file, GError *error)
{
	 ...
	 */
	athena_file_operation_remove (op);
	athena_file_changed (op->file);
	if (op->callback) {
		(* op->callback) (op->file, result_file, error, op->callback_data);
	}
	athena_file_operation_free (op);
}

void
athena_file_operation_cancel (AthenaFileOperation *op)
{
			  ...
			  gpointer callback_data)
{
	...
	if (new_info != NULL) {
		 ...
		 */
		existing_file = athena_directory_find_file_by_name (directory, new_name);
		if (existing_file != NULL) {
			athena_file_mark_gone (existing_file);
			athena_file_changed (existing_file);
		}
		
		old_uri = athena_file_get_uri (op->file);
		...
		new_uri = athena_file_get_uri (op->file);
		athena_directory_moved (old_uri, new_uri);
		g_free (new_uri);
		g_free (old_uri);
		...
		if (op->file->details->got_custom_display_name) {
			athena_file_invalidate_attributes (op->file,
							     ATHENA_FILE_ATTRIBUTE_INFO |
		 ...
		 gpointer callback_data)
{
	...
	if (new_file != NULL) {
		if (op->undo_info != NULL) {
			athena_file_undo_info_rename_set_data (ATHENA_FILE_UNDO_INFO_RENAME (op->undo_info),
...
static gboolean
name_is (AthenaFile *file, const char *new_name)
{
	const char *old_name;
...
void
athena_file_rename (AthenaFile *file,
		      const char *new_name,
		      AthenaFileOperationCallback callback,
		      gpointer callback_data)
{
	...
	if (ATHENA_IS_DESKTOP_ICON_FILE (file)) {
		...
		link = athena_desktop_icon_file_get_link (ATHENA_DESKTOP_ICON_FILE (file));
		old_name = athena_file_get_display_name (file);

		if ((old_name != NULL && strcmp (new_name, old_name) == 0)) {
			success = TRUE;
		} else {
			success = (link != NULL && athena_desktop_link_rename (link, new_name));
		}
	...
	if (is_renameable_desktop_file) {
		 ...
		 */
		uri = athena_file_get_uri (file);
		old_name = athena_link_local_get_text (uri);
		if (old_name != NULL && strcmp (new_name, old_name) == 0) {
			success = TRUE;
			name_changed = FALSE;
		} else {
			success = athena_link_local_set_text (uri, new_name);
			name_changed = TRUE;
		}
		g_free (old_name);
		g_free (uri);
	...
	if (!athena_file_undo_manager_pop_flag ()) {
		op->undo_info = athena_file_undo_info_rename_new ();
	}
...
gboolean
athena_file_rename_in_progress (AthenaFile *file)
{
	GList *node;
	AthenaFileOperation *op;
...
void
athena_file_cancel (AthenaFile *file,
		      AthenaFileOperationCallback callback,
		      gpointer callback_data)
{
	GList *node, *next;
	...
	for (node = file->details->operations_in_progress; node != NULL; node = next) {
		...
		if (op->callback == callback && op->callback_data == callback_data) {
			athena_file_operation_cancel (op);
		}
	}
}

gboolean         
athena_file_matches_uri (AthenaFile *file, const char *match_uri)
{
...
int
athena_file_compare_location (AthenaFile *file_1,
                                AthenaFile *file_2)
{
	GFile *loc_a, *loc_b;
	gboolean res;

	loc_a = athena_file_get_location (file_1);
	loc_b = athena_file_get_location (file_2);
...
gboolean
athena_file_is_local (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
	
	return athena_directory_is_local (file->details->directory);
}

static void
update_link (AthenaFile *link_file, AthenaFile *target_file)
{
	g_assert (ATHENA_IS_FILE (link_file));
	g_assert (ATHENA_IS_FILE (target_file));
...
static GList *
get_link_files (AthenaFile *target_file)
{
	char *uri;
	GList **link_files;
	...
	if (symbolic_links == NULL) {
		...
		uri = athena_file_get_uri (target_file);
		link_files = g_hash_table_lookup (symbolic_links, uri);
		g_free (uri);
	}
	if (link_files) {
		return athena_file_list_copy (*link_files);
	}
	return NULL;
}

static void
update_links_if_target (AthenaFile *target_file)
{
	GList *link_files, *p;

	link_files = get_link_files (target_file);
	for (p = link_files; p != NULL; p = p->next) {
		update_link (ATHENA_FILE (p->data), target_file);
	}
	athena_file_list_free (link_files);
}

static gboolean
update_info_internal (AthenaFile *file,
		      GFileInfo *info,
		      gboolean update_name)
{
	...
	changed |= athena_file_set_display_name (file,
	...
	changed |=
		athena_file_update_metadata_from_info (file, info);
	...
	if (update_name) {
		...
		if (file->details->name == NULL ||
			...
			node = athena_directory_begin_file_name_change
			...
			if (!file->details->got_custom_display_name &&
				 ...
				 * athena_file_set_display_name() did
				 * not unset the display name.
				 */
				athena_file_clear_display_name (file);
			}

			athena_directory_end_file_name_change
...
static gboolean
update_info_and_name (AthenaFile *file,
		      GFileInfo *info)
{
...
gboolean
athena_file_update_info (AthenaFile *file,
			   GFileInfo *info)
{
...
static gboolean
update_name_internal (AthenaFile *file,
		      const char *name,
...
gboolean
athena_file_update_name (AthenaFile *file, const char *name)
{
	gboolean ret;
...
gboolean
athena_file_update_name_and_directory (AthenaFile *file, 
					 const char *name,
					 AthenaDirectory *new_directory)
{
	AthenaDirectory *old_directory;
	FileMonitors *monitors;

	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
	g_return_val_if_fail (ATHENA_IS_DIRECTORY (file->details->directory), FALSE);
	g_return_val_if_fail (!file->details->is_gone, FALSE);
	g_return_val_if_fail (!athena_file_is_self_owned (file), FALSE);
	g_return_val_if_fail (ATHENA_IS_DIRECTORY (new_directory), FALSE);
	...
	monitors = athena_directory_remove_file_monitors (old_directory, file);
	athena_directory_remove_file (old_directory, file);

	file->details->directory = athena_directory_ref (new_directory);
	...
	athena_directory_add_file (new_directory, file);
	athena_directory_add_file_monitors (new_directory, file, monitors);
...
void
athena_file_set_directory (AthenaFile *file,
			     AthenaDirectory *new_directory)
{
	athena_file_update_name_and_directory (file, NULL, new_directory);
}

static Knowledge
get_item_count (AthenaFile *file,
		guint *count)
{
	gboolean known, unreadable;

	known = athena_file_get_directory_item_count
...
static int
compare_directories_by_count (AthenaFile *file_1, AthenaFile *file_2)
{
	/* Sort order:
...
static int
compare_files_by_size (AthenaFile *file_1, AthenaFile *file_2)
{
	/* Sort order:
...
static int
compare_by_size (AthenaFile *file_1, AthenaFile *file_2)
{
	/* Sort order:
	 *   Directories with n items
	 *   Directories with 0 items
	...
	is_directory_1 = athena_file_is_directory (file_1);
	is_directory_2 = athena_file_is_directory (file_2);
...
static int
compare_by_display_name (AthenaFile *file_1, AthenaFile *file_2)
{
	...
	name_1 = athena_file_peek_display_name (file_1);
	name_2 = athena_file_peek_display_name (file_2);
	...
	if (sort_last_1 && !sort_last_2) {
		...
		key_1 = athena_file_peek_display_name_collation_key (file_1);
		key_2 = athena_file_peek_display_name_collation_key (file_2);
...
static int
compare_by_directory_name (AthenaFile *file_1, AthenaFile *file_2)
{
	...
	directory_1 = athena_file_get_parent_uri_for_display (file_1);
	directory_2 = athena_file_get_parent_uri_for_display (file_2);
...
static gboolean
file_has_note (AthenaFile *file)
{
	char *note;
	gboolean res;

	note = athena_file_get_metadata (file, ATHENA_METADATA_KEY_ANNOTATION, NULL);
...
static GList *
prepend_automatic_keywords (AthenaFile *file,
			    GList *names)
{
	/* Prepend in reverse order. */
	AthenaFile *parent;

	parent = athena_file_get_parent (file);

#ifdef TRASH_IS_FAST_ENOUGH
	if (athena_file_is_in_trash (file)) {
		names = g_list_prepend
			(names, g_strdup (ATHENA_FILE_EMBLEM_NAME_TRASH));
	}
#endif
	if (file_has_note (file)) {
		names = g_list_prepend
			(names, g_strdup (ATHENA_FILE_EMBLEM_NAME_NOTE));
	}

	/* Trash files are assumed to be read-only, 
	 * so we want to ignore them here. */
	if (!athena_file_can_write (file) &&
	    !athena_file_is_in_trash (file) &&
	    (parent == NULL || athena_file_can_write (parent))) {
		names = g_list_prepend
			(names, g_strdup (ATHENA_FILE_EMBLEM_NAME_CANT_WRITE));
	}
	if (!athena_file_can_read (file)) {
		names = g_list_prepend
			(names, g_strdup (ATHENA_FILE_EMBLEM_NAME_CANT_READ));
	}
	if (athena_file_is_symbolic_link (file)) {
		names = g_list_prepend
			(names, g_strdup (ATHENA_FILE_EMBLEM_NAME_SYMBOLIC_LINK));
	}

	if (parent) {
		athena_file_unref (parent);
	}
...
static int
compare_by_type (AthenaFile *file_1, AthenaFile *file_2)
{
	...
	type_string_1 = athena_file_get_type_as_string (file_1);
	type_string_2 = athena_file_get_type_as_string (file_2);
...
static int
compare_by_time (AthenaFile *file_1, AthenaFile *file_2, AthenaDateType type)
{
	/* Sort order:
...
static int
compare_by_full_path (AthenaFile *file_1, AthenaFile *file_2)
{
	int compare;
...
static int
athena_file_compare_for_sort_internal (AthenaFile *file_1,
					 AthenaFile *file_2,
 ...
 **/
int
athena_file_compare_for_sort (AthenaFile *file_1,
				AthenaFile *file_2,
				...
				gboolean reversed)
{
	...
	result = athena_file_compare_for_sort_internal (file_1, file_2, directories_first, reversed);
	
	if (result == 0) {
		switch (sort_type) {
		case ATHENA_FILE_SORT_BY_DISPLAY_NAME:
			result = compare_by_display_name (file_1, file_2);
			if (result == 0) {
				result = compare_by_directory_name (file_1, file_2);
			}
			break;
		case ATHENA_FILE_SORT_BY_SIZE:
		...
		case ATHENA_FILE_SORT_BY_TYPE:
			...
			break;
		case ATHENA_FILE_SORT_BY_MTIME:
			result = compare_by_time (file_1, file_2, ATHENA_DATE_TYPE_MODIFIED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case ATHENA_FILE_SORT_BY_ATIME:
			result = compare_by_time (file_1, file_2, ATHENA_DATE_TYPE_ACCESSED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
		case ATHENA_FILE_SORT_BY_TRASHED_TIME:
			result = compare_by_time (file_1, file_2, ATHENA_DATE_TYPE_TRASHED);
			if (result == 0) {
				result = compare_by_full_path (file_1, file_2);
			}
			break;
...
int
athena_file_compare_for_sort_by_attribute_q   (AthenaFile                   *file_1,
						 AthenaFile                   *file_2,
						 GQuark                          attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	...
	/* Convert certain attributes into AthenaFileSortTypes and use
	 * athena_file_compare_for_sort()
	 */
	if (attribute == 0 || attribute == attribute_name_q) {
		return athena_file_compare_for_sort (file_1, file_2,
	...
	if (result == 0) {
		...
		value_1 = athena_file_get_string_attribute_q (file_1, 
								attribute);
		value_2 = athena_file_get_string_attribute_q (file_2, 
								attribute);
...
int
athena_file_compare_for_sort_by_attribute     (AthenaFile                   *file_1,
						 AthenaFile                   *file_2,
						 const char                     *attribute,
						 gboolean                        directories_first,
						 gboolean                        reversed)
{
	return athena_file_compare_for_sort_by_attribute_q (file_1, file_2,
 ...
 **/
int
athena_file_compare_display_name (AthenaFile *file,
				    const char *pattern)
{
	const char *name;
	int result;
	...
	name = athena_file_peek_display_name (file);
...
gboolean
athena_file_is_hidden_file (AthenaFile *file)
{
	return file->details->is_hidden;
}

static gboolean
is_file_hidden (AthenaFile *file)
{
			   ...
			   gboolean show_foreign)
{
	...
	if (athena_file_is_in_trash (file)) {
		...
		return (show_hidden || (!athena_file_is_hidden_file (file) && !is_file_hidden (file))) &&
			(show_foreign || !(athena_file_is_in_desktop (file) && athena_file_is_foreign_link (file)));
	}
}

gboolean
athena_file_is_home (AthenaFile *file)
{
	GFile *dir;
	...
	return athena_is_home_directory_file (dir,
						eel_ref_str_peek (file->details->name));
}

gboolean
athena_file_is_in_desktop (AthenaFile *file)
{
	if (file->details->directory->details->location) {
		return athena_is_desktop_directory (file->details->directory->details->location);
	}
	return FALSE;
}
...
GList *
athena_file_list_filter_hidden (GList    *files,
				  gboolean  show_hidden)
{
	...
	filtered_files = athena_file_list_copy (files);
	filtered_files = eel_g_list_partition (filtered_files,
					       ...
					       &removed_files);
	athena_file_list_free (removed_files);

	return filtered_files;
}

char *
athena_file_get_metadata (AthenaFile *file,
			    const char *key,
			    const char *default_metadata)
{
	...
	g_return_val_if_fail (ATHENA_IS_FILE (file), g_strdup (default_metadata));

	id = athena_metadata_get_id (key);
...
GList *
athena_file_get_metadata_list (AthenaFile *file,
				 const char *key)
{
	GList *res;
	guint id;
...
void
athena_file_set_metadata (AthenaFile *file,
			    const char *key,
			    const char *default_metadata,
			    const char *metadata)
{
	...
	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->set_metadata (file, key, val);
}

void
athena_file_set_metadata_list (AthenaFile *file,
				 const char *key,
				 GList *list)
{
	char **val;
	...
	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->set_metadata_as_list (file, key, val);

	g_free (val);
}

gboolean
athena_file_get_boolean_metadata (AthenaFile *file,
				    const char   *key,
				    gboolean      default_metadata)
{
	...
	g_return_val_if_fail (ATHENA_IS_FILE (file), default_metadata);

	result_as_string = athena_file_get_metadata
...
int
athena_file_get_integer_metadata (AthenaFile *file,
				    const char   *key,
...
time_t
athena_file_get_time_metadata (AthenaFile *file,
				 const char   *key)
{
	time_t time;
	char *time_string;

	time_string = athena_file_get_metadata (file, key, NULL);
...
void
athena_file_set_time_metadata (AthenaFile *file,
				 const char   *key,
				 time_t        time)
{
	char time_str[21];
	...
	athena_file_set_metadata (file, key, NULL, metadata);
}


void
athena_file_set_boolean_metadata (AthenaFile *file,
				    const char   *key,
				    gboolean      default_metadata,
				    gboolean      metadata)
{
	...
	athena_file_set_metadata (file, key,
				    default_metadata ? "true" : "false",
				    metadata ? "true" : "false");
}

void
athena_file_set_integer_metadata (AthenaFile *file,
				    const char   *key,
				    int           default_metadata,
				    int           metadata)
{
...
static const char *
athena_file_peek_display_name_collation_key (AthenaFile *file)
{
	const char *res;
...
static const char *
athena_file_peek_display_name (AthenaFile *file)
{
	const char *name;
	char *escaped_name;

	/* FIXME: for some reason we can get a AthenaFile instance which is
	 *        no longer valid or could be freed somewhere else in the same time.
	 *        There's race condition somewhere. See bug 602500.
	 */
	if (file == NULL || athena_file_is_gone (file))
		return "";
...
char *
athena_file_get_display_name (AthenaFile *file)
{
	return g_strdup (athena_file_peek_display_name (file));
}

char *
athena_file_get_edit_name (AthenaFile *file)
{
	const char *res;
...
char *
athena_file_get_name (AthenaFile *file)
{
 ...
 */
char *
athena_file_get_description (AthenaFile *file)
{
	return g_strdup (file->details->description);
}
   
void             
athena_file_monitor_add (AthenaFile *file,
			   ...
			   AthenaFileAttributes attributes)
{
	...
	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->monitor_add (file, client, attributes);
}   
			   
void
athena_file_monitor_remove (AthenaFile *file,
			      gconstpointer client)
{
	g_return_if_fail (ATHENA_IS_FILE (file));
	g_return_if_fail (client != NULL);

	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->monitor_remove (file, client);
}			      

gboolean
athena_file_is_launcher (AthenaFile *file)
{
	return file->details->is_launcher;
}

gboolean
athena_file_is_foreign_link (AthenaFile *file)
{
	return file->details->is_foreign_link;
}

gboolean
athena_file_is_trusted_link (AthenaFile *file)
{
	return file->details->is_trusted_link;
}

gboolean
athena_file_has_activation_uri (AthenaFile *file)
{
 ...
 * the actual uri if the file is an desktop file or a athena
 * xml link file.
 */
char *
athena_file_get_activation_uri (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), NULL);
...
GFile *
athena_file_get_activation_location (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), NULL);
	...
	return athena_file_get_location (file);
}


char *
athena_file_get_drop_target_uri (AthenaFile *file)
{
	char *uri, *target_uri;
	GFile *location;
	AthenaDesktopLink *link;
...
static char *
get_custom_icon_metadata_uri (AthenaFile *file)
{
	char *custom_icon_uri;
	char *uri;
	char *dir_uri;
	
	uri = athena_file_get_metadata (file, ATHENA_METADATA_KEY_CUSTOM_ICON, NULL);
	if (uri != NULL &&
	    athena_file_is_directory (file) &&
	    is_uri_relative (uri)) {
		dir_uri = athena_file_get_uri (file);
...
static char *
get_custom_icon_metadata_name (AthenaFile *file)
{
	char *icon_name;

	icon_name = athena_file_get_metadata (file,
						ATHENA_METADATA_KEY_CUSTOM_ICON_NAME, NULL);

	return icon_name;
}

static GIcon *
get_custom_icon (AthenaFile *file)
{
...
athena_file_get_filesystem_use_preview (AthenaFile *file)
{
...
gboolean
athena_file_should_show_thumbnail (AthenaFile *file)
{
	const char *mime_type;
	GFilesystemPreviewType use_preview;

	use_preview = athena_file_get_filesystem_use_preview (file);
	 ...
	 */
	if (athena_thumbnail_is_mimetype_limited_by_size (mime_type) &&
	    file->details->thumbnail_path == NULL &&
	    athena_file_get_size (file) > cached_thumbnail_limit) {
		return FALSE;
	}

	if (show_image_thumbs == ATHENA_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
		}
	} else if (show_image_thumbs == ATHENA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	} else {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			...
			/* only local files */
			return athena_file_is_local (file);
		}
	}
...
GIcon *
athena_file_get_gicon (AthenaFile *file,
			 AthenaFileIconFlags flags)
{
	...
	if (file->details->icon) {
		...
		if (flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON ||
		    flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) {
		...
		if (((flags & ATHENA_FILE_ICON_FLAGS_EMBEDDING_TEXT) ||
		     (flags & ATHENA_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT) ||
		     (flags & ATHENA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER) ||
		     (flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON) ||
		     (flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) ||
		     ((flags & ATHENA_FILE_ICON_FLAGS_IGNORE_VISITING) == 0 &&
		      athena_file_has_open_window (file))) &&
			...
			for (i = 0; names[i] != NULL; i++) {
				...
				if (strcmp (name, "text-x-generic") == 0 &&
				    (flags & ATHENA_FILE_ICON_FLAGS_EMBEDDING_TEXT)) {
					is_preview = TRUE;
				}
			}
			...
			if (is_folder && (flags & ATHENA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER)) {
		...
		if ((flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON) &&
			...
			g_object_unref (icon);
			icon = mount_icon;
		} else if ((flags & ATHENA_FILE_ICON_FLAGS_USE_MOUNT_ICON_AS_EMBLEM) &&
...
static GIcon *
get_default_file_icon (AthenaFileIconFlags flags)
{
...
AthenaIconInfo *
athena_file_get_icon (AthenaFile *file,
			int size,
			AthenaFileIconFlags flags)
{
	AthenaIconInfo *icon;
	...
	if (gicon) {
		icon = athena_icon_info_lookup (gicon, size);
		g_object_unref (gicon);
		return icon;
	}

	DEBUG ("Called file_get_icon(), at size %d, force thumbnail %d", size,
	       flags & ATHENA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE);
	
	if (flags & ATHENA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
		modified_size = size;
	} else {
		modified_size = size * cached_thumbnail_size / ATHENA_ICON_SIZE_STANDARD;
		DEBUG ("Modifying icon size to %d, as our cached thumbnail size is %d",
		       modified_size, cached_thumbnail_size);
	}

	if (flags & ATHENA_FILE_ICON_FLAGS_USE_THUMBNAILS &&
	    athena_file_should_show_thumbnail (file)) {
		if (file->details->thumbnail) {
			int w, h, s;
			double scale;
			...
			if (s <= cached_thumbnail_size) {
				scale = (double)size / ATHENA_ICON_SIZE_STANDARD;
			}
			else {
				scale = (double)modified_size / s;
			}
			/* Make sure that icons don't get smaller than ATHENA_ICON_SIZE_SMALLEST */
			if (s*scale <= ATHENA_ICON_SIZE_SMALLEST) {
				scale = (double) ATHENA_ICON_SIZE_SMALLEST / s;
			}
			...
			if (!gdk_pixbuf_get_has_alpha(raw_pixbuf) || s >= 128) {
				athena_thumbnail_frame_image (&scaled_pixbuf);
			}
			...
			if (modified_size > 128*1.25 &&
			    ...
			    athena_can_thumbnail_internally (file)) {
				/* Invalidate if we resize upward */
				file->details->thumbnail_wants_original = TRUE;
				athena_file_invalidate_attributes (file, ATHENA_FILE_ATTRIBUTE_THUMBNAIL);
			}

			DEBUG ("Returning thumbnailed image, at size %d %d",
			       (int) (w * scale), (int) (h * scale));
			
			icon = athena_icon_info_new_for_pixbuf (scaled_pixbuf);
			...
			if (athena_can_thumbnail (file)) {
				athena_create_thumbnail (file);
			}
		}
	}
	...
	if (file->details->is_thumbnailing &&
		...
		gicon = athena_file_get_gicon (file, flags);
	
	if (gicon) {
		icon = athena_icon_info_lookup (gicon, size);
		if (athena_icon_info_is_fallback (icon)) {
			g_object_unref (icon);
			icon = athena_icon_info_lookup (get_default_file_icon (flags), size);
		}
		g_object_unref (gicon);
		return icon;
	} else {
		return athena_icon_info_lookup (get_default_file_icon (flags), size);
	}
}

GdkPixbuf *
athena_file_get_icon_pixbuf (AthenaFile *file,
			       int size,
			       gboolean force_size,
			       AthenaFileIconFlags flags)
{
	...
	info = athena_file_get_icon (file, size, flags);
	if (force_size) {
		pixbuf =  athena_icon_info_get_pixbuf_at_size (info, size);
	} else {
		pixbuf = athena_icon_info_get_pixbuf (info);
	}
	g_object_unref (info);
...
gboolean
athena_file_get_date (AthenaFile *file,
			...
			time_t *date)
{
	...
	g_return_val_if_fail (date_type == ATHENA_DATE_TYPE_CHANGED
			      || date_type == ATHENA_DATE_TYPE_ACCESSED
			      || date_type == ATHENA_DATE_TYPE_MODIFIED
			      || date_type == ATHENA_DATE_TYPE_TRASHED
			      || date_type == ATHENA_DATE_TYPE_PERMISSIONS_CHANGED, FALSE);
	...
	return ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->get_date (file, date_type, date);
}

static char *
athena_file_get_where_string (AthenaFile *file)
{
	if (file == NULL) {
		return NULL;
	}
	...
	return ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->get_where_string (file);
}
...
static const char *TODAY_TIME_FORMATS [] = {
	 ...
	 * in the Athena version of strftime that can be
...
static char *
athena_file_fit_date_as_string (AthenaFile *file,
				  AthenaDateType date_type,
				  int width,
				  ...
				  void *measure_context)
{
	...
	if (!athena_file_get_date (file, date_type, &file_time_raw)) {
		return NULL;
	}

	file_time = localtime (&file_time_raw);
	date_format_pref = g_settings_get_enum (athena_preferences,
						ATHENA_PREFERENCES_DATE_FORMAT);

	if (date_format_pref == ATHENA_DATE_FORMAT_LOCALE) {
		return eel_strdup_strftime ("%c", file_time);
	} else if (date_format_pref == ATHENA_DATE_FORMAT_ISO) {
 ...
 * @file: AthenaFile representing the file in question.
 ...
 **/
char *
athena_file_fit_modified_date_as_string (AthenaFile *file,
					   int width,
					   AthenaWidthMeasureCallback measure_callback,
					   AthenaTruncateCallback truncate_callback,
					   void *measure_context)
{
	return athena_file_fit_date_as_string (file, ATHENA_DATE_TYPE_MODIFIED,
		width, measure_callback, truncate_callback, measure_context);
}

static char *
athena_file_get_trash_original_file_parent_as_string (AthenaFile *file)
{
	AthenaFile *orig_file, *parent;
	GFile *location;
	char *filename;

	if (file->details->trash_orig_path != NULL) {
		orig_file = athena_file_get_trash_original_file (file);
		parent = athena_file_get_parent (orig_file);
		location = athena_file_get_location (parent);

		filename = g_file_get_parse_name (location);

		g_object_unref (location);
		athena_file_unref (parent);
		athena_file_unref (orig_file);
 ...
 **/
static char *
athena_file_get_date_as_string (AthenaFile *file, AthenaDateType date_type)
{
	return athena_file_fit_date_as_string (file, date_type,
		0, NULL, NULL, NULL);
}

static AthenaSpeedTradeoffValue show_directory_item_count;
static AthenaSpeedTradeoffValue show_text_in_icons;

static void
show_text_in_icons_changed_callback (gpointer callback_data)
{
	show_text_in_icons = g_settings_get_enum (athena_preferences, ATHENA_PREFERENCES_SHOW_TEXT_IN_ICONS);
}

static void
show_directory_item_count_changed_callback (gpointer callback_data)
{
	show_directory_item_count = g_settings_get_enum (athena_preferences, ATHENA_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}

static gboolean
get_speed_tradeoff_preference_for_file (AthenaFile *file, AthenaSpeedTradeoffValue value)
{
	GFilesystemPreviewType use_preview;
	...
	if (value == ATHENA_SPEED_TRADEOFF_ALWAYS) {
		if (use_preview == G_FILESYSTEM_PREVIEW_TYPE_NEVER) {
			return FALSE;
		} else {
			return TRUE;
	...
	if (value == ATHENA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}

	g_assert (value == ATHENA_SPEED_TRADEOFF_LOCAL_ONLY);
...
gboolean
athena_file_should_show_directory_item_count (AthenaFile *file)
{
...
gboolean
athena_file_should_show_type (AthenaFile *file)
{
	char *uri;
	gboolean ret;
...
gboolean
athena_file_should_get_top_left_text (AthenaFile *file)
{
	...
	if (show_text_in_icons == ATHENA_SPEED_TRADEOFF_ALWAYS) {
		return TRUE;
	}
	
	if (show_text_in_icons == ATHENA_SPEED_TRADEOFF_NEVER) {
		return FALSE;
	}
 ...
 * @file: AthenaFile representing a directory.
 ...
 **/
gboolean
athena_file_get_directory_item_count (AthenaFile *file, 
					guint *count,
					gboolean *count_unreadable)
{
	...
	if (!athena_file_should_show_directory_item_count (file)) {
		return FALSE;
	}

	return ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->get_item_count 
 ...
 * @file: AthenaFile representing a directory or file.
 ...
 * athena_file_should_show_directory_item_count returns FALSE
 * for this file.
 * 
 ...
 **/
AthenaRequestStatus
athena_file_get_deep_counts (AthenaFile *file,
			       ...
			       gboolean force)
{
	...
	g_return_val_if_fail (ATHENA_IS_FILE (file), ATHENA_REQUEST_DONE);

	if (!force && !athena_file_should_show_directory_item_count (file)) {
		/* Set field so an existing value isn't treated as up-to-date
		 * when preference changes later.
		 */
		file->details->deep_counts_status = ATHENA_REQUEST_NOT_STARTED;
		return file->details->deep_counts_status;
	}

	return ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->get_deep_counts 
		(file, directory_count, file_count,
		 unreadable_directory_count, total_size);
}

void
athena_file_recompute_deep_counts (AthenaFile *file)
{
	if (file->details->deep_counts_status != ATHENA_REQUEST_IN_PROGRESS) {
		file->details->deep_counts_status = ATHENA_REQUEST_NOT_STARTED;
		if (file->details->directory != NULL) {
			athena_directory_add_file_to_work_queue (file->details->directory, file);
			athena_directory_async_state_changed (file->details->directory);
		}
	}
}
 ...
 * @file: AthenaFile representing a directory. It is an error to
 ...
 **/
gboolean
athena_file_get_directory_item_mime_types (AthenaFile *file,
					     GList **mime_list)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_can_get_size (AthenaFile *file)
{
 ...
 **/
goffset
athena_file_get_size (AthenaFile *file)
{
...
time_t
athena_file_get_mtime (AthenaFile *file)
{
				  ...
				  gpointer callback_data)
{
	...
	new_info = g_file_query_info_finish (G_FILE (source_object), res, &error);
	if (new_info != NULL) {
		if (athena_file_update_info (op->file, new_info)) {
			athena_file_changed (op->file);
		}
		g_object_unref (new_info);
	}
...
void
athena_file_set_attributes (AthenaFile *file, 
 ...
 */
gboolean
athena_file_can_get_permissions (AthenaFile *file)
{
 ...
 */
gboolean
athena_file_can_set_permissions (AthenaFile *file)
{
	uid_t user_id;

	if (file->details->uid != -1 &&
	    athena_file_is_local (file)) {
		/* Check the user. */
		user_id = geteuid();
...
guint
athena_file_get_permissions (AthenaFile *file)
{
	g_return_val_if_fail (athena_file_can_get_permissions (file), 0);
 ...
 * athena_file_can_set_permissions returned TRUE.
 * 
 * @file: AthenaFile representing the file in question.
 ...
 **/
void
athena_file_set_permissions (AthenaFile *file, 
			       ...
			       gpointer callback_data)
{
	...
	if (!athena_file_can_set_permissions (file)) {
	...
	if (!athena_file_undo_manager_pop_flag ()) {
		...
		undo_info = athena_file_undo_info_permissions_new (athena_file_get_location (file),
								     file->details->permissions,
								     new_permissions);
		athena_file_undo_manager_set_action (undo_info);
	}

	info = g_file_info_new ();
	g_file_info_set_attribute_uint32 (info, G_FILE_ATTRIBUTE_UNIX_MODE, new_permissions);
	athena_file_set_attributes (file, info, callback, callback_data);
	
	g_object_unref (info);
}

/**
 * athena_file_can_get_selinux_context:
 * 
 ...
 */
gboolean
athena_file_can_get_selinux_context (AthenaFile *file)
{
...
/**
 * athena_file_get_selinux_context:
 * 
 ...
 **/
char *
athena_file_get_selinux_context (AthenaFile *file)
{
	char *translated;
	char *raw;
	...
	if (!athena_file_can_get_selinux_context (file)) {
		return NULL;
	}
 ...
 */
gboolean
athena_file_can_get_owner (AthenaFile *file)
{
 ...
 */
char *
athena_file_get_owner_name (AthenaFile *file)
{
	return athena_file_get_owner_as_string (file, FALSE);
}

/**
 * athena_file_can_set_owner:
 * 
 * Check whether the current user is allowed to change
 * the owner of a file.
 * 
...
/**
 * athena_file_set_owner:
 * 
 * Set the owner of a file. This will only have any effect if
 * athena_file_can_set_owner returns TRUE.
 * 
 * @file: The file in question.
 ...
 */
void
athena_file_set_owner (AthenaFile *file, 
			 ...
			 gpointer callback_data)
{
	...
	if (!athena_file_can_set_owner (file)) {
	...
	if (!athena_file_undo_manager_pop_flag ()) {
		...
		current_owner = athena_file_get_owner_as_string (file, FALSE);

		undo_info = athena_file_undo_info_ownership_new (ATHENA_FILE_UNDO_OP_CHANGE_OWNER,
								   athena_file_get_location (file),
								   current_owner,
								   user_name_or_id);
		athena_file_undo_manager_set_action (undo_info);
 ...
 */
gboolean
athena_file_can_get_group (AthenaFile *file)
{
 ...
 **/
char *
athena_file_get_group_name (AthenaFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->group));
}

/**
 * athena_file_can_set_group:
 * 
 * Check whether the current user is allowed to change
 * the group of a file.
 * 
 ...
 */
gboolean
athena_file_can_set_group (AthenaFile *file)
{
	uid_t user_id;
	 ...
	 * systems.
	 */
	if (!athena_file_can_get_group (file)) {
		return FALSE;
	}
 ...
 */
static GList *
athena_get_group_names_for_user (void)
{
	GList *list;
	struct group *group;
	int count, i;
...
/**
 * athena_get_group_names:
 * 
 * Get a list of all group names.
 */
GList *
athena_get_all_group_names (void)
{
	GList *list;
	struct group *group;
 ...
 * 
 * @file: The AthenaFile in question.
 */
GList *
athena_file_get_settable_group_names (AthenaFile *file)
{
	uid_t user_id;
	GList *result;

	if (!athena_file_can_set_group (file)) {
		return NULL;
	}	
	...
	if (user_id == 0) {
		...
		result = athena_get_all_group_names ();
	} else if (user_id == (uid_t) file->details->uid) {
		/* Owner is allowed to set group to any that owner is member of. */
		result = athena_get_group_names_for_user ();
	} else {
		g_warning ("unhandled case in athena_get_settable_group_names");
		result = NULL;
	}
 ...
 * athena_file_can_set_group returns TRUE.
 * 
 * @file: The file in question.
 ...
 */
void
athena_file_set_group (AthenaFile *file, 
			 ...
			 gpointer callback_data)
{
	...
	if (!athena_file_undo_manager_pop_flag ()) {
		...
		current_group = athena_file_get_group_name (file);
		undo_info = athena_file_undo_info_ownership_new (ATHENA_FILE_UNDO_OP_CHANGE_GROUP,
								   athena_file_get_location (file),
								   current_group,
								   group_name_or_id);
		athena_file_undo_manager_set_action (undo_info);
 ...
 **/
static char *
athena_file_get_octal_permissions_as_string (AthenaFile *file)
{
	guint32 permissions;

	g_assert (ATHENA_IS_FILE (file));

	if (!athena_file_can_get_permissions (file)) {
		return NULL;
	}
 ...
 **/
static char *
athena_file_get_permissions_as_string (AthenaFile *file)
{
	guint32 permissions;
	...
	permissions = file->details->permissions;
	is_directory = athena_file_is_directory (file);
	is_link = athena_file_is_symbolic_link (file);
 ...
 **/
static char *
athena_file_get_owner_as_string (AthenaFile *file, gboolean include_real_name)
{
	char *user_name;
 ...
 **/
static char *
athena_file_get_size_as_string (AthenaFile *file)
{
	guint item_count;
	gboolean count_unreadable;
	...
	if (athena_file_is_directory (file)) {
		if (!athena_file_get_directory_item_count (file, &item_count, &count_unreadable)) {
			return NULL;
		}
 ...
 **/
static char *
athena_file_get_size_as_string_with_real_size (AthenaFile *file)
{
	guint item_count;
	gboolean count_unreadable;
...
static char *
athena_file_get_deep_count_as_string_internal (AthenaFile *file,
						 gboolean report_size,
						 gboolean report_directory_count,
						 gboolean report_file_count)
{
	AthenaRequestStatus status;
	guint directory_count;
	guint file_count;
	...
	g_assert (ATHENA_IS_FILE (file));
	g_assert (athena_file_is_directory (file));

	status = athena_file_get_deep_counts 
		(file, &directory_count, &file_count, &unreadable_count, &total_size, FALSE);

	/* Check whether any info is available. */
	if (status == ATHENA_REQUEST_NOT_STARTED) {
		return NULL;
	}
	 ...
	 * Callers can distinguish them using athena_file_get_deep_counts
	 * directly if desired.
	 */
	if (report_size) {
 ...
 **/
static char *
athena_file_get_deep_size_as_string (AthenaFile *file)
{
	return athena_file_get_deep_count_as_string_internal (file, TRUE, FALSE, FALSE);
}
 ...
 **/
static char *
athena_file_get_deep_total_count_as_string (AthenaFile *file)
{
	return athena_file_get_deep_count_as_string_internal (file, FALSE, TRUE, TRUE);
}
 ...
 **/
static char *
athena_file_get_deep_file_count_as_string (AthenaFile *file)
{
	return athena_file_get_deep_count_as_string_internal (file, FALSE, FALSE, TRUE);
}
 ...
 **/
static char *
athena_file_get_deep_directory_count_as_string (AthenaFile *file)
{
	return athena_file_get_deep_count_as_string_internal (file, FALSE, TRUE, FALSE);
}
 ...
 * athena_file_get_string_attribute_with_default if you want a non-NULL
 * default.
 * 
 * @file: AthenaFile representing the file in question.
 ...
 **/
char *
athena_file_get_string_attribute_q (AthenaFile *file, GQuark attribute_q)
{
	char *extension_attribute;

	if (attribute_q == attribute_name_q) {
		return athena_file_get_display_name (file);
	}
	if (attribute_q == attribute_type_q) {
		return athena_file_get_type_as_string (file);
	}
	if (attribute_q == attribute_mime_type_q) {
		return athena_file_get_mime_type (file);
	}
	if (attribute_q == attribute_size_q) {
		return athena_file_get_size_as_string (file);
	}
	if (attribute_q == attribute_size_detail_q) {
		return athena_file_get_size_as_string_with_real_size (file);
	}
	if (attribute_q == attribute_deep_size_q) {
		return athena_file_get_deep_size_as_string (file);
	}
	if (attribute_q == attribute_deep_file_count_q) {
		return athena_file_get_deep_file_count_as_string (file);
	}
	if (attribute_q == attribute_deep_directory_count_q) {
		return athena_file_get_deep_directory_count_as_string (file);
	}
	if (attribute_q == attribute_deep_total_count_q) {
		return athena_file_get_deep_total_count_as_string (file);
	}
	if (attribute_q == attribute_trash_orig_path_q) {
		return athena_file_get_trash_original_file_parent_as_string (file);
	}
	if (attribute_q == attribute_date_modified_q) {
		return athena_file_get_date_as_string (file, 
	...
	if (attribute_q == attribute_permissions_q) {
		return athena_file_get_permissions_as_string (file);
	}
	if (attribute_q == attribute_selinux_context_q) {
		return athena_file_get_selinux_context (file);
	}
	if (attribute_q == attribute_octal_permissions_q) {
		return athena_file_get_octal_permissions_as_string (file);
	}
	if (attribute_q == attribute_owner_q) {
		return athena_file_get_owner_as_string (file, TRUE);
	}
	if (attribute_q == attribute_group_q) {
		return athena_file_get_group_name (file);
	}
	if (attribute_q == attribute_uri_q) {
		return athena_file_get_uri (file);
	}
	if (attribute_q == attribute_where_q) {
		return athena_file_get_where_string (file);
	}
	if (attribute_q == attribute_link_target_q) {
		return athena_file_get_symbolic_link_target_path (file);
	}
	if (attribute_q == attribute_volume_q) {
		return athena_file_get_volume_name (file);
	}
	if (attribute_q == attribute_free_space_q) {
		return athena_file_get_volume_free_space (file);
	}
...
char *
athena_file_get_string_attribute (AthenaFile *file, const char *attribute_name)
{
	return athena_file_get_string_attribute_q (file, g_quark_from_string (attribute_name));
}
 ...
 * athena_file_get_string_attribute if you want NULL instead of a default
 * result.
 * 
 * @file: AthenaFile representing the file in question.
 * @attribute_name: The name of the desired attribute. See the description of
 * athena_file_get_string for the set of available attributes.
 * 
 ...
 **/
char *
athena_file_get_string_attribute_with_default_q (AthenaFile *file, GQuark attribute_q)
{
	char *result;
	guint item_count;
	gboolean count_unreadable;
	...
	result = athena_file_get_string_attribute_q (file, attribute_q);
	if (result != NULL) {
		return result;
	}
	...
	if (attribute_q == attribute_size_q) {
		...
		count_unreadable = FALSE;
		if (athena_file_is_directory (file)) {
			athena_file_get_directory_item_count (file, &item_count, &count_unreadable);
		}
		return g_strdup (count_unreadable ? _("? items") : "...");
	}
	if (attribute_q == attribute_deep_size_q) {
		status = athena_file_get_deep_counts (file, NULL, NULL, NULL, NULL, FALSE);
		if (status == ATHENA_REQUEST_DONE) {
...
char *
athena_file_get_string_attribute_with_default (AthenaFile *file, const char *attribute_name)
{
	return athena_file_get_string_attribute_with_default_q (file, g_quark_from_string (attribute_name));
}

gboolean
athena_file_is_date_sort_attribute_q (GQuark attribute_q)
{
 ...
 **/
static char *
get_description (AthenaFile *file)
{
	const char *mime_type;
	char *description;
	...
	if (g_content_type_is_unknown (mime_type) &&
	    athena_file_is_executable (file)) {
...
static char *
update_description_for_link (AthenaFile *file, char *string)
{
	char *res;
	
	if (athena_file_is_symbolic_link (file)) {
		g_assert (!athena_file_is_broken_symbolic_link (file));
		if (string == NULL) {
...
static char *
athena_file_get_type_as_string (AthenaFile *file)
{
	if (file == NULL) {
		return NULL;
	}

	if (athena_file_is_broken_symbolic_link (file)) {
 ...
 **/
GFileType
athena_file_get_file_type (AthenaFile *file)
{
	if (file == NULL) {
 ...
 **/
char *
athena_file_get_mime_type (AthenaFile *file)
{
	if (file != NULL) {
		g_return_val_if_fail (ATHENA_IS_FILE (file), NULL);
 ...
 **/
gboolean
athena_file_is_mime_type (AthenaFile *file, const char *mime_type)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
gboolean
athena_file_is_launchable (AthenaFile *file)
{
	...
	return type_can_be_executable &&
		athena_file_can_get_permissions (file) &&
		athena_file_can_execute (file) &&
		athena_file_is_executable (file) &&
		!athena_file_is_directory (file);
}
 ...
 **/
GList *
athena_file_get_emblem_icons (AthenaFile *file,
				char **exclude)
{
	GList *keywords, *l;
	GList *icons;
	...
	keywords = athena_file_get_keywords (file);
...
#ifdef TRASH_IS_FAST_ENOUGH
		if (strcmp (keyword, ATHENA_FILE_EMBLEM_NAME_TRASH) == 0) {
			char *uri;
 ...
 **/
GList *
athena_file_get_keywords (AthenaFile *file)
{
	GList *keywords;
	...
	keywords = g_list_concat (keywords, athena_file_get_metadata_list (file, ATHENA_METADATA_KEY_EMBLEMS));
 ...
 **/
gboolean
athena_file_is_symbolic_link (AthenaFile *file)
{
	return file->details->is_symlink;
}

gboolean
athena_file_is_mountpoint (AthenaFile *file)
{
	return file->details->is_mountpoint;
}

GMount *
athena_file_get_mount (AthenaFile *file)
{
	if (file->details->mount) {
		      ...
		      gpointer data)
{
	...
	athena_file_invalidate_attributes (file, ATHENA_FILE_ATTRIBUTE_MOUNT);
}

void
athena_file_set_mount (AthenaFile *file,
			 GMount *mount)
{
	if (file->details->mount) {
		g_signal_handlers_disconnect_by_func (file->details->mount, file_mount_unmounted, file);
...
/**
 * athena_file_is_broken_symbolic_link
 * 
 ...
 **/
gboolean
athena_file_is_broken_symbolic_link (AthenaFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return athena_file_get_file_type (file) == G_FILE_TYPE_SYMBOLIC_LINK;
}
 ...
 */
char *
athena_file_get_volume_free_space (AthenaFile *file)
{
	GFile *location;
	char *res;
	time_t now;
 ...
 */ 
char *
athena_file_get_volume_name (AthenaFile *file)
{
	GFile *location;
	char *res;
	GMount *mount;
...
/**
 * athena_file_get_symbolic_link_target_path
 * 
 ...
 * @file: AthenaFile representing the symbolic link in question.
 * 
 * Returns: newly-allocated copy of the file path of the target of the symbolic link.
 */
char *
athena_file_get_symbolic_link_target_path (AthenaFile *file)
{
	if (!athena_file_is_symbolic_link (file)) {
 ...
 */
char *
athena_file_get_symbolic_link_target_uri (AthenaFile *file)
{
 ...
 * 
 * Check if this file is a "athena link", meaning a historical
 * athena xml link file or a desktop file.
 * @file: AthenaFile representing the file in question.
 * 
 * Returns: True if the file is a athena link.
 * 
 **/
gboolean
athena_file_is_athena_link (AthenaFile *file)
{
	if (file->details->mime_type == NULL) {
		return FALSE;
	}
 ...
 **/
gboolean
athena_file_is_directory (AthenaFile *file)
{
	return athena_file_get_file_type (file) == G_FILE_TYPE_DIRECTORY;
}

/**
 * athena_file_is_user_special_directory
 *
 ...
 */
gboolean
athena_file_is_user_special_directory (AthenaFile *file,
...
gboolean
athena_file_is_archive (AthenaFile *file)
{
	char *mime_type;
	int i;
	...
	mime_type = athena_file_get_mime_type (file);
 ...
 **/
gboolean
athena_file_is_in_trash (AthenaFile *file)
{
	g_assert (ATHENA_IS_FILE (file));

	return athena_directory_is_in_trash (file->details->directory);
}

GError *
athena_file_get_file_info_error (AthenaFile *file)
{
	if (!file->details->get_info_failed) {
		return NULL;
	}
 ...
 **/
gboolean
athena_file_contains_text (AthenaFile *file)
{
	if (file == NULL) {
		return FALSE;
	}
	...
	return athena_file_is_mime_type (file, "text/plain");
}
 ...
 **/
gboolean
athena_file_is_executable (AthenaFile *file)
{
 ...
 **/
char *
athena_file_peek_top_left_text (AthenaFile *file,
				  gboolean  need_large_text,
				  gboolean *needs_loading)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), NULL);

	if (!athena_file_should_get_top_left_text (file)) {
		if (needs_loading) {
			*needs_loading = FALSE;
		}
		return NULL;
	...
	if (!file->details->got_top_left_text) {
		...
		if (athena_file_contains_text (file)) {
			return " ...";
		}
		return NULL;
	}
 ...
 **/
char *
athena_file_get_top_left_text (AthenaFile *file)
{
	return g_strdup (athena_file_peek_top_left_text (file, FALSE, NULL));
}

char *
athena_file_get_filesystem_id (AthenaFile *file)
{
	return g_strdup (eel_ref_str_peek (file->details->filesystem_id));
}

AthenaFile *
athena_file_get_trash_original_file (AthenaFile *file)
{
	GFile *location;
	...
	if (file->details->trash_orig_path != NULL) {
		...
		original_file = athena_file_get (location);
...
void
athena_file_mark_gone (AthenaFile *file)
{
	AthenaDirectory *directory;
 ...
 **/
void
athena_file_changed (AthenaFile *file)
{
	GList fake_list;
...
/**
 * athena_file_updated_deep_count_in_progress
 * 
 * Notify clients that a newer deep count is available for
 * the directory in question.
 */
void
athena_file_updated_deep_count_in_progress (AthenaFile *file) {
	...
	for (node = link_files; node != NULL; node = node->next) {
		athena_file_updated_deep_count_in_progress (ATHENA_FILE (node->data));
	}
	athena_file_list_free (link_files);	
}
 ...
 **/
void
athena_file_emit_changed (AthenaFile *file)
{
	GList *link_files, *p;
	...
	for (p = link_files; p != NULL; p = p->next) {
		if (p->data != file) {
			athena_file_changed (ATHENA_FILE (p->data));
		}
	}
	athena_file_list_free (link_files);
}
 ...
 **/
gboolean
athena_file_is_gone (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
/**
 * athena_file_is_not_yet_confirmed
 * 
 ...
 **/
gboolean
athena_file_is_not_yet_confirmed (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
 ...
 */
gboolean
athena_file_check_if_ready (AthenaFile *file,
			      AthenaFileAttributes file_attributes)
{
	...
	return ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->check_if_ready (file, file_attributes);
}			      

void
athena_file_call_when_ready (AthenaFile *file,
			       AthenaFileAttributes file_attributes,
...

{
	...
	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->call_when_ready
		(file, file_attributes, callback, callback_data);
}

void
athena_file_cancel_call_when_ready (AthenaFile *file,
				      AthenaFileCallback callback,
				      gpointer callback_data)
{
	g_return_if_fail (callback != NULL);
	...
	ATHENA_FILE_CLASS (G_OBJECT_GET_CLASS (file))->cancel_call_when_ready
		(file, callback, callback_data);
}

static void
invalidate_directory_count (AthenaFile *file)
{
	file->details->directory_count_is_up_to_date = FALSE;
}

static void
invalidate_deep_counts (AthenaFile *file)
{
	file->details->deep_counts_status = ATHENA_REQUEST_NOT_STARTED;
}

static void
invalidate_mime_list (AthenaFile *file)
{
	file->details->mime_list_is_up_to_date = FALSE;
}

static void
invalidate_top_left_text (AthenaFile *file)
{
	file->details->top_left_text_is_up_to_date = FALSE;
}

static void
invalidate_file_info (AthenaFile *file)
{
	file->details->file_info_is_up_to_date = FALSE;
}

static void
invalidate_link_info (AthenaFile *file)
{
	file->details->link_info_is_up_to_date = FALSE;
}

static void
invalidate_thumbnail (AthenaFile *file)
{
	file->details->thumbnail_is_up_to_date = FALSE;
}

static void
invalidate_mount (AthenaFile *file)
{
	file->details->mount_is_up_to_date = FALSE;
}

void
athena_file_invalidate_extension_info_internal (AthenaFile *file)
{
	if (file->details->pending_info_providers)
		g_list_free_full (file->details->pending_info_providers, g_object_unref);

	file->details->pending_info_providers =
		athena_module_get_extensions_for_type (ATHENA_TYPE_INFO_PROVIDER);
}

void
athena_file_invalidate_attributes_internal (AthenaFile *file,
					      AthenaFileAttributes file_attributes)
{
	Request request;
	...
	request = athena_directory_set_up_request (file_attributes);
...
gboolean
athena_file_has_open_window (AthenaFile *file)
{
	return file->details->has_open_window;
}

void
athena_file_set_has_open_window (AthenaFile *file,
...
gboolean
athena_file_is_thumbnailing (AthenaFile *file)
{
	g_return_val_if_fail (ATHENA_IS_FILE (file), FALSE);
...
void
athena_file_set_is_thumbnailing (AthenaFile *file,
...
void
athena_file_invalidate_attributes (AthenaFile *file,
				     AthenaFileAttributes file_attributes)
{
	/* Cancel possible in-progress loads of any of these attributes */
	athena_directory_cancel_loading_file_attributes (file->details->directory,
							   file,
							   file_attributes);
	
	/* Actually invalidate the values */
	athena_file_invalidate_attributes_internal (file, file_attributes);
...
athena_file_get_all_attributes (void)
{
	return  ATHENA_FILE_ATTRIBUTE_INFO |
		ATHENA_FILE_ATTRIBUTE_LINK_INFO |
		ATHENA_FILE_ATTRIBUTE_DEEP_COUNTS |
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT | 
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES | 
		ATHENA_FILE_ATTRIBUTE_TOP_LEFT_TEXT | 
		ATHENA_FILE_ATTRIBUTE_LARGE_TOP_LEFT_TEXT |
		ATHENA_FILE_ATTRIBUTE_EXTENSION_INFO |
		ATHENA_FILE_ATTRIBUTE_THUMBNAIL |
		ATHENA_FILE_ATTRIBUTE_MOUNT;
}

void
athena_file_invalidate_all_attributes (AthenaFile *file)
{
	AthenaFileAttributes all_attributes;

	all_attributes = athena_file_get_all_attributes ();
	athena_file_invalidate_attributes (file, all_attributes);
}
 ...
 **/
void
athena_file_dump (AthenaFile *file)
{
...
/**
 * athena_file_list_ref
 *
 * Ref all the files in a list.
 * @list: GList of files.
 **/
GList *
athena_file_list_ref (GList *list)
{
	g_list_foreach (list, (GFunc) athena_file_ref, NULL);
	return list;
}
 ...
 **/
void
athena_file_list_unref (GList *list)
{
	g_list_foreach (list, (GFunc) athena_file_unref, NULL);
}

/**
 * athena_file_list_free
 *
 * Free a list of files after unrefing them.
 * @list: GList of files.
 **/
void
athena_file_list_free (GList *list)
{
	athena_file_list_unref (list);
	g_list_free (list);
}

/**
 * athena_file_list_copy
 *
 * Copy the list of files, making a new ref of each,
 * @list: GList of files.
 **/
GList *
athena_file_list_copy (GList *list)
{
	return g_list_copy (athena_file_list_ref (list));
}

GList *
athena_file_list_from_uris (GList *uri_list)
{
	GList *l, *file_list;
	const char *uri;
	GFile *file;
...
static gboolean
get_attributes_for_default_sort_type (AthenaFile *file,
				      ...
				      gboolean *is_trash)
{
	...
	*is_trash = FALSE;
	...
	if (file && athena_file_is_directory (file)) {
		is_download_dir =
			athena_file_is_user_special_directory (file, G_USER_DIRECTORY_DOWNLOAD);
		is_desktop_dir =
			athena_file_is_user_special_directory (file, G_USER_DIRECTORY_DESKTOP);
		is_trash_dir =
			athena_file_is_in_trash (file);
...
AthenaFileSortType
athena_file_get_default_sort_type (AthenaFile *file,
				     gboolean *reversed)
{
	AthenaFileSortType retval;
	gboolean is_download, is_trash, res;

	retval = ATHENA_FILE_SORT_NONE;
	is_download = is_trash = FALSE;
	res = get_attributes_for_default_sort_type (file, &is_download, &is_trash);

	if (res) {
		if (is_download) {
			retval = ATHENA_FILE_SORT_BY_MTIME;
		} else if (is_trash) {
			retval = ATHENA_FILE_SORT_BY_TRASHED_TIME;
		}
...
const gchar *
athena_file_get_default_sort_attribute (AthenaFile *file,
					  gboolean *reversed)
{
	const gchar *retval;
	gboolean is_download, is_trash, res;
...
{
	return compare_by_display_name (ATHENA_FILE (a), ATHENA_FILE (b));
}
 ...
 **/
GList *
athena_file_list_sort_by_display_name (GList *list)
{
...
file_list_ready_data_free (FileListReadyData *data)
{
	...
	if (l != NULL) {
		...
		athena_file_list_free (data->file_list);
			  ...
			  gpointer callback_data)
{
	...
	data->file_list = athena_file_list_copy (file_list);
...
static void
file_list_file_ready_callback (AthenaFile *file,
			       gpointer user_data)
{
	FileListReadyData *data;
...
void 
athena_file_list_call_when_ready (GList *file_list,
				    AthenaFileAttributes attributes,
				    ...
				    gpointer callback_data)
{
	...
	if (handle) {
		*handle = (AthenaFileListHandle *) data;
	}
	...
	while (l != NULL) {
		...
		l = l->next; 
		athena_file_call_when_ready (file,
					       attributes,
...
void
athena_file_list_cancel_call_when_ready (AthenaFileListHandle *handle)
{
	GList *l;
	AthenaFile *file;
	FileListReadyData *data;
...
char *
athena_extract_top_left_text (const char *text,
				gboolean large,
				int length)
{
        GString* buffer;
	...
	if (large) {
		max_bytes = ATHENA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = ATHENA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = ATHENA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	} else {
		max_bytes = ATHENA_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES;
		max_lines = ATHENA_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
		max_cols = ATHENA_FILE_TOP_LEFT_TEXT_MAXIMUM_CHARACTERS_PER_LINE;
	}
...
{
	g_settings_get (athena_preferences,
			ATHENA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
...
{
	cached_thumbnail_size = g_settings_get_int (athena_icon_view_preferences,
						    ATHENA_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE);
...
{
	show_image_thumbs = g_settings_get_enum (athena_preferences, ATHENA_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS);
...
static void
real_set_metadata (AthenaFile  *file,
		   const char    *key,
		   const char    *value)
{
	/* Dummy default impl */
...
static void
real_set_metadata_as_list (AthenaFile *file,
			   const char   *key,
			   char         **value)
{
	/* Dummy default impl */
...
static void
athena_file_class_init (AthenaFileClass *class)
{
	GtkIconTheme *icon_theme;

	athena_file_info_getter = athena_file_get_internal;
	...
	G_OBJECT_CLASS (class)->constructor = athena_file_constructor;
	...
	signals[CHANGED] =
		g_signal_new ("changed",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaFileClass, changed),
		              NULL, NULL,
	...
	signals[UPDATED_DEEP_COUNT_IN_PROGRESS] =
		g_signal_new ("updated_deep_count_in_progress",
		              G_TYPE_FROM_CLASS (class),
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (AthenaFileClass, updated_deep_count_in_progress),
		              NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
		              G_TYPE_NONE, 0);

	g_type_class_add_private (class, sizeof (AthenaFileDetails));

	thumbnail_limit_changed_callback (NULL);
	g_signal_connect_swapped (athena_preferences,
				  "changed::" ATHENA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
				  G_CALLBACK (thumbnail_limit_changed_callback),
				  NULL);
	thumbnail_size_changed_callback (NULL);
	g_signal_connect_swapped (athena_preferences,
				  "changed::" ATHENA_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE,
	...
	g_signal_connect (athena_signaller_get_current (),
			  "mime_data_changed",
...
static void
athena_file_add_emblem (AthenaFile *file,
...
static void
athena_file_add_string_attribute (AthenaFile *file,
...
static void
athena_file_invalidate_extension_info (AthenaFile *file)
{
	athena_file_invalidate_attributes (file, ATHENA_FILE_ATTRIBUTE_EXTENSION_INFO);
}

void
athena_file_info_providers_done (AthenaFile *file)
{
	g_list_free_full (file->details->extension_emblems, g_free);
...
static void     
athena_file_info_iface_init (AthenaFileInfoIface *iface)
{
	iface->is_gone = athena_file_is_gone;
	iface->get_name = athena_file_get_name;
	iface->get_file_type = athena_file_get_file_type;
	iface->get_location = athena_file_get_location;
	iface->get_uri = athena_file_get_uri;
	iface->get_parent_location = athena_file_get_parent_location;
	iface->get_parent_uri = athena_file_get_parent_uri;
	iface->get_parent_info = athena_file_get_parent;
	iface->get_mount = athena_file_get_mount;
	iface->get_uri_scheme = athena_file_get_uri_scheme;
	iface->get_activation_uri = athena_file_get_activation_uri;
	iface->get_mime_type = athena_file_get_mime_type;
	iface->is_mime_type = athena_file_is_mime_type;
	iface->is_directory = athena_file_is_directory;
	iface->can_write = athena_file_can_write;
	iface->add_emblem = athena_file_add_emblem;
	iface->get_string_attribute = athena_file_get_string_attribute;
	iface->add_string_attribute = athena_file_add_string_attribute;
	iface->invalidate_extension_info = athena_file_invalidate_extension_info;
}

#if !defined (ATHENA_OMIT_SELF_CHECK)

void
athena_self_check_file (void)
{
	AthenaFile *file_1;
	AthenaFile *file_2;
	GList *list;
        ...
        EEL_CHECK_INTEGER_RESULT (athena_directory_number_outstanding (), 0);

	file_1 = athena_file_get_by_uri ("file:///home/");
	...
	EEL_CHECK_INTEGER_RESULT (G_OBJECT (file_1->details->directory)->ref_count, 1);
        EEL_CHECK_INTEGER_RESULT (athena_directory_number_outstanding (), 1);

	athena_file_unref (file_1);

        EEL_CHECK_INTEGER_RESULT (athena_directory_number_outstanding (), 0);
	
	file_1 = athena_file_get_by_uri ("file:///etc");
	file_2 = athena_file_get_by_uri ("file:///usr");
	...
	EEL_CHECK_STRING_RESULT (athena_file_get_name (file_1), "home");

	EEL_CHECK_BOOLEAN_RESULT (athena_file_get_by_uri ("file:///home/") == file_1, TRUE);
	athena_file_unref (file_1);
	 ...
	 * in real life */
	file_1 = athena_file_get_by_uri (":");
	EEL_CHECK_STRING_RESULT (athena_file_get_name (file_1), ":");
	athena_file_unref (file_1);
#endif

	file_1 = athena_file_get_by_uri ("eazel:");
	EEL_CHECK_STRING_RESULT (athena_file_get_name (file_1), "eazel");
	athena_file_unref (file_1);
	...
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_2, ATHENA_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) < 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_2, ATHENA_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) > 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_1, ATHENA_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_1, ATHENA_FILE_SORT_BY_DISPLAY_NAME, TRUE, FALSE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_1, ATHENA_FILE_SORT_BY_DISPLAY_NAME, FALSE, TRUE) == 0, TRUE);
	EEL_CHECK_BOOLEAN_RESULT (athena_file_compare_for_sort (file_1, file_1, ATHENA_FILE_SORT_BY_DISPLAY_NAME, TRUE, TRUE) == 0, TRUE);

	athena_file_unref (file_1);
	athena_file_unref (file_2);
}
