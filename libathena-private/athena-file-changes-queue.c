...
} AthenaFileChangeKind;

typedef struct {
	AthenaFileChangeKind kind;
	GFile *from;
	GFile *to;
	GdkPoint point;
	int screen;
...
typedef struct {
	...
	GList *tail;
	GMutex mutex;
} AthenaFileChangesQueue;

static AthenaFileChangesQueue *
athena_file_changes_queue_new (void)
{
	AthenaFileChangesQueue *result;

	result = g_new0 (AthenaFileChangesQueue, 1);
...
static AthenaFileChangesQueue *
athena_file_changes_queue_get (void)
{
	static AthenaFileChangesQueue *file_changes_queue;

	if (file_changes_queue == NULL) {
		file_changes_queue = athena_file_changes_queue_new ();
	}
...
static void
athena_file_changes_queue_add_common (AthenaFileChangesQueue *queue, 
	AthenaFileChange *new_item)
{
...
void
athena_file_changes_queue_file_added (GFile *location)
{
	AthenaFileChange *new_item;
	AthenaFileChangesQueue *queue;

	queue = athena_file_changes_queue_get();

	new_item = g_new0 (AthenaFileChange, 1);
	new_item->kind = CHANGE_FILE_ADDED;
	new_item->from = g_object_ref (location);
	athena_file_changes_queue_add_common (queue, new_item);
}

void
athena_file_changes_queue_file_changed (GFile *location)
{
	AthenaFileChange *new_item;
	AthenaFileChangesQueue *queue;
...
void
athena_file_changes_queue_file_removed (GFile *location)
{
	AthenaFileChange *new_item;
	AthenaFileChangesQueue *queue;
...
void
athena_file_changes_queue_file_moved (GFile *from,
					GFile *to)
{
	AthenaFileChange *new_item;
	AthenaFileChangesQueue *queue;
	...
	new_item = g_new (AthenaFileChange, 1);
...
void
athena_file_changes_queue_schedule_position_set (GFile *location, 
						   GdkPoint point,
						   int screen)
{
	AthenaFileChange *new_item;
...
void
athena_file_changes_queue_schedule_position_remove (GFile *location)
{
	AthenaFileChange *new_item;
	AthenaFileChangesQueue *queue;
...
static AthenaFileChange *
athena_file_changes_queue_get_change (AthenaFileChangesQueue *queue)
{
	GList *new_tail;
	AthenaFileChange *result;
 ...
 * in a list to the different athena_directory_notify calls
 */ 
void
athena_file_changes_consume_changes (gboolean consume_all)
{
	AthenaFileChange *change;
	GList *additions, *changes, *deletions, *moves;
	GList *position_set_requests;
	GFilePair *pair;
	AthenaFileChangesQueuePosition *position_set;
	guint chunk_count;
	AthenaFileChangesQueue *queue;
	...
	for (chunk_count = 0; ; chunk_count++) {
		change = athena_file_changes_queue_get_change (queue);
		...
		if (flush_needed) {
			...
			if (position_set_requests != NULL) {
				...
				athena_directory_schedule_position_set (position_set_requests);
		...
		case CHANGE_POSITION_SET:
			position_set = g_new (AthenaFileChangesQueuePosition, 1);
