...
get_builtin_columns (void)
{
	...
	columns = g_list_append (NULL,
				 g_object_new (ATHENA_TYPE_COLUMN,
					       "name", "name",
					       "attribute", "name",
					       "label", _("Name"),
...
get_extension_columns (void)
{
	...
	providers = athena_module_get_extensions_for_type (ATHENA_TYPE_COLUMN_PROVIDER);
	
	columns = NULL;
	
	for (l = providers; l != NULL; l = l->next) {
		AthenaColumnProvider *provider;
		GList *provider_columns;
		
		provider = ATHENA_COLUMN_PROVIDER (l->data);
		provider_columns = athena_column_provider_get_columns (provider);
...
get_trash_columns (void)
{
	...
	return athena_column_list_copy (columns);
}

GList *
athena_get_common_columns (void)
{
	static GList *columns = NULL;
...
GList *
athena_get_all_columns (void)
{
	GList *columns = NULL;

	columns = g_list_concat (athena_get_common_columns (),
	                         get_trash_columns ());
...
GList *
athena_get_columns_for_file (AthenaFile *file)
{
	GList *columns;
...
GList *
athena_column_list_copy (GList *columns) 
{
	GList *ret;
	GList *l;
...
void
athena_column_list_free (GList *columns)
{
	GList *l;
...
static int
column_compare (AthenaColumn *a, AthenaColumn *b, char **column_order)
{
	int index_a;
	int index_b;
	char *name;
...
GList *
athena_sort_columns (GList  *columns, 
		       char  **column_order)
{
	if (!column_order) {
		return NULL;
