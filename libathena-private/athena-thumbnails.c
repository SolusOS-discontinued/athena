...
/* The list of AthenaThumbnailInfo structs containing information about the
...
static AthenaThumbnailInfo *currently_thumbnailing = NULL;
...
static void
free_thumbnail_info (AthenaThumbnailInfo *info)
{
...
void
athena_update_thumbnail_file_copied (const char *source_file_uri,
...
void
athena_update_thumbnail_file_renamed (const char *source_file_uri,
					const char *destination_file_uri)
{
	athena_update_thumbnail_file_copied (source_file_uri, destination_file_uri);
	athena_remove_thumbnail_for_file (source_file_uri);
}

void 
athena_remove_thumbnail_for_file (const char *file_uri)
{
	char *thumbnail_path;
...
static GdkPixbuf *
athena_get_thumbnail_frame (void)
{
...
void
athena_thumbnail_frame_image (GdkPixbuf **pixbuf)
{
	GdkPixbuf *pixbuf_with_frame, *frame;
	int left_offset, top_offset, right_offset, bottom_offset;
		
	/* The pixbuf isn't already framed (i.e., it was not made by
	 * an old Athena), so we must embed it in a frame.
	 */

	frame = athena_get_thumbnail_frame ();
	if (frame == NULL) {
		return;
	}
	
	left_offset = ATHENA_THUMBNAIL_FRAME_LEFT;
	top_offset = ATHENA_THUMBNAIL_FRAME_TOP;
	right_offset = ATHENA_THUMBNAIL_FRAME_RIGHT;
	bottom_offset = ATHENA_THUMBNAIL_FRAME_BOTTOM;
...
GdkPixbuf *
athena_thumbnail_unframe_image (GdkPixbuf *pixbuf)
{
...
void
athena_thumbnail_remove_from_queue (const char *file_uri)
{
	GList *node;
...
void
athena_thumbnail_remove_all_from_queue (void)
{
	AthenaThumbnailInfo *info;
	GList *l, *next;
...
void
athena_thumbnail_prioritize (const char *file_uri)
{
	GList *node;
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   We do this in an idle callback as I don't think athena_file_changed() is
   thread-safe. */
static gboolean
thumbnail_thread_notify_file_changed (gpointer image_uri)
{
	...
	file = athena_file_get_by_uri ((char *) image_uri);
#ifdef DEBUG_THUMBNAILS
	g_message ("(Thumbnail Thread) Notifying file changed file:%p uri: %s\n", file, (char*) image_uri);
#endif

	if (file != NULL) {
		athena_file_set_is_thumbnailing (file, FALSE);
...
gboolean
athena_can_thumbnail_internally (AthenaFile *file)
{
	char *mime_type;
	gboolean res;
...
gboolean
athena_thumbnail_is_mimetype_limited_by_size (const char *mime_type)
{
	return pixbuf_can_load_type (mime_type);
}

gboolean
athena_can_thumbnail (AthenaFile *file)
{
	GnomeDesktopThumbnailFactory *factory;
	gboolean res;
	char *uri;
	...
	mime_type = athena_file_get_mime_type (file);
	mtime = athena_file_get_mtime (file);
...
void
athena_create_thumbnail (AthenaFile *file)
{
	time_t file_mtime = 0;
	AthenaThumbnailInfo *info;
	AthenaThumbnailInfo *existing_info;
	GList *existing, *node;

	athena_file_set_is_thumbnailing (file, TRUE);

	info = g_new0 (AthenaThumbnailInfo, 1);
	info->image_uri = athena_file_get_uri (file);
	info->mime_type = athena_file_get_mime_type (file);
	
	/* Hopefully the AthenaFile will already have the image file mtime,
...
thumbnail_thread_start (gpointer data)
{
	...
	for (;;) {
		...
		*/
		...
		/* We need to call athena_file_changed(), but I don't think that is
