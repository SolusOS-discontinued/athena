...
#include <config.h>
#include <math.h>
#include "athena-icon-canvas-item.h"
...
/* Private part of the AthenaIconCanvasItem structure. */
struct AthenaIconCanvasItemDetails {
	/* The image, text, font. */
	double x, y;
	GdkPixbuf *pixbuf;
...
static void athena_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType athena_icon_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (AthenaIconCanvasItem, athena_icon_canvas_item, EEL_TYPE_CANVAS_ITEM,
			 G_IMPLEMENT_INTERFACE (EEL_TYPE_ACCESSIBLE_TEXT,
						athena_icon_canvas_item_text_interface_init));

/* private */
static void     draw_label_text                      (AthenaIconCanvasItem        *item,
						      cairo_t                       *cr,
						      EelIRect                       icon_rect);
static void     measure_label_text                   (AthenaIconCanvasItem        *item);
static void     get_icon_canvas_rectangle            (AthenaIconCanvasItem        *item,
						      EelIRect                      *rect);
...
static PangoLayout *get_label_layout                 (PangoLayout                  **layout,
						      AthenaIconCanvasItem        *item,
						      const char                    *text);
static gboolean hit_test_stretch_handle              (AthenaIconCanvasItem        *item,
						      EelIRect                       canvas_rect,
						      GtkCornerType *corner);
static void      draw_embedded_text                  (AthenaIconCanvasItem        *icon_item,
                                                      cairo_t                       *cr,
						      int                            x,
						      int                            y);

static void       athena_icon_canvas_item_ensure_bounds_up_to_date (AthenaIconCanvasItem *icon_item);

/* Object initialization function for the icon item. */
static void
athena_icon_canvas_item_init (AthenaIconCanvasItem *icon_item)
{
	icon_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((icon_item), ATHENA_TYPE_ICON_CANVAS_ITEM, AthenaIconCanvasItemDetails);
	athena_icon_canvas_item_invalidate_label_size (icon_item);
}

static void
athena_icon_canvas_item_finalize (GObject *object)
{
	AthenaIconCanvasItemDetails *details;

	g_assert (ATHENA_IS_ICON_CANVAS_ITEM (object));

	details = ATHENA_ICON_CANVAS_ITEM (object)->details;
	...
	G_OBJECT_CLASS (athena_icon_canvas_item_parent_class)->finalize (object);
}
...
static void
athena_icon_canvas_item_invalidate_bounds_cache (AthenaIconCanvasItem *item)
{
...
void
athena_icon_canvas_item_invalidate_label_size (AthenaIconCanvasItem *item)
{
	...
	athena_icon_canvas_item_invalidate_bounds_cache (item);
...
static void
athena_icon_canvas_item_set_property (GObject        *object,
					guint           property_id,
					const GValue   *value,
					GParamSpec     *pspec)
{
	...
	item = ATHENA_ICON_CANVAS_ITEM (object);
	...
	case PROP_EDITABLE_TEXT:
		...
		athena_icon_canvas_item_invalidate_label_size (item);
	...
	default:
		g_warning ("athena_icons_view_item_item_set_arg on unknown argument");
		return;
	}
...
static void
athena_icon_canvas_item_get_property (GObject        *object,
					guint           property_id,
					GValue         *value,
					GParamSpec     *pspec)
{
...
cairo_surface_t *
athena_icon_canvas_item_get_drag_surface (AthenaIconCanvasItem *item)
{
	...
	g_return_val_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item), NULL);
	...
	gtk_style_context_add_class (context, "athena-canvas-item");
...
void
athena_icon_canvas_item_set_image (AthenaIconCanvasItem *item,
				     GdkPixbuf *image)
{
	AthenaIconCanvasItemDetails *details;	
	
	g_return_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item));
...
void 
athena_icon_canvas_item_set_attach_points (AthenaIconCanvasItem *item,
					     GdkPoint *attach_points,
					     int n_attach_points)
{
...
void
athena_icon_canvas_item_set_embedded_text_rect (AthenaIconCanvasItem       *item,
...
void
athena_icon_canvas_item_set_embedded_text (AthenaIconCanvasItem       *item,
					     const char                   *text)
{
 ...
 */
static void
recompute_bounding_box (AthenaIconCanvasItem *icon_item,
...
static EelIRect
compute_text_rectangle (const AthenaIconCanvasItem *item,
			...
			AthenaIconCanvasItemBoundsUsage usage)
{
	...
	if (ATHENA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
		if (!athena_icon_container_is_layout_rtl (ATHENA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas))) {
...
void
athena_icon_canvas_item_update_bounds (AthenaIconCanvasItem *item,
					 double i2w_dx, double i2w_dy)
{
...
static void
athena_icon_canvas_item_update (EelCanvasItem *item,
				  double i2w_dx, double i2w_dy,
				  gint flags)
{
	athena_icon_canvas_item_update_bounds (ATHENA_ICON_CANVAS_ITEM (item), i2w_dx, i2w_dy);

	eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

	EEL_CANVAS_ITEM_CLASS (athena_icon_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
in_single_click_mode (void)
{
	...
	click_policy = g_settings_get_enum (athena_preferences,
					    ATHENA_PREFERENCES_CLICK_POLICY);

	return click_policy == ATHENA_CLICK_POLICY_SINGLE;
}
...
#define IS_COMPACT_VIEW(container) \
        ((container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_L_R || \
...
static void
prepare_pango_layout_width (AthenaIconCanvasItem *item,
			    PangoLayout *layout)
{
	if (athena_icon_canvas_item_get_max_text_width (item) < 0) {
		pango_layout_set_width (layout, -1);
	} else {
		pango_layout_set_width (layout, floor (athena_icon_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
		pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
	}
}

static void
prepare_pango_layout_for_measure_entire_text (AthenaIconCanvasItem *item,
					      PangoLayout *layout)
{
	...
	container = ATHENA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
...
static void
prepare_pango_layout_for_draw (AthenaIconCanvasItem *item,
			       PangoLayout *layout)
{
	AthenaIconCanvasItemDetails *details;
	AthenaIconContainer *container;
	...
	if (IS_COMPACT_VIEW (container)) {
		...
		pango_layout_set_height (layout,
					 athena_icon_container_get_max_layout_lines_for_pango (container));
	}
}

static void
measure_label_text (AthenaIconCanvasItem *item)
{
	AthenaIconCanvasItemDetails *details;
	AthenaIconContainer *container;
	...
	if (have_editable) {
		...
		layout_get_size_for_layout (editable_layout,
					    athena_icon_container_get_max_layout_lines (container),
					    editable_height_for_entire_text,
...
static void
draw_label_text (AthenaIconCanvasItem *item,
                 cairo_t *cr,
		 EelIRect icon_rect)
{
	AthenaIconCanvasItemDetails *details;
	...
	needs_highlight = details->is_highlighted_for_selection || details->is_highlighted_for_drop;
	is_rtl_label_beside = athena_icon_container_is_layout_rtl (container) &&
			      container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE;
	...
	max_text_width = floor (athena_icon_canvas_item_get_max_text_width (item));
...
void
athena_icon_canvas_item_set_is_visible (AthenaIconCanvasItem       *item,
					  gboolean                      visible)
{
	...
	if (!visible) {
		athena_icon_canvas_item_invalidate_label (item);
	}
}

void
athena_icon_canvas_item_invalidate_label (AthenaIconCanvasItem     *item)
{
	athena_icon_canvas_item_invalidate_label_size (item);
...
static void
draw_stretch_handles (AthenaIconCanvasItem *item,
                      cairo_t *cr,
		      const EelIRect *rect)
{
	GtkWidget *widget;
...
static GdkPixbuf *
real_map_pixbuf (AthenaIconCanvasItem *icon_item)
{
	EelCanvas *canvas;
...
static GdkPixbuf *
map_pixbuf (AthenaIconCanvasItem *icon_item)
{
...
static void
draw_embedded_text (AthenaIconCanvasItem *item,
                    cairo_t *cr,
		    int x, int y)
{
	PangoLayout *layout;
...
static void
athena_icon_canvas_item_draw (EelCanvasItem *item,
                                cairo_t *cr,
                                cairo_region_t *region)
{
	AthenaIconContainer *container;
	AthenaIconCanvasItem *icon_item;
	AthenaIconCanvasItemDetails *details;
	EelIRect icon_rect;
	GdkPixbuf *temp_pixbuf;
	GtkStyleContext *context;

	container = ATHENA_ICON_CONTAINER (item->canvas);
	icon_item = ATHENA_ICON_CANVAS_ITEM (item);
...
static PangoLayout *
create_label_layout (AthenaIconCanvasItem *item,
		     const char *text)
{
	PangoLayout *layout;
	PangoContext *context;
	...
	container = ATHENA_ICON_CONTAINER (canvas_item->canvas);
...
static int
athena_icon_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	AthenaIconCanvasItem *icon_item;
	GdkCursor *cursor;
	GdkWindow *cursor_window;
	...
	case GDK_ENTER_NOTIFY:
		if (!icon_item->details->is_prelit) {
			...
			eel_canvas_item_send_behind (item,
						     ATHENA_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle);
...
static gboolean
hit_test (AthenaIconCanvasItem *icon_item, EelIRect canvas_rect)
{
	AthenaIconCanvasItemDetails *details;
...
static double
athena_icon_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
				 EelCanvasItem **actual_item)
{
	...
	*actual_item = item;
	...
	canvas_rect.x1 = cx + 1;
	canvas_rect.y1 = cy + 1;
	if (hit_test (ATHENA_ICON_CANVAS_ITEM (item), canvas_rect)) {
		return 0.0;
	} else {
...
static void
athena_icon_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
	AthenaIconCanvasItem *icon_item;
	AthenaIconCanvasItemDetails *details;
...
void
athena_icon_canvas_item_get_bounds_for_layout (AthenaIconCanvasItem *icon_item,
						 double *x1, double *y1, double *x2, double *y2)
{
	...
	athena_icon_canvas_item_ensure_bounds_up_to_date (icon_item);
...
void
athena_icon_canvas_item_get_bounds_for_entire_item (AthenaIconCanvasItem *icon_item,
						      double *x1, double *y1, double *x2, double *y2)
{
	AthenaIconCanvasItemDetails *details;
	EelIRect *total_rect;
...
static void
athena_icon_canvas_item_bounds (EelCanvasItem *item,
				  double *x1, double *y1, double *x2, double *y2)
{
	AthenaIconCanvasItem *icon_item;
	AthenaIconCanvasItemDetails *details;
...
static void
athena_icon_canvas_item_ensure_bounds_up_to_date (AthenaIconCanvasItem *icon_item)
{
	AthenaIconCanvasItemDetails *details;
...
EelDRect
athena_icon_canvas_item_get_icon_rectangle (const AthenaIconCanvasItem *item)
{
	EelDRect rectangle;
	double pixels_per_unit;
	GdkPixbuf *pixbuf;
	
	g_return_val_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item), eel_drect_empty);
...
EelDRect
athena_icon_canvas_item_get_text_rectangle (AthenaIconCanvasItem *item,
					      gboolean for_layout)
{
	/* FIXME */
	EelIRect icon_rectangle;
...
static void
get_icon_canvas_rectangle (AthenaIconCanvasItem *item,
			   EelIRect *rect)
{
	GdkPixbuf *pixbuf;

	g_assert (ATHENA_IS_ICON_CANVAS_ITEM (item));
	g_assert (rect != NULL);
...
void
athena_icon_canvas_item_set_show_stretch_handles (AthenaIconCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (AthenaIconCanvasItem *item,
...
gboolean
athena_icon_canvas_item_hit_test_stretch_handles (AthenaIconCanvasItem *item,
						    gdouble world_x,
						    gdouble world_y,
						    GtkCornerType *corner)
{
	...
	g_return_val_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item), FALSE);
 ...
 */
gboolean
athena_icon_canvas_item_hit_test_rectangle (AthenaIconCanvasItem *item, EelIRect canvas_rect)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item), FALSE);
...
const char *
athena_icon_canvas_item_get_editable_text (AthenaIconCanvasItem *icon_item)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (icon_item), NULL);

	return icon_item->details->editable_text;
}

void
athena_icon_canvas_item_set_renaming (AthenaIconCanvasItem *item, gboolean state)
{
	g_return_if_fail (ATHENA_IS_ICON_CANVAS_ITEM (item));
...
double
athena_icon_canvas_item_get_max_text_width (AthenaIconCanvasItem *item)
{
	EelCanvasItem *canvas_item;
	AthenaIconContainer *container;
	...
	if (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
		if (container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_L_R ||
		    container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_R_L) {
...
void
athena_icon_canvas_item_set_entire_text (AthenaIconCanvasItem       *item,
...
static void
athena_icon_canvas_item_class_init (AthenaIconCanvasItemClass *class)
{
	...
	object_class->finalize = athena_icon_canvas_item_finalize;
	object_class->set_property = athena_icon_canvas_item_set_property;
	object_class->get_property = athena_icon_canvas_item_get_property;
	...
	item_class->update = athena_icon_canvas_item_update;
	item_class->draw = athena_icon_canvas_item_draw;
	item_class->point = athena_icon_canvas_item_point;
	item_class->translate = athena_icon_canvas_item_translate;
	item_class->bounds = athena_icon_canvas_item_bounds;
	item_class->event = athena_icon_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       ATHENA_TYPE_ICON_CANVAS_ITEM,
				       athena_icon_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (AthenaIconCanvasItemDetails));
}

static GailTextUtil *
athena_icon_canvas_item_get_text (GObject *text)
{
	return ATHENA_ICON_CANVAS_ITEM (text)->details->text_util;
}

static void
athena_icon_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = athena_icon_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *athena_icon_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *athena_icon_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} AthenaIconCanvasItemAccessiblePrivate;

typedef struct {
	AthenaIconCanvasItem *item;
	gint action_number;
} AthenaIconCanvasItemAccessibleActionContext;

static GType athena_icon_canvas_item_accessible_get_type (void);
...
static gboolean
athena_icon_canvas_item_accessible_idle_do_action (gpointer data)
{
	AthenaIconCanvasItem *item;
	AthenaIconCanvasItemAccessibleActionContext *ctx;
	AthenaIcon *icon;
	AthenaIconContainer *container;
	GList* selection;
	GList file_list;
...
static gboolean
athena_icon_canvas_item_accessible_do_action (AtkAction *accessible,
						int i)
{
	AthenaIconCanvasItem *item;
	AthenaIconCanvasItemAccessibleActionContext *ctx;
	...
	item = ATHENA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (accessible)));
	if (!item) {
		return FALSE;
	}
	...
	case ACTION_MENU:
		...
		ctx = g_new (AthenaIconCanvasItemAccessibleActionContext, 1);
		ctx->action_number = i;
		ctx->item = item;
		g_queue_push_head (container->details->a11y_item_action_queue, ctx);
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler = g_idle_add (athena_icon_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
                g_warning ("Invalid action passed to AthenaIconCanvasItemAccessible::do_action");
                return FALSE;
        }
...
static int
athena_icon_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
athena_icon_canvas_item_accessible_action_get_description (AtkAction *accessible,
                                                             int i)
{
	AthenaIconCanvasItemAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return athena_icon_canvas_item_accessible_action_descriptions[i];
	}
}

static const char *
athena_icon_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return athena_icon_canvas_item_accessible_action_names[i];
}

static const char *
athena_icon_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
							    int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
athena_icon_canvas_item_accessible_action_set_description (AtkAction *accessible,
							     int i,
...
static void
athena_icon_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = athena_icon_canvas_item_accessible_do_action;
	iface->get_n_actions = athena_icon_canvas_item_accessible_get_n_actions;
	iface->get_description = athena_icon_canvas_item_accessible_action_get_description;
	iface->get_keybinding = athena_icon_canvas_item_accessible_action_get_keybinding;
	iface->get_name = athena_icon_canvas_item_accessible_action_get_name;
	iface->set_description = athena_icon_canvas_item_accessible_action_set_description;
}

static const gchar *
athena_icon_canvas_item_accessible_get_name (AtkObject *accessible)
{
	AthenaIconCanvasItem *item;
...
static const gchar*
athena_icon_canvas_item_accessible_get_description (AtkObject *accessible)
{
	AthenaIconCanvasItem *item;
...
static AtkObject *
athena_icon_canvas_item_accessible_get_parent (AtkObject *accessible)
{
	AthenaIconCanvasItem *item;
...
static int
athena_icon_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
	AthenaIconCanvasItem *item;
	AthenaIconContainer *container;
	GList *l;
...
static const gchar *
athena_icon_canvas_item_accessible_get_image_description (AtkImage *image)
{
	AthenaIconCanvasItemAccessiblePrivate *priv;
	AthenaIconCanvasItem *item;
	AthenaIcon *icon;
	...
	if (priv->image_description) {
		...
		item = ATHENA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (image)));
		if (item == NULL) {
			return NULL;
		}
		icon = item->user_data;
		container = ATHENA_ICON_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
		description = athena_icon_container_get_icon_description (container, icon->data);
...
static void
athena_icon_canvas_item_accessible_get_image_position
	(AtkImage		 *image,
	 gint                    *x,
	 gint	                 *y,
...
static gboolean
athena_icon_canvas_item_accessible_set_image_description (AtkImage    *image,
...
static void
athena_icon_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
	iface->get_image_description = athena_icon_canvas_item_accessible_get_image_description;
	iface->set_image_description = athena_icon_canvas_item_accessible_set_image_description;
	iface->get_image_size        = athena_icon_canvas_item_accessible_get_image_size;
	iface->get_image_position    = athena_icon_canvas_item_accessible_get_image_position;
}

/* accessible text interface */
static gint
athena_icon_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
                                                          gint           x,
                                                          gint           y,
                                                          AtkCoordType coords)
{
	...
	item = ATHENA_ICON_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (text)));
...
static void
athena_icon_canvas_item_accessible_get_character_extents (AtkText	   *text,
                                                            gint	   offset,
                                                            gint	   *x,
                                                            gint	   *y,
                                                            gint	   *width,
...
static void
athena_icon_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
	...
	iface->get_character_extents   = athena_icon_canvas_item_accessible_get_character_extents;
	iface->get_offset_at_point     = athena_icon_canvas_item_accessible_get_offset_at_point;
}

typedef struct {
	AtkGObjectAccessible parent;
} AthenaIconCanvasItemAccessible;

typedef struct {
	AtkGObjectAccessibleClass parent_class;
} AthenaIconCanvasItemAccessibleClass;

G_DEFINE_TYPE_WITH_CODE (AthenaIconCanvasItemAccessible,
			 athena_icon_canvas_item_accessible,
			 ATK_TYPE_GOBJECT_ACCESSIBLE,
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						athena_icon_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						athena_icon_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						athena_icon_canvas_item_accessible_action_interface_init));

static AtkStateSet*
athena_icon_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
	...
	state_set = ATK_OBJECT_CLASS (athena_icon_canvas_item_accessible_parent_class)->ref_state_set (accessible);
...
static void
athena_icon_canvas_item_accessible_finalize (GObject *object)
{
	AthenaIconCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (athena_icon_canvas_item_accessible_parent_class)->finalize (object);
}

static void
athena_icon_canvas_item_accessible_initialize (AtkObject *accessible,
						 gpointer widget)
{
	ATK_OBJECT_CLASS (athena_icon_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_ICON);
}

static void
athena_icon_canvas_item_accessible_class_init (AthenaIconCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = athena_icon_canvas_item_accessible_finalize;

	aclass->initialize = athena_icon_canvas_item_accessible_initialize;

	aclass->get_name = athena_icon_canvas_item_accessible_get_name;
	aclass->get_description = athena_icon_canvas_item_accessible_get_description;
	aclass->get_parent = athena_icon_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = athena_icon_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = athena_icon_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (AthenaIconCanvasItemAccessiblePrivate));
}

static void
athena_icon_canvas_item_accessible_init (AthenaIconCanvasItemAccessible *self)
{
}

/* dummy typedef */
typedef AtkObjectFactory      AthenaIconCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass AthenaIconCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (AthenaIconCanvasItemAccessibleFactory, athena_icon_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);

static AtkObject *
athena_icon_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
	AtkObject *accessible;
	AthenaIconCanvasItem *item;
	GString *item_text;

	item = ATHENA_ICON_CANVAS_ITEM (for_object);
	g_assert (item != NULL);
	...
	accessible = g_object_new (athena_icon_canvas_item_accessible_get_type (), NULL);
...
static GType
athena_icon_canvas_item_accessible_factory_get_accessible_type (void)
{
	return athena_icon_canvas_item_accessible_get_type ();
}

static void
athena_icon_canvas_item_accessible_factory_init (AthenaIconCanvasItemAccessibleFactory *self)
{
}

static void
athena_icon_canvas_item_accessible_factory_class_init (AthenaIconCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = athena_icon_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = athena_icon_canvas_item_accessible_factory_get_accessible_type;
}
