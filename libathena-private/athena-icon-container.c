...
/* athena-icon-container.c - Icon container widget.
...
#include "athena-icon-private.h"
#include "athena-lib-self-check-functions.h"
#include "athena-selection-canvas-item.h"
...
#define DEBUG_FLAG ATHENA_DEBUG_ICON_CONTAINER
...
/* From athena-icon-canvas-item.c */
...
/* Copied from AthenaIconContainer */
#define ATHENA_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT 5

/* Copied from AthenaFile */
...
} AthenaIconContainerAccessiblePrivate;

static GType         athena_icon_container_accessible_get_type (void);

static void          preview_selected_items                         (AthenaIconContainer *container);
static void          activate_selected_items                        (AthenaIconContainer *container);
static void          activate_selected_items_alternate              (AthenaIconContainer *container,
								     AthenaIcon          *icon);
static void          compute_stretch                                (StretchState          *start,
								     StretchState          *current);
static AthenaIcon *get_first_selected_icon                        (AthenaIconContainer *container);
static AthenaIcon *get_nth_selected_icon                          (AthenaIconContainer *container,
								     int                    index);
static gboolean      has_multiple_selection                         (AthenaIconContainer *container);
static gboolean      all_selected                                   (AthenaIconContainer *container);
static gboolean      has_selection                                  (AthenaIconContainer *container);
static void          icon_destroy                                   (AthenaIconContainer *container,
								     AthenaIcon          *icon);
static void          end_renaming_mode                              (AthenaIconContainer *container,
								     gboolean               commit);
static AthenaIcon *get_icon_being_renamed                         (AthenaIconContainer *container);
static void          finish_adding_new_icons                        (AthenaIconContainer *container);
static inline void   icon_get_bounding_box                          (AthenaIcon          *icon,
								     int                   *x1_return,
								     int                   *y1_return,
								     int                   *x2_return,
								     int                   *y2_return,
								     AthenaIconCanvasItemBoundsUsage usage);
static gboolean      is_renaming                                    (AthenaIconContainer *container);
static gboolean      is_renaming_pending                            (AthenaIconContainer *container);
static void          process_pending_icon_to_rename                 (AthenaIconContainer *container);
static void          athena_icon_container_stop_monitor_top_left  (AthenaIconContainer *container,
								     AthenaIconData      *data,
								     gconstpointer          client);
static void          athena_icon_container_start_monitor_top_left (AthenaIconContainer *container,
								     AthenaIconData      *data,
								     gconstpointer          client,
								     gboolean               large_text);
static void          handle_hadjustment_changed                     (GtkAdjustment         *adjustment,
								     AthenaIconContainer *container);
static void          handle_vadjustment_changed                     (GtkAdjustment         *adjustment,
								     AthenaIconContainer *container);
static GList *       athena_icon_container_get_selected_icons (AthenaIconContainer *container);
static void          athena_icon_container_update_visible_icons   (AthenaIconContainer *container);
static void          reveal_icon                                    (AthenaIconContainer *container,
								     AthenaIcon *icon);

static void	     athena_icon_container_set_rtl_positions (AthenaIconContainer *container);
static double	     get_mirror_x_position                     (AthenaIconContainer *container,
								AthenaIcon *icon,
								double x);
static void         text_ellipsis_limit_changed_container_callback  (gpointer callback_data);

static int compare_icons_horizontal (AthenaIconContainer *container,
				     AthenaIcon *icon_a,
				     AthenaIcon *icon_b);

static int compare_icons_vertical (AthenaIconContainer *container,
				   AthenaIcon *icon_a,
				   AthenaIcon *icon_b);

static void store_layout_timestamps_now (AthenaIconContainer *container);
static void remove_search_entry_timeout (AthenaIconContainer *container);

static gpointer accessible_parent_class;

static GQuark accessible_private_data_quark = 0;

static const char *athena_icon_container_accessible_action_names[] = {
	"activate",
	"menu",
	NULL
};

static const char *athena_icon_container_accessible_action_descriptions[] = {
	"Activate selected items",
	"Popup context menu",
	NULL
};

G_DEFINE_TYPE (AthenaIconContainer, athena_icon_container, EEL_TYPE_CANVAS);

/* The AthenaIconContainer signals.  */
enum {
	ACTIVATE,
	ACTIVATE_ALTERNATE,
	ACTIVATE_PREVIEWER,
...
/* Functions dealing with AthenaIcons.  */

static void
icon_free (AthenaIcon *icon)
{
...
static gboolean
icon_is_positioned (const AthenaIcon *icon)
{
...
static void
icon_set_position (AthenaIcon *icon,
		   double x, double y)
{	
	AthenaIconContainer *container;
	...
	container = ATHENA_ICON_CONTAINER (EEL_CANVAS_ITEM (icon->item)->canvas);

	if (icon == get_icon_being_renamed (container)) {
		end_renaming_mode (container, TRUE);
	}

	if (athena_icon_container_get_is_fixed_size (container)) {
		...
		*/
		...
		icon_bounds = athena_icon_canvas_item_get_icon_rectangle (icon->item);
...
static void
icon_get_size (AthenaIconContainer *container,
	       AthenaIcon *icon,
	       guint *size)
{
	if (size != NULL) {
		*size = MAX (athena_get_icon_size_for_zoom_level (container->details->zoom_level)
			       * icon->scale, ATHENA_ICON_SIZE_SMALLEST);
	}
}
 ...
 */
static void
icon_set_size (AthenaIconContainer *container,
	       AthenaIcon *icon,
	       guint icon_size,
	       gboolean snap,
	       gboolean update_position)
{
	...
	athena_icon_container_move_icon (container, icon,
					   icon->x, icon->y,
					   scale, FALSE,
...
static void
icon_raise (AthenaIcon *icon)
{
	EelCanvasItem *item, *band;
	
	item = EEL_CANVAS_ITEM (icon->item);
	band = ATHENA_ICON_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle;
	
	eel_canvas_item_send_behind (item, band);
}

static void
emit_stretch_started (AthenaIconContainer *container, AthenaIcon *icon)
{
...
static void
emit_stretch_ended (AthenaIconContainer *container, AthenaIcon *icon)
{
	g_signal_emit (container,
...
static void
icon_toggle_selected (AthenaIconContainer *container,
		      AthenaIcon *icon)
{		
	end_renaming_mode (container, TRUE);
	...
	if (icon == container->details->stretch_icon) {
		...
		athena_icon_canvas_item_set_show_stretch_handles (icon->item, FALSE);
		/* snap the icon if necessary */
		if (container->details->keep_aligned) {
			athena_icon_container_move_icon (container,
							   icon,
							   icon->x, icon->y,
							   icon->scale,
							   FALSE, TRUE, TRUE);
...
static gboolean
icon_set_selected (AthenaIconContainer *container,
		   AthenaIcon *icon,
		   gboolean select)
{
...
static inline void
icon_get_bounding_box (AthenaIcon *icon,
		       ...
		       AthenaIconCanvasItemBoundsUsage usage)
{
	...
	if (usage == BOUNDS_USAGE_FOR_DISPLAY) {
		...
		athena_icon_canvas_item_get_bounds_for_layout (icon->item,
								 &x1, &y1, &x2, &y2);
	} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
		athena_icon_canvas_item_get_bounds_for_entire_item (icon->item,
								      &x1, &y1, &x2, &y2);
	} else {
...
/* Utility functions for AthenaIconContainer.  */

gboolean
athena_icon_container_scroll (AthenaIconContainer *container,
...
static void
pending_icon_to_reveal_destroy_callback (AthenaIconCanvasItem *item,
					 AthenaIconContainer *container)
{
...
static AthenaIcon*
get_pending_icon_to_reveal (AthenaIconContainer *container)
{
	return container->details->pending_icon_to_reveal;
}

static void
set_pending_icon_to_reveal (AthenaIconContainer *container, AthenaIcon *icon)
{
	AthenaIcon *old_icon;
...
static void
icon_get_row_and_column_bounds (AthenaIconContainer *container,
				AthenaIcon *icon,
				EelIRect *bounds,
				gboolean safety_pad)
{
...
static void
reveal_icon (AthenaIconContainer *container,
	     AthenaIcon *icon)
{
	...
	if (athena_icon_container_is_auto_layout (container)) {
...
static void
process_pending_icon_to_reveal (AthenaIconContainer *container)
{
	AthenaIcon *pending_icon_to_reveal;
...
keyboard_icon_reveal_timeout_callback (gpointer data)
{
	...
	container = ATHENA_ICON_CONTAINER (data);
...
static void
unschedule_keyboard_icon_reveal (AthenaIconContainer *container)
{
...
static void
schedule_keyboard_icon_reveal (AthenaIconContainer *container,
			       AthenaIcon *icon)
{
	AthenaIconContainerDetails *details;
...
static void
clear_keyboard_focus (AthenaIconContainer *container)
{
...
static void inline
emit_atk_focus_tracker_notify (AthenaIcon *icon)
{
...
static void
set_keyboard_focus (AthenaIconContainer *container,
		    AthenaIcon *icon)
{
	g_assert (icon != NULL);
...
static void
set_keyboard_rubberband_start (AthenaIconContainer *container,
			       AthenaIcon *icon)
{
...
static void
clear_keyboard_rubberband_start (AthenaIconContainer *container)
{
	container->details->keyboard_rubberband_start = NULL;
}

/* carbon-copy of eel_canvas_group_bounds(), but
 * for AthenaIconContainerItems it returns the
				   ...
				   AthenaIconCanvasItemBoundsUsage usage)
{
	...
	for (list = group->item_list; list; list = list->next) {
		...
		if (!ATHENA_IS_ICON_CANVAS_ITEM (child)) {
			continue;
		}
		...
		if (child->flags & EEL_CANVAS_ITEM_VISIBLE) {
			...
			if (!ATHENA_IS_ICON_CANVAS_ITEM (child) ||
				...
				athena_icon_canvas_item_get_bounds_for_layout (ATHENA_ICON_CANVAS_ITEM (child),
										 &minx, &miny, &maxx, &maxy);
			} else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
				athena_icon_canvas_item_get_bounds_for_entire_item (ATHENA_ICON_CANVAS_ITEM (child),
										      &minx, &miny, &maxx, &maxy);
			} else {
				g_assert_not_reached ();
			}
...
static void
get_all_icon_bounds (AthenaIconContainer *container,
		     double *x1, double *y1,
		     double *x2, double *y2,
		     AthenaIconCanvasItemBoundsUsage usage)
{
...
void
athena_icon_container_reset_scroll_region (AthenaIconContainer *container)
{
...
void
athena_icon_container_update_scroll_region (AthenaIconContainer *container)
{
	double x1, y1, x2, y2;
	double pixels_per_unit;
	GtkAdjustment *hadj, *vadj;
	...
	reset_scroll_region = container->details->reset_scroll_region_trigger
		|| athena_icon_container_is_empty (container)
		|| athena_icon_container_is_auto_layout (container);
	 ...
	 */
	if (!athena_icon_container_is_empty (container)) {
	 ...
	 */
	if (athena_icon_container_is_layout_vertical (container)) {
		if (athena_icon_container_is_layout_rtl (container)) {
	...
	step_increment = athena_get_icon_size_for_zoom_level
...
{
	AthenaIconContainerClass *klass;
	const AthenaIcon *icon_a, *icon_b;

	icon_a = a;
	icon_b = b;
	klass  = ATHENA_ICON_CONTAINER_GET_CLASS (icon_container);

	return klass->compare_icons (icon_container, icon_a->data, icon_b->data);
}

static void
sort_icons (AthenaIconContainer *container,
	    GList                **icons)
{
	AthenaIconContainerClass *klass;

	klass = ATHENA_ICON_CONTAINER_GET_CLASS (container);
...
static void
resort (AthenaIconContainer *container)
{
...
#if 0
static double
get_grid_width (AthenaIconContainer *container)
{
	if (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
...
static void
lay_down_one_line (AthenaIconContainer *container,
		   GList *line_start,
		   GList *line_end,
		   double y,
		   double max_height,
		   ...
		   gboolean whole_text)
{
	...
	is_rtl = athena_icon_container_is_layout_rtl (container);
	...
	for (p = line_start; p != line_end; p = p->next) {
		...
		icon_set_position
			(icon,
			 ...
			 y + y_offset);
		athena_icon_canvas_item_set_entire_text (icon->item, whole_text);
...
static void
lay_down_one_column (AthenaIconContainer *container,
		     GList *line_start,
		     GList *line_end,
		     double x,
		     double y_start,
...
static void
lay_down_icons_horizontal (AthenaIconContainer *container,
			   GList *icons,
			   double start_y)
{
	GList *p, *line_start;
	...
	if (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
		...
		for (p = icons; p != NULL; p = p->next) {
			...
			text_bounds = athena_icon_canvas_item_get_text_rectangle (icon->item, TRUE);
	...
	line_width = container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE ? ICON_PAD_LEFT : 0;
	line_start = icons;
...
static void
lay_down_icons_vertical (AthenaIconContainer *container,
			 GList *icons,
			 double start_y)
{
	GList *p, *line_start;
	...
	g_assert (ATHENA_IS_ICON_CONTAINER (container));
	g_assert (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE);
...
static void
snap_position (AthenaIconContainer *container,
	       AthenaIcon *icon,
	       int *x, int *y)
{
	int center_x;
	...
	icon_position = athena_icon_canvas_item_get_icon_rectangle (icon->item);
...
placement_grid_new (AthenaIconContainer *container, gboolean tight)
{
	PlacementGrid *grid;
	int width, height;
	int num_columns;
...
static void
placement_grid_mark_icon (PlacementGrid *grid, AthenaIcon *icon)
{
	EelIRect icon_pos;
	EelIRect grid_pos;
...
static void
find_empty_location (AthenaIconContainer *container,
		     PlacementGrid *grid,
		     AthenaIcon *icon,
		     int start_x,
		     int start_y,
		     ...
		     int *y)
{
	...
	pixbuf_rect = athena_icon_canvas_item_get_icon_rectangle (icon->item);
...
static void
align_icons (AthenaIconContainer *container)
{
	...
	if (athena_icon_container_is_layout_rtl (container)) {
		athena_icon_container_set_rtl_positions (container);
	}
}

static double
get_mirror_x_position (AthenaIconContainer *container, AthenaIcon *icon, double x)
{
	EelDRect icon_bounds;
	GtkAllocation allocation;
...
static void
athena_icon_container_set_rtl_positions (AthenaIconContainer *container)
{
	GList *l;
	AthenaIcon *icon;
	double x;
...
static void
lay_down_icons_vertical_desktop (AthenaIconContainer *container, GList *icons)
{
	...
	if (placed > 0) {
		...
		if (grid) {
			...
			for (p = unplaced_icons; p != NULL; p = p->next) {
				...
				icon_rect = athena_icon_canvas_item_get_icon_rectangle (icon->item);
	 ...
	 */
	athena_icon_container_freeze_icon_positions (container);
}


static void
lay_down_icons (AthenaIconContainer *container, GList *icons, double start_y)
{
	switch (container->details->layout_mode)
	{
	case ATHENA_ICON_LAYOUT_L_R_T_B:
	case ATHENA_ICON_LAYOUT_R_L_T_B:
		lay_down_icons_horizontal (container, icons, start_y);
		break;
		
	case ATHENA_ICON_LAYOUT_T_B_L_R:
	case ATHENA_ICON_LAYOUT_T_B_R_L:
		if (athena_icon_container_get_is_desktop (container)) {
...
static void
redo_layout_internal (AthenaIconContainer *container)
{
	...
	athena_icon_container_update_scroll_region (container);

	process_pending_icon_to_reveal (container);
	process_pending_icon_to_rename (container);
	athena_icon_container_update_visible_icons (container);
}
...
redo_layout_callback (gpointer callback_data)
{
	...
	container = ATHENA_ICON_CONTAINER (callback_data);
...
static void
unschedule_redo_layout (AthenaIconContainer *container)
{
...
static void
schedule_redo_layout (AthenaIconContainer *container)
{
	if (container->details->idle_id == 0
...
static void
redo_layout (AthenaIconContainer *container)
{
...
static void
reload_icon_positions (AthenaIconContainer *container)
{
...
static void
invalidate_label_sizes (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		athena_icon_canvas_item_invalidate_label_size (icon->item);		
	}
}

/* invalidate the entire labels (i.e. their attributes) for all the icons */
static void
invalidate_labels (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		athena_icon_canvas_item_invalidate_label (icon->item);		
	}
}

static gboolean
select_range (AthenaIconContainer *container,
	      AthenaIcon *icon1,
	      AthenaIcon *icon2,
...
static gboolean
select_one_unselect_others (AthenaIconContainer *container,
			    AthenaIcon *icon_to_select)
{
	gboolean selection_changed;
	GList *p;
	AthenaIcon *icon;
...
static gboolean
unselect_all (AthenaIconContainer *container)
{
	return select_one_unselect_others (container, NULL);
}

void
athena_icon_container_move_icon (AthenaIconContainer *container,
				   AthenaIcon *icon,
				   int x, int y,
				   double scale,
				   gboolean raise,
				   ...
				   gboolean update_position)
{
	...
	if (scale != icon->scale) {
		icon->scale = scale;
		athena_icon_container_update_icon (container, icon);
		if (update_position) {
	...
	if (!details->auto_layout) {
		...
		icon->saved_ltr_x = athena_icon_container_is_layout_rtl (container) ? get_mirror_x_position (container, icon, icon->x) : icon->x;
	}
...
static void
rubberband_select (AthenaIconContainer *container,
		   ...
		   const EelDRect *current_rect)
{
	...
	for (p = container->details->icons; p != NULL; p = p->next) {
		...
		is_in = athena_icon_canvas_item_hit_test_rectangle (icon->item, canvas_rect);
...
rubberband_timeout_callback (gpointer data)
{
	AthenaIconContainer *container;
	GtkWidget *widget;
	AthenaIconRubberbandInfo *band_info;
	int x, y;
	double x1, y1, x2, y2;
	...
	athena_icon_container_scroll (container, x_scroll, y_scroll);
...
static void
start_rubberbanding (AthenaIconContainer *container,
		     GdkEventButton *event)
{
	...
	band_info->selection_rectangle = eel_canvas_item_new
		(eel_canvas_root
		 (EEL_CANVAS (container)),
		 ATHENA_TYPE_SELECTION_CANVAS_ITEM,
		 "x1", band_info->start_x,
		 "y1", band_info->start_y,
		 "x2", band_info->start_x,
		 "y2", band_info->start_y,
...
static void
stop_rubberbanding (AthenaIconContainer *container,
		    guint32 time)
{
	AthenaIconRubberbandInfo *band_info;
	GList *icons;
	...
	if (enable_animation) {
		athena_selection_canvas_item_fade_out (ATHENA_SELECTION_CANVAS_ITEM (band_info->selection_rectangle), 150);
	} else {
	...
	icons = athena_icon_container_get_selected_icons (container);
...
typedef gboolean (* IsBetterIconFunction) (AthenaIconContainer *container,
					   AthenaIcon *start_icon,
					   AthenaIcon *best_so_far,
					   AthenaIcon *candidate,
					   void *data);

static AthenaIcon *
find_best_icon (AthenaIconContainer *container,
		AthenaIcon *start_icon,
		IsBetterIconFunction function,
		void *data)
{
	GList *p;
	AthenaIcon *best, *candidate;
...
static AthenaIcon *
find_best_selected_icon (AthenaIconContainer *container,
			 AthenaIcon *start_icon,
			 IsBetterIconFunction function,
			 void *data)
{
...
static int
compare_icons_by_uri (AthenaIconContainer *container,
		      AthenaIcon *icon_a,
		      AthenaIcon *icon_b)
{
	char *uri_a, *uri_b;
	...
	uri_a = athena_icon_container_get_icon_uri (container, icon_a);
	uri_b = athena_icon_container_get_icon_uri (container, icon_b);
...
static int
get_cmp_point_x (AthenaIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
...
static int
get_cmp_point_y (AthenaIconContainer *container,
		 EelDRect icon_rect)
{
	if (container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
...
static int
compare_icons_horizontal (AthenaIconContainer *container,
			  AthenaIcon *icon_a,
			  AthenaIcon *icon_b)
{
	EelDRect world_rect;
	...
	world_rect = athena_icon_canvas_item_get_icon_rectangle (icon_a->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 ...
		 &ax,
		 NULL);
	world_rect = athena_icon_canvas_item_get_icon_rectangle (icon_b->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static int
compare_icons_vertical (AthenaIconContainer *container,
			AthenaIcon *icon_a,
			AthenaIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_horizontal_first (AthenaIconContainer *container,
				AthenaIcon *icon_a,
				AthenaIcon *icon_b)
{
	EelDRect world_rect;
...
static int
compare_icons_vertical_first (AthenaIconContainer *container,
			      AthenaIcon *icon_a,
			      AthenaIcon *icon_b)
{
	EelDRect world_rect;
...
static gboolean
leftmost_in_top_row (AthenaIconContainer *container,
		     AthenaIcon *start_icon,
		     AthenaIcon *best_so_far,
		     AthenaIcon *candidate,
		     void *data)
...
static gboolean
rightmost_in_top_row (AthenaIconContainer *container,
		      AthenaIcon *start_icon,
		      AthenaIcon *best_so_far,
		      AthenaIcon *candidate,
		      void *data)
...
static gboolean
rightmost_in_bottom_row (AthenaIconContainer *container,
			 AthenaIcon *start_icon,
			 AthenaIcon *best_so_far,
			 AthenaIcon *candidate,
			 void *data)
...
static int
compare_with_start_row (AthenaIconContainer *container,
			AthenaIcon *icon)
{
	EelCanvasItem *item;
...
static int
compare_with_start_column (AthenaIconContainer *container,
			   AthenaIcon *icon)
{
	EelCanvasItem *item;
...
static gboolean
same_row_right_side_leftmost (AthenaIconContainer *container,
			      AthenaIcon *start_icon,
			      AthenaIcon *best_so_far,
			      AthenaIcon *candidate,
			      void *data)
...
static gboolean
same_row_left_side_rightmost (AthenaIconContainer *container,
			      AthenaIcon *start_icon,
			      AthenaIcon *best_so_far,
			      AthenaIcon *candidate,
			      void *data)
...
static gboolean
next_row_leftmost (AthenaIconContainer *container,
		   AthenaIcon *start_icon,
	           AthenaIcon *best_so_far,
		   AthenaIcon *candidate,
		   void *data)
...
static gboolean
next_row_rightmost (AthenaIconContainer *container,
		    AthenaIcon *start_icon,
		    AthenaIcon *best_so_far,
		    AthenaIcon *candidate,
		    void *data)
...
static gboolean
next_column_bottommost (AthenaIconContainer *container,
			AthenaIcon *start_icon,
			AthenaIcon *best_so_far,
			AthenaIcon *candidate,
			void *data)
...
static gboolean
previous_row_rightmost (AthenaIconContainer *container,
		        AthenaIcon *start_icon,
			AthenaIcon *best_so_far,
			AthenaIcon *candidate,
			void *data)
...
static gboolean
same_column_above_lowest (AthenaIconContainer *container,
			  AthenaIcon *start_icon,
			  AthenaIcon *best_so_far,
			  AthenaIcon *candidate,
			  void *data)
...
static gboolean
same_column_below_highest (AthenaIconContainer *container,
			   AthenaIcon *start_icon,
			   AthenaIcon *best_so_far,
			   AthenaIcon *candidate,
			   void *data)
...
static gboolean
previous_column_highest (AthenaIconContainer *container,
			 AthenaIcon *start_icon,
			 AthenaIcon *best_so_far,
			 AthenaIcon *candidate,
			 void *data)
...
static gboolean
next_column_highest (AthenaIconContainer *container,
		     AthenaIcon *start_icon,
		     AthenaIcon *best_so_far,
		     AthenaIcon *candidate,
		     void *data)
...
static gboolean
previous_column_lowest (AthenaIconContainer *container,
		        AthenaIcon *start_icon,
			AthenaIcon *best_so_far,
			AthenaIcon *candidate,
			void *data)
...
static gboolean
last_column_lowest (AthenaIconContainer *container,
		    AthenaIcon *start_icon,
		    AthenaIcon *best_so_far,
		    AthenaIcon *candidate,
		    void *data)
...
static gboolean
closest_in_90_degrees (AthenaIconContainer *container,
		       AthenaIcon *start_icon,
		       AthenaIcon *best_so_far,
		       AthenaIcon *candidate,
		       void *data)
{
	...
	world_rect = athena_icon_canvas_item_get_icon_rectangle (candidate->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static EelDRect 
get_rubberband (AthenaIcon *icon1,
		AthenaIcon *icon2)
{
	EelDRect rect1;
	EelDRect rect2;
...
static void
keyboard_move_to (AthenaIconContainer *container,
		  AthenaIcon *icon,
		  AthenaIcon *from,
		  GdkEventKey *event)
{
...
static void
keyboard_home (AthenaIconContainer *container,
	       GdkEventKey *event)
{
	AthenaIcon *from;
	AthenaIcon *to;
...
static void
keyboard_end (AthenaIconContainer *container,
	      GdkEventKey *event)
{
	AthenaIcon *to;
	AthenaIcon *from;
	...
	to = find_best_icon (container, NULL,
			     athena_icon_container_is_layout_vertical (container) ?
			     last_column_lowest :
			     rightmost_in_bottom_row,
			     NULL);
...
static void
record_arrow_key_start (AthenaIconContainer *container,
			AthenaIcon *icon,
			GtkDirectionType direction)
{
	EelDRect world_rect;

	world_rect = athena_icon_canvas_item_get_icon_rectangle (icon->item);
	eel_canvas_w2c
		(EEL_CANVAS (container),
		 get_cmp_point_x (container, world_rect),
		 get_cmp_point_y (container, world_rect),
...
static void
keyboard_arrow_key (AthenaIconContainer *container,
		    GdkEventKey *event,
		    GtkDirectionType direction,
...
static void
keyboard_right (AthenaIconContainer *container,
		GdkEventKey *event)
{
	...
	keyboard_arrow_key (container,
			    ...
			    GTK_DIR_RIGHT,
			    rightmost_in_bottom_row,
			    athena_icon_container_is_layout_rtl (container) ?
...
static void
keyboard_left (AthenaIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_down (AthenaIconContainer *container,
	       GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_up (AthenaIconContainer *container,
	     GdkEventKey *event)
{
	IsBetterIconFunction fallback;
...
static void
keyboard_space (AthenaIconContainer *container,
		GdkEventKey *event)
{
	AthenaIcon *icon;
...
static void
select_previous_or_next_icon (AthenaIconContainer *container, 
			      gboolean next, 
			      GdkEventKey *event)
{
	AthenaIcon *icon;
...
destroy (GtkWidget *object)
{
	...
	container = ATHENA_ICON_CONTAINER (object);

        athena_icon_container_clear (container);
	...
	GTK_WIDGET_CLASS (athena_icon_container_parent_class)->destroy (object);
}
...
finalize (GObject *object)
{
	...
	details = ATHENA_ICON_CONTAINER (object)->details;
	...
	G_OBJECT_CLASS (athena_icon_container_parent_class)->finalize (object);
}
	       ...
	       GtkAllocation *allocation)
{
	...
	container = ATHENA_ICON_CONTAINER (widget);
	...
	GTK_WIDGET_CLASS (athena_icon_container_parent_class)->size_allocate (widget, allocation);
...
realize (GtkWidget *widget)
{
	...
	GTK_WIDGET_CLASS (athena_icon_container_parent_class)->realize (widget);
	...
	/* Set up DnD.  */
	athena_icon_dnd_init (container);
...
unrealize (GtkWidget *widget)
{
	...
	athena_icon_dnd_fini (container);
	remove_search_entry_timeout (container);

	GTK_WIDGET_CLASS (athena_icon_container_parent_class)->unrealize (widget);
}
...
style_updated (GtkWidget *widget)
{
	 ...
	 */
	if (!athena_icon_container_get_is_desktop (container)) {
		GTK_WIDGET_CLASS (athena_icon_container_parent_class)->style_updated (widget);
	}

	if (gtk_widget_get_realized (widget)) {
		invalidate_labels (container);
		athena_icon_container_request_update_all (container);
	}
}
		    ...
		    GdkEventButton *event)
{
	...
	clicked_on_icon = GTK_WIDGET_CLASS (athena_icon_container_parent_class)->button_press_event (widget, event);
...
static void
athena_icon_container_did_not_drag (AthenaIconContainer *container,
				      GdkEventButton *event)
{
	AthenaIconContainerDetails *details;
	gboolean selection_changed;
	...
	if (details->drag_icon != NULL &&
		...
		if (click_count == 0 &&
			 ...
			 * AthenaList goes the other way because its "links" seem
			 * much more link-like.
			 */
			if (event->button == MIDDLE_BUTTON) {
				activate_selected_items_alternate (container, NULL);
...
static gboolean
clicked_within_double_click_interval (AthenaIconContainer *container)
{
	static gint64 last_click_time = 0;
	static gint click_count = 0;
	gint double_click_time;
...
static void
clear_drag_state (AthenaIconContainer *container)
{
...
static gboolean
start_stretching (AthenaIconContainer *container)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	GtkWidget *toplevel;
	...
	if (!athena_icon_canvas_item_hit_test_stretch_handles (icon->item,
...
static gboolean
update_stretch_at_idle (AthenaIconContainer *container)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	double world_x, world_y;
...
static void
continue_stretching (AthenaIconContainer *container,
		     double world_x, double world_y)
{

	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));
...
static gboolean
keyboard_stretching (AthenaIconContainer *container,
		     GdkEventKey           *event)
{
	AthenaIcon *icon;
	guint size;
...
static void
ungrab_stretch_icon (AthenaIconContainer *container)
{
...
static void
end_stretching (AthenaIconContainer *container,
		double world_x, double world_y)
{
	AthenaIconPosition position;
	AthenaIcon *icon;
...
static gboolean
undo_stretching (AthenaIconContainer *container)
{
	AthenaIcon *stretched_icon;
		      ...
		      GdkEventButton *event)
{
	...
	if (event->button == details->drag_button) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			if (!details->drag_started) {
				athena_icon_container_did_not_drag (container, event);
			} else {
				athena_icon_dnd_end_drag (container);
	...
	return GTK_WIDGET_CLASS (athena_icon_container_parent_class)->button_release_event (widget, event);
}
		     ...
		     GdkEventMotion *event)
{
	...
	if (details->drag_button != 0) {
		...
		case DRAG_STATE_MOVE_OR_COPY:
			...
			if (gtk_drag_check_threshold (widget, 
				...
				athena_icon_dnd_begin_drag (container,
							      actions,
							      details->drag_button,
							      event, 
							      canvas_x,
	...
	return GTK_WIDGET_CLASS (athena_icon_container_parent_class)->motion_notify_event (widget, event);
}

static void
athena_icon_container_search_position_func (AthenaIconContainer *container,
...
static void
athena_icon_container_search_dialog_hide (GtkWidget *search_dialog,
					    AthenaIconContainer *container)
{
...
static gboolean
athena_icon_container_search_entry_flush_timeout (gpointer data)
{
	AthenaIconContainer *container = data;

	container->details->typeselect_flush_timeout = 0;
	athena_icon_container_search_dialog_hide (container->details->search_window, container);

	return FALSE;
}

static void
add_search_entry_timeout (AthenaIconContainer *container)
{
	container->details->typeselect_flush_timeout =
		g_timeout_add_seconds (ATHENA_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT,
				       athena_icon_container_search_entry_flush_timeout,
				       container);
}

static void
remove_search_entry_timeout (AthenaIconContainer *container)
{
...
static void
reset_search_entry_timeout (AthenaIconContainer *container)
{
	remove_search_entry_timeout (container);
 ...
 */
static void
athena_icon_container_search_preedit_changed (GtkEntry *entry,
						gchar *preedit,
						AthenaIconContainer *container)
{
...
static void
athena_icon_container_search_activate (GtkEntry *entry,
					 AthenaIconContainer *container)
{
	athena_icon_container_search_dialog_hide (container->details->search_window,
						    container);
...
static gboolean
athena_icon_container_search_delete_event (GtkWidget *widget,
					     GdkEventAny *event,
					     AthenaIconContainer *container)
{
	athena_icon_container_search_dialog_hide (widget, container);

	return TRUE;
}

static gboolean
athena_icon_container_search_button_press_event (GtkWidget *widget,
						   GdkEventButton *event,
						   AthenaIconContainer *container)
{
	athena_icon_container_search_dialog_hide (widget, container);
...
static gboolean
athena_icon_container_search_entry_button_press_event (GtkWidget *widget,
							 GdkEventButton *event,
							 AthenaIconContainer *container)
{
	reset_search_entry_timeout (container);
...
static void
athena_icon_container_search_populate_popup (GtkEntry *entry,
					       GtkMenu *menu,
					       AthenaIconContainer *container)
{
	remove_search_entry_timeout (container);
...
static void
athena_icon_container_get_icon_text (AthenaIconContainer *container,
				       AthenaIconData      *data,
				       char                 **editable_text,
...
static gboolean
athena_icon_container_search_iter (AthenaIconContainer *container,
				     const char *key, gint n)
{
	...
	for (p = container->details->icons; p != NULL && count != n; p = p->next) {
		icon = p->data;
		athena_icon_container_get_icon_text (container, icon->data, &name,
						       NULL, TRUE);
...
static void
athena_icon_container_search_move (GtkWidget *window,
				     AthenaIconContainer *container,
				     gboolean up)
{
	gboolean ret;
	...
	ret = athena_icon_container_search_iter (container, text,
	...
	if (ret) {
		...
		/* return to old iter */
		athena_icon_container_search_iter (container, text,
					container->details->selected_iter);
	}
}

static gboolean
athena_icon_container_search_scroll_event (GtkWidget *widget,
					     ...
					     AthenaIconContainer *container)
{
	...
	if (event->direction == GDK_SCROLL_UP) {
		athena_icon_container_search_move (widget, container, TRUE);
		retval = TRUE;
	} else if (event->direction == GDK_SCROLL_DOWN) {
		athena_icon_container_search_move (widget, container, FALSE);
		retval = TRUE;
	}
...
static gboolean
athena_icon_container_search_key_press_event (GtkWidget *widget,
						GdkEventKey *event,
						AthenaIconContainer *container)
{
	gboolean retval = FALSE;
	...
	if (event->keyval == GDK_KEY_Return && event->state & GDK_SHIFT_MASK) {
		athena_icon_container_search_dialog_hide (widget,
							    container);
...
static void
athena_icon_container_search_init (GtkWidget   *entry,
				     AthenaIconContainer *container)
{
	gint ret;
	gint len;
	...
	ret = athena_icon_container_search_iter (container, text, 1);
...
static void
athena_icon_container_ensure_interactive_directory (AthenaIconContainer *container)
{
	...
	g_signal_connect (container->details->search_window, "delete_event",
			  G_CALLBACK (athena_icon_container_search_delete_event),
			  container);
	g_signal_connect (container->details->search_window, "key_press_event",
			  G_CALLBACK (athena_icon_container_search_key_press_event),
			  container);
	g_signal_connect (container->details->search_window, "button_press_event",
			  G_CALLBACK (athena_icon_container_search_button_press_event),
			  container);
	g_signal_connect (container->details->search_window, "scroll_event",
			  G_CALLBACK (athena_icon_container_search_scroll_event),
			  container);
	...
	g_signal_connect (container->details->search_entry, "populate-popup",
			  G_CALLBACK (athena_icon_container_search_populate_popup),
			  container);
	g_signal_connect (container->details->search_entry, "activate",
			  G_CALLBACK (athena_icon_container_search_activate),
			  container);
	g_signal_connect (container->details->search_entry, "preedit-changed",
			  G_CALLBACK (athena_icon_container_search_preedit_changed),
			  container);
	g_signal_connect (container->details->search_entry, "button-press-event",
			  G_CALLBACK (athena_icon_container_search_entry_button_press_event),
			  container);
 ...
 */
static gboolean
athena_icon_container_start_interactive_search (AthenaIconContainer *container)
{
	...
	athena_icon_container_ensure_interactive_directory (container);

	/* done, show it */
	athena_icon_container_search_position_func (container, container->details->search_window);
	...
	if (container->details->search_entry_changed_id == 0) {
		container->details->search_entry_changed_id =
			g_signal_connect (container->details->search_entry, "changed",
				G_CALLBACK (athena_icon_container_search_init),
				container);
	}
	...
	* really non-kosher. */
	...
	athena_icon_container_search_init (container->details->search_entry, container);

	return TRUE;
}

static gboolean
handle_popups (AthenaIconContainer *container,
	       GdkEventKey           *event,
	       const char            *signal)
{
		 ...
		 GdkEventKey *event)
{
	...
	if (!handled) {
		handled = GTK_WIDGET_CLASS (athena_icon_container_parent_class)->key_press_event (widget, event);
	}
	...
	if (!handled &&
		...
		if (container->details->imcontext_changed ||    /* we're in a preedit */
			...
			if (athena_icon_container_start_interactive_search (container)) {
...
get_accessible (GtkWidget *widget)
{
	...
	accessible = g_object_new 
		(athena_icon_container_accessible_get_type (), NULL);
...
static GObject*
athena_icon_container_constructor (GType                  type,
				     ...
				     GObjectConstructParam *construct_params)
{
	...
	object = G_OBJECT_CLASS (athena_icon_container_parent_class)->constructor
		(type,
		 n_construct_params,
		 construct_params);
	...
	if (athena_icon_container_get_is_desktop (container)) {
		g_signal_connect_swapped (athena_desktop_preferences,
					  "changed::" ATHENA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT,
					  G_CALLBACK (text_ellipsis_limit_changed_container_callback),
					  container);
	} else {
		g_signal_connect_swapped (athena_icon_view_preferences,
					  "changed::" ATHENA_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT,
					  G_CALLBACK (text_ellipsis_limit_changed_container_callback),
					  container);
	}
...
static void
athena_icon_container_class_init (AthenaIconContainerClass *class)
{
	GtkWidgetClass *widget_class;
	EelCanvasClass *canvas_class;

	G_OBJECT_CLASS (class)->constructor = athena_icon_container_constructor;
	...
	signals[SELECTION_CHANGED]
		= g_signal_new ("selection_changed",
		                ...
		                G_SIGNAL_RUN_LAST,
		                G_STRUCT_OFFSET (AthenaIconContainerClass,
						 selection_changed),
		                NULL, NULL,
...
static void
update_selected (AthenaIconContainer *container)
{
	GList *node;
	AthenaIcon *icon;
...
{
	update_selected (ATHENA_ICON_CONTAINER (widget));
...
handle_focus_out_event (GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	...
	end_renaming_mode (ATHENA_ICON_CONTAINER (widget), TRUE);
	update_selected (ATHENA_ICON_CONTAINER (widget));
...
static int text_ellipsis_limits[ATHENA_ZOOM_LEVEL_N_ENTRIES];
...
text_ellipsis_limit_changed_callback (gpointer callback_data)
{
	...
	pref = g_settings_get_strv (athena_icon_view_preferences,
				    ATHENA_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT);

	/* set default */
	get_text_ellipsis_limit_for_zoom (pref, NULL, &one_limit);
	for (i = 0; i < ATHENA_ZOOM_LEVEL_N_ENTRIES; i++) {
...
desktop_text_ellipsis_limit_changed_callback (gpointer callback_data)
{
	...
	pref = g_settings_get_int (athena_desktop_preferences, ATHENA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
	desktop_text_ellipsis_limit = pref;
}

static void
athena_icon_container_init (AthenaIconContainer *container)
{
	AthenaIconContainerDetails *details;
	static gboolean setup_prefs = FALSE;

	details = g_new0 (AthenaIconContainerDetails, 1);

	details->icon_set = g_hash_table_new (g_direct_hash, g_direct_equal);
	details->layout_timestamp = UNDEFINED_TIME;
	details->zoom_level = ATHENA_ZOOM_LEVEL_STANDARD;

	details->font_size_table[ATHENA_ZOOM_LEVEL_SMALLEST] = -2 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_SMALLER] = -2 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_SMALL] = -0 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_STANDARD] = 0 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_LARGE] = 0 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_LARGER] = 0 * PANGO_SCALE;
	details->font_size_table[ATHENA_ZOOM_LEVEL_LARGEST] = 0 * PANGO_SCALE;
...
static gboolean
handle_icon_double_click (AthenaIconContainer *container,
			  AthenaIcon *icon,
			  GdkEventButton *event)
{
	AthenaIconContainerDetails *details;
...
static gboolean
handle_icon_button_press (AthenaIconContainer *container,
			  AthenaIcon *icon,
			  GdkEventButton *event)
{
	AthenaIconContainerDetails *details;
		     ...
		     gpointer data)
{
	...
	icon = ATHENA_ICON_CANVAS_ITEM (item)->user_data;
	g_assert (icon != NULL);
...
GtkWidget *
athena_icon_container_new (void)
{
	return gtk_widget_new (ATHENA_TYPE_ICON_CONTAINER, NULL);
}

/* Clear all of the icons in the container. */
void
athena_icon_container_clear (AthenaIconContainer *container)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	GList *p;
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		if (icon->is_monitored) {
			athena_icon_container_stop_monitor_top_left (container,
								       icon->data,
								       icon);
		}
		icon_free (p->data);
...
gboolean
athena_icon_container_is_empty (AthenaIconContainer *container)
{
	return container->details->icons == NULL;
}

AthenaIconData *
athena_icon_container_get_first_visible_icon (AthenaIconContainer *container)
{
	GList *l;
	AthenaIcon *icon, *best_icon;
	double x, y;
	double x1, y1, x2, y2;
	double *pos, best_pos;
...
void
athena_icon_container_scroll_to_icon (AthenaIconContainer  *container,
					AthenaIconData       *data)
{
	GList *l;
	AthenaIcon *icon;
	...
	athena_icon_container_layout_now (container);
...
void
athena_icon_container_for_each (AthenaIconContainer *container,
				  AthenaIconCallback callback,
				  gpointer callback_data)
{
	CallbackAndData callback_and_data;
...
static void
icon_destroy (AthenaIconContainer *container,
	      AthenaIcon *icon)
{
	AthenaIconContainerDetails *details;
	gboolean was_selected;
	AthenaIcon *icon_to_focus;
	GList *item;
...
static void
activate_selected_items (AthenaIconContainer *container)
{
	GList *selection;

	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));

	selection = athena_icon_container_get_selection (container);
	if (selection != NULL) {
	  	g_signal_emit (container,
				 signals[ACTIVATE], 0,
				 selection);
...
static void
preview_selected_items (AthenaIconContainer *container)
{
	GList *selection;
	GArray *locations;
	gint idx;
	...
	selection = athena_icon_container_get_selection (container);
	locations = athena_icon_container_get_selected_icon_locations (container);
...
static void
activate_selected_items_alternate (AthenaIconContainer *container,
				   AthenaIcon *icon)
{
	GList *selection;
...
static AthenaIcon *
get_icon_being_renamed (AthenaIconContainer *container)
{
	AthenaIcon *rename_icon;
...
athena_icon_container_get_icon_images (AthenaIconContainer *container,
					 AthenaIconData      *data,
					 int                    size,
					 char                 **embedded_text,
...
static void
athena_icon_container_freeze_updates (AthenaIconContainer *container)
{
	AthenaIconContainerClass *klass;
...
static void
athena_icon_container_unfreeze_updates (AthenaIconContainer *container)
{
	AthenaIconContainerClass *klass;
...
static void
athena_icon_container_start_monitor_top_left (AthenaIconContainer *container,
						AthenaIconData *data,
						gconstpointer client,
						gboolean large_text)
{
...
static void
athena_icon_container_stop_monitor_top_left (AthenaIconContainer *container,
					       AthenaIconData *data,
					       gconstpointer client)
{
	AthenaIconContainerClass *klass;
...
static void
athena_icon_container_prioritize_thumbnailing (AthenaIconContainer *container,
						 AthenaIcon *icon)
{
	AthenaIconContainerClass *klass;
...
static void
athena_icon_container_update_visible_icons (AthenaIconContainer *container)
{
	GtkAdjustment *vadj, *hadj;
	double min_y, max_y;
	double min_x, max_x;
	...
	for (node = g_list_last (container->details->icons); node != NULL; node = node->prev) {
		...
		if (icon_is_positioned (icon)) {
			...
			if (visible) {
				athena_icon_canvas_item_set_is_visible (icon->item, TRUE);
				athena_icon_container_prioritize_thumbnailing (container,
										 icon);
			} else {
				athena_icon_canvas_item_set_is_visible (icon->item, FALSE);
			}
		}
	}
}
...
void 
athena_icon_container_update_icon (AthenaIconContainer *container,
				     AthenaIcon *icon)
{
	AthenaIconContainerDetails *details;
	guint icon_size;
	...
	max_image_size = MAX (MAXIMUM_IMAGE_SIZE * EEL_CANVAS (container)->pixels_per_unit, ATHENA_ICON_MAXIMUM_SIZE);
	...
	icon_info = athena_icon_container_get_icon_images (container, icon->data, icon_size,
							     &embedded_text,
	...
	if (container->details->forced_icon_size > 0) {
		pixbuf = athena_icon_info_get_pixbuf_at_size (icon_info, icon_size);
	} else {
		pixbuf = athena_icon_info_get_pixbuf (icon_info);
	}

	athena_icon_info_get_attach_points (icon_info, &attach_points, &n_attach_points);
	has_embedded_text_rect = athena_icon_info_get_embedded_rect (icon_info,
								       &embedded_text_rect);
	...
	if (has_embedded_text_rect && embedded_text_needs_loading) {
		...
		athena_icon_container_start_monitor_top_left (container, icon->data, icon, large_embedded_text);
	}
	
	athena_icon_container_get_icon_text (container,
					       icon->data,
					       &editable_text,
					       &additional_text,
					       FALSE);
	...
	if (icon == get_icon_being_renamed (container) &&
	    g_strcmp0 (editable_text,
		       athena_icon_canvas_item_get_editable_text (icon->item)) != 0) {
		end_renaming_mode (container, FALSE);
	}
	...
	athena_icon_canvas_item_set_image (icon->item, pixbuf);
	athena_icon_canvas_item_set_attach_points (icon->item, attach_points, n_attach_points);
	athena_icon_canvas_item_set_embedded_text_rect (icon->item, &embedded_text_rect);
	athena_icon_canvas_item_set_embedded_text (icon->item, embedded_text);
...
static gboolean
assign_icon_position (AthenaIconContainer *container,
		      AthenaIcon *icon)
{
	gboolean have_stored_position;
	AthenaIconPosition position;
...
static void
finish_adding_icon (AthenaIconContainer *container,
		    AthenaIcon *icon)
{
	athena_icon_container_update_icon (container, icon);
...
static void
finish_adding_new_icons (AthenaIconContainer *container)
{
...
static gboolean
is_old_or_unknown_icon_data (AthenaIconContainer *container,
			     AthenaIconData *data)
{
	time_t timestamp;
	gboolean success;
 ...
 * @container: A AthenaIconContainer
 * @data: Icon data.
 * 
 ...
 **/
gboolean
athena_icon_container_add (AthenaIconContainer *container,
			     AthenaIconData *data)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	...
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);
	...
	icon->scale = 1.0;
 	icon->item = ATHENA_ICON_CANVAS_ITEM
...
void
athena_icon_container_layout_now (AthenaIconContainer *container)
{
	if (container->details->idle_id != 0) {
		unschedule_redo_layout (container);
		redo_layout_internal (container);
 ...
 **/
gboolean
athena_icon_container_remove (AthenaIconContainer *container,
				AthenaIconData *data)
{
	AthenaIcon *icon;
 ...
 **/
void
athena_icon_container_request_update (AthenaIconContainer *container,
					AthenaIconData *data)
{
	AthenaIcon *icon;
...
AthenaZoomLevel
athena_icon_container_get_zoom_level (AthenaIconContainer *container)
{
        return container->details->zoom_level;
}

void
athena_icon_container_set_zoom_level (AthenaIconContainer *container, int new_level)
{
	AthenaIconContainerDetails *details;
        int pinned_level;
	double pixels_per_unit;
	...
	pinned_level = new_level;
        if (pinned_level < ATHENA_ZOOM_LEVEL_SMALLEST) {
		pinned_level = ATHENA_ZOOM_LEVEL_SMALLEST;
        } else if (pinned_level > ATHENA_ZOOM_LEVEL_LARGEST) {
        	pinned_level = ATHENA_ZOOM_LEVEL_LARGEST;
	}
	...
	pixels_per_unit = (double) athena_get_icon_size_for_zoom_level (pinned_level)
 ...
 **/
void
athena_icon_container_request_update_all (AthenaIconContainer *container)
{
	GList *node;
	AthenaIcon *icon;
 ...
 */
void
athena_icon_container_reveal (AthenaIconContainer *container, AthenaIconData *data)
{
	AthenaIcon *icon;
 ...
 **/
GList *
athena_icon_container_get_selection (AthenaIconContainer *container)
{
	GList *list, *p;
...
static GList *
athena_icon_container_get_selected_icons (AthenaIconContainer *container)
{
	GList *list, *p;
 ...
 **/
void
athena_icon_container_invert_selection (AthenaIconContainer *container)
{
	GList *p;
...
static GArray *
athena_icon_container_get_icon_locations (AthenaIconContainer *container,
					    GList *icons)
{
	GArray *result;
	GList *node;
	...
	for (index = 0, node = icons; node != NULL; index++, node = node->next) {
	     	g_array_index (result, GdkPoint, index).x =
	     		((AthenaIcon *)node->data)->x;
	     	g_array_index (result, GdkPoint, index).y =
			((AthenaIcon *)node->data)->y;
	}
 ...
 **/
GArray *
athena_icon_container_get_selected_icon_locations (AthenaIconContainer *container)
{
	GArray *result;
	GList *icons;
	...
	icons = athena_icon_container_get_selected_icons (container);
	result = athena_icon_container_get_icon_locations (container, icons);
	g_list_free (icons);
...
/**
 * athena_icon_container_select_all:
 * @container: An icon container widget.
 * 
 * Select all the icons in @container at once.
 **/
void
athena_icon_container_select_all (AthenaIconContainer *container)
{
	gboolean selection_changed;
	GList *p;
	AthenaIcon *icon;
...
/**
 * athena_icon_container_select_list_unselect_others.
 * @container: An icon container widget.
 * @selection: A list of AthenaIcon *.
 * 
 * Set the selection to exactly the icons in @selection.
 **/
void
athena_icon_container_select_list_unselect_others (AthenaIconContainer *container,
						     GList *selection)
{
	gboolean selection_changed;
	GHashTable *hash;
...
/**
 * athena_icon_container_unselect_all:
 * @container: An icon container widget.
 * 
 * Deselect all the icons in @container.
 **/
void
athena_icon_container_unselect_all (AthenaIconContainer *container)
{
...
/**
 * athena_icon_container_get_icon_by_uri:
 * @container: An icon container widget.
 ...
 **/
AthenaIcon *
athena_icon_container_get_icon_by_uri (AthenaIconContainer *container,
					 const char *uri)
{
	AthenaIconContainerDetails *details;
	GList *p;
	...
	*/
	...
	for (p = details->icons; p != NULL; p = p->next) {
		...
		icon_uri = athena_icon_container_get_icon_uri
			(container, icon);
...
static AthenaIcon *
get_nth_selected_icon (AthenaIconContainer *container, int index)
{
	GList *p;
	AthenaIcon *icon;
	int selection_count;
...
static AthenaIcon *
get_first_selected_icon (AthenaIconContainer *container)
{
        return get_nth_selected_icon (container, 1);
}

static gboolean
has_multiple_selection (AthenaIconContainer *container)
{
        return get_nth_selected_icon (container, 2) != NULL;
}

static gboolean
all_selected (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
...
static gboolean
has_selection (AthenaIconContainer *container)
{
 ...
 **/
void
athena_icon_container_show_stretch_handles (AthenaIconContainer *container)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	guint initial_size;
	...
	athena_icon_canvas_item_set_show_stretch_handles (icon->item, TRUE);
...
/**
 * athena_icon_container_has_stretch_handles
 * @container: An icon container widget.
 * 
 * Returns true if the first selected item has stretch handles.
 **/
gboolean
athena_icon_container_has_stretch_handles (AthenaIconContainer *container)
{
	AthenaIcon *icon;
 ...
 **/
gboolean
athena_icon_container_is_stretched (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
 ...
 **/
void
athena_icon_container_unstretch (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
...
char *
athena_icon_container_get_icon_uri (AthenaIconContainer *container,
				      AthenaIcon *icon)
{
	char *uri;
...
char *
athena_icon_container_get_icon_drop_target_uri (AthenaIconContainer *container,
				   	     	  AthenaIcon *icon)
{
	char *uri;
 ...
 */
static void
reset_scroll_region_if_not_empty (AthenaIconContainer *container)
{
	if (!athena_icon_container_is_empty (container)) {
		athena_icon_container_reset_scroll_region (container);
	}
}
 ...
 */
void
athena_icon_container_set_auto_layout (AthenaIconContainer *container,
...
gboolean
athena_icon_container_is_keep_aligned (AthenaIconContainer *container)
{
...
static void
unschedule_align_icons (AthenaIconContainer *container)
{
        if (container->details->align_idle_id != 0) {
		g_source_remove (container->details->align_idle_id);
		container->details->align_idle_id = 0;
...
static void
schedule_align_icons (AthenaIconContainer *container)
{
	if (container->details->align_idle_id == 0
	    && container->details->has_been_allocated) {
...
void
athena_icon_container_set_keep_aligned (AthenaIconContainer *container,
...
void
athena_icon_container_set_layout_mode (AthenaIconContainer *container,
...
void
athena_icon_container_set_label_position (AthenaIconContainer *container,
					    AthenaIconLabelPosition position)
{
	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));
 ...
 */
void
athena_icon_container_freeze_icon_positions (AthenaIconContainer *container)
{
	gboolean changed;
	GList *p;
	AthenaIcon *icon;
...
void
athena_icon_container_sort (AthenaIconContainer *container)
{
	gboolean changed;
...
gboolean
athena_icon_container_is_auto_layout (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);
...
static void
pending_icon_to_rename_destroy_callback (AthenaIconCanvasItem *item, AthenaIconContainer *container)
{
...
static AthenaIcon*
get_pending_icon_to_rename (AthenaIconContainer *container)
{
	return container->details->pending_icon_to_rename;
}

static void
set_pending_icon_to_rename (AthenaIconContainer *container, AthenaIcon *icon)
{
	AthenaIcon *old_icon;
...
static void
process_pending_icon_to_rename (AthenaIconContainer *container)
{
	AthenaIcon *pending_icon_to_rename;
	...
	if (pending_icon_to_rename != NULL) {
		if (pending_icon_to_rename->is_selected && !has_multiple_selection (container)) {
			athena_icon_container_start_renaming_selected_item (container, FALSE);
		} else {
...
static gboolean
is_renaming_pending (AthenaIconContainer *container)
{
	return get_pending_icon_to_rename (container) != NULL;
}

static gboolean
is_renaming (AthenaIconContainer *container)
{
 ...
 **/
void
athena_icon_container_start_renaming_selected_item (AthenaIconContainer *container,
						      gboolean select_all)
{
	AthenaIconContainerDetails *details;
	AthenaIcon *icon;
	...
	editable_text = athena_icon_canvas_item_get_editable_text (icon->item);
	...
	athena_icon_container_freeze_updates (container);
	...
	if (details->rename_widget == NULL) {
		...
		if (details->label_position != ATHENA_ICON_LABEL_POSITION_BESIDE) {
	...
	icon_rect = athena_icon_canvas_item_get_icon_rectangle (icon->item);
	text_rect = athena_icon_canvas_item_get_text_rectangle (icon->item, TRUE);

	if (athena_icon_container_is_layout_vertical (container) &&
	    container->details->label_position == ATHENA_ICON_LABEL_POSITION_BESIDE) {
		/* for one-line editables, the width changes dynamically */
		width = -1;
	} else {
		width = athena_icon_canvas_item_get_max_text_width (icon->item);
	}
	...
	athena_icon_canvas_item_set_renaming (icon->item, TRUE);
}

static void
end_renaming_mode (AthenaIconContainer *container, gboolean commit)
{
	AthenaIcon *icon;
	...
	athena_icon_canvas_item_set_renaming (icon->item, FALSE);
	
	athena_icon_container_unfreeze_updates (container);
...
gboolean
athena_icon_container_has_stored_icon_positions (AthenaIconContainer *container)
{
	GList *p;
	AthenaIcon *icon;
	gboolean have_stored_position;
...
void
athena_icon_container_set_single_click_mode (AthenaIconContainer *container,
...
gboolean
athena_icon_container_get_is_fixed_size (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);
...
void
athena_icon_container_set_is_fixed_size (AthenaIconContainer *container,
...
gboolean
athena_icon_container_get_is_desktop (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);
...
void
athena_icon_container_set_is_desktop (AthenaIconContainer *container,
					   gboolean is_desktop)
{
	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));
	...
	if (is_desktop) {
		...
		context = gtk_widget_get_style_context (GTK_WIDGET (container));
		gtk_style_context_add_class (context, "athena-desktop");
	}
}

void
athena_icon_container_set_margins (AthenaIconContainer *container,
				     int left_margin,
				     int right_margin,
				     int top_margin,
				     int bottom_margin)
...
void
athena_icon_container_set_use_drop_shadows (AthenaIconContainer  *container,
...
void
athena_icon_container_set_font (AthenaIconContainer *container,
				  const char *font)
{
	g_return_if_fail (ATHENA_IS_ICON_CONTAINER (container));
...
void
athena_icon_container_set_font_size_table (AthenaIconContainer *container,
					     const int font_size_table[ATHENA_ZOOM_LEVEL_LARGEST + 1])
{
	...
	for (i = 0; i <= ATHENA_ZOOM_LEVEL_LARGEST; i++) {
...
/**
 * athena_icon_container_get_icon_description
 * @container: An icon container widget.
 * @data: Icon data 
 * 
 ...
 **/
char*
athena_icon_container_get_icon_description (AthenaIconContainer *container,
				              AthenaIconData      *data)
{
	AthenaIconContainerClass *klass;
...
gboolean
athena_icon_container_get_allow_moves (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);
...
void
athena_icon_container_set_allow_moves	(AthenaIconContainer *container,
...
void
athena_icon_container_set_forced_icon_size (AthenaIconContainer *container,
...
void
athena_icon_container_set_all_columns_same_width (AthenaIconContainer *container,
 ...
 **/
void
athena_icon_container_set_highlighted_for_clipboard (AthenaIconContainer *container,
...
/* AthenaIconContainerAccessible */

static AthenaIconContainerAccessiblePrivate *
accessible_get_priv (AtkObject *accessible)
{
	AthenaIconContainerAccessiblePrivate *priv;
...
static gboolean
athena_icon_container_accessible_do_action (AtkAction *accessible, int i)
{
	GtkWidget *widget;
	AthenaIconContainer *container;
	GList *selection;
	...
	case ACTION_MENU :
		...
		break;
	default :
		g_warning ("Invalid action passed to AthenaIconContainerAccessible::do_action");
		return FALSE;
	}
	return TRUE;
}
...
static int
athena_icon_container_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
athena_icon_container_accessible_action_get_description (AtkAction *accessible, 
							   int i)
{
	AthenaIconContainerAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return athena_icon_container_accessible_action_descriptions[i];
	}
}

static const char *
athena_icon_container_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return athena_icon_container_accessible_action_names[i];
}

static const char *
athena_icon_container_accessible_action_get_keybinding (AtkAction *accessible, 
							  int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
athena_icon_container_accessible_action_set_description (AtkAction *accessible, 
							   int i, 
...
static void
athena_icon_container_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = athena_icon_container_accessible_do_action;
	iface->get_n_actions = athena_icon_container_accessible_get_n_actions;
	iface->get_description = athena_icon_container_accessible_action_get_description;
	iface->get_name = athena_icon_container_accessible_action_get_name;
	iface->get_keybinding = athena_icon_container_accessible_action_get_keybinding;
	iface->set_description = athena_icon_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void
athena_icon_container_accessible_update_selection (AtkObject *accessible)
{
	AthenaIconContainer *container;
	AthenaIconContainerAccessiblePrivate *priv;
	GList *l;
	...
	container = ATHENA_ICON_CONTAINER (gtk_accessible_get_widget (GTK_ACCESSIBLE (accessible)));
...
static void
athena_icon_container_accessible_selection_changed_cb (AthenaIconContainer *container,
							 gpointer data)
{
...
static void
athena_icon_container_accessible_icon_added_cb (AthenaIconContainer *container,
...
static void
athena_icon_container_accessible_icon_removed_cb (AthenaIconContainer *container,
						    AthenaIconData *icon_data,
						    gpointer data)
{
	AthenaIcon *icon;
...
static void
athena_icon_container_accessible_cleared_cb (AthenaIconContainer *container, 
					       gpointer data)
{
...
static gboolean 
athena_icon_container_accessible_add_selection (AtkSelection *accessible, 
						  int i)
{
	GtkWidget *widget;
	AthenaIconContainer *container;
	...
	if (l) {
		...
		selection = g_list_prepend (selection, 
					    icon->data);
		athena_icon_container_set_selection (container, selection);
...
static gboolean
athena_icon_container_accessible_clear_selection (AtkSelection *accessible)
{
	GtkWidget *widget;
	AthenaIconContainer *container;
	...
	athena_icon_container_unselect_all (container);

	return TRUE;
}

static AtkObject *
athena_icon_container_accessible_ref_selection (AtkSelection *accessible, 
						  int i)
{
	...
	athena_icon_container_accessible_update_selection (ATK_OBJECT (accessible));
	priv = accessible_get_priv (ATK_OBJECT (accessible));
...
static int
athena_icon_container_accessible_get_selection_count (AtkSelection *accessible)
{
	int count;
	AthenaIconContainerAccessiblePrivate *priv;
...
static gboolean
athena_icon_container_accessible_is_child_selected (AtkSelection *accessible,
						      int i)
{
	AthenaIconContainer *container;
	GList *l;
...
static gboolean
athena_icon_container_accessible_remove_selection (AtkSelection *accessible,
						     int i)
{
	AthenaIconContainer *container;
	AthenaIconContainerAccessiblePrivate *priv;
...
static gboolean
athena_icon_container_accessible_select_all_selection (AtkSelection *accessible)
{
	AthenaIconContainer *container;
	GtkWidget *widget;
	...
	athena_icon_container_select_all (container);

	return TRUE;
}

void
athena_icon_container_widget_to_file_operation_position (AthenaIconContainer *container,
							   GdkPoint              *position)
{
	double x, y;
	...
	position->x -= athena_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
	position->y -= athena_get_icon_size_for_zoom_level (container->details->zoom_level) / 2;
}

static void 
athena_icon_container_accessible_selection_interface_init (AtkSelectionIface *iface)
{
	iface->add_selection = athena_icon_container_accessible_add_selection;
	iface->clear_selection = athena_icon_container_accessible_clear_selection;
	iface->ref_selection = athena_icon_container_accessible_ref_selection;
	iface->get_selection_count = athena_icon_container_accessible_get_selection_count;
	iface->is_child_selected = athena_icon_container_accessible_is_child_selected;
	iface->remove_selection = athena_icon_container_accessible_remove_selection;
	iface->select_all_selection = athena_icon_container_accessible_select_all_selection;
}


static gint 
athena_icon_container_accessible_get_n_children (AtkObject *accessible)
{
	AthenaIconContainer *container;
	GtkWidget *widget;
	gint i;
...
static AtkObject* 
athena_icon_container_accessible_ref_child (AtkObject *accessible, int i)
{
        AtkObject *atk_object;
        AthenaIconContainer *container;
        GList *item;
...
static void
athena_icon_container_accessible_initialize (AtkObject *accessible, 
					       gpointer data)
{
	AthenaIconContainer *container;
	AthenaIconContainerAccessiblePrivate *priv;
	...
	priv = g_new0 (AthenaIconContainerAccessiblePrivate, 1);
	...
	if (GTK_IS_ACCESSIBLE (accessible)) {
		...
		g_signal_connect (G_OBJECT (container), "selection_changed",
				  G_CALLBACK (athena_icon_container_accessible_selection_changed_cb), 
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_added",
				  G_CALLBACK (athena_icon_container_accessible_icon_added_cb), 
				  accessible);
		g_signal_connect (G_OBJECT (container), "icon_removed",
				  G_CALLBACK (athena_icon_container_accessible_icon_removed_cb), 
				  accessible);
		g_signal_connect (G_OBJECT (container), "cleared",
				  G_CALLBACK (athena_icon_container_accessible_cleared_cb), 
				  accessible);
	}
}

static void
athena_icon_container_accessible_finalize (GObject *object)
{
	AthenaIconContainerAccessiblePrivate *priv;
	int i;
...
static void
athena_icon_container_accessible_class_init (AtkObjectClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	accessible_parent_class = g_type_class_peek_parent (klass);

	gobject_class->finalize = athena_icon_container_accessible_finalize;

	klass->get_n_children = athena_icon_container_accessible_get_n_children;
	klass->ref_child = athena_icon_container_accessible_ref_child;
	klass->initialize = athena_icon_container_accessible_initialize;

	accessible_private_data_quark = g_quark_from_static_string ("icon-container-accessible-private-data");
}

static GType
athena_icon_container_accessible_get_type (void)
{
        static GType type = 0;

        if (!type) {
                static GInterfaceInfo atk_action_info = {
                        (GInterfaceInitFunc) athena_icon_container_accessible_action_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };              
		
                static GInterfaceInfo atk_selection_info = {
                        (GInterfaceInitFunc) athena_icon_container_accessible_selection_interface_init,
                        (GInterfaceFinalizeFunc) NULL,
                        NULL
                };              
...
void
athena_self_check_icon_container (void)
{
...
gboolean
athena_icon_container_is_layout_rtl (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), 0);

	return container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_R_L ||
		container->details->layout_mode == ATHENA_ICON_LAYOUT_R_L_T_B;
}

gboolean
athena_icon_container_is_layout_vertical (AthenaIconContainer *container)
{
	g_return_val_if_fail (ATHENA_IS_ICON_CONTAINER (container), FALSE);

	return (container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_L_R ||
		container->details->layout_mode == ATHENA_ICON_LAYOUT_T_B_R_L);
}

int
athena_icon_container_get_max_layout_lines_for_pango (AthenaIconContainer  *container)
{
	int limit;
...
int
athena_icon_container_get_max_layout_lines (AthenaIconContainer  *container)
{
	int limit;
...
void
athena_icon_container_begin_loading (AthenaIconContainer *container)
{
	gboolean dummy;

	if (athena_icon_container_get_store_layout_timestamps (container)) {
...
static void
store_layout_timestamps_now (AthenaIconContainer *container)
{
	AthenaIcon *icon;
	GList *p;
	gboolean dummy;
...
void
athena_icon_container_end_loading (AthenaIconContainer *container,
				     gboolean               all_icons_added)
{
	if (all_icons_added &&
	    athena_icon_container_get_store_layout_timestamps (container)) {
...
gboolean
athena_icon_container_get_store_layout_timestamps (AthenaIconContainer *container)
{
...
void
athena_icon_container_set_store_layout_timestamps (AthenaIconContainer *container,
