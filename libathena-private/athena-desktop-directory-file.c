/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   athena-desktop-directory-file.c: Subclass of AthenaFile to help implement the
   virtual desktop.
...
struct AthenaDesktopDirectoryFileDetails {
	AthenaDesktopDirectory *desktop_directory;

  	AthenaFile *real_dir_file;
...
typedef struct {
	AthenaDesktopDirectoryFile *desktop_file;
	AthenaFileCallback callback;
	gpointer callback_data;

	AthenaFileAttributes delegated_attributes;
	AthenaFileAttributes non_delegated_attributes;
...
G_DEFINE_TYPE (AthenaDesktopDirectoryFile, athena_desktop_directory_file,
	       ATHENA_TYPE_FILE);
...
static void
real_file_changed_callback (AthenaFile *real_file,
			    gpointer callback_data)
{
	AthenaDesktopDirectoryFile *desktop_file;
	
	desktop_file = ATHENA_DESKTOP_DIRECTORY_FILE (callback_data);
	athena_file_changed (ATHENA_FILE (desktop_file));
}

static AthenaFileAttributes 
get_delegated_attributes_mask (void)
{
	return ATHENA_FILE_ATTRIBUTE_DEEP_COUNTS |
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
		ATHENA_FILE_ATTRIBUTE_DIRECTORY_ITEM_MIME_TYPES;
}

static void
partition_attributes (AthenaFileAttributes attributes,
		      AthenaFileAttributes *delegated_attributes,
		      AthenaFileAttributes *non_delegated_attributes)
{
	AthenaFileAttributes mask;
...
static void
desktop_directory_file_monitor_add (AthenaFile *file,
				    ...
				    AthenaFileAttributes attributes)
{
	...
	desktop_file = ATHENA_DESKTOP_DIRECTORY_FILE (file);
	...
	athena_file_monitor_add (desktop_file->details->real_dir_file,
...
static void
desktop_directory_file_monitor_remove (AthenaFile *file,
				       gconstpointer client)
{
	AthenaDesktopDirectoryFile *desktop_file;
	DesktopMonitor *monitor;
...
desktop_callback_destroy (DesktopCallback *desktop_callback)
{
	...
	g_assert (ATHENA_IS_DESKTOP_DIRECTORY_FILE (desktop_callback->desktop_file));

	athena_file_unref (ATHENA_FILE (desktop_callback->desktop_file));
...
desktop_callback_check_done (DesktopCallback *desktop_callback)
{
	...
	athena_desktop_update_metadata_from_keyfile (ATHENA_FILE (desktop_callback->desktop_file), "directory");
	...
	(* desktop_callback->callback) (ATHENA_FILE (desktop_callback->desktop_file),
...
static void
ready_callback (AthenaFile *file,
		gpointer callback_data)
{
...
static void
desktop_directory_file_call_when_ready (AthenaFile *file,
					AthenaFileAttributes attributes,
					AthenaFileCallback callback,
					gpointer callback_data)
...
static void
desktop_directory_file_cancel_call_when_ready (AthenaFile *file,
					       AthenaFileCallback callback,
					       gpointer callback_data)
{
	AthenaDesktopDirectoryFile *desktop_file;
...
static gboolean
real_check_if_ready (AthenaFile *file,
		     AthenaFileAttributes attributes)
{
...
static gboolean
desktop_directory_file_check_if_ready (AthenaFile *file,
				       AthenaFileAttributes attributes)
{
	AthenaFileAttributes delegated_attributes, non_delegated_attributes;
	AthenaDesktopDirectoryFile *desktop_file;
	...
	return real_check_if_ready (file, non_delegated_attributes) &&
		athena_file_check_if_ready (desktop_file->details->real_dir_file,
					      delegated_attributes);
}

static gboolean
desktop_directory_file_get_item_count (AthenaFile *file, 
				       guint *count,
				       gboolean *count_unreadable)
{
	...
	got_count = athena_file_get_directory_item_count (desktop_file->details->real_dir_file,
							    count,
							    count_unreadable);
...
desktop_directory_file_get_deep_counts (AthenaFile *file,
					...
					goffset *total_size)
{
	...
	status = athena_file_get_deep_counts (desktop_file->details->real_dir_file,
						directory_count,
						file_count,
...
static gboolean
desktop_directory_file_get_date (AthenaFile *file,
				 ...
				 time_t *date)
{
	...
	return athena_file_get_date (desktop_file->details->real_dir_file,
				       date_type,
				       date);
}

static char *
desktop_directory_file_get_where_string (AthenaFile *file)
{
...
monitor_destroy (gpointer data)
{
	...
	athena_file_monitor_remove
		(ATHENA_FILE (monitor->desktop_file->details->real_dir_file), monitor);
	g_free (monitor);
}

static void
athena_desktop_directory_file_set_metadata (AthenaFile           *file,
					      const char             *key,
					      const char             *value)
{
	athena_desktop_set_metadata_string (file, "directory", key, value);
}

static void
athena_desktop_directory_file_set_metadata_as_list (AthenaFile           *file,
						      const char             *key,
						      char                  **value)
{
	athena_desktop_set_metadata_stringv (file, "directory", key, (const gchar **) value);
}

static void
athena_desktop_directory_file_init (AthenaDesktopDirectoryFile *desktop_file)
{
	AthenaDesktopDirectory *desktop_directory;
	AthenaDirectory *real_dir;
	AthenaFile *real_dir_file;

	desktop_file->details = G_TYPE_INSTANCE_GET_PRIVATE (desktop_file,
							     ATHENA_TYPE_DESKTOP_DIRECTORY_FILE,
							     AthenaDesktopDirectoryFileDetails);

	desktop_directory = ATHENA_DESKTOP_DIRECTORY (athena_directory_get_by_uri (EEL_DESKTOP_URI));
	...
	real_dir = athena_desktop_directory_get_real_directory (desktop_directory);
	real_dir_file = athena_directory_get_corresponding_file (real_dir);
	athena_directory_unref (real_dir);
				    ...
				    gpointer callback_data)
{
	desktop_callback_remove_file
		(value, ATHENA_FILE (callback_data));
}
...
desktop_finalize (GObject *object)
{
	...
	desktop_file = ATHENA_DESKTOP_DIRECTORY_FILE (object);
	...
	athena_file_unref (desktop_file->details->real_dir_file);
	athena_directory_unref (ATHENA_DIRECTORY (desktop_directory));

	G_OBJECT_CLASS (athena_desktop_directory_file_parent_class)->finalize (object);
}

static void
athena_desktop_directory_file_class_init (AthenaDesktopDirectoryFileClass *klass)
{
	...
	file_class->set_metadata = athena_desktop_directory_file_set_metadata;
	file_class->set_metadata_as_list = athena_desktop_directory_file_set_metadata_as_list;

	g_type_class_add_private (klass, sizeof (AthenaDesktopDirectoryFileDetails));
}
