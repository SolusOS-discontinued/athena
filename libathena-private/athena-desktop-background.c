...
#include "athena-desktop-background.h"
...
static void init_fade (AthenaDesktopBackground *self);
static void free_fade (AthenaDesktopBackground *self);
static void queue_background_change (AthenaDesktopBackground *self);

static AthenaDesktopBackground *singleton = NULL;

G_DEFINE_TYPE (AthenaDesktopBackground, athena_desktop_background, G_TYPE_OBJECT);
...
struct AthenaDesktopBackgroundDetails {
...
static void
free_fade (AthenaDesktopBackground *self)
{
...
static void
free_background_surface (AthenaDesktopBackground *self)
{
...
static void
athena_desktop_background_set_image_uri (AthenaDesktopBackground *self,
...
static gboolean
background_changed_cb (AthenaDesktopBackground *self)
{
	self->details->change_idle_id = 0;

	athena_desktop_background_unrealize (self);
	athena_desktop_background_set_up_widget (self);
...
static void
queue_background_change (AthenaDesktopBackground *self)
{
...
static void
athena_desktop_background_changed (GnomeBG *bg,
                                     gpointer user_data)
{
        AthenaDesktopBackground *self;
...
static void
athena_desktop_background_set_property (GObject *object,
                                          guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec)
{
...
static GObject *
athena_desktop_background_constructor (GType type,
                                         ...
                                         GObjectConstructParam *construct_params)
{
        ...
        retval = G_OBJECT_CLASS (athena_desktop_background_parent_class)->constructor
                (type, n_construct_params, construct_params);

        singleton = ATHENA_DESKTOP_BACKGROUND (retval);
...
static void
athena_desktop_background_class_init (AthenaDesktopBackgroundClass *klass)
{
	GObjectClass *object_class;
        GParamSpec *pspec;

	object_class = G_OBJECT_CLASS (klass);
	object_class->finalize = athena_desktop_background_finalize;
        object_class->set_property = athena_desktop_background_set_property;
        object_class->constructor = athena_desktop_background_constructor;
        object_class->constructed = athena_desktop_background_constructed;

        pspec = g_param_spec_object ("widget", "The widget for this background",
                                     "The widget that gets its background set",
                                     ATHENA_TYPE_ICON_CONTAINER,
                                     G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
        g_object_class_install_property (object_class, PROP_WIDGET, pspec);

	g_type_class_add_private (klass, sizeof (AthenaDesktopBackgroundDetails));
}
...
void
athena_desktop_background_receive_dropped_background_image (AthenaDesktopBackground *self,
                                                              const char *image_uri)
{
	...
	athena_desktop_background_set_image_uri (self, image_uri);
...
athena_desktop_background_new (AthenaIconContainer *container)
{
        return g_object_new (ATHENA_TYPE_DESKTOP_BACKGROUND,
                             "widget", container,
                             NULL);
}
