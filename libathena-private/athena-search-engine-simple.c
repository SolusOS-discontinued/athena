...
typedef struct {
	AthenaSearchEngineSimple *engine;
...
struct AthenaSearchEngineSimpleDetails {
	AthenaQuery *query;
...
G_DEFINE_TYPE (AthenaSearchEngineSimple, athena_search_engine_simple,
	       ATHENA_TYPE_SEARCH_ENGINE);

static void
finalize (GObject *object)
{
	AthenaSearchEngineSimple *simple;

	simple = ATHENA_SEARCH_ENGINE_SIMPLE (object);
	...
	G_OBJECT_CLASS (athena_search_engine_simple_parent_class)->finalize (object);
}

static SearchThreadData *
search_thread_data_new (AthenaSearchEngineSimple *engine,
			AthenaQuery *query)
{
	...
	data->mime_types = athena_query_get_mime_types (query);
...
search_thread_done_idle (gpointer user_data)
{
	...
	if (!g_cancellable_is_cancelled (data->cancellable)) {
		athena_search_engine_finished (ATHENA_SEARCH_ENGINE (data->engine));
...
search_thread_add_hits_idle (gpointer user_data)
{
	...
	if (!g_cancellable_is_cancelled (hits->thread_data->cancellable)) {
		athena_search_engine_hits_added (ATHENA_SEARCH_ENGINE (hits->thread_data->engine),
						   hits->uris);
	}
...
static void
athena_search_engine_simple_start (AthenaSearchEngine *engine)
{
	AthenaSearchEngineSimple *simple;
	SearchThreadData *data;
	GThread *thread;
	
	simple = ATHENA_SEARCH_ENGINE_SIMPLE (engine);
	...
	thread = g_thread_new ("athena-search-simple", search_thread_func, data);
...
static void
athena_search_engine_simple_stop (AthenaSearchEngine *engine)
{
	AthenaSearchEngineSimple *simple;
...
static void
athena_search_engine_simple_set_query (AthenaSearchEngine *engine, AthenaQuery *query)
{
	AthenaSearchEngineSimple *simple;
...
static void
athena_search_engine_simple_class_init (AthenaSearchEngineSimpleClass *class)
{
	...
	engine_class->set_query = athena_search_engine_simple_set_query;
	engine_class->start = athena_search_engine_simple_start;
	engine_class->stop = athena_search_engine_simple_stop;

	g_type_class_add_private (class, sizeof (AthenaSearchEngineSimpleDetails));
}

static void
athena_search_engine_simple_init (AthenaSearchEngineSimple *engine)
{
	engine->details = G_TYPE_INSTANCE_GET_PRIVATE (engine, ATHENA_TYPE_SEARCH_ENGINE_SIMPLE,
						       AthenaSearchEngineSimpleDetails);
}

AthenaSearchEngine *
athena_search_engine_simple_new (void)
{
	AthenaSearchEngine *engine;

	engine = g_object_new (ATHENA_TYPE_SEARCH_ENGINE_SIMPLE, NULL);
