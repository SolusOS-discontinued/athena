/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   athena-monitor.c: file and directory change monitoring for athena
...
#include <config.h>
#include "athena-monitor.h"
#include "athena-file-changes-queue.h"
#include "athena-file-utilities.h"

#include <gio/gio.h>

struct AthenaMonitor {
	GFileMonitor *monitor;
};

gboolean
athena_monitor_active (void)
{
	     ...
	     gpointer user_data)
{
	...
	case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT:
		athena_file_changes_queue_file_changed (child);
		break;
	case G_FILE_MONITOR_EVENT_DELETED:
		athena_file_changes_queue_file_removed (child);
		break;
	case G_FILE_MONITOR_EVENT_CREATED:
		athena_file_changes_queue_file_added (child);
		break;
