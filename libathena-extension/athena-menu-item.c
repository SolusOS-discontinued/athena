/*
 *  athena-menu-item.c - Menu items exported by AthenaMenuProvider
 *                         objects.
 *
 ...
 *
 * Returns: a newly create #AthenaMenuItem
 */
AthenaMenuItem *
athena_menu_item_new (const char *name,
			const char *label,
			const char *tip,
			const char *icon)
{
	...
	item = g_object_new (ATHENA_TYPE_MENU_ITEM, 
			     "name", name,
			     "label", label,
			     "tip", tip,
			     "icon", icon,
...
/**
 * athena_menu_item_activate:
 * @item: pointer to a #AthenaMenuItem
 *
 * emits the activate signal.
 */
void
athena_menu_item_activate (AthenaMenuItem *item)
{
 ...
 * @item: pointer to a #AthenaMenuItem
 * @menu: pointer to a #AthenaMenu to attach to the button
 *
 * Attachs a menu to the given #AthenaMenuItem.
 */
void
athena_menu_item_set_submenu (AthenaMenuItem *item, AthenaMenu *menu)
{
	g_object_set (item, "menu", menu, NULL);
}

static void
athena_menu_item_get_property (GObject *object,
				 guint param_id,
				 GValue *value,
				 GParamSpec *pspec)
{
...
static void
athena_menu_item_set_property (GObject *object,
				 guint param_id,
				 const GValue *value,
				 GParamSpec *pspec)
{
...
static void
athena_menu_item_finalize (GObject *object)
{
	AthenaMenuItem *item;
...
static void
athena_menu_item_instance_init (AthenaMenuItem *item)
{
	item->details = g_new0 (AthenaMenuItemDetails, 1);
	item->details->sensitive = TRUE;
	item->details->menu = NULL;
}

static void
athena_menu_item_class_init (AthenaMenuItemClass *class)
{
	parent_class = g_type_class_peek_parent (class);
	
	G_OBJECT_CLASS (class)->finalize = athena_menu_item_finalize;
	G_OBJECT_CLASS (class)->get_property = athena_menu_item_get_property;
	G_OBJECT_CLASS (class)->set_property = athena_menu_item_set_property;

        signals[ACTIVATE] =
                g_signal_new ("activate",
                              G_TYPE_FROM_CLASS (class),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (AthenaMenuItemClass, 
					       activate),
                              NULL, NULL,
...
GType 
athena_menu_item_get_type (void)
{
	static GType type = 0;
	
	if (!type) {
		const GTypeInfo info = {
			sizeof (AthenaMenuItemClass),
			NULL,
			NULL,
			(GClassInitFunc)athena_menu_item_class_init,
			NULL,
			NULL,
			sizeof (AthenaMenuItem),
			0,
			(GInstanceInitFunc)athena_menu_item_instance_init
		};
